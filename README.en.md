...
<div align="center">
  <img src="public/logo/icons/1024x1024.png" alt="Mesa Code" width="128" height="128" />
</div>
<p align="center">
  <a href="https://applink.feishu.cn/client/chat/chatter/add_by_link?link_token=47ag983c-8fcb-4d6d-814b-5395193a712c&amp;qr_code=true">Community</a> ·
  <a href="https://discord.gg/z9aBcQXZQ3">Discord</a>
</p>
<p align="center">English-only documentation</p>

Mesa Code is an AI coding workspace with desktop, browser, and terminal interfaces. This repository contains the clients, backend services, shared UI, and Agent CLI and runtime source code.

| Interface                    | Purpose                                                                                   | Development command            |
| ---------------------------- | ----------------------------------------------------------------------------------------- | ------------------------------ |
| Desktop                      | Windows Electron desktop application                                                      | `pnpm dev:desktop`             |
| Web / Mesa Code CLI distribution | Terminal and browser workspace; packages the TUI, Web client, backend, and Agent together | `pnpm dev:web`                 |
| Agent CLI                    | The `mesacode` terminal interface, which also provides the Agent runtime for Desktop and Web | `pnpm --filter @mesacode/cli dev` |
...
The Agent CLI and runtime source code lives in [apps/mesacode-cli/](apps/mesacode-cli/) as a regular directory included when you clone this repository. No separate checkout or Git submodule initialization is required.
...
Set `MESACODE_DATA_BASE_DIR` to use a separate development data directory. For example, on macOS / Linux:

```bash
MESACODE_DATA_BASE_DIR="$HOME/.mesacode-dev-home" pnpm dev:desktop:test
```
...
MESACODE_SERVER_WORKSPACE=/path/to/project pnpm dev:web
```
...
After changing Agent source code, run `pnpm --filter @mesacode/cli... build` and restart the service. To validate the complete distribution, extract and run it as described under Packaging → Mesa Code CLI distribution below.

### Mesa Code CLI distribution

The command-line distribution includes the TUI, Web client, and Agent behind one `mesacode` command. With no arguments it starts the TUI; a leading `--web` starts Web mode; all other arguments go to the existing Agent CLI. Both modes run locally without Electron.
...
mesacode --web --workspace /path/to/project --port 3030 --no-open
...
When starting the general Web service's HTTP entry directly, configure API/WebSocket authentication with `MESACODE_SERVER_AUTH_TOKEN`. When creating the service programmatically, use the `authToken` option.

See Packaging below for build instructions. `pnpm build:mesacode` only creates the distribution; it does not replace an existing `mesacode` on `PATH`. If the command still points to an older installation or another checkout, check it with `command -v mesacode` on macOS / Linux or `where.exe mesacode` on Windows.
...
```bash
pnpm --filter @mesacode/cli dev --help
pnpm --filter @mesacode/cli dev

# Build the CLI and its workspace dependencies
pnpm --filter @mesacode/cli... build
node apps/mesacode-cli/packages/cli/dist/mesacode.cjs --help
```

This entry runs the Agent CLI directly and does not handle the distribution's `--web` switch. Use `pnpm dev:web` for Web development, or the extracted `bin/mesacode.mjs` shown below to test the unified command.
...
| Setting                              | Purpose                                                                                 |
| ------------------------------------ | --------------------------------------------------------------------------------------- |
| `MESACODE_DATA_BASE_DIR`                | Base directory for application data, stored under its `.mesacode/` subdirectory            |
| `MESACODE_SERVER_WORKSPACE`             | Workspace path for the Web backend                                                      |
| `MESACODE_BUILTIN_PROVIDER_CONFIG_FILE` | Path to a local provider configuration file; uses the built-in configuration when unset |
| `MESACODE_DIST_BASE_URL`                | Download base URL used by the CLI distribution installer                                |
...
Run `pnpm build:mesacode` to build the CLI/TUI, backend, and Web client, collect the TUI native libraries, workers, and runtime dependencies, then assemble the distribution. Running the distribution still requires Node.js; use the version specified in `mise.toml`.

Before packaging, set the download base URL with `MESACODE_DIST_BASE_URL` in `.env`, `.env.local`, or the process environment, or pass it through `--base-url`. The URL below is a placeholder; replace it with your hosting URL when publishing:

```bash
pnpm build:mesacode --base-url https://downloads.example.com/mesacode/

# When MESACODE_DIST_BASE_URL is already configured
pnpm build:mesacode

# Repackage existing Agent, backend, and Web build outputs
pnpm build:mesacode --skip-build

# Show options for the version, output directory, and more
pnpm build:mesacode --help
```

The version defaults to the root `package.json` version. Output is written to `dist/mesacode/`:

- `releases/<version>/mesacode-<version>.tar.gz`: runtime package.
- `releases/<version>/sha256.txt`: checksum file.
- `latest.json` and `install.sh`: version index and installer.

Upload the entire directory to the configured download base URL. The installer downloads the runtime package from that URL, installs it to `~/.mesacode/runtime` by default, and creates the `mesacode` command in `~/.local/bin`. Override these directories with `MESACODE_DIST_HOME` and `MESACODE_DIST_BIN_DIR`, respectively.
...
```bash
mesacode_version=$(node -p "require('./dist/mesacode/latest.json').version")
mkdir -p dist/mesacode/debug
tar -xzf "dist/mesacode/releases/$mesacode_version/mesacode-$mesacode_version.tar.gz" \
  -C dist/mesacode/debug
# Start the TUI by default
node dist/mesacode/debug/mesacode/bin/mesacode.mjs

# Start Web mode
node dist/mesacode/debug/mesacode/bin/mesacode.mjs --web \
...
| `packages/mesacode-server-cli`                          | Standalone server startup and process management                                        |
