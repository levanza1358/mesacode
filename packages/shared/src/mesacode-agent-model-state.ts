// ── 旧协议兼容面（过渡期）──────────────────────────────
// 剩余 5 个导出：旧 configOptions 投影函数（formatModelPickerValue/normalizeAvailableMesacodeMode/
// getMesacodeAgentModeSelectOptions/getMesacodeAgentAvailableModes/
// mesacodeSessionSettingsToMesacodeConfigOptions）。
// 消费者：services mesacodeConfigOptions、UI mesacodeSessionProjection 等旧栈。
import { formatModelPickerValue } from "./model-selection.js";
import type { MesacodeSessionMode, MesacodeSessionSettingsState } from "./mesacode-protocol/index.js";
import type { MesacodeConfigOption, MesacodeTaskModeInfo } from "./mesacode-task-types-core.js";
...
export function normalizeAvailableMesacodeMode(mode: MesacodeSessionMode): string {
  return MESACODE_AGENT_MODE_ID_SET.has(mode) ? mode : "build";
}

export function getMesacodeAgentModeSelectOptions(): NonNullable<MesacodeConfigOption["options"]> {
...
export function getMesacodeAgentAvailableModes(): MesacodeTaskModeInfo[] {
  return MESACODE_AGENT_MODE_OPTIONS.map((mode) => ({ ...mode }));
}

export function mesacodeSessionSettingsToMesacodeConfigOptions(
