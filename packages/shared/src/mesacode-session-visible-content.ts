...
// 消费者：mesacodeTaskServiceAdapter/mesacodeSessionProjection/mesacodeTaskIndexSyncer、
// CLI bootstrap session-mapper。textFromMesacodeMessageParts 已迁
// mesacode-protocol-legacy-types.ts（幸存面）。本文件与旧投影栈同生命周期。
import type { MesacodeSessionGoal } from "./mesacode-protocol/index.js";
...
export function isMesacodeGoalContinuationReminderText(text: string): boolean {
...
export function isMesacodeGoalContinuationReminderMessage(message: MesacodeMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
    (message.info.source === "goal-continuation" ||
      String(message.info.metadata?.["source"] ?? "") === "goal-continuation" ||
      isMesacodeGoalContinuationReminderText(textFromMesacodeMessageParts(message.parts)))
  );
}

export function isMesacodeGoalStateReminderText(text: string): boolean {
  const normalized = text.trimStart();
  return normalized.startsWith("<system-reminder>") && normalized.includes(GOAL_STATE_TEXT_MARKER);
}

export function isMesacodeGoalModelOnlyReminderMessage(message: MesacodeMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
    (isMesacodeGoalContinuationReminderMessage(message) ||
      isMesacodeGoalStateReminderText(textFromMesacodeMessageParts(message.parts)))
  );
}

export function isMesacodeModelOnlySyntheticUserMessage(message: MesacodeMessageWithParts): boolean {
  if (
    message.info.role === "user" &&
...
export function isMesacodeCompactSummaryMessage(message: MesacodeMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
...
export function getMesacodeUserVisibleMessages(
  messages: readonly MesacodeMessageWithParts[],
  _options: { target?: MesacodeSessionGoal | null } = {},
): MesacodeMessageWithParts[] {
  const visibleMessages: MesacodeMessageWithParts[] = [];
  for (const message of messages) {
    if (isMesacodeModelOnlySyntheticUserMessage(message) || isMesacodeCompactSummaryMessage(message)) {
...
export function getMesacodeGoalIterationByAssistantMessageId(
  messages: readonly MesacodeMessageWithParts[],
  options: { maxGoalIteration?: number; target?: MesacodeSessionGoal | null } = {},
): Map<string, number> {
  ...
  const sortedMessages = [...messages].sort(compareMesacodeMessagesByCreatedTime);
  ...
  for (const message of sortedMessages) {
    if (message.info.role === "user") {
      if (isMesacodeGoalContinuationReminderMessage(message)) {
...
export interface MesacodeGoalIterationCountTimelineItem {
  goalIteration?: number;
  status: "started" | "completed" | "failed_closed" | "cancelled";
  verification?: { passed?: boolean | null } | null;
}

export function getMesacodeGoalActiveIterationCount(input: {
  targetStatus?: string | null;
  timeline?: readonly MesacodeGoalIterationCountTimelineItem[] | null;
}): number {
...
function compareMesacodeMessagesByCreatedTime(
  left: MesacodeMessageWithParts,
  right: MesacodeMessageWithParts,
) {
...
function isVisibleRealGoalUserMessage(message: MesacodeMessageWithParts, target: MesacodeSessionGoal) {
  if (
    message.info.role !== "user" ||
...
export function resolveMesacodeVisibleSessionTitle(input: {
  title?: string;
  messages: readonly MesacodeMessageWithParts[];
  target?: MesacodeSessionGoal | null;
  fallback?: string;
}): string {
  const normalizedTitle = input.title?.trim() ?? "";
  if (
    normalizedTitle &&
    !isMesacodeGoalContinuationReminderText(normalizedTitle) &&
    !isMesacodeGoalStateReminderText(normalizedTitle)
  ) {
  ...
  const firstUser = input.messages.find(
    (message) =>
      message.info.role === "user" &&
      getConversationMessageProjectionPolicy(message) === "realUserInput" &&
      !isMesacodeGoalModelOnlyReminderMessage(message),
  );
  const firstUserText = textFromMesacodeMessageParts(firstUser?.parts ?? []).trim();
  if (firstUserText) {
