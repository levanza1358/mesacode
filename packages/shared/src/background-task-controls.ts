export type MesacodeBackgroundTaskControlStatus =
  | "pending"
  | "running"
  | "completed"
  | "failed"
...
export interface MesacodeBackgroundTaskControlItem {
  jobId: string;
  toolCallId?: string;
  command: string;
  taskKind: "agent" | "bash";
  cancellable?: boolean;
  title?: string;
  status: MesacodeBackgroundTaskControlStatus;
  startedAt?: number;
  elapsedMs?: number;
  pid?: number;
  stdoutTail?: string;
...
const ACTIVE_BACKGROUND_TASK_CONTROL_STATUSES = new Set<MesacodeBackgroundTaskControlStatus>([
  "running",
]);
...
type BackgroundTaskControlKind = MesacodeBackgroundTaskControlItem["taskKind"];
 ...
 */
export function resolveMesacodeBackgroundTaskControlKind(
  value: unknown,
): BackgroundTaskControlKind | undefined {
...
function normalizeStatus(value: string | undefined): MesacodeBackgroundTaskControlStatus {
...
function readJobId(record: Record<string, unknown>, command: string): string {
  const explicitId = readStringField(record, [
    // Mesacode Protocol 后台任务取消入口按 taskId 查找 runtime task 记录；
...
export function parseMesacodeBackgroundTaskControlItems(
  value: unknown,
): MesacodeBackgroundTaskControlItem[] {
  if (!Array.isArray(value)) {
    return [];
  }
  const jobsById = new Map<string, MesacodeBackgroundTaskControlItem>();
  for (const item of value) {
    if (!isPlainRecord(item)) {
      continue;
    }
    const taskKind = resolveMesacodeBackgroundTaskControlKind(item);
    if (!taskKind) {
      continue;
    }
    ...
    const job: MesacodeBackgroundTaskControlItem = {
      jobId,
...
export function isActiveMesacodeBackgroundTaskControlItem(
  job: MesacodeBackgroundTaskControlItem,
): boolean {
  return ACTIVE_BACKGROUND_TASK_CONTROL_STATUSES.has(job.status);
}

export function getMesacodeBackgroundTaskControlItemElapsedMs(
  job: MesacodeBackgroundTaskControlItem,
  now = Date.now(),
): number {
...
export function collectVisibleMesacodeBackgroundTaskControlItems(
  jobs: readonly MesacodeBackgroundTaskControlItem[],
  now = Date.now(),
  thresholdMs = 30_000,
): Array<MesacodeBackgroundTaskControlItem & { elapsedMs: number }> {
  return jobs
    .filter(isActiveMesacodeBackgroundTaskControlItem)
    .map((job) => ({
      ...job,
      elapsedMs: getMesacodeBackgroundTaskControlItemElapsedMs(job, now),
    }))
