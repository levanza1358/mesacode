...
export type FeedbackTicketFramework = "mesacode-agent";
...
export interface FeedbackDeviceInfo {
  ...
  osVersion?: string;
  osArch?: string;
  /** 提交反馈时的当前 Agent。单 Mesacode Agent 模式下固定为 Mesacode Agent。 */
  agentProvider?: string;
  /** 提交反馈时归一到反馈平台的框架标识，当前固定为 mesacode-agent。 */
...
export const DEFAULT_FEEDBACK_TICKET_FRAMEWORK: FeedbackTicketFramework = "mesacode-agent";

export const FEEDBACK_TICKET_FRAMEWORK_OPTIONS: {
  value: FeedbackTicketFramework;
  label: string;
}[] = [{ value: "mesacode-agent", label: "Mesacode Agent" }];
