import type { MesacodeApiRetryStatus } from "./mesacode-task-types-core.js";

export function normalizeMesacodeApiRetryStatus(
  value: unknown,
...
export function mesacodeApiRetryFromModelNetworkStatusPayload(
  payload: Record<string, unknown>,
): MesacodeApiRetryStatus | null | undefined {
  const type = stringValue(payload.type);
  if (type === "model_retry_scheduled") {
    return normalizeMesacodeApiRetryStatus(payload);
  }
  if (type === "model_request_started") {
    const streamRecoveryRetry = mesacodeApiRetryFromStreamRecoveryPayload(payload.streamRecovery);
...
export function isMesacodeModelRetryRecoveryProgressPayload(
  payload: Record<string, unknown>,
): boolean {
...
export function mesacodeApiRetryFromStreamRecoveryPayload(
  value: unknown,
): MesacodeApiRetryStatus | undefined {
  const record = asRecord(value);
