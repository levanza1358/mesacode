export type MesacodeAgentBinaryKind = "native-binary";

export interface MesacodeAgentRuntimeDescriptor {
  binaryKind: MesacodeAgentBinaryKind;
  binaryEnvVar: string;
  ...
  /**
   * 桌面端把 agent 的 JS bundle（mesacode.cjs）打进 resources/glm，由 app 内置的 Electron Node runtime
...
export const MESACODE_AGENT_RUNTIME: MesacodeAgentRuntimeDescriptor = {
  binaryKind: "native-binary",
  binaryEnvVar: "GLM_BINARY_PATH",
  bundledResourceDir: "glm",
  version: "0.13.3",
  spawnArgs: ["app-server", "--stdio"],
  nativeConfigDir: ".mesacode/cli",
  nativeConfigFileName: "config.json",
  missingBinaryMessage:
    "[Mesacode Agent] glm binary 未找到，请设置 GLM_BINARY_PATH 或先准备 GLM 运行时资源",
  resolveEntrySegments: (platform) => [resolvePlatformBinaryName("mesacode-agent", platform)],
  nodeBundleEntryFile: "mesacode.cjs",
  resolveNodeBundleSegments() {
    return [this.nodeBundleEntryFile];
  },
};

export function getMesacodeAgentRuntime(): MesacodeAgentRuntimeDescriptor {
  return MESACODE_AGENT_RUNTIME;
}
