...
import { z } from "zod";
import type { MesacodeTaskMigrationSource, MesacodeTaskMode } from "./mesacode-task-types-core.js";
import { mesacodeAgentProviderSchema } from "./mesacode-agent-policy.js";
import { mesacodePermissionResponseSchema } from "./mesacode-protocol-legacy-types.js";
// merge 冲突解决：两侧分别在相邻行新增独立 import（本分支 hook trust review
// 决策 schema、staging telemetry error attribution schema），二者无语义交集，均保留。
import { workspaceHookReviewDecisionSchema } from "./mesacode-protocol-v4/workspace-hook-review.js";
import { errorAttributionSchema } from "./mesacode-protocol-v4/snapshot.js";

const nonEmptyString = z.string().trim().min(1);
const mesacodeTaskModeRealtimeValues = [
  "yolo",
  "plan",
  "edit",
  "auto",
  "autoEdit",
  "build",
] as const satisfies readonly MesacodeTaskMode[];
const mesacodeTaskMigrationSourceRealtimeValues = [
  "claudeCode",
] as const satisfies readonly MesacodeTaskMigrationSource[];
const mesacodeTaskChangeSummaryRealtimeSchema = z
  .object({
...
const taskMetaRealtimeSchema = z.object({
  ...
  // schema 推导类型因此无法回到 MesacodeTaskMeta，host typecheck 也就无法覆盖这条链路。
  mode: z.enum(mesacodeTaskModeRealtimeValues),
  model: z.string().optional(),
  runtimeEpoch: z.number().int().nonnegative().optional(),
  provider: mesacodeAgentProviderSchema.optional(),
  migrationSource: z.enum(mesacodeTaskMigrationSourceRealtimeValues).optional(),
  ...
  lastError: z
    ...
    .optional(),
  changeSummary: mesacodeTaskChangeSummaryRealtimeSchema.optional(),
});
...
export const workspaceTaskListInvalidatedEventSchema = taskRealtimeInvalidationBaseEventSchema
  ...
  .strict();
const mesacodePromptAttachmentSchema = z.discriminatedUnion("kind", [
  z
    .object({
...
const mesacodeTaskRuntimeCommandBaseSchema = z
  .object({
  ...
  .strict();
const mesacodeTaskRuntimeCommandSchema = z.discriminatedUnion("type", [
  mesacodeTaskRuntimeCommandBaseSchema
    .extend({
...
const taskEnqueueCommandOwnerCommandRequestSchema = taskOwnerCommandBaseSchema
  .extend({
    ...
    taskCommand: mesacodeTaskRuntimeCommandSchema,
  })
  .strict();
...
export const taskOwnerCommandResultSchema = z.discriminatedUnion("success", [
  z
    .object({
      ...
      taskCommand: mesacodeTaskRuntimeCommandSchema.optional(),
    })
    .strict(),
  z
    .object({
