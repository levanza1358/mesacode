...
export function buildNativeSupplierKey(mesacodeProvider: MesacodeProvider): string {
  return `${NATIVE_SUPPLIER_KEY_PREFIX}${mesacodeProvider}`;
}
...
export function buildGhostSupplierKey(
  mesacodeProvider: MesacodeProvider,
...
export function resolveSupplierKeyFromModelDisplayValue(
  ...
  return buildNativeSupplierKey(mesacodeProvider);
}
