import type { MesacodePersistedMessage, MesacodePersistedMessagePart } from "./mesacode-task-types-core.js";

// Mesacode runtime 把同一个 user turn 里**每一轮 LLM 调用**落成独立的 assistant 消息（各带 time.created/completed）。
...
export function coalesceConsecutiveMesacodeAssistants(
  messages: readonly MesacodePersistedMessage[],
): MesacodePersistedMessage[] {
  const merged: MesacodePersistedMessage[] = [];
...
function mergeAssistantPair(
  first: MesacodePersistedMessage,
  next: MesacodePersistedMessage,
): MesacodePersistedMessage {
  ...
  const shiftedNextParts: MesacodePersistedMessagePart[] = (next.parts ?? []).map((part) =>
    part.type === "tool-call"
