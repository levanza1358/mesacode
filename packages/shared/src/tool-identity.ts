export const MESACODE_KNOWN_TOOL_NAMES = [
  "Read",
  "Write",
  "Edit",
  "ApplyPatch",
...
export type MesacodeKnownToolName = (typeof MESACODE_KNOWN_TOOL_NAMES)[number];

export type MesacodeToolFamily =
  | "file-read"
  | "file-write"
  | "shell"
  | "search"
...
const TOOL_FAMILY_BY_NAME: Record<MesacodeKnownToolName, MesacodeToolFamily> = {
  Read: "file-read",
  Write: "file-write",
  Edit: "file-write",
...
const TOOL_NAME_BY_LOWER = new Map<string, MesacodeKnownToolName>(
  MESACODE_KNOWN_TOOL_NAMES.map((toolName) => [toolName.toLowerCase(), toolName]),
);

export function normalizeMesacodeToolName(
  value: string | null | undefined,
): MesacodeKnownToolName | null {
...
export function getMesacodeToolFamilyForName(
  value: string | null | undefined,
): MesacodeToolFamily | null {
  const toolName = normalizeMesacodeToolName(value);
  return toolName ? TOOL_FAMILY_BY_NAME[toolName] : null;
}

export function isMesacodeToolFamily(
  value: string | null | undefined,
  family: MesacodeToolFamily,
): boolean {
  return getMesacodeToolFamilyForName(value) === family;
}

export function isMesacodeFileContentWriteToolName(value: string | null | undefined): boolean {
  return normalizeMesacodeToolName(value) === "Write";
}
