...
export type WorkspaceHookConfigFileKind = "mesacode.json" | ".mesacode/config.json" | "explicit";
...
function buildWorkspaceHookCandidatePaths(directories: readonly string[]): string[] {
  return directories.flatMap((directory) => [
    join(directory, "mesacode.json"),
    join(directory, ".mesacode", "config.json"),
  ]);
}
...
export function createWorkspaceHookSourceInput(input: {
  ...
  return {
    canonicalPath,
    baseDir: basename(configDirectory) === ".mesacode" ? dirname(configDirectory) : configDirectory,
    discoveryOrder: input.discoveryOrder,
    configFileKind: explicitProjectConfig
      ? "explicit"
      : basename(canonicalPath) === "mesacode.json"
        ? "mesacode.json"
        : ".mesacode/config.json",
    explicitProjectConfig,
    editable:
      !explicitProjectConfig &&
      canonicalPath === resolve(input.workingDirectory, ".mesacode", "config.json"),
    hooks: input.hooks,
  };
}
