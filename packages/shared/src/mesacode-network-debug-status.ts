import type {
  ...
  TraceId,
  MesacodeStreamEvent,
  MesacodeTaskNetworkDebugStatusType,
} from "./mesacode-task-types-core.js";

const MODEL_NETWORK_STATUS_TYPES = new Set<MesacodeTaskNetworkDebugStatusType>([
...
export function mesacodeTaskNetworkDebugStatusFromPayload(params: {
  taskId: string;
  traceId: TraceId;
  inputId?: InputId;
  queryId?: QueryId;
...
}): Extract<MesacodeStreamEvent, { type: "task_network_debug_status" }> | null {
...
function networkStatusTypeValue(value: unknown): MesacodeTaskNetworkDebugStatusType | undefined {
  return typeof value === "string" && MODEL_NETWORK_STATUS_TYPES.has(value as never)
    ? (value as MesacodeTaskNetworkDebugStatusType)
    : undefined;
}
