...
import { z } from "zod";
import { mesacodeProcessDiagnosticSchema } from "./process-diagnostic.js";
...
import { mesacodeProviderSchema } from "./providers.js";
import { mesacodeAgentProviderSchema } from "./mesacode-agent-policy.js";
...
} from "./mesacode-protocol/index.js";
...
export const helloAckMessageSchema = z.object({
  type: z.literal("mesacode-hello-ack"),
  version: z.string(),
...
export const hostInitLocalMessageSchema = z.object({
  ...
  mesacodeBuiltinProviderConfigFilePath: nonEmptyStringSchema,
...
export const hostOffPeakRunMessageSchema = z.object({
  ...
  workspaceIdentity: z.string().optional(),
  prompt: nonEmptyStringSchema,
  // 权限四档映射现有 MesacodeTaskMode；与 cron-run 的 mode 同样按宽松 string 传输
...
export { mesacodeProviderSchema };

export const mesacodeTaskMigrationSourceSchema = z.enum(["claudeCode"]);
...
export const hostAgentProcessSpawnedResponseSchema = z.object({
  ...
  provider: mesacodeProviderSchema,
  workspacePath: nonEmptyStringSchema,
  command: z.string(),
...
export const hostAgentProcessExceptionResponseSchema = z
  .object({
    ...
    runtimeGeneration: z.number().int().positive(),
    runtimeInstanceId: nonEmptyStringSchema,
    diagnostic: mesacodeProcessDiagnosticSchema,
  })
  .strict();
...
export const agentLaneResourceSampleSchema = mesacodeProcessResourceSampleSchema
...
export const nodeSelfResourceSampleSchema = z
  .object({
     ...
     * 上限刻意放宽（与 CLI 的 `mesacodeProcessResourceSampleSchema` 同口径）：读数异常时宁可让
...
export const hostMcpResourceSamplesResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    environmentKey: resourceTelemetryEnvironmentKeySchema.optional(),
    samples: mesacodeMcpResourceSamplesSchema,
  })
  .strict();
...
export const hostToolExecResourceResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    sample: mesacodeToolExecResourceSchema,
  })
  .strict();
...
export const hostMcpTelemetryResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    event: mesacodeMcpTelemetryEventSchema,
  })
  .strict();
...
export const mesacodeTaskPersistStatusSchema = z.enum(["running", "completed", "error"]);

export const mesacodePromptImageAttachmentSchema = z.object({
...
export const mesacodePromptVideoAttachmentSchema = z.object({
...
export const mesacodePromptPdfAttachmentSchema = z.object({
  kind: z.literal("pdf"),
  filename: z.string(),
  mimeType: z.string(),
  sizeBytes: z.number().int().nonnegative().optional(),
...
export const mesacodePromptFileAttachmentSchema = z.object({
  kind: z.literal("file"),
  filename: z.string(),
  mimeType: z.string(),
...
export const mesacodePromptAttachmentSchema = z.discriminatedUnion("kind", [
  mesacodePromptImageAttachmentSchema,
  mesacodePromptVideoAttachmentSchema,
  mesacodePromptPdfAttachmentSchema,
  mesacodePromptFileAttachmentSchema,
]);

export const mesacodePersistedToolCallSchema = z.object({
...
const mesacodePersistedMessagePartSchema = z.discriminatedUnion("type", [
...
export const mesacodePersistedMessageSchema = z.object({
  ...
  attachments: z.array(mesacodePromptAttachmentSchema).optional(),
  tools: z.array(mesacodePersistedToolCallSchema).optional(),
  thought: z.string().optional(),
  parts: z.array(mesacodePersistedMessagePartSchema).optional(),
...
export const mesacodeTaskGoalStatusSchema = z.enum(["active", "paused", "budget_limited", "complete"]);

export const mesacodeTaskTargetChangedActionSchema = z.enum([
  "set",
  "status_updated",
  "cleared",
  "usage_accounted",
...
export const mesacodeTaskTargetChangedSourceSchema = z.enum(["command", "tool", "runtime"]);

export const mesacodeTaskGoalSchema = z.object({
  sessionID: nonEmptyStringSchema,
  ...
  status: mesacodeTaskGoalStatusSchema,
...
export const mesacodeTaskGoalChangedPatchSchema = z.object({
  action: mesacodeTaskTargetChangedActionSchema,
  source: mesacodeTaskTargetChangedSourceSchema,
  target: mesacodeTaskGoalSchema.nullable(),
  previousTarget: mesacodeTaskGoalSchema.nullable().optional(),
});

export const mesacodeTaskTargetStatusSchema = mesacodeTaskGoalStatusSchema;
export const mesacodeTaskTargetSchema = mesacodeTaskGoalSchema;
export const mesacodeTaskTargetChangedPatchSchema = mesacodeTaskGoalChangedPatchSchema;

export const mesacodeTaskMetaSchema = z.object({
  taskId: nonEmptyStringSchema,
  ...
  mode: mesacodeTaskModeSchema,
  model: z.string().optional(),
  thoughtLevel: nonEmptyStringSchema.optional(),
  runtimeEpoch: z.number().int().nonnegative().optional(),
  provider: mesacodeAgentProviderSchema.optional(),
  migrationSource: mesacodeTaskMigrationSourceSchema.optional(),
  ...
  status: mesacodeTaskPersistStatusSchema.optional(),
  lastError: z
    .object({
  ...
  changeSummary: z
    ...
    .optional(),
  target: mesacodeTaskGoalSchema.nullable().optional(),
});

export const mesacodeTaskIndexEntrySchema = z.object({
  workspaceHash: nonEmptyStringSchema,
  taskId: nonEmptyStringSchema,
});

export const mesacodePinnedTasksFileSchema = z.object({
  version: z.literal("1"),
  tasks: z.array(mesacodeTaskIndexEntrySchema),
});

const mesacodePersistedFileSnapshotSchema = z.object({
  path: z.string(),
...
const mesacodePersistedFileChangeSchema = z.object({
  turnIndex: z.number().int().nonnegative(),
  snapshots: z.array(mesacodePersistedFileSnapshotSchema),
  fileState: z.enum(["applied", "reverted"]).optional(),
});

const mesacodePersistedTurnCheckpointSchema = z.object({
  turnIndex: z.number().int().nonnegative(),
  baseFileCheckpointId: nonEmptyStringSchema,
  resultFileCheckpointId: nonEmptyStringSchema.optional(),
});

export const mesacodeSessionFileSchema = z.object({
  meta: mesacodeTaskMetaSchema,
  messages: z.array(mesacodePersistedMessageSchema),
  fileChanges: z.array(mesacodePersistedFileChangeSchema).optional(),
  turnCheckpoints: z.array(mesacodePersistedTurnCheckpointSchema).optional(),
});
