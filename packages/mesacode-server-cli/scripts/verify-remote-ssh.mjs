 ...
 *
 * 前置：docker 可用；已运行 `pnpm --filter @mesacode/server-cli stage --target <target>`。
...
const HOST_CAPABILITY_HEADER = "x-mesacode-rpc-host-capability";
...
const releaseArchive = join(packageRoot, "dist-release", `mesacode-server-${target}.tar.gz`);
const containerName = `mesacode-server-verify-${process.pid}`;
const imageTag = "mesacode-server-verify-sshd:ubuntu22";
...
async function main() {
  ...
  await run("ls", [releaseArchive], { quiet: true }).catch(() => {
    throw new Error(
      `Release archive missing: ${releaseArchive}; run pnpm --filter @mesacode/server-cli stage --target ${target}`,
    );
  });

  // 一次性 ssh 密钥，容器仅信任本次运行生成的公钥。
  const workDir = await mkdtemp(join(tmpdir(), "mesacode-server-verify-"));
  ...
  await ssh(
    `mkdir -p /root/mesacode-server && tar -xzf /root/mesacode-server-${target}.tar.gz -C /root/mesacode-server --strip-components=1`,
  );

  log("start daemon on remote");
  const { stdout: daemonOutput } = await ssh("/root/mesacode-server/bin/mesacode serve --daemon --json");
  ...
  await ssh(
    "cd /root/mesacode-server/runtime && ./node -e \"const pty=require('node-pty');const p=pty.spawn('/bin/echo',['pty-ok'],{cols:80,rows:24});let o='';p.onData(d=>o+=d);p.onExit(()=>{process.exit(o.includes('pty-ok')?0:1)})\"",
  );
  log("node-pty ok");

  log("verify remote lifecycle status/stop");
  const { stdout: statusOutput } = await ssh("/root/mesacode-server/bin/mesacode status --json");
  assert(
    JSON.parse(statusOutput.trim().split("\n").pop()).state === "ready",
    "remote status ready",
  );
  await ssh("/root/mesacode-server/bin/mesacode stop --json");
  const { stdout: stoppedOutput } = await ssh("/root/mesacode-server/bin/mesacode status --json");
