...
import { createServiceLogger } from "@mesacode/services/node";
import {
...
function exposeWebSocket(
  ...
  services.exposeOnChannelServer(
    server,
    scope ? new Map([[IMesacodeAgentService.channelName, scope.service]]) : new Map(),
  );
  socket.onClose(() => {
    void scope?.dispose();
    rawServer.dispose();
...
export async function createCoreHttpServer(
  ...
  const info: ServerRemoteInfo = {
    serverId: options.serverId ?? hostname() ?? "mesacode-server",
  ...
  app.use("/ws/host", async (context, next) => {
    const capability = context.req.header(MESACODE_RPC_HOST_CAPABILITY_HEADER);
