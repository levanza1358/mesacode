...
import { IMesacodeAgentService } from "@mesacode/services";
...
export async function runServerCore(generation: number): Promise<void> {
  ...
  const explicitMesacodeBuiltinProviderConfigFilePath =
    process.env[MESACODE_BUILTIN_PROVIDER_CONFIG_FILE_ENV]?.trim();
  const mesacodeBuiltinProviderConfigFilePath = explicitMesacodeBuiltinProviderConfigFilePath
    ? explicitMesacodeBuiltinProviderConfigFilePath
    : typeof __MESACODE_BUILTIN_PROVIDER_CONFIG_JSON__ === "string"
      ? await materializeMesacodeBuiltinProviderConfig({
          environmentConfigRoot: getAppConfigDir(),
          content: __MESACODE_BUILTIN_PROVIDER_CONFIG_JSON__,
        })
      : undefined;
  if (!mesacodeBuiltinProviderConfigFilePath) {
    throw new Error(
      `当前构建未嵌入 Mesacode Built-in Provider Config，且未设置 ${MESACODE_BUILTIN_PROVIDER_CONFIG_FILE_ENV}`,
    );
  }
  ...
  const taskActivityTracker = createTaskActivityTracker(services.getOptional(IMesacodeAgentService));
