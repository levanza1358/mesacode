...
function getDefaultServerDataRoot(): string {
  const configured = process.env.MESACODE_DATA_BASE_DIR?.trim();
  return join(configured || homedir(), ".mesacode", "server");
}
  ...
  return {
    ...
    controlEndpoint:
      process.platform === "win32"
        ? `\\\\.\\pipe\\mesacode-server-${stablePathId(root)}`
...
function inferDataBaseDir(serverRoot: string): string {
  ...
  if (basename(serverRoot) === "server" && basename(parent) === ".mesacode") {
