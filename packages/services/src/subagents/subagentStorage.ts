...
export async function resolveUserSubagentRoot(options?: SubagentStorageOptions): Promise<string> {
  return join(await resolveMesacodeStorageRoot(options), "agents");
}

export function resolveWorkspaceSubagentRoot(workspacePath: string): string {
  return join(workspacePath, ".mesacode", "agents");
}

export async function resolveSubagentStateFile(options?: SubagentStorageOptions): Promise<string> {
  return join(await resolveMesacodeStorageRoot(options), "v2", "agents-state.json");
}

export async function resolveMesacodeStorageRoot(options?: SubagentStorageOptions): Promise<string> {
...
async function readUserCliConfig(
  ...
  try {
    const raw = await readFile(
      join(resolveUserHomeDir(options), ".mesacode", "cli", "config.json"),
      "utf8",
    );
