...
const PLUGIN_MANIFEST_PATHS = [
  join(".mesacode-plugin", "plugin.json"),
...
async function discoverPluginAgents(params: {
  ...
  const storageRoot = await resolveMesacodeStorageRoot(params.storageOptions);
...
async function readPluginConfig(options?: SubagentStorageOptions): Promise<PluginConfigSummary> {
  try {
    const configPath = join(resolveUserHomeDir(options), ".mesacode", "cli", "config.json");
...
async function readEnabledPluginRecords(
  ...
  const records = installed.filter(
    (record) =>
      ...
      !(
        record.id.endsWith(`@${MESACODE_OFFICIAL_PLUGIN_MARKETPLACE_ID}`) &&
  ...
  for (const cacheRoot of await scanOfficialPluginCacheRoots(pluginStorageRoot)) {
    const id = `${cacheRoot.name}@${MESACODE_OFFICIAL_PLUGIN_MARKETPLACE_ID}`;
...
export function createSubagentsService(options?: SubagentsServiceOptions): ISubagentsService & {
  ...
  return {
    ...
    async getPrimaryUserAgentsDirectory(_params: { provider: MesacodeProvider }): Promise<{
      path: string;
    }> {
