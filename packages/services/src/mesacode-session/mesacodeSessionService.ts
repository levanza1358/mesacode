/* eslint-disable max-lines -- mesacodeSessionService 聚合 desktop-continuous session 操作、draft lifecycle 和 task index 同步边界，拆分需要单独设计。 */
import type { IMesacodeAgentService } from "#src/mesacode-agent/mesacodeAgent.js";
import type { MesacodeTaskIndexSyncer } from "#src/mesacode-agent/mesacodeTaskIndexSyncer.js";
...
import type {
  ...
  MesacodeSessionReadWorkspacePresentationParams,
...
import { formatModelPickerValue } from "#src/mesacode-agent/mesacodeConfigOptions.js";
import { createMesacodeSessionApiRetryRuntimeTracker } from "#src/mesacode-session/mesacodeSessionApiRetry.js";
import { appendWorkspaceToFilesystemMcpServers } from "#src/session/mcpWorkspaceScope.js";
import { repairEmptyImportedClaudeSessionSnapshot } from "#src/mesacode-session/importedClaudeSessionRepair.js";
import { createMesacodeDeferredDraftRegistry } from "#src/mesacode-session/mesacodeSessionDraftRegistry.js";
...
interface CreateMesacodeSessionServiceOptions {
...
export function createMesacodeSessionService({
  agentService,
  taskIndexSyncer,
  cuaProductMcpServerResolver,
}: CreateMesacodeSessionServiceOptions): IMesacodeSessionService {
  const { withApiRetryRuntime } = createMesacodeSessionApiRetryRuntimeTracker();
  const deferredDraftSessions = createMesacodeDeferredDraftRegistry();

  function notifySyncer(
    target: MesacodeTaskTarget,
    options?: {
  ...
  function getSessionSnapshotDiagnostics(snapshot: MesacodeSessionStateSnapshot) {
  ...
  // mesacodeSessionService 是 desktop-continuous 主路径，状态变更后必须
  ...
  async function broadcastSnapshot(
    ...
    try {
      ...
      logger.warn(
        undefined,
        `[mesacode-session-service] ${tag} syncSnapshotAndBroadcast 失败 taskId=${snapshot.session.sessionId}`,
        error,
      );
    }
  }
  ...
  async function repairEmptyImportedClaudeSession(
    snapshot: MesacodeSessionStateSnapshot,
    params: MesacodeSessionResumeParams | MesacodeSessionReadParams,
  ...
  async function withResolvedMcpServers<
    T extends MesacodeSessionCreateParams | MesacodeSessionResumeParams,
  >(params: T): Promise<T> {
  ...
  return {
    async initializeWorkspace(params: MesacodeSessionWorkspaceTarget) {
    ...
    getWorkspaceRuntimeIdentity(params: MesacodeSessionWorkspaceTarget) {
      return agentService.getWorkspaceRuntimeIdentity(params);
    },

    readWorkspacePresentation(params: MesacodeSessionReadWorkspacePresentationParams) {
      return agentService.readWorkspacePresentation(params);
    },

    async createSession(params: MesacodeSessionCreateParams) {
      ...
      logger.info(sessionTraceId, "[mesacode-session-service] createSession 分配 session trace", {
      ...
      logger.info(sessionTraceId, "[mesacode-session-service] createSession agent 返回", {
      ...
      // desktop-continuous 路径不会经过 Mesacode task adapter，sqlite 的 task index 全靠
      ...
      logger.info(sessionTraceId, "[mesacode-session-service] createSession task index 同步完成", {
    ...
    async resumeSession(params: MesacodeSessionResumeParams) {
      const startedAt = Date.now();
      ...
      if (
        ...
        if (
          ...
          logger.warn(
            undefined,
            "[mesacode-session-service] resumeSession 跳过不支持的 task 思考强度",
            {
      ...
      if (thoughtLevelOverride && snapshot.settings.thoughtLevel.current !== thoughtLevelOverride) {
        logger.info(undefined, "[mesacode-session-service] resumeSession 重放 task 思考强度", {
      ...
      logger.info(undefined, "[mesacode-session-service] resumeSession 历史恢复完成", {
        agentDurationMs,
    ...
    listSessions(params: MesacodeSessionListParams) {
      return agentService.listSessions(params);
    },

    async readSession(params: MesacodeSessionReadParams) {
      const startedAt = Date.now();
      const snapshot = await repairEmptyImportedClaudeSession(
        withApiRetryRuntime(await agentService.readSession(params)),
        params,
      );
      logger.info(undefined, "[mesacode-session-service] readSession 历史快照读取完成", {
    ...
    readSessionMessages(params: MesacodeSessionMessagesParams) {
      return agentService.readSessionMessages(params);
    },

    readSessionEvents(params: MesacodeSessionEventsParams) {
      return agentService.readSessionEvents(params);
    },

    promoteDeferredDraftSession(params: MesacodeTaskTarget) {
      ...
      notifySyncer(params);
      logger.info(undefined, "[mesacode-session-service] deferred draft session 已提升为 task", {
        sessionId: params.sessionId,
        workspaceIdentity: params.workspaceIdentity ?? null,
        workspacePath: params.workspacePath,
      });
    ...
    closeSession(params: MesacodeTaskTarget) {
      deferredDraftSessions.forget(params);
      return agentService.closeSession(params).then(() => undefined);
    },

    async closeDeferredDraftSession(params: MesacodeTaskTarget) {
      try {
        ...
        logger.warn(
          undefined,
          "[mesacode-session-service] 条件关闭 deferred draft 失败，保留旧 session",
          {
    ...
    async setModel(params: MesacodeSessionSetModelParams) {
    ...
    async setThoughtLevel(params: MesacodeSessionSetThoughtLevelParams) {
      return withApiRetryRuntime(await agentService.setThoughtLevel(params));
    },

    async setMode(params: MesacodeSessionSetModeParams) {
