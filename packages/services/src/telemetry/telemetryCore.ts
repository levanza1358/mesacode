...
function sessionCreateEventId(userId: string, sessionId: string): string {
  const bytes = createHash("sha256")
    .update(JSON.stringify(["mesacode:session_create:v1", userId, sessionId]))
    .digest()
    .subarray(0, 16);
...
function resolveTelemetryStateFile(homeDir?: string): string {
  if (homeDir) {
    return join(homeDir, ".mesacode", "v2", "telemetry-state.json");
  }
...
function resolveTelemetryLockFile(homeDir?: string): string {
  if (homeDir) {
    return join(homeDir, ".mesacode", "v2", "telemetry-state.lock");
  }
...
export function createTelemetryCore(dependencies: TelemetryCoreDependencies = {}) {
  ...
  const appVersion = dependencies.appVersion ?? MESACODE_VERSION;
  ...
  async function sendReport(
    ...
    if (!MESACODE_TELEMETRY_ENABLED || !MESACODE_TELEMETRY_REPORT_ENDPOINT) {
      return;
    }
    ...
    const endpoint = String(
      rewriteMesacodeEndpointUrl(
        MESACODE_TELEMETRY_REPORT_ENDPOINT,
        (await dependencies.resolveMesacodeEndpointOrigin?.()) ?? MESACODE_TELEMETRY_REPORT_ENDPOINT,
      ),
    );

    const headers = buildMesacodeSourceHeadersFromContext({
      appVersion,
      platform,
      arch: dependencies.arch ?? process.arch,
      osVersion,
      releaseChannel: dependencies.releaseChannel ?? MESACODE_ENV,
