...
export interface HostResourceUsageAgent {
  ...
  children: readonly MesacodeProcessChildProcess[];
}
...
type Owner =
  ...
  | { kind: "mcp"; child: MesacodeProcessChildProcess }
...
function ownerToRow(
  ...
  if (owner.kind === "agent") {
    return {
      pid: sample.pid,
      name: isOwnerRoot
        ? formatMesacodeAgentProcessName(owner.agent.provider, owner.agent.workspacePath)
