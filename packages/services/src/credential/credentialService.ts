...
async function readAll(credentialsFile = getCredentialsFile()): Promise<Record<string, string>> {
  ...
  try {
    ...
    throw new Error(`Unable to read Mesacode credentials: ${credentialsFile}`, { cause: error });
  }
  ...
  try {
    ...
    throw new Error(`Mesacode credentials are corrupt: ${credentialsFile}`, { cause: error });
  }
}
