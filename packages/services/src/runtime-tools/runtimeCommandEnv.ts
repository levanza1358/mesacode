...
import { buildMesacodeToolEnvPassthroughEnv, sanitizeMesacodeRuntimeEnvInPlace } from "@mesacode/shared";
...
function buildLoginShellEnvPatch(
  ...
  return {
    ...patch,
    ...buildMesacodeToolEnvPassthroughEnv(snapshot),
  };
}
...
export function buildRuntimeProcessEnvPatch(
  ...
  const toolEnvPassthroughPatch = buildMesacodeToolEnvPassthroughEnv({
    ...normalizedBaseEnv,
    ...loginShellEnvPatch,
  });
  ...
  const envPatch: Record<string, string> = {
    ...
    // encoding is set. Mesacode/Bash tool output is consumed as UTF-8, so force Python
...
export function initializeRuntimeProcessEnv(
  ...
  // 网络变量会先封存为 MESACODE_TOOL_ENV_PASSTHROUGH_JSON，只有 Bash/tool 子进程边界才恢复原名。
  sanitizeMesacodeRuntimeEnvInPlace(process.env);
