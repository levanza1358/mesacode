...
import type { GitCheckpointDiff, GitCheckpointFileDiff } from "@mesacode/shared";
...
export function getCheckpointRefName(workspacePath: string, checkpointId: string): string {
  ...
  return `refs/mesacode/checkpoints/${workspaceHash}/${checkpointId}`;
}
...
export function buildCheckpointEnv(tempIndexPath: string): NodeJS.ProcessEnv {
  return {
    ...
    GIT_AUTHOR_NAME: "Mesacode Checkpoint",
    GIT_AUTHOR_EMAIL: "checkpoint@mesacode.local",
    GIT_COMMITTER_NAME: "Mesacode Checkpoint",
    GIT_COMMITTER_EMAIL: "checkpoint@mesacode.local",
  };
}
