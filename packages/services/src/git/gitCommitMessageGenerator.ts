...
interface GitCommitMessageCurrentModelProvider {
  ...
  }): Promise<MesacodeWorkspaceGenerateTextParams["selection"] | null>;
}
...
interface GitCommitMessageTextGenerator {
  generateText(params: {
    ...
    prompt: string;
    querySource: string;
  }): Promise<{ text: string; selection: MesacodeWorkspaceGenerateTextParams["selection"] }>;
}
...
export class GitCommitMessageGenerator {
  ...
  private async resolveCurrentModel(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<MesacodeWorkspaceGenerateTextParams["selection"]> {
