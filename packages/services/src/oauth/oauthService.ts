...
export class OAuthService implements IOAuthService {
  ...
  async restoreCachedSessionState(): Promise<OAuthCachedSessionRestoreResult> {
    ...
    // 启动缓存恢复只需要检查共享 mesacode JWT；若通过 loadActiveTokenSet 连带读取
    ...
    if (mesacodeJwtToken && resolveJwtExpiration(mesacodeJwtToken, this.now()).kind === "expired") {
      serviceLog.info("cached session invalidated because mesacode JWT expired", {
    ...
    if (activeProvider === ZAI_PROVIDER_ID) {
      if (!mesacodeJwtToken) {
        // sidebar 登录入口之前只看缓存 user_info，会把“缺少 mesacodejwttoken”的状态误判成已登录。
        // 这里补充 mesacodejwttoken 门槛，确保没有后端 JWT 时统一按未登录处理。
        log("restoreCachedSession skipped: missing mesacodejwttoken:", activeProvider);
        return { status: "signed-out" };
      }
    }
  ...
  private async invalidateExpiredCachedSession(
    ...
    const invalidated = await this.runSessionMutation(async () => {
      ...
      const currentJwt = (await this.credentialService.load(MESACODE_JWT_TOKEN_KEY))?.trim() ?? "";
      if (
  ...
  private async persistOAuthSession(
    ...
    if (inactiveProvider) {
      ...
      // 再保存当前 token；反序会让 clearProvider 误删共享的 mesacodejwttoken。
  ...
  async startOAuthWithPolling(provider: OAuthProviderId): Promise<OAuthStartResponse> {
    ...
    const initUrl = buildMesacodeApiUrlFromEnv(this.env, "/api/v1/oauth/cli/init");
    ...
    if (provider === BIGMODEL_PROVIDER_ID) {
      ...
      // flow 仍由 Host 轮询，但浏览器回调恢复到官网中转页，再透传到 mesacode://oauth/callback。
      ...
      // Desktop 统一改写为官网中转页，再由官网透传到 mesacode://oauth/callback。
    ...
    this.pendingState = {
      ...
      polling: {
        ...
        pollIntervalMs,
        pollToken,
        pollUrl: buildMesacodeApiUrlFromEnv(
          this.env,
  ...
  async pollPendingOAuth(): Promise<OAuthCallbackResult | null> {
    ...
    try {
      const result = await this.runPendingSessionCompletion(pending, async () => {
        ...
        const mesacodeJwtToken = readTrimmedString(ready.token);
        const userId = readTrimmedString(user?.user_id);
        if (!mesacodeJwtToken || !providerAccessToken || !userId) {
          throw new Error("OAuth flow 查询响应无效");
        }
