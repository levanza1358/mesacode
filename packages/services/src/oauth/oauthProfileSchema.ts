...
function getCachedProfileSchemaVersion(profile: OAuthUserProfile): number | null {
  ...
  const version = (rawProfile as { mesacodeProfileSchemaVersion?: unknown }).mesacodeProfileSchemaVersion;
...
function getCachedProfileMigrationRetryAfter(profile: OAuthUserProfile): number | null {
  ...
  const retryAfter = (rawProfile as { mesacodeProfileMigrationRetryAfter?: unknown })
    .mesacodeProfileMigrationRetryAfter;
...
export function withProviderProfileSchema(
  ...
  delete nextRawProfile.mesacodeProfileMigrationRetryAfter;

  return {
    ...profile,
    rawProfile: {
      ...nextRawProfile,
      mesacodeProfileSchemaVersion: BIGMODEL_PROFILE_SCHEMA_VERSION,
    },
  };
}
...
function withBigModelProfileMigrationRetryAfter(
  ...
  return {
    ...
    rawProfile: {
      ...
      mesacodeProfileMigrationRetryAfter: now + BIGMODEL_PROFILE_MIGRATION_RETRY_DELAY_MS,
    },
  };
}
...
export async function refreshLegacyBigModelCachedProfile(
  ...
  try {
    ...
    if (isBigModelUserInfoFallback(refreshedProfile)) {
      // 旧版本可能把 mesacode JWT 写进 BigModel access token。
