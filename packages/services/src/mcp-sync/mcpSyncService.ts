...
const MESACODE_MCP_DESCRIPTOR: DirectoryMcpDescriptor = {
  source: "mesacode",
  directorySource: "mesacode",
  userConfigDirSegments: [".mesacode", "cli"],
  workspaceConfigDirSegments: [".mesacode"],
...
function getUserZcodeMcpConfigPath(): string {
  return buildUserConfigPath(MESACODE_MCP_DESCRIPTOR);
}
...
async function collectEffectiveUserMcpRecords(): Promise<UserMcpRecord[]> {
  const mesacodeRecords = await readUserMcpRecordsFromFile(MESACODE_MCP_DESCRIPTOR);
  if (mesacodeRecords.length > 0) {
    return sortMcpRecords(mesacodeRecords);
  }
...
async function saveMcpToUserDirectory(payload: SaveCliMcpToUserDirectoryRequest): Promise<void> {
  if (payload.action === "set-enabled") {
    ...
    const location =
      payload.location ??
      buildDirectoryMcpLocation(MESACODE_MCP_DESCRIPTOR, scope, payload.projectPath);
  ...
  await writeMesacodeServersToFile(scope, nextServers, payload.projectPath);
}
...
async function readDirectoryServersFromPreferredSources(
  ...
  const mesacodeServers = await readDirectoryServersFromFile(
    MESACODE_MCP_DESCRIPTOR,
    scope,
    workspacePath,
  );
  if (mesacodeServers.length > 0) {
    return mesacodeServers;
  }
...
async function writeMesacodeServersToFile(
  scope: Exclude<McpScope, "common">,
  servers: Record<string, Record<string, unknown>>,
  workspacePath?: string,
): Promise<void> {
  const filePath = buildDirectoryConfigPath(MESACODE_MCP_DESCRIPTOR, scope, workspacePath);
  const current = (await readJsonObject(filePath)) ?? {};
  const next = writeServerMapToJson(current, MESACODE_MCP_DESCRIPTOR.configKeyName, servers);
...
async function importMcpServers(params: {
  ...
  const targetServers = readServerMapFromJson(current, MESACODE_MCP_DESCRIPTOR.configKeyName);
  const existingByName = await collectEffectiveUserMcpRecordByName();
  ...
  if (changed) {
    await writeTextAtomic(
      targetPath,
      `${JSON.stringify(writeServerMapToJson(current, MESACODE_MCP_DESCRIPTOR.configKeyName, targetServers), null, 2)}\n`,
    );
  }
