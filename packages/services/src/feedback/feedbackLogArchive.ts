...
import { redactFeedbackText, MESACODE_VERSION, MESACODE_COMMIT } from "@mesacode/shared";
...
export async function createFeedbackDiagnosticArchive(options: {
  ...
  const path = join(outputDir, "mesacode-diagnostic-logs.zip");
  ...
  try {
    ...
    zip.addBuffer(
      Buffer.from(
        [
          "Mesacode diagnostic logs",
          `timestamp: ${now.toISOString()}`,
          `appVersion: ${MESACODE_VERSION}`,
          `commit: ${MESACODE_COMMIT}`,
