/* oxlint-disable eslint(max-lines) -- 已发布旧 Mesacode config.json 的多版 Provider 结构读取集中在同一边界。 */
...
function normalizeBigModelCodingPlanAnthropicBaseUrlForEnv(
  ...
  const fallbackBaseUrl =
    resolveRuntimeMesacodeEnv(env) === "production"
  ...
  if (resolveRuntimeMesacodeEnv(env) === "production") {
...
function getMesacodeConfigFilePath(): string {
...
interface MesacodeConfigFile {
  $schema?: string;
  provider?: Record<string, MesacodeOpenCodeProviderConfig>;
  [key: string]: unknown;
}

interface MesacodeOpenCodeProviderConfig {
  api?: string;
  name?: string;
  env?: string[];
  id?: string;
  ...
  createdAt?: number;
  updatedAt?: number;
  models?: Record<string, MesacodeOpenCodeModelConfig>;
  mesacode?: MesacodeProviderConfigExtension;
  [key: string]: unknown;
}

interface MesacodeOpenCodeModelConfig {
  id?: string;
  name?: string;
  family?: string;
  release_date?: string;
  attachment?: boolean;
  reasoning?: boolean | MesacodeReasoningConfigExtension;
  temperature?: boolean;
  tool_call?: boolean;
  interleaved?: unknown;
  cost?: JsonObject;
  ...
  modified?: boolean;
  deleted?: boolean;
  mesacode?: MesacodeModelConfigExtension;
  [key: string]: unknown;
}

interface MesacodeReasoningConfigExtension {
  enabled?: boolean;
  variants?: string[];
...
interface MesacodeProviderConfigExtension {
  enabled?: boolean;
  systemDisabledReason?: ModelProviderSystemDisabledReason;
  endpoints?: ModelProviderEndpoints;
  apiFormat?: ModelProviderApiFormat;
...
interface MesacodeModelConfigExtension {
  kinds?: ModelProviderKind[];
  defaultKind?: ModelProviderKind;
  modelIdByKind?: Partial<Record<ModelProviderKind, string>>;
  disabledReason?: string;
...
const mesacodeModelProviderModalitySchema = z.enum(["text", "image", "video", "audio", "pdf"]);
const mesacodeProviderModelMappingsSchema = z.record(z.string(), z.unknown());

const mesacodeReasoningConfigExtensionSchema = z
  .object({
...
const mesacodeModelConfigExtensionSchema = z
  .object({
...
const mesacodeOpenCodeModelConfigSchema: z.ZodType<MesacodeOpenCodeModelConfig> = z
  .object({
    id: z.string().optional(),
    name: z.string().optional(),
    reasoning: z.union([z.boolean(), mesacodeReasoningConfigExtensionSchema]).optional(),
    limit: z
      .object({
    ...
    modalities: z
      .object({
        input: z.array(mesacodeModelProviderModalitySchema).optional(),
        output: z.array(mesacodeModelProviderModalitySchema).optional(),
      })
      .optional(),
    ...
    modified: z.boolean().optional(),
    deleted: z.boolean().optional(),
    mesacode: mesacodeModelConfigExtensionSchema.optional(),
  })
  .passthrough();

const mesacodeProviderConfigExtensionSchema = z
  .object({
    enabled: z.boolean().optional(),
    ...
    defaultKind: modelProviderKindSchema.optional(),
    providerMappings: mesacodeProviderModelMappingsSchema.optional(),
...
const mesacodeOpenCodeProviderConfigSchema: z.ZodType<MesacodeOpenCodeProviderConfig> = z
  .object({
    ...
    createdAt: z.number().optional(),
    updatedAt: z.number().optional(),
    models: z.record(z.string(), mesacodeOpenCodeModelConfigSchema).optional(),
    mesacode: mesacodeProviderConfigExtensionSchema.optional(),
  })
  .passthrough();

const mesacodeConfigFileSchema = z
  .object({
    $schema: z.string().optional(),
    provider: z.record(z.string(), mesacodeOpenCodeProviderConfigSchema).optional(),
  })
  .passthrough();
...
function inferModelProviderKindFromOpenCodeProvider(
  provider: MesacodeOpenCodeProviderConfig,
): ModelProviderKind {
  if (provider.kind) {
...
function hasOpenCodeProviderRuntimeFields(provider: MesacodeOpenCodeProviderConfig): boolean {
  // config.json 的权威运行态字段是 kind/options.baseURL。
  // 旧 endpoints/apiFormat/defaultKind/mesacode 可能是历史迁移残留，若继续优先读取会覆盖新配置。
  return Boolean(
    provider.kind ||
...
function resolvePresetProviderKindFromRuntimeBaseUrl(
  ...
  if (path.includes("/api/anthropic") || path.includes("/mesacode-plan/anthropic")) {
    return "anthropic";
  }
  return undefined;
}
...
function resolveOpenCodeProviderEndpoints(
  ...
  return provider.endpoints ?? provider.mesacode?.endpoints ?? {};
}

function stripMesacodePrefixedProviderMappings(
  ...
  const entries = Object.entries(providerMappings ?? {}).filter(
    ([key]) => !key.toLowerCase().startsWith("mesacode"),
  );
  return Object.fromEntries(entries);
}

function openCodeReasoningToModelReasoning(
  model: MesacodeOpenCodeModelConfig,
  ...
  if (!options.preferOpenCodeFields && model.mesacode?.reasoning) {
    return stripModelProviderReasoningPatches(model.mesacode.reasoning);
  }
...
function openCodeModelToModelProviderModel(
  ...
  const mesacode = options.preferOpenCodeFields ? undefined : model.mesacode;
  const mesacodeExtensions = model.mesacode;
  const contextWindow =
  ...
  return createModelProviderModelConfig({
    ...
    priority: readFiniteNumber(model.priority) ?? readFiniteNumber(mesacodeExtensions?.priority),
    disabledReason: model.disabledReason ?? mesacode?.disabledReason,
    supportsTools: model.supportsTools ?? mesacode?.supportsTools,
    supportsStructuredOutput: model.supportsStructuredOutput ?? mesacode?.supportsStructuredOutput,
    modified: model.modified ?? mesacodeExtensions?.modified,
    deleted: model.deleted ?? mesacodeExtensions?.deleted,
  });
}
...
function openCodeProviderToModelProviderConfig(
  ...
  const mesacode = provider.mesacode;
  ...
  for (const deletedModelId of normalizeDeletedModelIds(mesacode?.deletedModels)) {
  ...
  const apiKeyRequired =
    provider.apiKeyRequired ?? mesacode?.apiKeyRequired ?? readBoolean(options.apiKeyRequired);
  const providerMappings = preferOpenCodeFields
    ? undefined
    : stripMesacodePrefixedProviderMappings(provider.providerMappings ?? mesacode?.providerMappings);
  return normalizeProviderForStore({
    ...
    ...(provider.enabled !== undefined
      ...
      : mesacode?.enabled !== undefined
        ? { enabled: mesacode.enabled }
        : {}),
    ...((provider.systemDisabledReason ?? mesacode?.systemDisabledReason)
      ? {
          systemDisabledReason: provider.systemDisabledReason ?? mesacode?.systemDisabledReason,
        }
      : {}),
    endpoints,
    apiFormat: preferOpenCodeFields
      ? resolveModelProviderKindApiFormat(defaultKind)
      : (provider.apiFormat ?? mesacode?.apiFormat ?? resolveModelProviderKindApiFormat(defaultKind)),
    ...((provider.source ?? mesacode?.source)
      ? { source: provider.source ?? mesacode?.source }
      : { source: "custom" }),
    ...((provider.catalogSourceId ?? mesacode?.catalogSourceId)
      ? { catalogSourceId: provider.catalogSourceId ?? mesacode?.catalogSourceId }
      : {}),
    ...((provider.catalogProviderId ?? mesacode?.catalogProviderId)
      ? {
          catalogProviderId: provider.catalogProviderId ?? mesacode?.catalogProviderId,
        }
      : {}),
    ...((provider.modelsDevProviderId ?? mesacode?.modelsDevProviderId)
      ? {
          modelsDevProviderId: provider.modelsDevProviderId ?? mesacode?.modelsDevProviderId,
        }
      : {}),
    ...(apiKeyRequired !== undefined ? { apiKeyRequired } : {}),
    ...((provider.headers ?? mesacode?.headers ?? readStringRecord(options.headers))
      ? {
          headers: provider.headers ?? mesacode?.headers ?? readStringRecord(options.headers),
        }
      : {}),
    ...((provider.logoUrl ?? mesacode?.logoUrl)
      ? { logoUrl: provider.logoUrl ?? mesacode?.logoUrl }
      : {}),
    apiKey,
    ...((provider.apiKeyUrl ?? mesacode?.apiKeyUrl)
      ? { apiKeyUrl: provider.apiKeyUrl ?? mesacode?.apiKeyUrl }
      : {}),
    defaultKind,
    models,
    ...(providerMappings ? { providerMappings } : {}),
    createdAt:
      (!preferOpenCodeFields ? (provider.createdAt ?? mesacode?.createdAt) : undefined) ?? now,
    updatedAt:
      (!preferOpenCodeFields ? (provider.updatedAt ?? mesacode?.updatedAt) : undefined) ?? now,
  });
}

async function readRawMesacodeConfigFile(): Promise<MesacodeConfigFile | null> {
  const filePath = getMesacodeConfigFilePath();
  let raw: string;
  try {
  ...
  const parsed = mesacodeConfigFileSchema.safeParse(json);
  if (!parsed.success) {
...
async function readMesacodeConfigProviders(): Promise<ModelProviderConfig[] | null> {
  const config = await readRawMesacodeConfigFile();
  if (!config?.provider) {
    return null;
  }
...
function normalizeProviderForStore(provider: ModelProviderConfig): ModelProviderConfig {
  ...
  return {
    ...
    providerMappings: stripMesacodePrefixedProviderMappings(
...
function applyProviderStoreMigrations(providers: ModelProviderConfig[]): ModelProviderConfig[] {
  ...
  // reasoning mapping 由 Mesacode Built-in/Personal ModelConfigRules 在 Registry 中解析；
...
export async function readLegacyMesacodeConfigProviders(): Promise<ModelProviderConfig[]> {
  const configProviders = await readMesacodeConfigProviders();
