import { BIGMODEL_PROVIDER_ID } from "@mesacode/shared";
...
export async function resolveBigModelStartPlanZcodeJwt(params: {
  ...
  if (params.trustCachedZcodeJwt === true || activeProvider === BIGMODEL_PROVIDER_ID) {
    const credentialJwt = (await params.credentialService?.load(MESACODE_JWT_TOKEN_KEY))?.trim() || "";
    if (credentialJwt) {
      return credentialJwt;
    }
  }

  // mesacode JWT 必须在 BigModel OAuth callback 阶段用授权码 body 落盘。
