...
interface TeamPlanRequestKeyDependencies {
  ...
  readonly access: Extract<MesacodeAccountAccess, { planKind: "team-coding-plan" }>;
}
...
export async function resolveAccountTeamPlanRuntimeApiKey(
  ...
  const mesacodeJwtToken = (await params.credentialService?.load(MESACODE_JWT_TOKEN_KEY))?.trim() ?? "";
  if (family === "bigmodel" && mesacodeJwtToken && token === mesacodeJwtToken) {
    // BigModel /api/biz 只接受登录 access token，不能使用旧版本误存的 Mesacode JWT。
    log.warn(undefined, "Team Plan runtime key projection skipped: stale mesacode JWT token", {
      family,
      projectId,
    });
    return null;
