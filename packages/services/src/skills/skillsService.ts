...
const SKILL_SETTINGS_DIR = join(resolveUserHomeDir(), ".mesacode", "v2");
const SKILL_CLI_SETTINGS_DIR = join(resolveUserHomeDir(), ".mesacode", "cli");
...
/** 对齐 apps/mesacode-cli/packages/adapters/src/skills/index.ts:19 */
 ...
 * `.mesacode/skills` 与 `.agents/skills` 都收集起来。
 * 对齐 apps/mesacode-cli/packages/adapters/src/skills/roots.ts:60-72。
...
async function resolveAncestorWorkspaceRoots(workspacePath: string): Promise<string[]> {
  ...
  for (const dir of baseDirectories) {
    ...
    // 当成 `.mesacode` 的 fallback，导致同层 `.mesacode` 只要有一个技能，`/`、`$` 和设置页
...
async function discoverSkills(params: {
  ...
  if (params.includeUserSkills) {
    // 用户级技能是全局资源，`.mesacode/skills` 里只要存在一个技能就截断
  ...
  for (const rootPath of scanRootPaths) {
    ...
    for (const skillPath of skillPaths) {
      ...
      // 这些字段不影响 Mesacode 读取 name/description，继续报 warning 只会制造无操作价值的噪音。
...
function resolveCapabilities(options?: SkillsServiceOptions): SkillsCapability {
  const isDesktopRuntime = options?.isDesktopRuntime ?? Boolean(process.env.MESACODE_PROCESS_LABEL);
  if (isDesktopRuntime) {
...
export function createSkillsService(options?: SkillsServiceOptions): ISkillsService {
  ...
  return {
    ...
    async deleteSkill(params: {
      ...
      // sourcePath 才指向 `~/.mesacode/skills/<name>` 下的目录项本身。
      ...
      // 收集工作区各层级（沿 worktree 向上）的 .mesacode/skills 与 .agents/skills，外加用户级两根。
