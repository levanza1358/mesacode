...
import { parseWorkspaceHookTrustStoreContent } from "@mesacode/shared/workspace-hook-trust-store-file";
...
function getRootDir(source: SettingsDirectorySource, workspacePath?: string): string {
  ...
  if (source === "mesacode") {
    return workspacePath ? join(baseDir, ".mesacode") : join(baseDir, ".mesacode", "cli");
  }
...
function getConfigPath(source: SettingsDirectorySource, workspacePath?: string): string {
  return join(
    ...
    source === "mesacode" ? MESACODE_CONFIG_FILE : SETTINGS_FILE,
  );
}
...
async function readUserMesacodeSource(): Promise<WorkspaceHookSourceInput | undefined> {
  const path = getConfigPath("mesacode");
...
async function readPersistentWorkspaceHookTrustDigests(
  ...
  const userConfig = (await readJsonFile<Record<string, unknown>>(getConfigPath("mesacode"))) ?? {};
...
async function loadHooksImpl(
  ...
  const hooks = [
    ...
    ...fromUserMesacodeSource({
      ...
      runtimeRoot,
      workspacePath,
      location: buildLocation("mesacode"),
    }),
...
async function writeMesacodeHooksConfig(
  workspacePath: string | undefined,
  hooks: Hook[],
): Promise<void> {
  const configPath = getConfigPath("mesacode", workspacePath);
  const existingConfig = (await readJsonFile<MesacodeConfigFile>(configPath)) ?? {};
  ...
  await atomicWriteWorkspaceHookConfig(configPath, {
    ...
    hooks: {
      ...
      events: toMesacodeHooksEvents(hooks),
    },
  });
}
...
async function saveHooksImpl(params: {
  ...
  const currentProjectConfigPath = resolve(params.workspacePath, ".mesacode", "config.json");
  const userHooks = params.hooks.filter(
    (hook) =>
      hook.editable !== false &&
      (!hook.location || (hook.location.source === "mesacode" && hook.location.scope === "user")),
  );
  const projectHooks = params.hooks.filter(
    (hook) =>
      hook.editable !== false &&
      hook.location?.source === "mesacode" &&
      hook.location.scope === "project" &&
      (!hook.configuredState ||
        resolve(hook.configuredState.sourcePath) === currentProjectConfigPath),
  );
  await writeMesacodeHooksConfig(undefined, userHooks);
  await writeMesacodeHooksConfig(params.workspacePath, projectHooks);
}
