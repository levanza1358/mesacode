import { resolveWorkspaceKey, MESACODE_AGENT_PROVIDER } from "@mesacode/shared";
import type { TaskIndexRepo } from "#src/session/taskIndexRepo.js";
import type { IMesacodeAgentService, MesacodeAgentWorkspaceTarget } from "#src/mesacode-agent/mesacodeAgent.js";
...
export async function repairSubagentTaskIndex(params: {
  ...
  agentService: Pick<IMesacodeAgentService, "listSessions">;
  taskIndexRepo: Pick<TaskIndexRepo, "listTaskMetas" | "updateTaskState">;
  isCurrent: () => boolean;
  onRemoved: () => void;
}): Promise<void> {
  const { target, taskIndexRepo, agentService, isCurrent } = params;
  const rows = await taskIndexRepo.listTaskMetas({ ...target, provider: MESACODE_AGENT_PROVIDER });
  const ids = rows
