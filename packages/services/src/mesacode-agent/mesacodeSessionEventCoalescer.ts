import type { MesacodeSessionEvent } from "@mesacode/shared";
import type { MesacodeAgentServiceEvent } from "#src/mesacode-agent/mesacodeAgent.js";
...
type SessionServiceEvent = Extract<MesacodeAgentServiceEvent, { type: "session.event" }>;
...
interface BackgroundSessionEventCoalescer {
  accept(event: MesacodeAgentServiceEvent): void;
  flush(): void;
  dispose(): void;
}
...
function getBackgroundSessionEventCoalesceKey(event: MesacodeSessionEvent): string | null {
...
function mergeBackgroundSessionEvents(
  current: MesacodeSessionEvent,
  next: MesacodeSessionEvent,
...
export function createBackgroundSessionEventCoalescer(params: {
  emit: (event: MesacodeAgentServiceEvent) => void;
  flushDelayMs?: number;
  maxItems?: number;
