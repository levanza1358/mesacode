...
} from "#src/session/mesacodeTaskService.js";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import {
  readTrajectoryFileTail,
  resolveModelIODirs,
  sanitizeSessionSegment,
} from "#src/mesacode-agent/modelTrajectoryFileTail.js";
import type { TrajectoryFileTail } from "#src/mesacode-agent/modelTrajectoryFileTail.js";
...
/**
 * 解析 ~/.mesacode/cli/{debug,rollout} 下的 model-io JSONL，按 sessionId 还原某个 task 的模型调用轨迹。
 *
 * 设计说明：
 * - model-io 由 adapters/model/runner-debug.ts 落盘；一个 session 一个
 *   `model-io-<sanitizedSessionId>.jsonl`。
 * - Mesacode Agent 把 taskId 当作 sessionId（见 mesacodeTaskServiceAdapter），所以这里只读取
...
function mapRecord(record: Record<string, unknown>): MesacodeModelTrajectoryRecord {
  ...
  const mapped: MesacodeModelTrajectoryRecord = {
...
function mapMessages(value: unknown): MesacodeModelTrajectoryMessage[] {
...
function mapResponseToolCall(rawToolCall: unknown): MesacodeModelTrajectoryContentPart {
...
function mapUsage(value: unknown): MesacodeModelTrajectoryUsage | undefined {
