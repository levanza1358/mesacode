import { resolveMesacodeAgentSpawnCwd } from "#src/mesacode-agent/mesacodeAgentSpawnCwd.js";
import type { MesacodeAgentStorageStartupSnapshot } from "#src/mesacode-agent/mesacodeAgent.js";
/* eslint-disable max-lines -- mesacodeAgentProcessManager 集中维护 agent 子进程启动、复用、超时回收和 runtime identity，拆分会扩大进程生命周期状态同步面 */
...
import {
  parseMesacodeProcessDiagnostic,
  MESACODE_AGENT_LIFECYCLE_LOG_MARKER,
  MESACODE_PROCESS_DIAGNOSTIC_NAME_MAX_CHARS,
  MESACODE_PROCESS_DIAGNOSTIC_MESSAGE_MAX_CHARS,
  MESACODE_PROCESS_DIAGNOSTIC_STACK_MAX_CHARS,
} from "@mesacode/shared/process-diagnostic";
import {
  MESACODE_AGENT_RUNTIME,
  MESACODE_AGENT_PROVIDER,
  MESACODE_RUNTIME_ENV_KEY,
  resolveWorkspaceKey,
  resolveMesacodeRuntimeEnv,
  sanitizeMesacodeRuntimeEnv,
} from "@mesacode/shared";
import {
  findMesacodeAgentRuntimeBinary,
  findMesacodeAgentRuntimeNodeBundle,
...
import { MesacodeProtocolClient } from "./mesacodeProtocolClient.js";
import { MesacodeStdioTransport } from "./mesacodeStdioTransport.js";
import { readMesacodeStdioTapDevState } from "./mesacodeStdioTapDevConfig.js";
import type { MesacodeAgentPresentationSurface } from "./mesacodeAgentPresentationSurface.js";
...
export interface MesacodeAgentCommand {
...
export interface MesacodeAgentCommandResolverContext {
  presentationSurface?: MesacodeAgentPresentationSurface;
  workspacePath: string;
  workspaceIdentity?: string;
  workspaceKey: string;
}

export type MesacodeAgentCommandResolver = (
  context: MesacodeAgentCommandResolverContext,
) => Promise<MesacodeAgentCommand | null> | MesacodeAgentCommand | null;

export interface MesacodeAgentProcessManagerOptions {
  commandResolver?: MesacodeAgentCommandResolver;
  presentationSurface?: MesacodeAgentPresentationSurface;
...
interface ManagedMesacodeAgentProcess {
  ...
  runtimeIdentity: MesacodeAgentRuntimeIdentity;
...
function buildE2EAgentCoverageEnv(env: NodeJS.ProcessEnv = process.env): Record<string, string> {
  const artifactDir = env.MESACODE_E2E_ARTIFACT_DIR?.trim();
  if (env.MESACODE_E2E_COVERAGE !== "1" || !artifactDir) {
    return {};
  }
  const directory = resolve(artifactDir, "coverage", "raw", "cli");
  mkdirSync(directory, { recursive: true });
  const preloadPath = resolve(directory, "mesacode-e2e-coverage-preload.cjs");
 ...
 */
interface MesacodeAgentRuntimeRestartedEvent {
  workspaceKey: string;
  runtimeIdentity: MesacodeAgentRuntimeIdentity;
}

export interface MesacodeAgentRuntimeLifecycleEvent {
  workspacePath: string;
  workspaceIdentity?: string;
  workspaceKey: string;
  runtimeIdentity: MesacodeAgentRuntimeIdentity;
...
interface MesacodeAgentRuntimeIdentity {
  generation: number;
  identity: string;
  processId?: number;
  workspaceKey: string;
...
interface MesacodeAgentSpawnPreflight {
  command: string;
  args: string[];
  requestedCwd: string;
  cwd: string;
...
const serviceLog = createServiceLogger("mesacode-agent");
...
function parseArgsJson(raw: string | undefined): string[] | undefined {
  ...
  if (!Array.isArray(parsed) || parsed.some((item) => typeof item !== "string")) {
    throw new Error("MESACODE_AGENT_SERVER_ARGS_JSON must be a JSON string array");
  }
  return parsed;
}
...
async function buildMesacodeAgentSpawnPreflight(
  command: MesacodeAgentCommand,
  workspacePath: string,
  spawnFallbackCwd?: string,
): Promise<MesacodeAgentSpawnPreflight> {
  ...
  const {
    ...
    cwdExists,
  } = await resolveMesacodeAgentSpawnCwd({ requestedCwd, workspacePath, spawnFallbackCwd });
  return {
...
function resolveBundledWorkspaceMesacodeAgentCommand(
  context: MesacodeAgentCommandResolverContext,
): MesacodeAgentCommand | null {
  const distEntrypoint = findUpward("apps/mesacode-cli/packages/cli/dist/mesacode.cjs");
  if (distEntrypoint) {
    const useBytecode =
      process.versions.electron && process.env.MESACODE_DESKTOP_AGENT_BYTECODE === "1";
    const entrypoint = useBytecode
      ? join(dirname(distEntrypoint), "mesacode.bytecode.cjs")
      : distEntrypoint;
    ...
    return {
      ...
      // 这里显式启用 Node 运行模式，避免内置 mesacode-agent 被当成 Electron/Chromium 子进程启动并卡在 GPU 初始化。
      env: { ELECTRON_RUN_AS_NODE: "1" },
    };
  }

  const sourceEntrypoint = findUpward("apps/mesacode-cli/packages/cli/src/main.ts");
...
function resolveDeployedMesacodeAgentBinaryCommand(
  context: MesacodeAgentCommandResolverContext,
): MesacodeAgentCommand | null {
  // 旧 resolver 只识别 MESACODE_AGENT_SERVER_COMMAND env 和 monorepo 源码树。
  // SSH 远端把 mesacode-server.cjs 单文件部署到 ~/.mesacode/server/，宿主进程的 cwd 不在仓库内、
  // env 也不会被 ssh exec 继承，即使 mesacode-agent 已经部署到 ~/.mesacode/server/agents/glm/，
  // resolver 也找不到，第一次 getClient 就抛 "Mesacode agent server command is not configured"。
  // 这里复用 findMesacodeAgentRuntimeBinary 的候选链（含 GLM_BINARY_PATH env、
  // packagedResourcesPath、~/.mesacode/server/agents/glm、bundled-agents 等），
  // 把已部署的原生 binary 当成最终兜底，远端/桌面打包形态都能命中。
  const binaryPath = findMesacodeAgentRuntimeBinary();
  if (!binaryPath) {
    return null;
  }
  return {
    command: binaryPath,
    args: MESACODE_AGENT_RUNTIME.spawnArgs,
    cwd: context.workspacePath,
  };
}

function resolveElectronRuntimeMesacodeAgentCommand(
  context: MesacodeAgentCommandResolverContext,
): MesacodeAgentCommand | null {
  // 桌面打包态：host 跑在 Electron utility process 里，process.execPath 指向 Electron Helper，
  // 它内置的 Node runtime 与 mesacode-cli 目标版本一致（Electron 41 = Node 24.x）。
  // 这里直接用 app 自带的 Electron Node 执行打进 resources/glm 的 mesacode.cjs，
  ...
  const bundlePath = findMesacodeAgentRuntimeNodeBundle();
  if (!bundlePath) {
    return null;
  }
  return {
    command: process.execPath,
    args: [bundlePath, ...MESACODE_AGENT_RUNTIME.spawnArgs],
...
export function resolveDefaultMesacodeAgentCommand(
  context: MesacodeAgentCommandResolverContext,
): MesacodeAgentCommand | null {
  const command = process.env.MESACODE_AGENT_SERVER_COMMAND?.trim();
  if (command) {
    return applyPresentationSurfaceToCommand(
      {
        command,
        args: parseArgsJson(process.env.MESACODE_AGENT_SERVER_ARGS_JSON) ?? ["app-server", "--stdio"],
        cwd: process.env.MESACODE_AGENT_SERVER_CWD?.trim() || context.workspacePath,
      },
  ...
  // 抢先匹配）→ 桌面打包态 Electron Node runtime 跑 mesacode.cjs → 已部署 native binary（远端 SSH 兜底）。
  const bundled =
    resolveBundledWorkspaceMesacodeAgentCommand(context) ??
    resolveElectronRuntimeMesacodeAgentCommand(context);
  return applyPresentationSurfaceToCommand(
    bundled
      ? { ...bundled, supportsStorageStartup: true }
      : resolveDeployedMesacodeAgentBinaryCommand(context),
    context.presentationSurface,
  );
}

function applyPresentationSurfaceToCommand(
  command: MesacodeAgentCommand | null,
  presentationSurface: MesacodeAgentCommandResolverContext["presentationSurface"],
): MesacodeAgentCommand | null {
...
function wrapMesacodeAgentCommandWithStdioTapDevProxy(
  command: MesacodeAgentCommand,
  workspaceKey: string,
): MesacodeAgentCommand {
  const tapState = readMesacodeStdioTapDevState();
  if (!tapState.enabled) {
    return command;
  }

  const tapScript = findUpward("scripts/dev/mesacode-stdio-tap.mjs");
  if (!tapScript) {
    debugLog("Mesacode stdio tap proxy enabled but script not found");
    return command;
  }
...
export class MesacodeAgentProcessManager {
  private readonly processesByWorkspaceKey = new Map<string, ManagedMesacodeAgentProcess>();
  private readonly ownedProcesses = new Set<ManagedMesacodeAgentProcess>();
  private readonly startingByWorkspaceKey = new Map<string, Promise<MesacodeProtocolClient>>();
  ...
  private readonly storageStartupEmitter = new Emitter<{
    workspaceKey: string;
    snapshot: MesacodeAgentStorageStartupSnapshot;
  }>();
  ...
  getStorageStartupState(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): MesacodeAgentStorageStartupSnapshot | null {
  ...
  private readonly commandResolver: MesacodeAgentCommandResolver;
  private readonly presentationSurface: MesacodeAgentProcessManagerOptions["presentationSurface"];
  private readonly requestTimeoutMs: number | undefined;
  private readonly processLifecycleReporter: RuntimeProcessLifecycleReporter | undefined;
  private readonly resolveSpawnEnv: MesacodeAgentProcessManagerOptions["resolveSpawnEnv"];
  private readonly waitForSpawnAdmission: MesacodeAgentProcessManagerOptions["waitForSpawnAdmission"];
  ...
  private readonly runtimeRestartedEmitter = new Emitter<MesacodeAgentRuntimeRestartedEvent>();
  private readonly runtimeLifecycleEmitter = new Emitter<MesacodeAgentRuntimeLifecycleEvent>();
  private disposeAllInFlight: Promise<void> | undefined;
  private disposed = false;

  /** 进程换代通知（generation>1 时触发）；v4 订阅方据此重订，见 MesacodeAgentRuntimeRestartedEvent。 */
  ...
  constructor(options?: MesacodeAgentProcessManagerOptions) {
    this.commandResolver = options?.commandResolver ?? resolveDefaultMesacodeAgentCommand;
  ...
  private reportProcessLifecycle(
    ...
    try {
      ...
      warnLog("Mesacode agent process lifecycle reporter failed", error);
    }
  }

  private clearIdleTimer(managed: ManagedMesacodeAgentProcess): void {
   ...
   */
  private scheduleIdleReclaim(workspaceKey: string, managed: ManagedMesacodeAgentProcess): void {
    ...
    const timer = setTimeout(() => {
      ...
      log("Mesacode agent process idle timeout; reclaiming", {
        workspaceKey,
  ...
  private recordTerminationIntent(
    managed: ManagedMesacodeAgentProcess,
  ...
  private reportRuntimeUnavailable(managed: ManagedMesacodeAgentProcess): void {
  ...
  private reportRuntimeReady(managed: ManagedMesacodeAgentProcess): void {
    if (
      !managed.spawned ||
      managed.exited ||
  ...
  private cleanupManagedProcess(
    ...
    const cleanupPromise = managed.client
      ...
      .then(() => {
        ...
        log("Mesacode agent process cleanup completed", {
    ...
    if (options.reportError !== false) {
      void cleanupPromise.catch((error) => {
        errorLog("Mesacode agent process cleanup failed", {
          workspaceKey: managed.runtimeIdentity.workspaceKey,
          pid: managed.child.pid,
          runtimeIdentity: managed.runtimeIdentity.identity,
          reason,
  ...
  private async cleanupManagedProcessForShutdown(managed: ManagedMesacodeAgentProcess): Promise<void> {
  ...
  private async cleanupManagedProcessWithRetry(
    ...
    try {
      ...
      warnLog(`Mesacode agent process cleanup retrying during ${retryScope}`, {
        workspaceKey: managed.runtimeIdentity.workspaceKey,
        pid: managed.child.pid,
        runtimeIdentity: managed.runtimeIdentity.identity,
        cleanupStage: reason,
  ...
  async getClient(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<MesacodeProtocolClient> {
    if (this.disposed) {
      throw new Error("Mesacode agent process manager is disposed.");
    }
    ...
    if (starting) {
      ...
      log("Mesacode agent process start already in progress", {
        workspaceKey,
      });
      const client = await starting;
      log("Mesacode agent process start wait completed", {
        workspaceKey,
  ...
  getExistingClient(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): MesacodeProtocolClient | undefined {
    const managed = this.processesByWorkspaceKey.get(resolveWorkspaceKey(params));
  ...
  private async startClient(
    ...
    if (!command) {
      throw new Error(
        "Mesacode agent server command is not configured. Set MESACODE_AGENT_SERVER_COMMAND before integration.",
      );
    }
    if (admissionSignal.aborted) {
      throw admissionSignal.reason ?? new Error("Mesacode agent process start was cancelled.");
    }
    const effectiveCommand = wrapMesacodeAgentCommandWithStdioTapDevProxy(command, workspaceKey);
    log("Mesacode agent command resolved", {
      workspaceKey,
      command: command.command,
    ...
    if ((this.restartGenerationByWorkspaceKey.get(workspaceKey) ?? 0) !== startGeneration) {
      ...
      throw new Error("Mesacode agent process start was cancelled.");
    }
    // cwd 探测也让出事件循环，必须放在最终 admission 与销毁/代际检查之前。
    const spawnPreflight = await buildMesacodeAgentSpawnPreflight(
      effectiveCommand,
    ...
    // app 以本地开发方式启动时，让 agent 子进程也带上 MESACODE_RUNTIME_ENV=development；
    // 不再传 NODE_ENV，避免用户 shell/runtime 变量影响 Mesacode 运行模式或泄漏到 Bash 工具。
    const runtimeEnv = resolveMesacodeRuntimeEnv(process.env);
    log("Mesacode agent spawn preflight", {
      workspaceKey,
      spawnPreflight,
    });
    ...
    const child = spawn(effectiveCommand.command, spawnPreflight.args, {
      ...
      env: {
        ...sanitizeMesacodeRuntimeEnv(process.env),
        [MESACODE_RUNTIME_ENV_KEY]: runtimeEnv,
        ...spawnEnv,
    ...
    const transport = new MesacodeStdioTransport(child, {
      onStderrLine: (line) => {
        const diagnostic = parseMesacodeProcessDiagnostic(line);
    ...
    const client = new MesacodeProtocolClient(transport, {
    ...
    const runtimeIdentity: MesacodeAgentRuntimeIdentity = {
    ...
    const managed: ManagedMesacodeAgentProcess = {
      child,
      client,
      exited: false,
      readyReported: false,
    ...
    child.once("spawn", () => {
      ...
      this.reportRuntimeReady(managed);
      log("Mesacode agent process started", {
        workspaceKey,
        command: effectiveCommand.command,
        cwd: spawnPreflight.cwd,
        pid: child.pid,
    ...
    child.once("error", (error) => {
      errorLog(
        `Mesacode agent process error${this.processLifecycleReporter?.onError ? ` ${MESACODE_AGENT_LIFECYCLE_LOG_MARKER}` : ""}`,
        {
          workspaceKey,
          pid: child.pid,
    ...
    child.once("exit", async (code, signal) => {
      ...
      log("Mesacode agent process exited", exitContext);
      if (terminationKind === "unexpected") {
        ...
        errorLog(
          `Mesacode agent process exited unexpectedly${this.processLifecycleReporter ? ` ${MESACODE_AGENT_LIFECYCLE_LOG_MARKER}` : ""}`,
          {
            ...exitContext,
            stderr,
          },
    ...
    client.onRequestTimeout((event) => {
      ...
      if (event.method === "workspace/cancelGenerateText") {
        warnLog(
          "Mesacode agent cancel notification timed out; keeping client (best-effort control plane)",
          {
            workspaceKey,
            method: event.method,
      ...
      warnLog("Mesacode agent request timed out; disposing stale protocol client", {
        workspaceKey,
        method: event.method,
        requestId: event.requestId,
        timeoutMs: event.timeoutMs,
    ...
    client.onClose(() => {
      ...
      log("Mesacode agent protocol client closed", {
        workspaceKey,
        pid: child.pid,
        runtimeIdentity: runtimeIdentity.identity,
        wasActiveClient,
  ...
  async getRuntimeIdentity(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<MesacodeAgentRuntimeIdentity> {
    const workspaceKey = resolveWorkspaceKey(params);
    ...
    if (!managed || managed.exited || managed.child.killed) {
      throw new Error("Mesacode agent runtime identity is unavailable.");
    }
    ...
    try {
      ...
      return command
        ...
        : {
            available: false,
            workspaceKey,
            reason: "MESACODE_AGENT_SERVER_COMMAND is not configured",
          };
    } catch (error) {
      return {
        available: false,
  ...
  private abortPendingStarts(
    workspaceKey: string,
    reason = new Error("Mesacode agent process start was cancelled."),
  ): void {
  ...
  private abortAllPendingStarts(
    reason = new Error("Mesacode agent process manager is disposed."),
  ): void {
  ...
  async disposeAllAndWait(): Promise<void> {
    ...
    // 依赖 unref timer，host 自己退出后 timer 不会再执行，mesacode-cli 会残留为孤儿进程。
