import type {
  ...
  ModelSelection,
  MesacodeSessionMode,
  MesacodeSessionSettingsState,
  MesacodeSessionStateSnapshot,
  MesacodeTaskModeInfo,
} from "@mesacode/shared";
import {
  formatModelPickerValue as formatSharedModelSelection,
  getMesacodeAgentAvailableModes as getSharedMesacodeAgentAvailableModes,
  normalizeAvailableMesacodeMode as normalizeSharedAvailableMesacodeMode,
...
function resolveTaskMetaModelSelectionFromSnapshot(
  snapshot: Pick<MesacodeSessionStateSnapshot, "messages" | "settings">,
): ModelSelection | undefined {
...
export function normalizeAvailableMesacodeMode(mode: MesacodeSessionMode): string {
  return normalizeSharedAvailableMesacodeMode(mode);
}

export function getMesacodeAgentAvailableModes(): MesacodeTaskModeInfo[] {
  return getSharedMesacodeAgentAvailableModes();
}

export function settingsToConfigOptions(settings: MesacodeSessionSettingsState): MesacodeConfigOption[] {
  // service 侧曾经维护了一份三项 mode 白名单，edit 模式上线后没有同步，
  // 切换模式时 mode_update 会把 UI 菜单覆盖成 build/plan/yolo。这里统一复用 shared 的事实源。
  return mesacodeSessionSettingsToMesacodeConfigOptions(settings);
}
