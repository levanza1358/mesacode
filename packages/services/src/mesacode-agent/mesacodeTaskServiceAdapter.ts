/* oxlint-disable eslint(max-lines) -- 迁移期需要在一个门面里集中维护旧 task projection 到 Mesacode session 的协议适配。 */
...
import {
  coalesceConsecutiveMesacodeAssistants,
  createSessionTraceId,
  decodeCustomModelValue,
  deriveMesacodeTaskStatusFromSessionSnapshot,
  extractPlanStepsFromToolInput,
  extractPlanStepsFromToolOutput,
  generateTraceId,
  getMesacodeGoalActiveIterationCount,
  getMesacodeGoalIterationByAssistantMessageId,
  getMesacodeUserVisibleMessages,
  isMainAgentToolProjectionSource,
  normalizeMesacodeApiRetryStatus,
  attachMesacodeBackgroundTaskNotificationToRaw,
  collectMesacodeBackgroundTaskNotificationsByToolUseId,
  mergeMesacodeBackgroundTaskControlItems,
  parseMesacodeBackgroundTaskControlItems,
  parseMesacodeBackgroundTaskNotificationText,
  parseModelPickerValue as parseSharedModelSelection,
  resolveWorkspaceKey,
  resolveMesacodeVisibleSessionTitle,
  textFromMesacodeMessageParts,
  MESACODE_AGENT_PROVIDER,
  mesacodeBackgroundTaskNotificationToolUpdateStatus,
  appendMesacodeStreamingToolInputDelta,
  buildMesacodeStreamingToolInputPreview,
  createMesacodeToolProjectionMemory,
  finalizeMesacodeToolProjectionInput,
  forgetMesacodeToolProjectionMetadata,
  isMesacodeModelRetryRecoveryProgressPayload,
  markMesacodeStreamingToolInputPreviewMaterialized,
  resolveMesacodeToolProjectionMetadata,
  mesacodeApiRetryFromModelNetworkStatusPayload,
  mesacodeApiRetryFromStreamRecoveryPayload,
  mesacodeTaskNetworkDebugStatusFromPayload,
  shouldMaterializeMesacodeStreamingToolInputPreview,
  type MesacodeApiRetryStatus,
  type MesacodeAssistantMessageFeedback,
  type MesacodeBackgroundTaskNotificationInfo,
  type MesacodeBackgroundTaskControlItem,
  type MesacodeBackgroundTurnAttribution,
  type MesacodeCancelTaskCommandResult,
  type MesacodeConfigOption,
  type MesacodeEnqueueTaskCommandResult,
  type MesacodeError,
  type MesacodeGoalVerificationTimelineMeta,
  type MesacodeImportSessionsResult,
  type MesacodeImportableSessionCandidate,
  type MesacodeUsage,
  type MesacodePersistedMessage,
  type MesacodePersistedMessagePart,
  type MesacodePersistedToolCall,
  type MesacodePromptAttachment,
  type MesacodeProvider,
  type MesacodeSessionFile,
  type MesacodeTaskGoal,
  type MesacodeTaskGoalStats,
  type MesacodeTaskMode,
  type MesacodePlanStep,
  type MesacodeSlashCommand,
  type MesacodeStreamEvent,
  type MesacodeTaskCreateResult,
  type MesacodeTaskMeta,
  type MesacodeTaskClientMode,
  type MesacodeTaskRuntimeCommand,
  type MesacodeTaskSnapshot,
  type MesacodeTaskSnapshotBody,
  type MesacodeTaskSnapshotRefContent,
  type MesacodeTaskSnapshotToolCallsSlice,
  type MesacodeTaskTokenUsageResult,
  type MesacodeTodoGroup,
  type MesacodeTurnSteerCommandKind,
  type MesacodeTurnSteerSource,
  type MesacodeWorkspaceEvent,
  type MesacodeWorkspaceTaskListChanged,
  type InputId,
  type TraceId,
  mesacodeContextUsageBreakdownSchema,
  mesacodeSessionSettingsStateSchema,
  type MesacodeDeliveryKind,
  type MesacodeMessagePart,
  type MesacodeMessageWithParts,
  type ModelSelection,
  type MesacodePermissionOption,
  type MesacodePermissionRequestParams,
  type MesacodePermissionRequest,
  type MesacodeSessionEvent,
  type MesacodeSessionMode,
  type MesacodeSessionSettingsState,
  type MesacodeSessionStateSnapshot,
  type MesacodeStateUpdatedNotification,
  type MesacodeContextCompactionTimelineMeta,
  type MesacodeTimelineMeta,
  type MesacodeTimelineStatus,
  type MesacodeTimelineTrigger,
  type MesacodeToolProjectionMemory,
  type MesacodeUserInputRequestParams,
  type MesacodeUserInputResponse,
  type MesacodeAgentMcpServer,
} from "@mesacode/shared";
import type {
  MesacodeTaskListQuery,
  MesacodeTaskListResult,
  MesacodeWorkspaceEventSubscriptionParams,
  IMesacodeTaskService,
  MesacodeArchivedTaskDeletionResult,
  MesacodeTaskReadyOutcome,
  MesacodeTaskTerminalOutcome,
...
import type {
  MesacodeTaskIndexReadyEvent,
  MesacodeTaskIndexSyncer,
  MesacodeTaskIndexTerminalEvent,
} from "./mesacodeTaskIndexSyncer.js";
import { readModelTrajectory } from "./modelTrajectory.js";
import { errorAttributionSchema, type CommandPayloadMap } from "@mesacode/shared/mesacode-protocol-v4";
import {
  assertV4CommandAckOk,
  createHostCommandEnvelope,
  sendHostCasCommandV4,
} from "./mesacodeV4HostCommand.js";
...
import {
  ...
  getMesacodeAgentAvailableModes,
  normalizeAvailableMesacodeMode,
  settingsToConfigOptions,
} from "./mesacodeConfigOptions.js";
...
interface CreateMesacodeTaskServiceAdapterOptions {
  mesacodeAgentService: IMesacodeAgentService;
  ...
  taskIndexSyncer: MesacodeTaskIndexSyncer;
...
type WorkspaceEventInput = string | MesacodeWorkspaceEventSubscriptionParams;
type MesacodeSendPromptRuntimeCommand = Extract<MesacodeTaskRuntimeCommand, { type: "send_prompt" }>;
type MesacodeTerminalStreamEvent =
  | Extract<MesacodeStreamEvent, { type: "task_complete" }>
  | Extract<MesacodeStreamEvent, { type: "task_error" }>;

const GLM_PROVIDER: MesacodeProvider = MESACODE_AGENT_PROVIDER;
const EMPTY_SLASH_COMMANDS: MesacodeSlashCommand[] = [];
const logger = createServiceLogger("mesacode-task-service");
...
interface LiveToolProjection {
  ...
  tool: MesacodePersistedToolCall;
  toolId: string;
}
...
function formatMesacodeAgentLogDate(now: Date): string {
  return [now.getFullYear(), padDatePart(now.getMonth() + 1), padDatePart(now.getDate())].join("-");
}

function resolveMesacodeAgentCurrentLogFilePath(now = new Date()): string {
  const configuredLogDir = process.env.MESACODE_LOG_DIR?.trim();
  const logDir = configuredLogDir || join(homedir(), ".mesacode", "cli", "log");
  return join(logDir, `mesacode-${formatMesacodeAgentLogDate(now)}.jsonl`);
}

export function createMesacodeTaskServiceAdapter(
  options: CreateMesacodeTaskServiceAdapterOptions,
): IMesacodeTaskService {
  const errorEmitter = new Emitter<MesacodeError>();
  const taskEmitters = new Map<string, Emitter<MesacodeStreamEvent>>();
  const globalTaskEmitters = new Map<string, Emitter<MesacodeStreamEvent>>();
  const overlays = new Map<string, TaskOverlay>();
  const taskTargets = new Map<string, TaskTarget>();
  const runtimeCommands = new Map<string, MesacodeTaskRuntimeCommand[]>();
  const runtimeCommandDrains = new Map<string, Promise<void>>();
  const apiRetryByTaskKey = new Map<string, MesacodeApiRetryStatus | null>();
  const backgroundTaskControlsByTaskKey = new Map<string, MesacodeBackgroundTaskControlItem[]>();
  const streamedTurnKeys = new Set<string>();
  const activePromptInputIds = new Map<string, InputId>();
  const toolProjectionMemoryByTaskKey = new Map<string, MesacodeToolProjectionMemory>();
  ...
  function unsupported(name: string): never {
    throw Object.assign(
      new Error(`Mesacode task service adapter does not support IMesacodeTaskService.${name} yet.`),
      {
        code: "MESACODE_AGENT_UNSUPPORTED_LEGACY_TASK_METHOD",
      },
    );
  }
  ...
  function turnAttributionOf(
    params: MesacodeBackgroundTurnAttribution,
  ...
  async function resolveProductMcpServers(
    servers: MesacodeAgentMcpServer[] | undefined,
  ): Promise<MesacodeAgentMcpServer[] | undefined> {
  ...
  async function sendPromptToAgent(
    ...
    params: {
      ...
      messageId?: string;
      content: string;
      attachments?: MesacodePromptAttachment[];
      toolDenylist?: string[];
      clientId?: string;
    ...
    // Mesacode task wrapper 的字段仍叫 traceId，但这里语义已经是单次输入 inputId。
    // 先记录 inputId，后续 Mesacode session 事件回投 Mesacode Agent 时才能让 UI 终态按输入轮次收口。
    activePromptInputIds.set(taskKey(target), params.traceId);
    logger.info(params.traceId, "Mesacode task facade sendPrompt 开始", {
    ...
    try {
      ...
      if (params.attachments?.length) {
        ...
        await options.mesacodeAgentService.sendPrompt({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
        ...
        const ack = await options.mesacodeAgentService.sendConversationCommandV4({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          ...(target.remoteSessionId ? { remoteSessionId: target.remoteSessionId } : {}),
          ...(params.clientMode ? { clientMode: params.clientMode } : {}),
      ...
      logger.info(params.traceId, "Mesacode task facade sendPrompt ACK", {
      ...
      logger.warn(params.traceId, "Mesacode task facade sendPrompt 失败", {
        durationMs: Date.now() - startedAt,
  ...
  function setRuntimeCommands(params: TaskTarget, commands: MesacodeTaskRuntimeCommand[]): void {
    const key = taskKey(params);
  ...
  function markRuntimeCommandRunning(
    params: TaskTarget,
    command: MesacodeSendPromptRuntimeCommand,
  ): MesacodeSendPromptRuntimeCommand {
    const key = taskKey(params);
    const runningCommand: MesacodeSendPromptRuntimeCommand = {
      ...command,
      status: "running",
      updatedAt: Date.now(),
    };
  ...
  function markRuntimeCommandFailed(
    ...
    const key = taskKey(params);
    const failedCommand: MesacodeSendPromptRuntimeCommand = {
      ...command,
      status: "failed",
      updatedAt: Date.now(),
      error: error instanceof Error ? error.message : String(error),
  ...
  function completeRuntimeCommandByInputId(
    ...
    logger.info(command.traceId, "Mesacode task command 终态收口", {
  ...
  function completeRuntimeCommandForTerminalEvent(
    params: TaskTarget,
    event: MesacodeTerminalStreamEvent,
  ): void {
  ...
  async function drainRuntimeCommands(params: TaskTarget, reason: string): Promise<void> {
    ...
    const command = commands.find(
      (candidate): candidate is MesacodeSendPromptRuntimeCommand =>
    ...
    logger.info(runningCommand.traceId, "Mesacode task command drain 开始", {
    ...
    try {
      ...
      logger.warn(runningCommand.traceId, "Mesacode task command drain 失败", {
        commandId: runningCommand.commandId,
        error: error instanceof Error ? error.message : String(error),
        reason,
        taskId: params.taskId,
  ...
  async function drainRuntimeCommandsSafely(params: TaskTarget, reason: string): Promise<void> {
    try {
      ...
      logger.warn(undefined, "Mesacode task command drain 调度失败", {
        error: error instanceof Error ? error.message : String(error),
        reason,
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
  ...
  function persistedStatusFromToolUpdate(
    status: Extract<MesacodeStreamEvent, { type: "tool_call_update" }>["status"],
  ): MesacodePersistedToolCall["status"] | undefined {
  ...
  function rememberLiveToolProjection(params: TaskTarget, event: MesacodeStreamEvent): void {
    ...
    const nextTool: MesacodePersistedToolCall =
  ...
  function persistedToolId(tool: MesacodePersistedToolCall): string | undefined {
  ...
  function mergePersistedToolCall(
    snapshotTool: MesacodePersistedToolCall | undefined,
    liveTool: MesacodePersistedToolCall,
  ): MesacodePersistedToolCall {
    if (!snapshotTool) {
      return liveTool;
    }
    return {
  ...
  function findToolLocation(
    messages: readonly MesacodePersistedMessage[],
    toolId: string,
  ...
  function latestAssistantMessageIndex(messages: readonly MesacodePersistedMessage[]): number {
  ...
  function upsertPersistedToolPart(
    parts: readonly MesacodePersistedMessagePart[] | undefined,
    toolIndex: number,
  ): MesacodePersistedMessagePart[] {
  ...
  function updateSnapshotMessageTool(
    message: MesacodePersistedMessage,
    toolIndex: number,
    tool: MesacodePersistedToolCall,
  ): MesacodePersistedMessage {
  ...
  function mergeLiveToolProjectionIntoSnapshotMessages(
    ...
    if (changed) {
      logger.debug(undefined, "Mesacode snapshot 合并 live tool projection", {
        event: "mesacode_task.snapshot.live_tool_projection.merged",
  ...
  function getTaskTarget(taskId: string): TaskTarget {
    ...
    if (!target) {
      throw Object.assign(new Error(`Mesacode session target is not loaded: ${taskId}`), {
        code: "MESACODE_SESSION_TARGET_NOT_FOUND",
      });
    }
    return target;
  }
  ...
  function getToolProjectionMemory(params: TaskTarget): MesacodeToolProjectionMemory {
    const key = taskKey(params);
    let memory = toolProjectionMemoryByTaskKey.get(key);
    if (!memory) {
      memory = createMesacodeToolProjectionMemory();
  ...
  function getWorkspaceEmitter(workspace: WorkspaceEventInput): Emitter<MesacodeWorkspaceEvent> {
    return taskIndexSyncer.getWorkspaceEmitter(workspace);
  }

  function getTaskEmitter(params: TaskTarget): Emitter<MesacodeStreamEvent> {
    const key = taskKey(params);
    let emitter = taskEmitters.get(key);
    if (!emitter) {
      emitter = new Emitter<MesacodeStreamEvent>();
      taskEmitters.set(key, emitter);
    }
    return emitter;
  }

  function getGlobalTaskEmitter(taskId: string): Emitter<MesacodeStreamEvent> {
  ...
  function emitTaskEvent(params: TaskTarget, event: MesacodeStreamEvent): void {
  ...
  function emitWorkspaceTaskListChanged(
    ...
    taskMeta: MesacodeTaskMeta | undefined,
    reason: MesacodeWorkspaceTaskListChanged["reason"],
  ) {
  ...
  function emitWorkspaceConfig(
    ...
    settings: MesacodeSessionSettingsState,
  ) {
  ...
  async function resumeSnapshot(
    ...
    return options.mesacodeAgentService.resumeSession({
      workspacePath: params.workspacePath,
      workspaceIdentity: params.workspaceIdentity,
  ...
  async function sendConfigCasCommandV4<T extends "switchModelConfig" | "switchCollaborationMode">(
    ...
    await sendHostCasCommandV4({
      send: (envelope) =>
        options.mesacodeAgentService.sendConversationCommandV4({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          envelope,
        }),
   ...
   * command.ts 裁决），而旧协议 MesacodeSessionMode 含 auto 且旧 op 接受它——为 UI 行为
  ...
  async function switchCollaborationModeViaProtocol(
    ...
    if (mode === "auto") {
      await options.mesacodeAgentService.setMode({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: target.taskId,
        mode,
  ...
  async function repairEmptyImportedClaudeSnapshot(
    ...
    const repaired = await repairImportedClaudeSessionSnapshot({
      snapshot,
      target: params,
      createSession: (input) => options.mesacodeAgentService.createSession(input),
      onRepair: (history) => {
        logger.warn(
          undefined,
  ...
  function snapshotToMeta(snapshot: MesacodeSessionStateSnapshot): MesacodeTaskMeta {
    const target = {
    ...
    return applyOverlayToMeta(
      {
        ...
        mode: fromMesacodeMode(snapshot.session.mode),
        ...
        status: deriveMesacodeTaskStatusFromSessionSnapshot(snapshot),
        ...
        target: snapshot.projection.target
          ? fromMesacodeGoal(snapshot.projection.target)
  ...
  function rememberIndexedTaskMeta(meta: MesacodeTaskMeta): MesacodeTaskMeta {
  ...
  async function resolveTaskIndexResumeHints(
    ...
    if (model || thoughtLevel) {
      ...
      logger.info(undefined, "从 task index 回填 Mesacode session resume 配置", {
        model: model || null,
  ...
  async function syncTaskIndexMeta(meta: MesacodeTaskMeta): Promise<MesacodeTaskMeta> {
  ...
  async function updateIndexedTaskState(
    ...
    try {
      ...
      // 旧 Mesacode session 可能还没有轻量 task index 行。
  ...
  function updateTaskIndexFromStreamEvent(params: TaskTarget, event: MesacodeStreamEvent): void {
  ...
  function updateTaskApiRetryFromStreamEvent(params: TaskTarget, event: MesacodeStreamEvent): void {
    const key = taskKey(params);
  ...
  function snapshotToMesacode(
    snapshot: MesacodeSessionStateSnapshot,
    options?: { includeEmptyPendingElicitations?: boolean },
  ): MesacodeTaskSnapshot {
    const meta = snapshotToMeta(snapshot);
    const activeGoalIterationCount = getSnapshotGoalActiveIterationCount(snapshot);
    const goalIterationByAssistantMessageId = getMesacodeGoalIterationByAssistantMessageId(
      snapshot.messages,
      {
    ...
    const backgroundTaskNotifications = collectMesacodeBackgroundTaskNotificationsByToolUseId(
      snapshot.messages,
    );
      ...
      normalizeGoalVerificationTimelineMessageOrder(
        addGoalVerificationTimelineSnapshotFallback(
          addSessionForkSnapshotFallback(
            coalesceConsecutiveMesacodeAssistants(
              getMesacodeUserVisibleMessages(snapshot.messages, {
                target: snapshot.projection.target,
              }).map((message) =>
                mapMessage(
                  message,
    ...
    const pendingPermissions: MesacodePermissionRequest[] = snapshot.projection.pendingPermissions
    ...
    const pendingElicitations = snapshot.projection.pendingPermissions
      ...
      .filter(
        (event): event is Extract<MesacodeStreamEvent, { type: "elicitation_request" }> =>
          event !== null,
      );
    const backgroundTaskControls = parseMesacodeBackgroundTaskControlItems(
  ...
  function mapServiceEvent(params: TaskTarget, event: MesacodeAgentServiceEvent): void {
    if (event.type === "snapshot") {
      void syncTaskIndexSnapshot(event.snapshot).catch((error) => {
        logger.warn(undefined, "同步 Mesacode snapshot 到 task index 失败", error);
      });
      const snapshotEvent: MesacodeStreamEvent = {
        type: "task_snapshot_updated",
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        workspaceKey: workspaceKey(params),
  ...
  function handleTaskIndexTerminalEvent(event: MesacodeTaskIndexTerminalEvent): void {
  ...
  function handleTaskIndexReadyEvent(event: MesacodeTaskIndexReadyEvent): void {
    const params: TaskTarget = {
      taskId: event.target.sessionId,
      workspacePath: event.target.workspacePath,
      workspaceIdentity: event.target.workspaceIdentity,
  ...
  async function disposeMesacodeAgentServiceAndWait(): Promise<void> {
    const agentService = options.mesacodeAgentService as IMesacodeAgentService & {
  ...
  const service: IMesacodeTaskService & {
    disposeAll(): void;
    disposeAllAndWait(): Promise<void>;
  } = {
    async initialize(params) {
      const result = await options.mesacodeAgentService.initialize(params);
      return {
    ...
    async releaseWorkspacePreparation(params): Promise<void> {
      ...
      await options.mesacodeAgentService.disposeWorkspace(normalizeWorkspaceParams(params));
    },

    async createTask(params): Promise<MesacodeTaskCreateResult> {
      ...
      const mcpServers = await resolveProductMcpServers(params.mcpServers);
      let snapshot: MesacodeSessionStateSnapshot | null = null;
      if (draftSessionId && !mcpServers) {
        try {
          snapshot = await options.mesacodeAgentService.readSession({
            ...target,
          ...
          if (requestedSelection) {
            if (!sameModelSelection(snapshot.settings.model.current, requestedSelection)) {
              ...
              snapshot = await options.mesacodeAgentService.setModel({
                ...target,
                sessionId: draftSessionId,
          ...
          if (
            ...
            snapshot = await options.mesacodeAgentService.setThoughtLevel({
              ...target,
              sessionId: draftSessionId,
      ...
      if (!snapshot) {
        ...
        // 迁移属 v4 生命周期收口。
        snapshot = await options.mesacodeAgentService.createSession({
          ...target,
          sessionTraceId: createSessionTraceId(),
          mode: toMesacodeMode(params.mode),
          model: requestedSelection,
          thoughtLevel: requestedSelection?.options?.reasoningLevel,
    ...
    async enqueueTaskCommand(params): Promise<MesacodeEnqueueTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const workspaceKeyValue = workspaceKey(params);
      const command: MesacodeTaskRuntimeCommand = {
    ...
    async promoteTaskCommand(params): Promise<MesacodeEnqueueTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const key = taskKey(params);
      const commands = runtimeCommands.get(key) ?? [];
    ...
    async cancelTaskCommand(params): Promise<MesacodeCancelTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const key = taskKey(params);
      const commands = runtimeCommands.get(key) ?? [];
      const command = commands.find((candidate) => candidate.commandId === params.commandId);
      if (!command) {
        logger.info(undefined, "Mesacode task command 取消时已不存在", {
          commandId: params.commandId,
          taskId: params.taskId,
          workspaceIdentity: params.workspaceIdentity ?? null,
          workspaceKey: resolveWorkspaceKey(params),
      ...
      if (command.status === "running") {
        logger.info(command.traceId, "Mesacode task command 已开始运行，跳过取消", {
          commandId: command.commandId,
          taskId: params.taskId,
          workspaceIdentity: params.workspaceIdentity ?? null,
          workspaceKey: resolveWorkspaceKey(params),
      ...
      logger.info(command.traceId, "Mesacode task command 已取消", {
        commandId: command.commandId,
        status: command.status,
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
    ...
    async stopGeneration(params): Promise<void> {
      ...
      logger.info(params.runId, "Mesacode task facade stopGeneration 开始", {
      ...
      logger.info(params.runId, "Mesacode task facade stopGeneration ACK", {
        durationMs: Date.now() - startedAt,
        taskId: params.taskId,
        workspaceIdentity: target.workspaceIdentity ?? null,
        workspaceKey: resolveWorkspaceKey(target),
    ...
    async compactSession(params) {
      ...
      try {
        const result = await options.mesacodeAgentService.compactSession({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          sessionId: params.taskId,
    ...
    async goalSession(params) {
      ...
      logger.info(params.inputId, "[mesacode-task-service] goalSession start", {
      ...
      const result = await options.mesacodeAgentService.goalSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
        inputId: params.inputId,
      ...
      logger.info(params.inputId, "[mesacode-task-service] goalSession agent 返回", {
        action: params.action,
        durationMs: Date.now() - startedAt,
      ...
      emitWorkspaceTaskListChanged(target, meta, "task_status_changed");
      logger.info(params.inputId, "[mesacode-task-service] goalSession task index 同步完成", {
        action: params.action,
        durationMs: Date.now() - startedAt,
        startedTurn: result.startedTurn,
        status: result.snapshot.session.status,
    ...
    async closeTask(params): Promise<void> {
      ...
      await options.mesacodeAgentService.closeSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async resumeTask(params): Promise<MesacodeTaskMeta> {
    ...
    async listTasks(params): Promise<MesacodeTaskMeta[]> {
    ...
    async listPinnedTasks(params): Promise<MesacodeTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async listTaskList(params: MesacodeTaskListQuery): Promise<MesacodeTaskListResult> {
    ...
    async listArchivedTasks(params): Promise<MesacodeTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async archiveStaleTasks(params): Promise<MesacodeTaskMeta[]> {
    ...
    async archiveWorkspaceTasks(params): Promise<MesacodeTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async getTaskSnapshot(params): Promise<MesacodeTaskSnapshot | null> {
      const startedAt = Date.now();
      ...
      const mesacodeSnapshot = limitTaskSnapshotMessages(
        snapshotToMesacode(snapshot, {
      ...
      const indexedMeta = await syncTaskIndexMeta(mesacodeSnapshot.meta);
      const indexDurationMs = Date.now() - indexStartedAt;
      logger.info(undefined, "[mesacode-task-service] 历史快照读取完成", {
        ...
        stats: getTaskSnapshotMessageDiagnostics(mesacodeSnapshot),
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
        workspacePath: params.workspacePath,
      });
      return { ...mesacodeSnapshot, meta: indexedMeta };
    },
    ...
    async getTaskSnapshotWithEtag(params) {
      ...
      if (!snapshot) {
        logger.info(undefined, "[mesacode-task-service] 历史快照 ETag 读取为空", {
          clientMode: params.clientMode ?? "unknown",
          durationMs: Date.now() - startedAt,
          messageLimit: params.messageLimit ?? null,
          taskId: params.taskId,
      ...
      if (params.ifNoneMatch && params.ifNoneMatch === etag) {
        logger.info(undefined, "[mesacode-task-service] 历史快照 ETag 命中缓存", {
          clientMode: params.clientMode ?? "unknown",
          durationMs: Date.now() - startedAt,
          etagDurationMs,
          messageLimit: params.messageLimit ?? null,
      ...
      logger.info(undefined, "[mesacode-task-service] 历史快照 ETag 生成完成", {
        clientMode: params.clientMode ?? "unknown",
        durationMs: Date.now() - startedAt,
        etagDurationMs,
        messageLimit: params.messageLimit ?? null,
    ...
    async getTaskSnapshotBody(): Promise<MesacodeTaskSnapshotBody | null> {
      return null;
    },

    async getTaskSnapshotRef(): Promise<MesacodeTaskSnapshotRefContent | null> {
      return null;
    },

    async getTaskSnapshotToolCallsSlice(): Promise<MesacodeTaskSnapshotToolCallsSlice | null> {
      return null;
    },

    async getTaskMeta(params): Promise<MesacodeTaskMeta | null> {
      const meta = await taskIndexRepo.getTaskMeta(params);
      return meta ? rememberIndexedTaskMeta(meta) : null;
    },

    async getTaskConfigOptions(params): Promise<MesacodeConfigOption[]> {
    ...
    async getTaskModelSelection(params): Promise<ModelSelection | null> {
      const target = getTaskTarget(params.taskId);
      const snapshot = await options.mesacodeAgentService.readSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async setAssistantMessageFeedback(params): Promise<MesacodeSessionFile> {
      const snapshot = await service.getTaskSnapshot(params);
      if (!snapshot) {
      ...
      if (targetMessage) {
        targetMessage.feedback = params.feedback as MesacodeAssistantMessageFeedback | undefined;
      }
      return snapshot;
    },
    ...
    async scanImportableClaudeSessions(params: {
      ...
      limit?: number;
    }): Promise<MesacodeImportableSessionCandidate[]> {
    ...
    async importClaudeSessions(params: {
      ...
      workspaceIdentity?: string;
      sessionIds: string[];
    }): Promise<MesacodeImportSessionsResult> {
      return importClaudeNativeSessions({
        ...
        createImportedSession: async (source) => {
          const targetWorkspaceIdentity = params.workspacePath
            ...
            : undefined;
          const snapshot = await options.mesacodeAgentService.createSession({
          ...
          // 导入后的任务必须是真实 Mesacode session，setModel/sendPrompt 才能继续命中 runtime。
    ...
    async setMode(params): Promise<void> {
      ...
      const target = getTaskTarget(params.taskId);
      await switchCollaborationModeViaProtocol(target, toMesacodeMode(params.mode) ?? "build");
      const snapshot = await resumeSnapshot(target);
      await syncTaskIndexSnapshot(snapshot);
    },

    async setConfigOption(params): Promise<MesacodeConfigOption[]> {
      const target = getTaskTarget(params.taskId);
      ...
      if (params.configId === THOUGHT_LEVEL_CONFIG_ID) {
        ...
        const current = await options.mesacodeAgentService.readSession({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          sessionId: params.taskId,
        });
        ...
        await switchCollaborationModeViaProtocol(
          target,
          toMesacodeMode(params.value as MesacodeTaskMode) ?? "build",
        );
      }
      const snapshot = await resumeSnapshot(target);
      await syncTaskIndexSnapshot(snapshot);
    ...
    async setModel(params): Promise<MesacodeConfigOption[]> {
      ...
      const target = getTaskTarget(params.taskId);
      await options.mesacodeAgentService.setModel({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
    ...
    async setAutomationSessionConfig(params): Promise<MesacodeConfigOption[]> {
      const target = getTaskTarget(params.taskId);
    ...
    async getTaskNativeSessionLogFile() {
      const path = resolveMesacodeAgentCurrentLogFilePath();
      // 返回 Mesacode Agent 的结构化日志 JSONL；日志行中的 sessionId 用于按当前任务排查。
      return { provider: GLM_PROVIDER, path, exists: existsSync(path) };
    },

    async getModelTrajectory(params) {
      // Mesacode Agent 把 taskId 当作 sessionId 落盘 model-io（见本文件其它 sessionId: params.taskId 用法），
      // 这里按 sessionId 还原该 task 的模型调用轨迹，供 UI 侧边栏可视化。
      const trajectory = await readModelTrajectory(params.taskId, params.limit);
      logger.info(
        `[MesacodeTaskService] getModelTrajectory taskId=${params.taskId} records=${trajectory.records.length} files=${trajectory.sourceFiles.length} truncated=${trajectory.truncated}`,
      );
      return trajectory;
    },

    async getTaskTokenUsage(params): Promise<MesacodeTaskTokenUsageResult> {
      // 摘要面板需要展示 task 的累计模型消耗，不能复用 usage_update 的 context window。
      // 这里通过 Mesacode Protocol 读 agent SQLite 的 model_usage 聚合，保持桌面和远控同一事实源。
      return options.mesacodeAgentService.getTaskTokenUsage({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async getTaskSessionFilePath(params) {
      return {
        path: `${params.workspacePath}/${params.taskId}.mesacode-session`,
        exists: false,
      };
    },
    ...
    async deleteArchivedTasks(params): Promise<MesacodeArchivedTaskDeletionResult> {
      const result: MesacodeArchivedTaskDeletionResult = {
        deletedTaskIds: [],
        skippedTaskIds: [],
        failedTaskIds: [],
      };
    ...
    async renameTask(params): Promise<MesacodeTaskMeta> {
      const renamedAt = Date.now();
      logger.info(undefined, "[MesacodeTaskService] renameTask start", {
        taskId: params.taskId,
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        workspaceKey: resolveWorkspaceKey(params),
      ...
      try {
        ...
        logger.info(undefined, "[MesacodeTaskService] renameTask overlay set", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        });
        ...
        logger.info(undefined, "[MesacodeTaskService] renameTask index updated", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        ...
        logger.info(undefined, "[MesacodeTaskService] renameTask event emitted", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        });
        return meta;
      } catch (error) {
        logger.error(undefined, "[MesacodeTaskService] renameTask failed", {
          taskId: params.taskId,
          workspacePath: params.workspacePath,
          workspaceIdentity: params.workspaceIdentity,
          workspaceKey: resolveWorkspaceKey(params),
    ...
    async setTaskPinned(params): Promise<MesacodeTaskMeta> {
    ...
    async setTaskUnread(params): Promise<MesacodeTaskMeta> {
    ...
    async archiveTask(params): Promise<MesacodeTaskMeta> {
    ...
    async unarchiveTask(params): Promise<MesacodeTaskMeta> {
    ...
    async branchTaskFromPrompt(): Promise<MesacodeTaskCreateResult> {
      unsupported("branchTaskFromPrompt");
    },

    onDynamicStreamEvent(taskId: string): Event<MesacodeStreamEvent> {
      return getGlobalTaskEmitter(taskId).event;
    },

    onDynamicTaskTerminalOutcome(taskId: string): Event<MesacodeTaskTerminalOutcome> {
    ...
    onDynamicTaskReady(taskId: string): Event<MesacodeTaskReadyOutcome> {
      return (listener) =>
    ...
    onDynamicTaskEvent(params): Event<MesacodeStreamEvent> {
      const target = {
        taskId: params.taskId,
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
      ...
      return (listener) => {
        ...
        // 已收敛 v4 命令；读路径消费方是
        // host 镜像 taskRealtimePort（词表同为 MesacodeStreamEvent），镜像换 v4 帧 = relay
        // 协议与手机 store 整链重做。
        // 过渡归宿 = replayable 读路径 v4 store，与 host/index.ts 镜像、
        // mapStateUpdated/mapServiceEvent、agentService.onDynamicSessionEvent 同批摘除。
        const upstreamDisposable = options.mesacodeAgentService.onDynamicSessionEvent({
          workspacePath: params.workspacePath,
          workspaceIdentity: params.workspaceIdentity,
          sessionId: params.taskId,
          deliveryKind: toMesacodeDeliveryKind(params.deliveryKind),
    ...
    onDynamicWorkspaceEvent(workspace): Event<MesacodeWorkspaceEvent> {
    ...
    disposeAll(): void {
      ...
      options.mesacodeAgentService.disposeAll();
    ...
    async disposeAllAndWait(): Promise<void> {
      ...
      // 否则 host 退出时会把 mesacode-cli 的 SIGKILL 兜底 timer 一起带走。
      taskIndexSyncer.disposeAll();
      await disposeMesacodeAgentServiceAndWait();
      disposeLocalTaskState();
    },
  };
...
function applyOverlayToMeta(meta: MesacodeTaskMeta, overlay: TaskOverlay): MesacodeTaskMeta {
  return {
    ...meta,
...
function deriveTitleFromSnapshot(snapshot: MesacodeSessionStateSnapshot): string {
  return resolveMesacodeVisibleSessionTitle({
...
function buildSnapshotHistory(
  ...
  messageLimit: number,
): NonNullable<MesacodeTaskSnapshot["history"]> {
  return {
...
function getTaskSnapshotMessageDiagnostics(snapshot: MesacodeTaskSnapshot) {
...
function parseModelPickerValue(value: string): ModelSelection {
  ...
  if (customModel?.providerId && customModel.modelName) {
    // UI 下拉的 custom:provider:model 只是展示态，不能原样传给 mesacode-cli。
...
function toMesacodeMode(mode: MesacodeTaskMode | undefined): MesacodeSessionMode | undefined {
  switch (mode) {
    case "plan":
      return "plan";
    case "edit":
...
function fromMesacodeMode(mode: MesacodeSessionMode): MesacodeTaskMode {
...
function addSessionForkSnapshotFallback(
  ...
  return [
    ...messages,
    {
      id: `mesacode-timeline-fork-${parentSessionId}-`,
      role: "user",
      content: "",
...
function addGoalVerificationTimelineSnapshotFallback(
  ...
  const timelineMessages = timeline
    ...
    .map<MesacodePersistedMessage>((item) => ({
...
function goalVerificationTimelineIdentityKey(
  item: Extract<MesacodeGoalVerificationTimelineMeta, { type: "goal_verification" }>,
): string {
...
function goalVerificationTimelineMessageId(
  ...
  if (typeof item.goalIteration === "number") {
    return `mesacode-goal-verification-${item.targetId}-${item.goalIteration}`;
  }
  return `mesacode-goal-verification-${item.verificationId}`;
}

function insertGoalVerificationTimelineMessages(
  messages: MesacodePersistedMessage[],
  timelineMessages: MesacodePersistedMessage[],
): MesacodePersistedMessage[] {
...
function getSnapshotGoalActiveIterationCount(snapshot: MesacodeSessionStateSnapshot): number {
  ...
  return getMesacodeGoalActiveIterationCount({
    targetStatus: snapshot.projection.target?.status ?? null,
    timeline,
  });
}

function toMesacodeDeliveryKind(
...
function backgroundTaskNotificationToolUpdateFromInput(params: {
  ...
  taskId: string;
  traceId: TraceId;
}): Extract<MesacodeStreamEvent, { type: "tool_call_update" }> | null {
  const parsed = parseMesacodeBackgroundTaskNotificationText(params.input);
  if (!parsed) {
    return null;
  }
  const status = mesacodeBackgroundTaskNotificationToolUpdateStatus(parsed.notification.status);
  return {
    ...
    ...(status === "failed" && parsed.notification.error
      ...
      : {}),
    raw: attachMesacodeBackgroundTaskNotificationToRaw(
...
function mapMessage(
  message: MesacodeMessageWithParts,
  goalIteration?: number,
  backgroundTaskNotifications?: ReadonlyMap<string, MesacodeBackgroundTaskNotificationInfo>,
): MesacodePersistedMessage {
  const tools: MesacodePersistedToolCall[] = [];
  const parts: MesacodePersistedMessagePart[] = [];
  const attachments =
    message.info.role === "user" ? mapPromptAttachmentsFromParts(message.parts) : undefined;
  let syntheticTimeline: MesacodeTimelineMeta | undefined;
...
function mapPromptAttachmentsFromParts(
  parts: readonly MesacodeMessagePart[],
): MesacodePromptAttachment[] | undefined {
  const attachments = parts
    .filter((part): part is Extract<MesacodeMessagePart, { type: "file" }> => part.type === "file")
    .map(mapPromptAttachmentFromFilePart)
    .filter((attachment): attachment is MesacodePromptAttachment => attachment !== undefined);
  return attachments.length > 0 ? attachments : undefined;
}

function mapPromptAttachmentFromFilePart(
  part: Extract<MesacodeMessagePart, { type: "file" }>,
): MesacodePromptAttachment | undefined {
...
function extractSyntheticTimelineFromTextPart(
  part: Extract<MesacodeMessagePart, { type: "text" }>,
): MesacodeTimelineMeta | undefined {
...
function synthesizeCompactionTimeline(
  part: Extract<MesacodeMessagePart, { type: "compaction" }>,
): MesacodeTimelineMeta | undefined {
  const metadata = asRecord(part.metadata);
...
function mapToolPart(
  part: Extract<MesacodeMessagePart, { type: "tool" }>,
  backgroundTaskNotifications?: ReadonlyMap<string, MesacodeBackgroundTaskNotificationInfo>,
): MesacodePersistedToolCall {
  const state = part.state;
  const taskNotification = backgroundTaskNotifications?.get(part.callId);
  // Mesacode Protocol 的 part.callId 是实时流和终态 snapshot 共同的工具身份。
  // 以前只保存 metadata 会丢掉 toolCallId，手机 replayable 里 result-only 临时工具就无法被终态快照覆盖。
  const raw = attachMesacodeBackgroundTaskNotificationToRaw(
  ...
  if (state.status === "completed") {
    ...
    const notificationStatus = taskNotification?.status
      ? mesacodeBackgroundTaskNotificationToolUpdateStatus(taskNotification.status)
      : undefined;
...
function textFromParts(parts: readonly MesacodeMessagePart[]): string {
  return textFromMesacodeMessageParts(parts);
}

function reasoningFromParts(parts: readonly MesacodeMessagePart[]): string | undefined {
  const text = parts
    .filter(
      (part): part is Extract<MesacodeMessagePart, { type: "reasoning" }> => part.type === "reasoning",
    )
    .map((part) => part.text)
    .join("");
...
function fromMesacodeGoal(goal: unknown): MesacodeTaskGoal {
  ...
  return {
    ...
    status: isMesacodeTaskGoalStatus(status) ? status : "active",
...
function isMesacodeTaskGoalStatus(status: string | undefined): status is MesacodeTaskGoal["status"] {
  return (
    status === "active" ||
    status === "paused" ||
...
function sessionTodosToPlanSteps(
  todos: MesacodeSessionStateSnapshot["todos"],
): MesacodePlanStep[] | null {
...
function sessionGoalStatsToRuntime(
  stats: MesacodeSessionStateSnapshot["goalStats"],
): MesacodeTaskGoalStats | null {
  return stats ? { ...stats } : null;
}

function sessionTodoGroupsToRuntime(
  groups: MesacodeSessionStateSnapshot["todoGroups"],
): MesacodeTodoGroup[] | null {
...
function mapStateUpdated(
  ...
  const parsedSettings = mesacodeSessionSettingsStateSchema.safeParse(notification.patch);
  ...
  return [
    {
      ...
      taskId: params.taskId,
      traceId,
      currentModeId: normalizeAvailableMesacodeMode(parsedSettings.data.mode.current),
      availableModes: getMesacodeAgentAvailableModes(),
    },
    {
...
function mapSessionEvent(
  ...
  toolProjectionMemory?: MesacodeToolProjectionMemory,
  backgroundTaskControlsByTaskKey?: Map<string, MesacodeBackgroundTaskControlItem[]>,
  ...
  // Mesacode Protocol runtime trace 只在事件没有 inputId 时兜底，避免同一轮 chunk/tool/complete 被拆成不同 trace。
  ...
  if (eventInputId && eventInputId !== protocolTraceId) {
    logger.debug(
      eventInputId,
      `对齐 Mesacode prompt inputId eventType=${event.type} protocolTrace=${protocolTraceId}`,
    );
  }
  ...
  if (event.type === "turn.started") {
    ...
    const runStartedEvent: MesacodeStreamEvent = {
  ...
  if (event.type === "turn.completed") {
    const events: MesacodeStreamEvent[] = [];
    ...
    if (stringValue(payload.turnPhase) === "compact") {
      return [
        compactFailureToTimelineEvent(
          ...
          traceId,
          eventInputId,
          stringValue(errorPayload.message) ?? "Mesacode compact failed",
        ),
      ];
    }
    ...
    return [
      {
        ...
        traceId,
        ...(eventInputId ? { inputId: eventInputId } : {}),
        error: stringValue(errorPayload.message) ?? "Mesacode session failed",
...
): Extract<MesacodeStreamEvent, { type: "session_info_update" }> | null {
  if (!hasActiveApiRetry || !isMesacodeModelRetryRecoveryProgressPayload(payload)) {
    return null;
  }
...
function mapToolInputStreaming(
  ...
  if (kind === "tool_input_delta") {
    const state = appendMesacodeStreamingToolInputDelta(
      streamingToolInputById?.get(toolId),
      stringValue(payload.delta) ?? "",
    );
    streamingToolInputById?.set(toolId, state);
    if (!shouldMaterializeMesacodeStreamingToolInputPreview(state, { toolName })) {
    ...
    const preview = buildMesacodeStreamingToolInputPreview(state.rawInput);
    markMesacodeStreamingToolInputPreviewMaterialized(state);
  ...
  if (kind === "tool_call") {
    ...
    const preview = buildMesacodeStreamingToolInputPreview(
      rawInput,
    ...
    if ((hasCompleteInput || preview.complete) && toolProjectionMemory) {
      finalizeMesacodeToolProjectionInput(toolId, preview.input, toolProjectionMemory);
    }
...
function logStreamingToolInputProjection(
  ...
  logger.debug(traceId, "Mesacode streaming tool input projected", {
    ...details,
    event: "mesacode.task.streaming_tool_input.projected",
  });
}
...
function mapCompactTimelinePayload(
  ...
  inputId: InputId | undefined,
  payload: Record<string, unknown>,
): Extract<MesacodeStreamEvent, { type: "agent_message_chunk" }> | null {
...
function mapSyntheticTimelinePartPayload(
  ...
  const timeline = extractSyntheticTimelineFromTextPart(
    part as Extract<MesacodeMessagePart, { type: "text" }>,
  );
  if (!timeline) {
    return null;
  }
...
function compactTimelineMetaFromPayload(
  payload: Record<string, unknown>,
  inputId: InputId | undefined,
): MesacodeContextCompactionTimelineMeta | null {
...
function compactFailureToTimelineEvent(
  ...
  inputId: InputId | undefined,
  reason: string,
): Extract<MesacodeStreamEvent, { type: "agent_message_chunk" }> {
  return {
    type: "agent_message_chunk",
    taskId,
    traceId,
...
function mapToolUpdated(
  ...
  const rememberedTool = resolveMesacodeToolProjectionMetadata(payload, toolId, memory);
  ...
  if ("input" in payload && "toolName" in payload) {
    ...
    toolNameById?.set(toolId, toolName);
    finalizeMesacodeToolProjectionInput(toolId, payload.input, memory);
    const toolEvent: MesacodeStreamEvent = {
      type: "tool_call",
      taskId,
      traceId,
      ...(inputId ? { inputId } : {}),
  ...
  if ("result" in payload) {
    ...
    // Mesacode Protocol 的 ToolCallResult 只有 toolCallId/result，不再重复带 toolName。
    // 去掉 Mesacode Agent 后如果不记住前序 ToolCallScheduled 的 TodoWrite 名称，result 里的 todos
    ...
    if (status !== "in_progress") {
      forgetMesacodeToolProjectionMetadata(toolId, memory);
    }
    const planSteps = isMainAgentToolProjectionSource(payload)
      ? extractPlanStepsFromToolOutput({
          title: toolName,
...
function parentToolUseIdFromToolPayload(payload: Record<string, unknown>): string | null {
  // Mesacode Protocol 发送的父级字段叫 parentToolCallId；
...
function permissionRequestToStreamEvent(
  taskId: string,
  request: MesacodePermissionRequestParams,
): MesacodePermissionRequest {
  return {
...
function pendingPermissionToStreamEvent(
  taskId: string,
  permission: MesacodeSessionStateSnapshot["projection"]["pendingPermissions"][number],
): MesacodePermissionRequest {
  return {
    type: "permission_request",
    taskId,
...
function userInputRequestToElicitationStreamEvent(
  taskId: string,
  request: MesacodeUserInputRequestParams,
): MesacodeStreamEvent {
  const questions =
...
): Extract<MesacodeStreamEvent, { type: "elicitation_response" }> {
  return {
...
function pendingUserInputBackedPermissionToElicitationEvent(
  taskId: string,
  permission: MesacodeSessionStateSnapshot["projection"]["pendingPermissions"][number],
): Extract<MesacodeStreamEvent, { type: "elicitation_request" }> | null {
...
function pendingExitPlanModeToElicitationEvent(
  taskId: string,
  permission: MesacodeSessionStateSnapshot["projection"]["pendingPermissions"][number],
): Extract<MesacodeStreamEvent, { type: "elicitation_request" }> {
...
function setTaskBackgroundTaskControlCache(
  cache: Map<string, MesacodeBackgroundTaskControlItem[]> | undefined,
  cacheKey: string,
  jobs: MesacodeBackgroundTaskControlItem[],
) {
...
function updateTaskBackgroundTaskControlCacheFromPayload(
  ...
  cacheKey: string,
  payload: Record<string, unknown>,
): MesacodeBackgroundTaskControlItem[] | null {
  const parsedJobs = parseMesacodeBackgroundTaskControlItems([payload]);
  ...
  const nextJobs = mergeMesacodeBackgroundTaskControlItems(cache.get(cacheKey) ?? [], parsedJobs);
...
function permissionOptionsFromPayload(payload: Record<string, unknown>): MesacodePermissionOption[] {
  return Array.isArray(payload.options) ? (payload.options as MesacodePermissionOption[]) : [];
}
...
function permissionResolvedPayloadToStreamEvents(
  ...
  } as Extract<MesacodeStreamEvent, { type: "permission_response" }>;
...
function mapSessionInfoLikePayload(
  ...
  const networkDebugStatus = mesacodeTaskNetworkDebugStatusFromPayload({
    taskId,
    traceId,
    ...(inputId ? { inputId } : {}),
    ...(eventId ? { eventId } : {}),
  ...
  if ("target" in payload && ("action" in payload || "source" in payload)) {
    events.push({
      ...
      target: {
        ...
        target: payload.target ? fromMesacodeGoal(payload.target as never) : null,
        previousTarget: payload.previousTarget
          ? fromMesacodeGoal(payload.previousTarget as never)
          : undefined,
      },
    });
  }
  ...
  if (Array.isArray(projection.backgroundJobs)) {
    const jobs = parseMesacodeBackgroundTaskControlItems(projection.backgroundJobs);
    setTaskBackgroundTaskControlCache(
      backgroundTaskControlsByTaskKey,
...
function taskTokenUsageDeltaFromPayload(
  ...
  eventId: string | undefined,
  payload: Record<string, unknown>,
): Extract<MesacodeStreamEvent, { type: "task_token_usage_delta" }> | null {
...
function apiRetryFromSessionInfoPayload(
  payload: Record<string, unknown>,
): MesacodeApiRetryStatus | null | undefined {
...
function recordAgentModelNetworkTelemetry(event: MesacodeSessionEvent): void {
...
function buildAgentModelNetworkInterface(payload: Record<string, unknown>): string {
  ...
  return `mesacode_agent.model.${providerKind}.${transport}.${base}`;
}
...
type ContextUsageUpdate = Pick<
  Extract<MesacodeStreamEvent, { type: "usage_update" }>,
  "used" | "size" | "cost" | "cache" | "breakdown"
>;

function contextUsageFromProjection(
  projection: MesacodeSessionStateSnapshot["projection"],
...
function contextUsageFromRuntime(
  runtimeUsage: MesacodeSessionStateSnapshot["runtime"]["contextUsage"] | undefined,
): ContextUsageUpdate | null {
...
function contextUsageFromPayload(payload: Record<string, unknown>): ContextUsageUpdate | null {
  ...
  // Mesacode Protocol 的 session.updated 里 contextUsed/contextWindow 是 projection 事实源；
...
function optionalContextUsageBreakdownFromPayload(
  ...
  const parsed = mesacodeContextUsageBreakdownSchema.safeParse(payload.contextUsageBreakdown);
...
function usageFromPayload(value: unknown): MesacodeUsage | undefined {
...
function timelineStatusValue(value: unknown): MesacodeTimelineStatus | undefined {
...
function timelineTriggerValue(value: unknown): MesacodeTimelineTrigger | undefined {
  return value === "manual" ||
    value === "auto" ||
    value === "reactive" ||
    value === "partial" ||
...
function turnSteerSourceValue(value: unknown): MesacodeTurnSteerSource | undefined {
  return value === "plan_approval_feedback" || value === "workflow_refine_feedback"
    ? value
    : undefined;
}

function turnSteerCommandKindValue(value: unknown): MesacodeTurnSteerCommandKind | undefined {
