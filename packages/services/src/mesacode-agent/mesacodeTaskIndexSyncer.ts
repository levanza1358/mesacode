...
import { repairSubagentTaskIndex } from "#src/mesacode-agent/repairSubagentTaskIndex.js";
import {
  deriveMesacodeTaskStatusFromSessionSnapshot,
  generateTraceId,
  getMesacodeUserVisibleMessages,
  isMesacodeGoalContinuationReminderText,
  isMesacodeModelOnlySyntheticUserMessage,
  resolveWorkspaceKey,
...
import { Emitter, type Event, type IDisposable } from "@mesacode/rpc";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import { TaskIndexRepo } from "#src/session/taskIndexRepo.js";
import type { MesacodeWorkspaceEventSubscriptionParams } from "#src/session/mesacodeTaskListTypes.js";
import type {
...
import { formatTaskMetaModelSelectionFromSnapshot } from "./mesacodeConfigOptions.js";

const logger = createServiceLogger("mesacode-task-index-syncer");
...
export interface MesacodeTaskIndexTerminalEvent {
...
export interface MesacodeTaskIndexReadyEvent {
  target: MesacodeAgentSessionTarget;
  /** v4 phase 终态映射：agent 收口后可接受下一条输入的 ready 边界。 */
  reason: "prompt_completed" | "prompt_failed";
}

export interface MesacodeTaskIndexSyncer {
  /**
   ...
   */
  ensureWorkspaceSubscription(target: MesacodeAgentWorkspaceTarget): void;
  /**
   ...
   * desktop-continuous 路径上，mesacodeSessionService 在 createSession/resumeSession/setModel
  ...
  syncSnapshotAndBroadcast(
    ...
    options: {
      ...
      unreadSignal?: MesacodeWorkspaceTaskListChanged["unreadSignal"];
      /**
       ...
       */
      broadcastReason: MesacodeWorkspaceTaskListChanged["reason"];
    },
   ...
   */
  syncTaskModel(target: MesacodeAgentSessionTarget, model: string): Promise<MesacodeTaskMeta | null>;
  /**
  ...
  emitWorkspaceTaskListChanged(
    ...
    meta: MesacodeTaskMeta | undefined,
    reason: MesacodeWorkspaceTaskListChanged["reason"],
    options?: Pick<MesacodeWorkspaceTaskListChanged, "unreadSignal">,
  ): void;
  /**
...
): MesacodeWorkspaceTaskListChanged["unreadSignal"] {
...
function taskStatusFromSummaryPhase(phase: SessionPhase): MesacodeTaskMeta["status"] {
  switch (phase) {
    case "running":
    case "prewarming":
      return "running";
...
function buildBaselineMetaFromSummary(
  target: MesacodeAgentWorkspaceTarget,
...
export function createMesacodeTaskIndexSyncer(
  options: CreateMesacodeTaskIndexSyncerOptions,
  ...
  const workspaceEmitters = new Map<string, Emitter<MesacodeWorkspaceEvent>>();
  const terminalEventEmitter = new Emitter<MesacodeTaskIndexTerminalEvent>();
  const readyEventEmitter = new Emitter<MesacodeTaskIndexReadyEvent>();
  let disposed = false;
  ...
  const isRuntimeUnavailableError = (error: unknown): boolean =>
    typeof error === "object" &&
    error !== null &&
    (error as { code?: unknown }).code === MESACODE_AGENT_RUNTIME_UNAVAILABLE_CODE;
  ...
  function getWorkspaceEmitter(workspace: WorkspaceEventInput): Emitter<MesacodeWorkspaceEvent> {
    ...
    if (!emitter) {
      emitter = new Emitter<MesacodeWorkspaceEvent>();
  ...
  function broadcastTargetFrom(target: MesacodeAgentSessionTarget): WorkspaceBroadcastTarget {
    return {
      workspacePath: target.workspacePath,
      workspaceIdentity: target.workspaceIdentity,
  ...
  function emitTerminalAndReady(target: MesacodeAgentSessionTarget, summary: SessionSummary): void {
  ...
  function applyTerminalTransition(
    ...
    void taskIndexRepo
      ...
      .then((meta) => {
        ...
        // 不经过 mesacodeSessionService 的 op 驱动 snapshot 同步，行缺失/正文搜索/lastError
  ...
  function applyTitleChange(target: MesacodeAgentSessionTarget, title: string): void {
    const updatedAt = Date.now();
    void taskIndexRepo
      .applyAgentPatch({
        workspacePath: target.workspacePath,
      ...
      .then((meta) => {
        if (meta) {
          ...
          // v4 createSession 不经过 mesacodeSessionService.createSession；首个标题到达且
  ...
  function processSummary(
    ...
    if (becameVisibleTask) {
      ...
      // 不经过 mesacodeSessionService.createSession。此处是最早且不依赖标题时序的新任务边界；
  ...
  async function seedMissingRowsFromInitialSnapshot(
    ...
    // 纯 V4 UI 不经过 mesacodeSessionService.initializeWorkspace；若把首帧
  ...
  function applyWorkspaceConfigFrame(
    ...
    if (!config || config.configOptions.length === 0) {
      // 空目录（无 live session 的订阅种子）不下发：下游 useMesacodeConfig 收到空
      // configOptions 会把聊天工具栏的模型目录清掉。
      completeConfigRecoveryFrame(state, deliveryKind);
      return;
    }
    // v4 载荷与 MesacodeConfigOption 结构对齐（shared 黄金测试背书），零映射直通，
    // 下游 workspace_config_options_update 消费面（useMesacodeConfig 等）不改。
    getWorkspaceEmitter({
  ...
  function ensureWorkspaceSubscription(target: MesacodeAgentWorkspaceTarget): void {
    if (disposed) {
      return;
    }
...
function hasUserVisibleContent(snapshot: MesacodeSessionStateSnapshot): boolean {
  const title = snapshot.session.title?.trim() ?? "";
  if (title && !isMesacodeGoalContinuationReminderText(title)) {
    return true;
  }
  ...
  return snapshot.messages.some((message) => {
    ...
    if (isMesacodeModelOnlySyntheticUserMessage(message)) return false;
...
// 与 UI 默认折叠后展示的范围一致。
function buildSearchableTextFromSnapshot(snapshot: MesacodeSessionStateSnapshot): string {
  const parts: string[] = [];
  let total = 0;
  // /goal 自动续跑会把内部 system-reminder 作为 runtime user turn 持久化；
  // 这条内容是 model-only 输入，不是用户可见 query，不能写进侧边栏搜索正文。
  for (const message of getMesacodeUserVisibleMessages(snapshot.messages, {
    target: snapshot.projection.target,
  })) {
    const textParts = message.parts.filter(
      (part): part is Extract<MesacodeMessagePart, { type: "text" }> => part.type === "text",
    );
...
function fromMesacodeGoal(
  goal: NonNullable<MesacodeSessionStateSnapshot["projection"]["target"]>,
): MesacodeTaskGoal {
  return {
