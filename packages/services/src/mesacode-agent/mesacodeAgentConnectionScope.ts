...
import { Emitter, Event as RpcEvent, type Event, type IDisposable } from "@mesacode/rpc";
import {
...
export type MesacodeAgentV4ClientMode = "desktop-continuous" | "web-remote-replayable";

export interface MesacodeAgentV4ConnectionContext {
  connectionId: string;
  clientMode: MesacodeAgentV4ClientMode;
  role?: "terminal-client" | "trusted-host-relay";
}

const TRUSTED_CONNECTION_FIELD = "__mesacodeTrustedV4Connection";
const TRUSTED_UNSUBSCRIBE_ROUTE_FIELD = "__mesacodeTrustedV4UnsubscribeRoute";

type TrustedConnectionCarrier = {
  [TRUSTED_CONNECTION_FIELD]?: MesacodeAgentV4ConnectionContext;
};

interface TrustedMesacodeAgentV4UnsubscribeRoute {
  topic: string;
  connectionId: string;
}

type TrustedUnsubscribeRouteCarrier = {
  [TRUSTED_UNSUBSCRIBE_ROUTE_FIELD]?: TrustedMesacodeAgentV4UnsubscribeRoute;
};

export function readTrustedMesacodeAgentV4UnsubscribeRoute(
  value: unknown,
): TrustedMesacodeAgentV4UnsubscribeRoute | null {
 ...
 */
export function readTrustedMesacodeAgentV4Connection(
  value: unknown,
): MesacodeAgentV4ConnectionContext | null {
  if (typeof value !== "object" || value === null) return null;
...
function withTrustedConnection<T extends object>(
  value: T,
  context: MesacodeAgentV4ConnectionContext,
): T {
...
function withTrustedUnsubscribeRoute<T extends object>(
  value: T,
  route: TrustedMesacodeAgentV4UnsubscribeRoute,
): T {
  const forwarded = {
    ...(value as unknown as Record<string, unknown>),
  };
...
function workspaceKey(target: MesacodeAgentWorkspaceTarget): string {
  return target.workspaceIdentity?.trim() || target.workspacePath;
}

function workspaceTarget(target: MesacodeAgentWorkspaceTarget): MesacodeAgentWorkspaceTarget {
  return {
...
interface ConnectionFlowRoute {
  key: string;
  target: MesacodeAgentWorkspaceTarget;
  connection: MesacodeAgentV4ConnectionContext;
}
...
function deliveryProfileFor(clientMode: MesacodeAgentV4ClientMode): HelloMessage["deliveryProfile"] {
  return clientMode === "desktop-continuous" ? "continuous" : "replayable";
}

function createHello(context: MesacodeAgentV4ConnectionContext): HelloMessage {
...
export interface MesacodeAgentConnectionScope {
  readonly service: IMesacodeAgentService;
...
export function createMesacodeAgentConnectionScope(
  base: IMesacodeAgentService,
  context: MesacodeAgentV4ConnectionContext,
  ...
  const forwardedConnection = (params: unknown): MesacodeAgentV4ConnectionContext => {
    const downstream = readTrustedMesacodeAgentV4Connection(params);
  ...
  const ownershipKey = (kind: SubscriptionKind, target: MesacodeAgentWorkspaceTarget, topic: string) =>
    `${kind}\0${workspaceKey(target)}\0${topic}`;
  const routedEventKey = (kind: SubscriptionKind, target: MesacodeAgentWorkspaceTarget) =>
    `${kind}\0${workspaceKey(target)}`;
  const flowRouteKey = (target: MesacodeAgentWorkspaceTarget, connectionId: string) =>
  ...
  const flowRouteForEntry = (entry: OwnedSubscription): ConnectionFlowRoute => ({
    ...
    } satisfies MesacodeAgentV4ConnectionContext,
  });
  ...
  const findBySubscription = (
    ...
    target: MesacodeAgentWorkspaceTarget,
    subscriptionId: string,
    expectedRoute: TrustedMesacodeAgentV4UnsubscribeRoute | null,
  ...
  const forwardedUnsubscribeRoute = (
    ...
    const downstream = readTrustedMesacodeAgentV4UnsubscribeRoute(params);
  ...
  const unsubscribeBase = (
    ...
    const params: MesacodeAgentConversationUnsubscribeParams = {
      ...entry.target,
  ...
  const resyncBase = (entry: OwnedSubscription, params: MesacodeAgentConversationResyncParams) => {
    const forwarded = withTrustedUnsubscribeRoute(params, {
      topic: entry.topic,
      connectionId: entry.connectionId,
    });
  ...
  const resyncOwned = (kind: SubscriptionKind, params: MesacodeAgentConversationResyncParams) => {
    assertReady();
  ...
  const overrides: Partial<IMesacodeAgentService> = {
