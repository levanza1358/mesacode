import { MesacodeStorageStartupGate } from "#src/mesacode-agent/mesacodeStorageStartupGate.js";
...
import type {
  ...
  MesacodeProtocolTrace,
} from "@mesacode/shared";
import type { V4Method } from "@mesacode/shared/mesacode-protocol-v4";
import type { z } from "zod";
import type { MesacodeProtocolTransport } from "./mesacodeProtocolTransport.js";

/** 客户端可发的方法名：旧 mesacodeProtocolMethods + v4/*（并存，收敛为 v4）。 */
type MesacodeProtocolClientMethod = MesacodeProtocolMethod | V4Method;

interface MesacodeProtocolClientOptions {
  requireStorageStartup?: boolean;
  requestTimeoutMs?: number;
}

interface MesacodeProtocolRequestTimeoutEvent {
  method: MesacodeProtocolClientMethod;
  requestId: MesacodeProtocolRequestId;
  timeoutMs: number;
}
...
interface MesacodeProtocolClientRequestOptions {
...
const DEFAULT_MESACODE_PROTOCOL_REQUEST_TIMEOUT_MS = 3 * 60_000;

class MesacodeProtocolClientError extends Error {
  constructor(
    message: string,
    ...
    super(message);
    this.name = "MesacodeProtocolClientError";
  }
}

export class MesacodeProtocolRequestTimeoutError extends Error {
  constructor(
    readonly method: MesacodeProtocolClientMethod,
    readonly requestId: MesacodeProtocolRequestId,
    readonly timeoutMs: number,
  ) {
    super(`Mesacode Protocol request timed out: ${method}`);
    this.name = "MesacodeProtocolRequestTimeoutError";
  }
}

export class MesacodeProtocolClient implements IDisposable {
  readonly storageStartup: MesacodeStorageStartupGate;
  private readonly pending = new Map<string, PendingRequest<unknown>>();
  private readonly notificationEmitter = new Emitter<MesacodeProtocolNotification>();
  private readonly requestEmitter = new Emitter<MesacodeProtocolRequest>();
  private readonly requestTimeoutEmitter = new Emitter<MesacodeProtocolRequestTimeoutEvent>();
   ...
   * 否则会立即抛 "Mesacode Protocol client is disposed"。
   */
  ...
  constructor(
    private readonly transport: MesacodeProtocolTransport,
    options?: MesacodeProtocolClientOptions,
  ) {
    this.storageStartup = new MesacodeStorageStartupGate(options?.requireStorageStartup ?? false);
    this.requestTimeoutMs = options?.requestTimeoutMs ?? DEFAULT_MESACODE_PROTOCOL_REQUEST_TIMEOUT_MS;
    this.disposables.push(
      transport.onMessage((message) => this.handleMessage(message)),
      transport.onClose((event) => {
        this.storageStartup.dispose();
        const suffix = event.reason ? `: ${event.reason}` : "";
        this.rejectAll(new Error(`Mesacode agent transport closed${suffix}`));
  ...
  async request<T = unknown>(
    ...
    const resultPromise = new Promise<T>((resolve, reject) => {
      const expire = () => {
        ...
        const error = new MesacodeProtocolRequestTimeoutError(method, id, requestTimeoutMs);
  ...
  async notify(method: MesacodeProtocolClientMethod, params?: unknown): Promise<void> {
    this.assertNotDisposed();
    await this.transport.send({ method, params });
  }

  async respond(id: MesacodeProtocolRequestId, result: unknown): Promise<void> {
    this.assertNotDisposed();
  ...
  private handleMessage(message: unknown): void {
    ...
    if ("result" in message && "id" in message) {
      this.resolveResponse(
        (message as { id: MesacodeProtocolRequestId; result: unknown }).id,
    ...
    if ("error" in message && "id" in message) {
      ...
      this.rejectResponse(
        errorMessage.id,
        new MesacodeProtocolClientError(
    ...
    if ("method" in message && "id" in message) {
      this.requestEmitter.fire(message as MesacodeProtocolRequest);
      return;
    }
    ...
    if ("method" in message) {
      if (
        ...
        this.storageStartup.accept((message as MesacodeProtocolNotification).params)
      ) {
      ...
      this.notificationEmitter.fire(message as MesacodeProtocolNotification);
    }
  }

  private resolveResponse(id: MesacodeProtocolRequestId, result: unknown): void {
    const requestKey = String(id);
    const pending = this.pending.get(requestKey);
    if (!pending) {
      return;
    ...
    try {
      ...
      pending.reject(
        error instanceof Error
          ? error
          : new Error(`Mesacode Protocol response parse failed: ${pending.method}`),
      );
    }
  }

  private rejectResponse(id: MesacodeProtocolRequestId, error: Error): void {
    const requestKey = String(id);
    const pending = this.pending.get(requestKey);
    if (!pending) {
      return;
  ...
  private disposeLocalResources(): void {
    this.rejectAll(new Error("Mesacode Protocol client disposed"));
  ...
  private assertNotDisposed(): void {
    if (this.disposed) {
      throw new Error("Mesacode Protocol client is disposed");
    }
  }
}
