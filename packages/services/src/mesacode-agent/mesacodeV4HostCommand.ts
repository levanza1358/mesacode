...
class MesacodeV4CommandRejectedError extends Error {
  readonly code = "MESACODE_V4_COMMAND_REJECTED";
  ...
  constructor(
    ...
    this.name = "MesacodeV4CommandRejectedError";
  }
}
...
export function assertV4CommandAckOk(
  ...
  throw new MesacodeV4CommandRejectedError(commandType, ack, contextMessage);
}
...
export async function sendHostCasCommandV4<T extends CommandType>(input: {
  ...
  throw new MesacodeV4CommandRejectedError(
    input.type,
    lastAck ?? {
      commandId: "",
      status: "stale",
