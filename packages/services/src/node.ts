...
export {
  ...
  MESACODE_E2E_FS_FAULTS_ALLOW_ENV,
  MESACODE_E2E_FS_FAULTS_ENV,
...
export {
  ...
  MESACODE_WINDOWS_APP_INSTALL_DIR_ENV,
} from "./paths.js";
...
export { createMesacodeAgentService } from "./mesacode-agent/mesacodeAgentService.js";
export { createMesacodeTaskServiceAdapter } from "./mesacode-agent/mesacodeTaskServiceAdapter.js";
export { createMesacodeSessionService } from "./mesacode-session/mesacodeSessionService.js";
export {
  resolveDefaultMesacodeAgentCommand,
  MesacodeAgentProcessManager,
} from "./mesacode-agent/mesacodeAgentProcessManager.js";
export type {
  MesacodeAgentCommand,
  MesacodeAgentCommandResolver,
  MesacodeAgentCommandResolverContext,
  MesacodeAgentProcessManagerOptions,
} from "./mesacode-agent/mesacodeAgentProcessManager.js";
export { MesacodeProtocolClient } from "./mesacode-agent/mesacodeProtocolClient.js";
export type { MesacodeProtocolTransport } from "./mesacode-agent/mesacodeProtocolTransport.js";
export { MesacodeStdioTransport } from "./mesacode-agent/mesacodeStdioTransport.js";
export {
  getMesacodeStdioTapDevLogDir,
  readMesacodeStdioTapDevState,
  setMesacodeStdioTapDevEnabled,
} from "./mesacode-agent/mesacodeStdioTapDevConfig.js";
export type { MesacodeStdioTapDevState } from "@mesacode/shared";
export {
...
import { IMesacodeAgentService } from "./mesacode-agent/mesacodeAgent.js";
import type { CuaOperationStateReporter } from "./mesacode-agent/cuaOperationTurnTracker.js";
import { IMesacodeSessionService } from "./mesacode-session/mesacodeSession.js";
import {
...
import { createMesacodeAgentService } from "./mesacode-agent/mesacodeAgentService.js";
import type { MesacodeAgentCommandResolver } from "./mesacode-agent/mesacodeAgentProcessManager.js";
import { buildAgentTelemetrySpawnEnv } from "./mesacode-agent/agentTelemetryEnv.js";
import { resolveMesacodeAgentPresentationSurface } from "./mesacode-agent/mesacodeAgentPresentationSurface.js";
import { createMesacodeTaskServiceAdapter } from "./mesacode-agent/mesacodeTaskServiceAdapter.js";
import { createMesacodeSessionService } from "./mesacode-session/mesacodeSessionService.js";
import { createMesacodeTaskIndexSyncer } from "./mesacode-agent/mesacodeTaskIndexSyncer.js";
...
import { readLegacyMesacodeConfigProviders } from "./model-provider/legacyMesacodeConfigProviderReader.js";
...
import { fetchMesacodeBuiltinRemoteRelease } from "./model-provider/mesacodeBuiltinRemoteConfig.js";
import {
  createProviderRuntimeFromConfigRuntime,
...
import { buildHelperOpenArgs, isCuaLocalDevelopmentRuntime } from "@mesacode/cua/broker/server";
...
import {
  ...
  isPotentialMesacodeCuaAgentMcpServer,
...
import { DEV_HELPER_APP_NAME, HELPER_APP_NAME } from "@mesacode/cua/broker/helperConstants";
import { resolveBrokerSocketPath } from "@mesacode/cua/broker/socketPath";
import {
  DEFAULT_MESACODE_MODEL_CONTEXT_BUDGET_STRATEGY,
  resolveSafeEndpointHostname,
  MESACODE_JWT_INVALID_BROADCAST_CHANNEL,
  formatLogPrefix,
  ...
  type BrowserCommand,
  isMesacodeCuaMcpCommand,
  isMesacodeCuaMcpPackageArg,
  isMesacodeCuaInternalFeatureEnabled,
  MESACODE_CUA_PLUGIN_AUTHORITY_ENV_KEY,
  type MesacodeAutomation,
  type MesacodeAutomationRun,
  getCapturedMesacodeAgentTelemetryEnv,
  MESACODE_DESKTOP_CONTEXT_PROMPT_ENABLED_ENV,
  ZAI_PROVIDER_ID,
  mesacodeAccountAccessSchema,
  mesacodeProviderAccountAccessSchema,
  MESACODE_VERSION,
  buildRuntimeMesacodeApiUrl,
} from "@mesacode/shared";

// 这些 conversation-share 实现依赖 Node 文件系统；仅通过 @mesacode/services/node 暴露，
...
export function shouldEnableDefaultCuaProductHelper(
  ...
  // CUA 已随正式版默认开启（isMesacodeCuaInternalFeatureEnabled 默认 ON，仅显式 0/false/off 关闭；2026-08 注释更正——旧注释称默认关闭已过期）。显式开启后 macOS 使用既有产品 Helper，Windows 使用安装包内 runtime；
  // 两端都保持按需启动。关闭时不创建 host、不探测资源、不产生子进程或权限提示。
  const env = options.env ?? process.env;
  if (!isMesacodeCuaInternalFeatureEnabled(env)) return false;
  ...
  return {
    async resolveMcpServers(servers, context) {
      ...
      return options.isResolverCurrent?.(resolver, context) === false
        ? servers?.filter((server) => !isPotentialMesacodeCuaAgentMcpServer(server))
        : resolved;
    },
    async restart() {
      ...
      if (!resolver) {
        throw new Error("Mesacode Computer Use is not enabled (plugin off or not product mode).");
      }
...
export const MESACODE_CUA_BUNDLED_HELPER_APP_PATH_ENV = "MESACODE_CUA_BUNDLED_HELPER_APP_PATH";

export function resolveBundledCuaHelperAppPath(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const injectedPath = env[MESACODE_CUA_BUNDLED_HELPER_APP_PATH_ENV]?.trim();
  if (injectedPath) {
    return injectedPath;
  }
  const resourcesPath = (
...
export function hasGlobalCliMesacodeCuaServer(env: NodeJS.ProcessEnv = process.env): boolean {
  const home = env.HOME?.trim() || homedir();
  const configPath = join(home, ".mesacode", "cli", "config.json");
  let parsed: unknown;
  try {
  ...
  return Object.entries(parsed.mcp.servers).some(([name, config]) =>
    isGlobalCliMesacodeCuaServer(name, config),
  );
}

function isGlobalCliMesacodeCuaServer(name: string, config: unknown): boolean {
  if (!isRecord(config)) return false;
  if (config.enabled === false) return false;
  if (typeof config.type === "string" && config.type !== "stdio") return false;
  if (name === "computer-use") return true;
  // 与 desktop/services resolver 和 CLI bootstrap 共用 @mesacode/shared 的单一事实源，避免第三处
  // 判定漂移：git/.git/本地路径形态的 mesacode-cua 若这里漏判，全局 CLI env 注入不会带 broker
  // socket/token，agent 会回退成 Python/uvx 自己持有 macOS TCC（违反 product broker 边界）。
  if (typeof config.command === "string" && isMesacodeCuaMcpCommand(config.command)) {
    return true;
  }
  return (
    Array.isArray(config.args) &&
    config.args.some((arg) => typeof arg === "string" && isMesacodeCuaMcpPackageArg(arg))
  );
}
...
export async function buildCuaProductHelperAgentEnv(
  ...
  //
  // fail-closed 理由：CLI 全局配置里存在 mesacode-cua 时，返回空 env 会让 agent 按原始
  ...
  try {
    ...
    if (host.waitForTransport) {
      ...
      return {
        ...
        [MESACODE_CUA_PLUGIN_AUTHORITY_ENV_KEY]: transport.pluginAuthority,
      };
    }
    ...
    if (reserved && !hasCuaProductHelperAgentEnvUnavailable(host)) {
      ...
      return {
        ...
        [MESACODE_CUA_PLUGIN_AUTHORITY_ENV_KEY]: reserved.pluginAuthority,
      };
    }
    ...
    return {
      ...
      [MESACODE_CUA_PLUGIN_AUTHORITY_ENV_KEY]: handle.pluginAuthority,
    };
  } catch (error) {
    ...
    logger.warn(
      undefined,
      `Computer Use Helper broker_unavailable; disabling workspace mesacode-cua MCP server for this agent spawn (${cuaHelperStartErrorDetail(error)})`,
    );
    return {
...
export function createLocalServices(options: {
  ...
  mesacodeAgentCommandResolver?: MesacodeAgentCommandResolver;
  ...
  mesacodeAgentSpawnFallbackCwd?: string;
  /** desktop-attached remote server 从 Desktop Host 收到的一次性 Agent 网络配置。 */
  remoteAgentNetwork?: {
    httpProxy?: string;
    noProxy?: string;
  };
  /** 所属 Environment 的 Mesacode Built-in Provider Config 物理路径。 */
  mesacodeBuiltinProviderConfigFilePath: string;
  ...
  // 导致 bun 这类只在 shell profile 里追加的命令在 Mesacode Agent/终端里不可见。
  // 这里在所有本地服务启动前统一修正运行时环境，并顺带把内置 rg 注入 PATH，
  // 让 Mesacode Agent、终端、认证 runtime 共用同一套命令解析结果。
  initializeRuntimeProcessEnv(options?.runtimeProcessEnvPatch);

  const desktopContextPromptEnabledRaw =
    process.env[MESACODE_DESKTOP_CONTEXT_PROMPT_ENABLED_ENV]?.trim();
  ...
  const resolveCurrentMesacodeEndpointOrigin = async () =>
    resolveRuntimeMesacodeEndpointOrigin(process.env, {
      overrideOrigin: (await settingService.get()).mesacodeEndpointOrigin,
    });
  ...
  const mesacodeJwtLogoutHandlerRef: {
  ...
  const apiClient = createNodeApiClient({
    ...
    onZcodeJwtInvalid: (input, headers) => mesacodeJwtLogoutHandlerRef.current?.(input, headers),
  ...
  const providerConfigRuntime = createProviderConfigRuntime({
    mesacodeBuiltinFilePath: options.mesacodeBuiltinProviderConfigFilePath,
    mesacodeBuiltinEnvironment: {
      environmentConfigRoot: resolveAppConfigDir(),
      platform: clientConfigPlatform,
      appVersion: MESACODE_VERSION,
      resolveEndpointOrigin: resolveCurrentMesacodeEndpointOrigin,
      onRefreshResult: (event) => {
        if (event.result === "updated")
          providerConfigLog.info(undefined, "Mesacode Built-in CDN 配置已更新", event);
        else providerConfigLog.debug(undefined, "Mesacode Built-in 刷新检查", event);
      },
      fetchRelease: (endpointOrigin, signal) =>
        fetchMesacodeBuiltinRemoteRelease({
          apiClient,
          endpointOrigin,
          signal,
          appVersion: MESACODE_VERSION,
    ...
    onMesacodeBuiltinRefreshError: (error) => {
      providerConfigLog.warn(undefined, "Mesacode Built-in Config 远端刷新失败", { error });
    },
    ...
    // 已发布 config.json 保存的是 Mesacode 用户配置；清理第三方 ACP 不能移除这条升级路径。
    // Repository 仅在新 Personal 配置不存在时导入，并保留旧文件以便回滚。
    readLegacyProviders: () => readLegacyMesacodeConfigProviders(),
  });
  ...
  let providerConnectivityAgentService:
    | Pick<IMesacodeAgentService, "testModelConnectivity">
    | undefined;
  ...
  // mcpSync/hooks 里引用 mesacodeAgentService 的闭包是惰性调用，声明顺序不影响初始化。
  ...
  const mcpSyncService = createMcpSyncService({
    ...
    listMcpServerStatuses: (params) => mesacodeAgentService.listMcpServerStatuses(params),
  });
  ...
  const hooksService = createHooksService({
    grantWorkspaceHookTrust: (params) => mesacodeAgentService.grantWorkspaceHookTrust(params),
  });
  ...
  // createDefaultCuaProductHelper() 在 mesacodeAgentService 存在之前就要组装 resolver，
  // 但"是否有活跃 turn"这个信号只有 mesacodeAgentService 建好之后才能查询。用前向引用占位——resolver 真正
  ...
  const isCuaEnabledForContext = (context?: CuaProductMcpServerResolverContext): boolean =>
    // 保留 main 原有 gate 行为（避免回归）：dev/internal 特性开启时（MESACODE_CUA_DEV_MODE=1 或
    // MESACODE_CUA_PRODUCT_HELPER=1）即视为启用，不依赖 config.json 显式 enable——main 的 bootstrap
    // 用 isMesacodeCuaInternalFeatureEnabled 门控 bundled plugin，与 feat 的 workspace enablement 不同。
    // 生产路径（dev mode off）回落到官方插件 workspace enablement 判定（与 feat 一致）。
    isMesacodeCuaInternalFeatureEnabled(process.env) ||
    isOfficialCuaPluginEnabledForWorkspace({
      env: process.env,
  ...
  const createManagedDefaultCuaProductHelper = (
    ...
    const helper = createDefaultCuaProductHelper({
      // 转发活跃-turn 查询（前向引用，mesacodeAgentService 建好后赋值）。
  ...
  //
  // dev 判定直接用上游的 isCuaLocalDevelopmentRuntime（@mesacode/cua/broker/server，
  // 即本文件已经用来 import buildHelperOpenArgs 的那个 subpath，可正常导入）。
  //
  // 行为等价性（别误读成安全加固）：上游是 `COMPILED_LOCAL_DEVELOPMENT_RUNTIME &&
  // MESACODE_RUNTIME_ENV!=="production"`，而那个编译期常量只有 scripts/build-cua-helper-app.mjs
  // 会用 define 折叠（Helper bundle）；desktop host bundle 没有该 define，于是回退成
  // `process.env.NODE_ENV !== "production"` —— 正是复制品写的那一项。所以在**当前**打包形态下
  // 两者逐字等价，关门靠的是 MESACODE_RUNTIME_ENV=production（打包态显式注入且不传 NODE_ENV）。
  //
  // 换成上游的收益是消除漂移面：折叠点、因子个数与 fail-closed 方向都由上游一处决定，
  // 哪天 host bundle 也补上 __MESACODE_LOCAL_DEVELOPMENT_RUNTIME__ define（Helper 侧已经有），
  // 编译期门自动生效，不需要再回来改这里。
  ...
  const launchStandaloneCuaHelperForStatus = async (): Promise<string | null> => {
    ...
    const home = process.env.MESACODE_HOME?.trim() || join(homedir(), ".mesacode");
  ...
  const cuaPipSessionService = createCuaPipSessionService({
    ...
    resolveCredentials: async () => {
      ...
      // PiP 客户端以 role=presentation 声明，资格由 Helper 按对端（Mesacode 主进程）签名 identifier 裁决。
  ...
  const cuaPermissionService: ICuaPermissionService = {
    async getStatus(
      ...
      if (
        ...
        return {
          available: false,
          reason: "Mesacode Computer Use is not enabled (plugin off or not product mode).",
        };
      }
      ...
      if (!host || !host.running) {
        ...
        if (!stable) {
          return {
            available: false,
            reason:
              "Mesacode Computer Use is not running; it will start automatically on first Computer Use use.",
            idle: true,
          } satisfies { available: false; reason: string; idle: true };
        }
        // standalone Helper 上直接查权限真值（身份模式，无 token）。
        try {
          const { callBrokerMethod } = await import("@mesacode/cua/broker/helperHealth");
          ...
          return {
            available: false,
            reason: "Mesacode Computer Use is starting up; retry in a moment.",
            idle: true,
          } satisfies { available: false; reason: string; idle: true };
        }
      }
      try {
        ...
        if (!helper || !isDefaultCuaProductHelperCurrent(helper)) {
          return {
            available: false,
            reason: "Mesacode Computer Use lifecycle is disposed.",
          };
        }
    ...
    async restartHelper(
      ...
      try {
        ...
        return {
          ok: false,
          reason: `Failed to restart Mesacode Computer Use: ${
            error instanceof Error ? error.message : String(error)
          }`,
        };
      }
  ...
  const mesacodeAgentService = createMesacodeAgentService({
    ...
    commandResolver: options?.mesacodeAgentCommandResolver,
    presentationSurface: resolveMesacodeAgentPresentationSurface({
    ...
    // MesacodeAgentProcessManager，导致 host 永远不向 main 上报 Agent spawn/exit，进程监控器
    // 因而看不到实际运行的 Agent，也无法验证只读到可写升级是否复用同一进程。
    processLifecycleReporter: options?.processLifecycleReporter,
    spawnFallbackCwd: options?.mesacodeAgentSpawnFallbackCwd,
    ...
    // adapter 的 fetch wrapper。判定实现与 CLI 侧共用 @mesacode/shared 的同一份，避免分叉。
    // origin 解析复用 resolveCurrentMesacodeEndpointOrigin——与闲时任务同口径（含 settings
    ...
    officialMcpTrustedOrigins: createOfficialMcpTrustedOriginRegistry({
      ...
      resolveMesacodeApiOrigin: resolveCurrentMesacodeEndpointOrigin,
    }),
    cuaOperationStateReporter: shouldEnableCuaOperationStateReporter({
      ...
      : undefined,
    // Mesacode 只发布 turn/session 事实；面板 terminal policy 由 producer coordinator 决定。
    ...
    resolveSpawnEnv: async (context) => {
      ...
      if (!helper && cuaPluginEnabled && process.platform === "darwin") {
        ...
        cuaProductHelperEnv = {
          ...
          [MESACODE_CUA_PLUGIN_AUTHORITY_ENV_KEY]: randomBytes(16).toString("hex"),
        };
      ...
      const telemetryEnv = getCapturedMesacodeAgentTelemetryEnv();
      ...
      return {
        ...
        ...buildAgentEndpointOriginEnv(await resolveCurrentMesacodeEndpointOrigin()),
        // broker 凭据（socket/token）注入 agent spawn env，让内置 mesacode-cua plugin 的
        // computer-use MCP server 经 __mesacode-plugin-host 恢复 token 后连上 broker。
        ...
        ...createNodeProviderRuntimePathEnv({
          ...
          mesacodeBuiltinFilePath: await providerConfigRuntime.resolveMesacodeBuiltinActiveFilePath(),
    ...
    ...(isDesktopAttachedRemote
      ...
      : {
          ...
          resolveSessionRuntimePreferences: async (scope) => {
            ...
            const settings = await settingService.get();
            const modelContextBudgetStrategy = DEFAULT_MESACODE_MODEL_CONTEXT_BUDGET_STRATEGY;
            return {
  ...
  providerConnectivityAgentService = mesacodeAgentService;
  ...
  hasActiveTurnRef = () => mesacodeAgentService.hasActiveCuaOperationTurn();
  // desktop-continuous UI 直接订阅 mesacodeSessionService，绕开 Mesacode task adapter 的
  ...
  const mesacodeTaskIndexSyncer = createMesacodeTaskIndexSyncer({
    agentService: mesacodeAgentService,
    taskIndexRepo,
  });
  ...
  const mesacodeSessionService = createMesacodeSessionService({
    agentService: mesacodeAgentService,
  ...
  const gitCommitMessageGenerator = new GitCommitMessageGenerator({
    ...
    textGenerator: {
      async generateText(params) {
        return await mesacodeAgentService.generateWorkspaceText({
  ...
  // task wrapper 由 Mesacode task service adapter 提供；核心 session 状态由 Mesacode agent server 维护。
  const mesacodeTaskService = createMesacodeTaskServiceAdapter({
  ...
  const mesacodeJwtLogoutLogger = createServiceLogger("mesacode-jwt-logout");
  mesacodeJwtLogoutHandlerRef.current = (input, headers) => {
      ...
      .then((invalidated) => {
        ...
        if (invalidated) {
          void broadcastService.send({
            channel: MESACODE_JWT_INVALID_BROADCAST_CHANNEL,
            payload: {},
          });
        }
      })
      .catch((error) => {
        mesacodeJwtLogoutLogger.warn("Mesacode JWT logout failed", { error });
      });
  };
  ...
  const offPeakCredentialResolverDeps = {
    ...
    resolveAccountProvider: async () => {
      ...
      const staticAccess = mesacodeProviderAccountAccessSchema.parse(config.access.toJSON());
      const access = await accountRequestAuthService.resolveAccessCurrent(staticAccess);
      if (!access) return null;
      return {
        providerId: provider.providerId,
        access: mesacodeAccountAccessSchema.parse(access),
  ...
  const conversationShareClient = new ConversationShareHttpClient({
    ...
    apiClient,
    baseUrl: buildRuntimeMesacodeApiUrl(process.env, "/api/v1"),
    tokenProvider: async (): Promise<string | null> => {
      ...
      return tokenSet?.mesacodeJwtToken ?? tokenSet?.accessToken ?? null;
    },
  });
  ...
  const services = new ServiceCollection()
    ...
    .register(IMesacodeAgentService, mesacodeAgentService)
    .register(IMesacodeSessionService, mesacodeSessionService)
    ...
    .register(
      ...
      createClientConfigService({
        ...
        resolveRequestContext: async () => ({
          endpointOrigin: await resolveCurrentMesacodeEndpointOrigin(),
          appVersion: MESACODE_VERSION,
      ...
      (() => {
        ...
        const offPeakTaskService = new OffPeakTaskService({
          ...
          stopRunningTask: async (params) => {
            await mesacodeTaskService.stopGeneration({
        ...
        // 回写前向引用，供 mesacodeAgentService 的 offPeak/create、offPeak/list 协议 handler 调用。
    ...
    .register(IPluginManagementService, createPluginManagementService({ mesacodeAgentService }))
...
/** 仅给同一事件账号返回当前 Mesacode JWT；不缓存、不修改登录凭据。 */
export function createTelemetryAuthorizationLoader(
  ...
  return async (userId) => {
    ...
    try {
      ...
      const jwt = (await credentialService.load("mesacodejwttoken"))?.trim();
...
): () => Promise<import("@mesacode/shared").OAuthLoginAttribution | null> {
...
export function disposeServiceResources(services: ServiceCollection): void {
  ...
  const disposableServices = [
    ...
    services.getOptional(IMesacodeTaskService),
    services.getOptional(IMesacodeAgentService),
    services.getOptional(IMesacodeSessionService),
...
export async function disposeServiceResourcesAndWait(services: ServiceCollection): Promise<void> {
  ...
  // 旧的同步 dispose 会在 host 退出时丢掉强杀 timer，导致 mesacode-cli/app-server 变成孤儿进程。
  const disposableServices = [
    services.getOptional(ITerminalService),
    services.getOptional(IMesacodeTaskService),
    services.getOptional(IMesacodeAgentService),
