...
const MESACODE_PLUGIN_MANIFEST_PATH = [".mesacode-plugin", "plugin.json"] as const;
...
function getWorkspaceZcodeCommandRoot(workspacePath: string): string {
  return join(workspacePath, ".mesacode", "commands");
}

function getUserZcodeCommandRoot(): string {
  return join(resolveUserHomeDir(), ".mesacode", "commands");
}

function getWorkspaceZcodePluginRoot(workspacePath: string): string {
  return join(workspacePath, ".mesacode", "plugins");
}

function getUserZcodePluginRoot(): string {
  return join(resolveUserHomeDir(), ".mesacode", "plugins");
}
...
function getWorkspaceZcodeConfigPath(workspacePath: string): string {
  return join(workspacePath, ".mesacode", "config.json");
}
...
function getUserZcodeAgentsFilePath(): string {
  return join(resolveUserHomeDir(), ".mesacode", "AGENTS.md");
}
...
async function findPluginManifestPath(pluginPath: string): Promise<string | null> {
  const mesacodeManifestPath = join(pluginPath, ...MESACODE_PLUGIN_MANIFEST_PATH);
  if (await pathExists(mesacodeManifestPath)) {
    return mesacodeManifestPath;
  }
