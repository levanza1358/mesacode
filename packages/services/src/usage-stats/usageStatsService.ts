...
import { isCodingPlanModelProviderId } from "@mesacode/shared";
...
import type { OfficialMcpCredentialSource } from "./providers/mesacodeMcpQuotaProvider.js";
...
interface UsageStatsServiceDependencies {
  ...
  /** App Usage 经 Mesacode Protocol 读取 agent 数据库真实统计。 */
  mesacodeAgentService: Pick<IMesacodeAgentService, "getAppUsageStats">;
  /**
...
export function createUsageStatsService(
  ...
  return {
    async getAppUsageSnapshot(request: AppUsageRequest): Promise<AppUsageSnapshot> {
      ...
      // 经 Mesacode Protocol usage/stats 取回。不再读本地 session JSON 估算。
      return dependencies.mesacodeAgentService.getAppUsageStats({
        range: request.range,
