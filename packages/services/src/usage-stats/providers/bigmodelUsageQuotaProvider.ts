...
import type { ProviderFamilyDomain } from "@mesacode/shared";
...
} from "./mesacodeMcpQuotaProvider.js";
...
export class BigModelUsageQuotaProvider {
  ...
  ): Promise<MesacodeAccountAccess | undefined> {
  ...
  private async getStartPlanSnapshot(
    providerId: string,
    accountAccess: MesacodeAccountAccess | undefined,
  ...
  async getCodingPlanResetStatus(
    ...
    const payload = await readCodingPlanResetApiJson(
      this.apiClient,
      buildRuntimeMesacodeApiUrl(this.env, `${CODING_PLAN_RESET_BASE_PATH}/status`),
      {
        method: "GET",
        timeoutMs: REQUEST_TIMEOUT_MS,
  ...
  async useCodingPlanReset(request: CodingPlanResetUseRequest): Promise<CodingPlanResetUseResult> {
    ...
    const payload = await readCodingPlanResetApiJson(
      this.apiClient,
      buildRuntimeMesacodeApiUrl(this.env, `${CODING_PLAN_RESET_BASE_PATH}/use`),
      {
        method: "POST",
        timeoutMs: REQUEST_TIMEOUT_MS,
        headers: {
  ...
  async requestCodingPlanResetOpportunity(
    ...
    try {
      payload = await readCodingPlanResetApiJson(
        this.apiClient,
        buildRuntimeMesacodeApiUrl(this.env, `${CODING_PLAN_RESET_BASE_PATH}/opportunity`),
        {
          method: "POST",
          timeoutMs: REQUEST_TIMEOUT_MS,
          headers: {
  ...
  async markCodingPlanResetHistoryRead(request: CodingPlanResetScopeRequest): Promise<void> {
    ...
    const payload = await readCodingPlanResetApiJson(
      this.apiClient,
      buildRuntimeMesacodeApiUrl(this.env, `${CODING_PLAN_RESET_BASE_PATH}/history/read`),
      {
        method: "POST",
        timeoutMs: REQUEST_TIMEOUT_MS,
  ...
  private async resolveCodingPlanResetAuthorization(
    ...
    const mesacodeJwt = (await this.credentialService?.load(MESACODE_JWT_TOKEN_KEY))?.trim() ?? "";
    if (!mesacodeJwt) {
      throw new Error("coding_plan_reset_mesacode_jwt_required");
    }
    ...
    return {
      mesacodeAuthorization: /^Bearer\s/i.test(mesacodeJwt) ? mesacodeJwt : `Bearer ${mesacodeJwt}`,
  ...
  private async resolveAuthorization(
    ...
    if (request.allowEnvApiKey !== false && request.requirePreferredProvider !== true) {
      const envApiKey =
        readEnv(this.env, "MESACODE_BIGMODEL_USAGE_API_KEY") ??
...
function createCodingPlanResetHeaders(
  ...
  const headers: Record<string, string> = {
    Authorization: authorization.mesacodeAuthorization,
...
function resolveTeamPlanContext(accountAccess: MesacodeAccountAccess): TeamPlanContext | null {
...
function resolveQuotaUrlFromEnv(env: NodeJS.ProcessEnv): string | undefined {
  return readEnv(env, "MESACODE_BIGMODEL_USAGE_QUOTA_URL") ?? readEnv(env, "BIGMODEL_USAGE_QUOTA_URL");
}
