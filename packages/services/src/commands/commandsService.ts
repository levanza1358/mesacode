...
import {
  ...
  MESACODE_COMMAND_AGENT_SOURCES,
  ...
  type UserCommand,
  type MesacodeCommand,
} from "@mesacode/shared";
import { DEFAULT_ENABLED_OFFICIAL_PLUGIN_IDS } from "@mesacode/shared";
...
const DEFAULT_COMMAND_AGENT_SOURCE: CommandAgentSource = MESACODE_COMMAND_AGENT_SOURCE;
const COMMAND_AGENT_SOURCE_ORDER: readonly CommandAgentSource[] = MESACODE_COMMAND_AGENT_SOURCES;
const ENABLE_OVERRIDE_KEY = "enable";
const HOME_PREFIX = "~/";
const MESACODE_OFFICIAL_PLUGIN_MARKETPLACE = "mesacode-plugins-official";
const MESACODE_INLINE_PLUGIN_MARKETPLACE = "inline";
const MESACODE_PLUGIN_MANIFEST_PATH = join(".mesacode-plugin", "plugin.json");
...
const MESACODE_COMMAND_DESCRIPTOR: CommandAgentSourceDescriptor = {
  agentSource: "mesacodeAgent",
  directorySource: "mesacode",
  userDirectorySegments: [".mesacode", "commands"],
  workspaceDirectorySegments: [".mesacode", "commands"],
  fileExtension: ".md",
  format: "markdown",
  namespaceSeparator: "/",
...
function getUserCliConfigPath(): string {
  return join(resolveUserHomeDir(), ".mesacode", "cli", "config.json");
}
...
async function scanOfficialPluginCacheRoots(pluginStorageRoot: string): Promise<string[]> {
  const cacheRoot = join(pluginStorageRoot, "cache", MESACODE_OFFICIAL_PLUGIN_MARKETPLACE);
...
async function findPluginManifestPath(rootPath: string): Promise<string | null> {
  for (const manifestPath of [
    join(rootPath, MESACODE_PLUGIN_MANIFEST_PATH),
...
async function resolvePluginCommandRootDescriptors(): Promise<PluginCommandRootDescriptor[]> {
  ...
  const candidates: PluginRootCandidate[] = [
    ...config.dirs.map((dir) => ({
      defaultEnabled: true,
      marketplace: MESACODE_INLINE_PLUGIN_MARKETPLACE,
  ...
  for (const candidate of candidates) {
    ...
    // 仍会从 cache 贡献命令。
    if (
      candidate.marketplace === MESACODE_OFFICIAL_PLUGIN_MARKETPLACE &&
...
export function createCommandsService(_options?: CommandsServiceOptions): ICommandsService {
  async function list(params: {
    ...
    // Mesacode Agent 需要先合并所有 workspace 目录，再合并所有 user 目录；
    // 按每个目录交错读取 project/user 会让 user .mesacode 抢在 workspace .agents 前面。
    ...
    const pluginCommands =
      !params.agentSource || params.agentSource === MESACODE_COMMAND_AGENT_SOURCE
        ? await discoverPluginCommands(enabledOverrides)
        : [];

    return {
      commands: [...dedupedUserCommands, ...pluginCommands] as MesacodeCommand[],
...
async function discoverPluginCommandsRecursive(
  ...
  for (const entry of entries) {
    ...
    if (!entry.toLowerCase().endsWith(MESACODE_COMMAND_DESCRIPTOR.fileExtension)) {
      continue;
    }
    ...
    try {
      ...
      const parsed = CommandFileParser.parseCommandFile(
        content,
        fullPath,
        MESACODE_COMMAND_DESCRIPTOR.format,
      );
      if (!parsed) {
        continue;
      }
      const name = getCommandName(rootDir, fullPath, MESACODE_COMMAND_DESCRIPTOR);
...
async function discoverCommandsFromDirectorySources(params: {
  ...
  for (const descriptor of params.descriptors) {
    ...
    // `.mesacode` 是强优先级来源；只要读到有效命令，同 scope 的 `.agents` 就不再参与。
    if (descriptor.directorySource === "mesacode" && discoveredCount > 0) {
      break;
    }
  }
}
