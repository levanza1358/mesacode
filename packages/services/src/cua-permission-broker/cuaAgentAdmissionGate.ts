...
export class CuaAgentAdmissionGate {
  ...
  waitForSpawnAdmission(context: CuaAgentSpawnAdmissionContext): Promise<void> {
    ...
    if (signal?.aborted) {
      return Promise.reject(signal.reason ?? new Error("Mesacode agent process start was cancelled."));
    }
    if (this.activeEpoch === undefined) {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      const onAbort = (): void => {
        ...
        reject(signal?.reason ?? new Error("Mesacode agent process start was cancelled."));
      };
      waiter = {
        resolve: () => {
