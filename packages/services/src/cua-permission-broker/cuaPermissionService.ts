...
// to @mesacode/cua/src/broker/ports.ts. The descriptor registration itself stays in
// services (host control plane — depends on services' createServiceDescriptor
// + @mesacode/shared ServiceChannels), so services internal callers (node.ts,
// accessor.ts, services/index.ts) and ui consumers (via @mesacode/services root
...
// at compile time; Vite never resolves @mesacode/cua for these).
import type {
  CuaPermissionState,
  CuaPermissionStatus,
...
} from "@mesacode/cua/broker/ports";
