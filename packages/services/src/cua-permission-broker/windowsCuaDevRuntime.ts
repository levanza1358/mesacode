...
const DEV_ROOT_ENV = "MESACODE_CUA_DEV_ROOT";
const EXPECTED_PACKAGE_NAME = "@mesacode/cua";
...
async function requireExpectedPackage(
  ...
  try {
    ...
    const contract = pkg.mesacodeCuaRuntime;
    if (
  ...
  throw new WindowsCuaDevRuntimeResolutionError(
    "invalid-package",
    `Windows CUA development root package.json must name ${EXPECTED_PACKAGE_NAME} and expose a valid mesacodeCuaRuntime contract.`,
    PACKAGE_JSON,
  );
}
