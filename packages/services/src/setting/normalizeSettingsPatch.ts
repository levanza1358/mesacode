...
export function normalizeSettingsPatch(patch: Partial<AppSettings>): Partial<AppSettings> {
  ...
  if (
    "mesacodeEndpointOrigin" in normalizedPatch &&
    typeof normalizedPatch.mesacodeEndpointOrigin === "string"
  ) {
    // 非生产 endpoint override 需要支持 Reset 清空；RPC/JSON 对 undefined 不稳定时，用空串也能回到默认生产域。
    const trimmedMesacodeEndpointOrigin = normalizedPatch.mesacodeEndpointOrigin.trim();
    normalizedPatch.mesacodeEndpointOrigin =
      trimmedMesacodeEndpointOrigin.length > 0 ? trimmedMesacodeEndpointOrigin : undefined;
  }
