...
import {
  ...
  type MesacodeOffPeakTaskCreateParams,
...
import type { ModelSelection, ModelSelectionValidation } from "@mesacode/provider";
...
function isValidCreateParams(params: MesacodeOffPeakTaskCreateParams): boolean {
  return (
...
export class OffPeakTaskService implements IOffPeakTaskService {
  ...
  async createTask(params: MesacodeOffPeakTaskCreateParams): Promise<OffPeakTaskCreateResult> {
    let providerName = "";
    try {
    ...
    const normalizedParams: MesacodeOffPeakTaskCreateParams = {
      ...params,
    ...
    let created: MesacodeOffPeakTask;
    try {
   ...
   */
  async cancelTask(offPeakTaskId: string): Promise<MesacodeOffPeakTask | null> {
  ...
  async pauseTask(offPeakTaskId: string): Promise<MesacodeOffPeakTask | null> {
   ...
   */
  async continueTask(offPeakTaskId: string): Promise<MesacodeOffPeakTask | null> {
  ...
  async deleteHistory(offPeakTaskId: string): Promise<MesacodeOffPeakTask | null> {
  ...
  ): Promise<MesacodeOffPeakTask | null> {
    const existing = await this.deps.repo.get(offPeakTaskId);
  ...
  async list(): Promise<MesacodeOffPeakTask[]> {
    return this.projectModelSelectionIssues(await this.deps.repo.list());
  }

  async get(offPeakTaskId: string): Promise<MesacodeOffPeakTask | null> {
  ...
  ): Promise<MesacodeOffPeakTask[]> {
    const repaired: MesacodeOffPeakTask[] = [];
  ...
  private async settleOne(task: MesacodeOffPeakTask): Promise<void> {
