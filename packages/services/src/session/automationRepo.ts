...
import {
  ...
  mesacodeTaskModeSchema,
  type MesacodeAutomation,
  type MesacodeAutomationCreateParams,
  type MesacodeAutomationDispatchStatus,
  type MesacodeAutomationLifecycleStatus,
  type ModelSelection,
  type MesacodeAutomationRun,
  type MesacodeAutomationRunDispatchStatus,
  type MesacodeAutomationRunOutcome,
  type MesacodeAutomationTrigger,
  type MesacodeAutomationUpdateParams,
...
function rowToAutomation(row: AutomationRow): MesacodeAutomation {
  ...
  return {
    ...
    scheduleRule: row.schedule_rule
      ? (JSON.parse(row.schedule_rule) as MesacodeAutomation["scheduleRule"])
      : undefined,
    ...(row.schedule_edited_by_user === 1 ? { scheduleEditedByUser: true } : {}),
    runCount: row.run_count,
    enabled: row.enabled === 1,
    lifecycleStatus: row.lifecycle_status as MesacodeAutomationLifecycleStatus,
    nextRunAt: row.next_run_at ?? undefined,
    lastRunAt: row.last_run_at ?? undefined,
    dispatchStatus: row.dispatch_status as MesacodeAutomationDispatchStatus,
...
function readAutomationModelSelection(row: AutomationRow): MesacodeAutomation["modelSelection"] {
...
function serializeAutomationModelSelection(
  selection: MesacodeAutomation["modelSelection"],
): string | null {
...
function normalizeAutomationMode(mode: string | null): MesacodeAutomation["mode"] | undefined {
  const parsed = mesacodeTaskModeSchema.safeParse(mode);
  return parsed.success ? parsed.data : undefined;
}

function assertValidAutomationMode(mode: unknown): void {
  if (mode === undefined || mode === null) return;
  if (!mesacodeTaskModeSchema.safeParse(mode).success) {
    // 读取兼容历史脏数据不代表允许继续写脏数据；Repo 是绕过 RPC 时的最终持久化边界。
    throw new Error(`Invalid automation mode: ${String(mode)}`);
  }
}

function rowToRun(row: AutomationRunRow): MesacodeAutomationRun {
  ...
  return {
    ...
    trigger: row.trigger as MesacodeAutomationTrigger,
    ...(modelSelection ? { modelSelection } : {}),
    dispatchStatus: row.dispatch_status as MesacodeAutomationRunDispatchStatus,
    outcome: (row.outcome as MesacodeAutomationRunOutcome | null) ?? undefined,
...
export class AutomationRepo {
  ...
  // 互相覆盖全局值，导致 repo 与裸 SQL 操作在并发窗口内写进真实库 ~/.mesacode/v2（历史脏数据
  ...
  async create(
    params: MesacodeAutomationCreateParams,
    options: { nextRunAt: number | null; lifecycleStatus?: MesacodeAutomationLifecycleStatus },
  ): Promise<MesacodeAutomation> {
  ...
  async get(automationId: string, workspaceKey?: string): Promise<MesacodeAutomation | null> {
    await this.ensureReady();
    const row = this.getRow(automationId, workspaceKey);
  ...
  async update(
    automationId: string,
    params: MesacodeAutomationUpdateParams,
    options?: {
      nextRunAt?: number | null;
      lifecycleStatus?: MesacodeAutomationLifecycleStatus;
      resetRetry?: boolean;
    },
    workspaceKey?: string,
  ): Promise<MesacodeAutomation | null> {
    assertValidAutomationMode(params.mode);
    await this.ensureReady();
   ...
   */
  async claimDue(now: number): Promise<MesacodeAutomation[]> {
    await this.ensureReady();
    const db = this.getDatabase();
    db.exec("BEGIN IMMEDIATE");
    try {
      ...
      const claimed: MesacodeAutomation[] = [];
  ...
  async ensureRunClaimed(params: {
    ...
    workspaceKey: string;
    scheduledAt: number | null;
    trigger: MesacodeAutomationTrigger;
  }): Promise<void> {
    await this.ensureReady();
    const now = Date.now();
    this.getDatabase()
  ...
  async markRunDispatch(params: {
    runId: string;
    dispatchStatus: MesacodeAutomationRunDispatchStatus;
  ...
  async listRuns(automationId: string, workspaceKey?: string): Promise<MesacodeAutomationRun[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
  ...
  async getRun(runId: string): Promise<MesacodeAutomationRun | null> {
    await this.ensureReady();
    const row = this.getDatabase()
