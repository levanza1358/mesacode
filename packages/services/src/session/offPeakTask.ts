import type {
  ...
  MesacodeOffPeakTask,
  MesacodeOffPeakTaskCreateParams,
  ModelSelection,
...
export interface IOffPeakTaskService {
  ...
  createTask(params: MesacodeOffPeakTaskCreateParams): Promise<OffPeakTaskCreateResult>;
  cancelTask(offPeakTaskId: string): Promise<MesacodeOffPeakTask | null>;
  pauseTask(offPeakTaskId: string): Promise<MesacodeOffPeakTask | null>;
  continueTask(offPeakTaskId: string): Promise<MesacodeOffPeakTask | null>;
  deleteTask(offPeakTaskId: string): Promise<void>;
  /** 仅隐藏本地 History 行；不删除 task/session/执行字段。 */
  deleteHistory(offPeakTaskId: string): Promise<MesacodeOffPeakTask | null>;
  updateTask(
    offPeakTaskId: string,
    params: OffPeakUpdateTaskParams,
  ): Promise<MesacodeOffPeakTask | null>;
  list(): Promise<MesacodeOffPeakTask[]>;
  get(offPeakTaskId: string): Promise<MesacodeOffPeakTask | null>;
}
