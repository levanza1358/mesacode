...
import type { MesacodeImportSessionsResult, MesacodeTaskMeta } from "@mesacode/shared";
...
export async function importClaudeNativeSessions(params: {
  ...
  sessionIds: string[];
  createImportedSession?: (source: ClaudeNativeImportedSessionSource) => Promise<MesacodeTaskMeta>;
  onTaskImported: (meta: MesacodeTaskMeta) => void;
  ...
  const result: MesacodeImportSessionsResult = {
    imported: [],
    skipped: [],
    failed: [],
  };
