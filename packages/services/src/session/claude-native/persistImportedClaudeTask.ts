...
export function buildSearchableTextFromMessages(messages: MesacodeSessionFile["messages"]): string {
  const parts: string[] = [];
  let total = 0;
...
export async function persistImportedClaudeTask(params: {
  ...
  const indexMeta: MesacodeTaskMeta = {
    ...meta,
...
export async function writeImportedClaudeTaskSnapshot(params: {
  ...
  // ~/.mesacode/v2/sessions/{hash}/{taskId}.json。现在真实 Mesacode session 承担续聊，legacy snapshot
