...
} from "#src/session/mesacodeTaskListTypes.js";
import type {
...
function appendMesacodeAgentIndexedProviderFilter(
  where: string[],
...
interface TaskIndexStatePatch {
  ...
  unreadAt?: number;
  model?: string;
  status?: MesacodeTaskMeta["status"];
  lastError?: MesacodeTaskMeta["lastError"];
  target?: MesacodeTaskMeta["target"];
  updatedAt?: number;
}
...
const GROUPED_WORKSPACE_BOOTSTRAP_ONCE_KEY = "__mesacode_internal_grouped_workspace_bootstrap_once__";
const DEFAULT_TASK_GROUP_COLOR: MesacodeTaskGroupColor = "gray";
const WORKSPACE_BOOTSTRAP_TASK_GROUP_COLORS = [
  ...
  "blue",
  "purple",
] satisfies MesacodeTaskGroupColor[];
...
function isTerminalTaskStatus(status: MesacodeTaskMeta["status"]): boolean {
  return status === "completed" || status === "error";
}

function shouldPreserveNewerTerminalStatus(
  existingMeta: MesacodeTaskMeta | null,
  incomingMeta: MesacodeTaskMeta,
): boolean {
...
function rowToMeta(row: TaskIndexRow): MesacodeTaskMeta {
  const workspaceIdentity = resolveTaskIndexRowWorkspaceIdentity(row);
  try {
    const parsed = mesacodeTaskMetaSchema.safeParse(JSON.parse(row.meta_json));
    if (parsed.success) {
      return {
        ...(parsed.data as MesacodeTaskMeta),
  ...
  return {
    ...
    mode: row.mode as MesacodeTaskMeta["mode"],
    model: row.model ?? undefined,
    provider: row.provider === MESACODE_AGENT_PROVIDER ? MESACODE_AGENT_PROVIDER : undefined,
    migrationSource: (row.migration_source as MesacodeTaskMeta["migrationSource"]) ?? undefined,
    ...
    unreadAt: row.unread_at ?? undefined,
    status: (row.task_status as MesacodeTaskMeta["status"]) ?? undefined,
  };
}

/** 序列化 meta 到 meta_json。cron 身份随 meta 一起写入（单一来源），另在 writeRecord 投影到 cron_automation_id 索引列。 */
function serializeMetaJson(meta: MesacodeTaskMeta): string {
...
function normalizeWorkspaceBootstrapScopes(
  scopes: Array<{
    workspacePath: string;
    workspaceIdentity?: string;
    workspacePurpose?: import("@mesacode/shared").WorkspacePurpose;
  }>,
...
function rowToTaskListItem(row: TaskIndexRow, search: string | null): MesacodeTaskListItem {
...
function isTaskGroupColor(value: string): value is MesacodeTaskGroupColor {
  return (
    value === "gray" ||
    value === "red" ||
    value === "orange" ||
...
function rowToTaskGroup(row: TaskGroupRow): MesacodeTaskGroup {
  return {
    id: row.group_id,
    title: row.title,
...
function workspaceGroupColor(targetWorkspaceKey: string): MesacodeTaskGroupColor {
...
function groupedTopNodeOrderRef(node: MesacodeGroupedTaskViewNode): {
  nodeType: "group" | "task";
  nodeKey: string;
  mapKey: string;
} {
...
function compareGroupedNodes(
  left: MesacodeGroupedTaskViewNode,
  right: MesacodeGroupedTaskViewNode,
): number {
...
function compareCronGroupTasks(left: MesacodeTaskListItem, right: MesacodeTaskListItem): number {
...
function compareGroupTasks(
  left: MesacodeTaskListItem,
  right: MesacodeTaskListItem,
...
export class TaskIndexRepo {
  ...
  async archiveStaleTasks(params: {
    ...
    if (params.provider) {
      appendMesacodeAgentIndexedProviderFilter(where, args, params.provider);
    }
  ...
  private writeRecord(record: TaskIndexWriteRecord): MesacodeTaskMeta {
  ...
  async syncTaskMetaAtGroupedTop(params: {
    ...
    titleOverridden?: boolean;
    searchableText?: string;
  }): Promise<{ meta: MesacodeTaskMeta; initializedGroupedOrder: boolean }> {
      ...
      try {
        ...
        const meta: MesacodeTaskMeta = {
          ...
          target: Object.prototype.hasOwnProperty.call(params.meta, "target")
            ? params.meta.target
            : existingMeta?.target,
          // Claude Code 导入升级成真实 Mesacode session 后，protocol snapshot
          // 本身不知道迁移来源。同步运行态快照时保留已有 migrationSource，避免
          // 列表过滤和后续切模型把导入任务重新当成普通 Mesacode 任务。
   ...
   */
  private ensureCronGroupMembership(meta: MesacodeTaskMeta): void {
  ...
  private ensureOffPeakGroupMembership(
    meta: Pick<MesacodeTaskMeta, "workspacePath" | "workspaceIdentity" | "taskId">,
  ): void {
   ...
   */
  async seedTaskMetaIfMissing(meta: MesacodeTaskMeta): Promise<MesacodeTaskMeta> {
    await this.ensureReady();
  ...
  async clearTaskUnreadIfMatches(params: {
  ...
  }): Promise<{ meta: MesacodeTaskMeta; cleared: boolean }> {
    await this.ensureReady();
    return this.enqueueWrite(params, () => {
      ...
      try {
        ...
        const nextMeta: MesacodeTaskMeta = {
          ...current,
          unreadAt: undefined,
        };
   ...
   */
  async listSessionsByAutomation(automationId: string): Promise<MesacodeTaskMeta[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
        `SELECT
  ...
  async queryTaskList(
    params: MesacodeTaskListQuery & { provider?: MesacodeProvider },
  ): Promise<MesacodeTaskListResult> {
    await this.ensureReady();
  ...
  async renameTaskGroup(params: { groupId: string; title: string }): Promise<MesacodeTaskGroup> {
    await this.ensureReady();
    const title = params.title.trim() || "New Group";
    const now = Date.now();
    const database = this.getDatabase();
  ...
  async initializeGroupedTaskAtTop(params: MesacodeGroupedTaskRef): Promise<boolean> {
    await this.ensureReady();
    return this.initializeGroupedTaskAtTopReady(params);
  }

  private initializeGroupedTaskAtTopReady(params: MesacodeGroupedTaskRef): boolean {
    const row = this.getTaskRow(params);
  ...
  async queryGroupedTaskView(
    params: MesacodeGroupedTaskViewQuery & { provider?: MesacodeProvider },
  ): Promise<MesacodeGroupedTaskView> {
    await this.ensureReady();
    ...
    if (params.provider) {
      // grouped 和 workspace 都是 Mesacode Agent 任务列表入口，必须共享旧 provider
      // 残留过滤口径；否则历史 claude/codex/gemini 索引行会只在 grouped 里冒出来。
      appendMesacodeAgentIndexedProviderFilter(activeTaskWhere, activeTaskArgs, params.provider);
    }
    const activeTasks =
  ...
  async queryGroupedTaskViewStructure(params: {
  ...
  }): Promise<MesacodeGroupedTaskViewStructure> {
    await this.ensureReady();
    ...
    const memberRows = this.getDatabase()
      ...
      .all() as unknown as TaskGroupMemberRow[];
    const members: MesacodeGroupedTaskViewStructureMember[] = memberRows.map((row) => ({
    ...
    const orderRows = this.getDatabase()
      ...
      .all() as unknown as TaskGroupViewNodeOrderRow[];
    const topLevelOrders: MesacodeGroupedTaskViewStructureTopOrder[] = [];
  ...
  async applyGroupedTaskViewOrder(
    params: MesacodeGroupedTaskViewOrderInput & { provider?: MesacodeProvider },
  ): Promise<MesacodeGroupedTaskView> {
    await this.ensureReady();
    ...
    const validateTaskRef = (task: MesacodeGroupedTaskRef): string | null => {
      ...
      if (params.provider && row.provider !== params.provider) {
        ...
        // 带 provider 的 Mesacode Agent 视图只接受当前 glm task；旧 provider 引用作为不可见遗留数据跳过。
        return null;
      }
      return key;
    };
    ...
    const visibleTopLevelNodes: MesacodeGroupedTaskViewTopLevelNodeRef[] = [];
    const visibleGroups: Array<{ groupId: string; taskRefs: MesacodeGroupedTaskRef[] }> = [];
    ...
    for (const group of params.groups) {
      ...
      const visibleTaskRefs: MesacodeGroupedTaskRef[] = [];
