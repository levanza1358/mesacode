...
import type { MesacodeAutomation, MesacodeAutomationScheduleRule } from "@mesacode/shared";
...
export function scheduleRuleDefinition(rule: MesacodeAutomationScheduleRule): string {
...
export function isOneShotAutomation(
  automation: Pick<MesacodeAutomation, "recurring" | "maxRuns">,
): boolean {
...
export function buildRelativeDelaySchedule(
  delayMinutes: number,
  from = Date.now(),
): Pick<MesacodeAutomation, "cronExpr" | "scheduleRule"> {
...
export function computeInitialAutomationNextRunAt(
  automation: Pick<MesacodeAutomation, "cronExpr" | "recurring" | "scheduleRule">,
  from = Date.now(),
): number | null {
...
export function computeScheduleRuleNextRunAt(
  rule: MesacodeAutomationScheduleRule,
  from = Date.now(),
): number | null {
...
export function inferMinuteIntervalScheduleRule(
  cronExpr: string,
  anchorAt: number,
): MesacodeAutomationScheduleRule | undefined {
...
export function buildIntervalScheduleRule(
  intervalUnit: MesacodeAutomationScheduleRule["unit"],
  interval: number,
  cronExpr: string,
  anchorAt: number,
...
export function computeAutomationNextRunAt(
  automation: Pick<MesacodeAutomation, "cronExpr" | "scheduleRule">,
  from?: number,
): number | null {
