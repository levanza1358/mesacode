/* eslint-disable max-lines -- Mesacode task wrapper service 接口集中承载 app/runtime API，拆散会让替换阶段更难追踪。 */
import type { Event } from "@mesacode/rpc";
import { ServiceChannels } from "@mesacode/shared";
import type { CommandPayloadMap } from "@mesacode/shared/mesacode-protocol-v4";
import { createServiceDescriptor } from "#src/descriptors.js";
import type {
  MesacodeImportSessionsResult,
  MesacodeImportableSessionCandidate,
  MesacodeSessionCompactResult,
  MesacodeSessionGoalAction,
  MesacodeSessionGoalResult,
  MesacodeTaskCreateResult,
  MesacodeTaskMeta,
  MesacodeStreamEvent,
  MesacodeTaskMode,
  MesacodeConfigOption,
  MesacodeError,
  MesacodeAssistantMessageFeedback,
  MesacodePromptAttachment,
  MesacodeProvider,
  MesacodeWorkspaceEvent,
  TraceId,
  MesacodeSessionFile,
  MesacodeAgentMcpServer,
  MesacodeTaskSnapshot,
  MesacodeTaskSnapshotBody,
  MesacodeTaskSnapshotToolCallsSlice,
  MesacodeTaskSnapshotRefContent,
  MesacodeEnqueueTaskCommandResult,
  MesacodeCancelTaskCommandResult,
  MesacodeTaskClientMode,
  MesacodeTaskTokenUsageResult,
...
export interface MesacodeTaskSnapshotWithEtagResult {
  snapshot: MesacodeTaskSnapshot | null;
  etag?: string;
  notModified?: boolean;
}

/** 同一 workspace 的固定归档删除集合；每个去重后的目标恰好属于一种结果。 */
export interface MesacodeArchivedTaskDeletionResult {
...
export type MesacodeModelTrajectoryContentPart =
...
export interface MesacodeModelTrajectoryMessage {
  role: string;
  parts: MesacodeModelTrajectoryContentPart[];
}

/** model_io 记录里归一化出来的 token 用量。 */
export interface MesacodeModelTrajectoryUsage {
  inputTokens?: number;
  outputTokens?: number;
  totalTokens?: number;
...
export type MesacodeModelTrajectoryCallSourceKind =
  | "main"
  | "sidecar"
  | "subagent"
  | "compact"
...
export interface MesacodeModelTrajectoryCallSource {
  kind: MesacodeModelTrajectoryCallSourceKind;
  querySource?: string;
}
 ...
 */
export interface MesacodeModelTrajectoryRecord {
  requestId: string;
  attempt: number;
  startedAt: string;
  completedAt?: string;
  ...
  turnId?: string;
  traceId?: string;
  callSource?: MesacodeModelTrajectoryCallSource;
  model: {
    modelId?: string;
    providerId?: string;
    role?: string;
  ...
  request: {
    messages: MesacodeModelTrajectoryMessage[];
    toolNames: string[];
  };
  response?: {
    finishReason?: string;
    ...
    toolCalls: MesacodeModelTrajectoryContentPart[];
    usage?: MesacodeModelTrajectoryUsage;
    responseId?: string;
    modelId?: string;
  };
  error?: {
...
export interface MesacodeModelTrajectory {
  taskId: string;
  /** runtime 是否支持读取 model-io 轨迹（仅 Mesacode Agent 落盘 model-io）。 */
  available: boolean;
  records: MesacodeModelTrajectoryRecord[];
  /** 命中的源文件绝对路径，便于排查。 */
  sourceFiles: string[];
...
export interface MesacodeTaskTerminalOutcome {
  taskId: string;
...
export interface MesacodeTaskReadyOutcome {
  taskId: string;
  reason: "prompt_completed" | "prompt_failed";
}

/**
 * IMesacodeTaskService — Mesacode task wrapper API 服务接口
 *
 * UI、remote controller 通过这层访问 task wrapper 状态；核心 session 状态由
 * Mesacode Agent server 维护，新功能应优先走 IMesacodeSessionService。
 */
export interface IMesacodeTaskService {
  // ---- 生命周期 ----
  ...
  /** 创建 Mesacode session 并同步 task 索引。 */
  createTask(params: {
    workspacePath: string;
    workspaceIdentity?: string;
    provider?: MesacodeProvider;
  ...
  }): Promise<MesacodeTaskCreateResult>;
  ...
  enqueueTaskCommand(params: {
    ...
    automationId?: string;
    ownerRunId?: TraceId;
  }): Promise<MesacodeEnqueueTaskCommandResult>;
  ...
  cancelTaskCommand(params: {
    ...
    ownerRunId?: TraceId;
    clientMode: MesacodeTaskClientMode;
  }): Promise<MesacodeCancelTaskCommandResult>;
  ...
  }): Promise<MesacodeSessionCompactResult>;
  ...
  goalSession(params: {
    ...
    objective?: string;
    expectedRevision?: number;
  }): Promise<MesacodeSessionGoalResult>;
   ...
   */
  listTaskList(params: MesacodeTaskListQuery): Promise<MesacodeTaskListResult>;
  ...
  applyGroupedTaskViewOrder(params: MesacodeGroupedTaskViewOrderInput): Promise<MesacodeGroupedTaskView>;
  ...
  getTaskSnapshot(params: {
    ...
    model?: string;
    thoughtLevel?: string;
  }): Promise<MesacodeTaskSnapshot | null>;
  ...
  getTaskSnapshotWithEtag(params: {
    ...
    model?: string;
    thoughtLevel?: string;
  }): Promise<MesacodeTaskSnapshotWithEtagResult>;
  ...
  getTaskSnapshotBody(params: {
    ...
    workspaceIdentity?: string;
    refId: string;
  }): Promise<MesacodeTaskSnapshotBody | null>;
  ...
  getTaskSnapshotRef(params: {
    ...
    workspaceIdentity?: string;
    refId: string;
  }): Promise<MesacodeTaskSnapshotRefContent | null>;
  ...
  getTaskSnapshotToolCallsSlice(params: {
    ...
    limit: number;
  }): Promise<MesacodeTaskSnapshotToolCallsSlice | null>;
  ...
  getTaskConfigOptions(params: { taskId: string }): Promise<MesacodeConfigOption[]>;
  ...
  setAssistantMessageFeedback(params: {
    ...
    workspaceIdentity?: string;
    turnIndex: number;
    feedback: MesacodeAssistantMessageFeedback | null;
  }): Promise<MesacodeSessionFile>;
  ...
  /** 切换 task 模式 */
  setMode(params: { taskId: string; mode: MesacodeTaskMode }): Promise<void>;
  ...
  /**
   * 读取 task 对应的模型调用轨迹（来自 ~/.mesacode/cli/{debug,rollout} 的 model-io JSONL）。
   * taskId 即 Mesacode Agent 的 sessionId，按 sessionId 匹配 model-io 记录。
   */
  getModelTrajectory(params: {
    taskId: string;
    /** 最多返回的调用条数（按时间倒序保留最近 N 条），默认 200。 */
    limit?: number;
  }): Promise<MesacodeModelTrajectory>;
  ...
  getTaskTokenUsage(params: {
    ...
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<MesacodeTaskTokenUsageResult>;
  ...
  /**
   * 重启指定 workspace 的 Mesacode Agent 共享进程。
   * 配置变更需要重新读取进程环境时使用。
  ...
  onDynamicWorkspaceEvent(
    workspace: string | MesacodeWorkspaceEventSubscriptionParams,
  ): Event<MesacodeWorkspaceEvent>;

  /** 全局错误事件 */
  onError: Event<MesacodeError>;
}

export const IMesacodeTaskService = createServiceDescriptor<IMesacodeTaskService>(
  ServiceChannels.MesacodeTask,
);
