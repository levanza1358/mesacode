...
/* off-peak 任务仓库：off_peak_tasks 的 sqlite schema、状态机守卫写入与调度认领。
   ...
   禁止往 automations 表或 MesacodeAutomation 类型上加字段。 */
...
import {
  ...
  type MesacodeOffPeakTaskStatus,
...
function rowToTask(row: OffPeakTaskRow): MesacodeOffPeakTask {
  ...
  return {
    ...
    prompt: row.prompt,
    permissionMode: row.permission_mode as MesacodeTaskMode,
    ...
    status: row.status as MesacodeOffPeakTaskStatus,
...
function readOffPeakModelSelection(row: OffPeakTaskRow): MesacodeOffPeakTask["modelSelection"] | null {
...
function serializeOffPeakModelSelection(
  selection: NonNullable<MesacodeOffPeakTask["modelSelection"]>,
): string {
...
export class OffPeakTaskRepo {
   ...
   */
  async create(
    params: MesacodeOffPeakTaskCreateParams,
    options?: {
      /** 测试注入用；缺省 Date.now()。 */
      now?: number;
  ...
  async invalidateModelSelection(
    offPeakTaskId: string,
    modelSelection: NonNullable<MesacodeOffPeakTask["modelSelection"]>,
    options?: { now?: number },
  ...
  async updateEditableFields(
    ...
    params: {
      ...
      modelSelection?: MesacodeOffPeakTask["modelSelection"] | null;
    },
    options?: { now?: number },
  ): Promise<MesacodeOffPeakTask | null> {
    await this.ensureReady();
   ...
   */
  async claimDue(now: number): Promise<MesacodeOffPeakTask[]> {
    await this.ensureReady();
    const db = this.getDatabase();
    db.exec("BEGIN IMMEDIATE");
    try {
      ...
      const claimed: MesacodeOffPeakTask[] = [];
  ...
  async listNonTerminal(): Promise<MesacodeOffPeakTask[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
        `SELECT * FROM off_peak_tasks
  ...
  async listUnsettledTerminal(): Promise<MesacodeOffPeakTask[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
        `SELECT * FROM off_peak_tasks
