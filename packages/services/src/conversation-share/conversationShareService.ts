...
import type { ConversationRow } from "@mesacode/shared/mesacode-protocol-v4";
import {
  PROTOCOL_V4_LIMITS,
  MESACODE_ATTACHMENT_FAULT_CODES,
  readMesacodeAttachmentFaultCode,
} from "@mesacode/shared/mesacode-protocol-v4";
import { Emitter } from "@mesacode/rpc";

import type { IMesacodeAgentService } from "../mesacode-agent/mesacodeAgent.js";
import type { IMesacodeSessionService } from "#src/mesacode-session/mesacodeSession.js";
...
interface ConversationShareServiceOptions {
  mesacodeAgentService: ConversationShareAgentService;
  ...
  mesacodeSessionService?: Pick<IMesacodeSessionService, "createSession" | "listSessions">;
...
function hasUnsupportedArtifactReference(value: unknown): boolean {
  if (typeof value === "string") return /^mesacode-artifact:\/\//iu.test(value);
 ...
 * 仅在 fault 码缺席时保留一条 errno 文本兜底，用于尚未带结构化码的旧 mesacode-cli。
 */
function isDefiniteMissingAttachment(error: unknown): boolean {
  const faultCode = readMesacodeAttachmentFaultCode(error);
  if (faultCode) {
    return (
      faultCode === MESACODE_ATTACHMENT_FAULT_CODES.shareStatNotFound ||
      faultCode === MESACODE_ATTACHMENT_FAULT_CODES.statNotFile
    );
  }
...
function isAttachmentAuthorizationError(error: unknown): boolean {
  ...
  if (faultCode) {
    return (
      faultCode === MESACODE_ATTACHMENT_FAULT_CODES.shareStatNotAuthorized ||
      faultCode === MESACODE_ATTACHMENT_FAULT_CODES.shareReadNotAuthorized ||
      faultCode === MESACODE_ATTACHMENT_FAULT_CODES.shareStatConnectionUntrusted ||
      faultCode === MESACODE_ATTACHMENT_FAULT_CODES.shareReadConnectionUntrusted
    );
  }
  const message = error instanceof Error ? error.message : String(error);
...
function isAttachmentTooLargeError(error: unknown): boolean {
  const faultCode = readMesacodeAttachmentFaultCode(error);
  return (
    faultCode === MESACODE_ATTACHMENT_FAULT_CODES.shareStatTooLarge ||
    faultCode === MESACODE_ATTACHMENT_FAULT_CODES.previewTooLarge
  );
}
...
export class ConversationShareService implements IConversationShareService {
  private readonly mesacodeAgentService: ConversationShareAgentService;
  ...
  >();
  private readonly mesacodeSessionService?: Pick<
  ...
  constructor(options: ConversationShareServiceOptions) {
    this.mesacodeAgentService = options.mesacodeAgentService;
    ...
    this.mesacodeSessionService = options.mesacodeSessionService;
    ...
    // 兜底写死生产站 https://mesacode.z.ai/cn/share，于是测试环境（API base 走
    // 配置的 Mesacode origin）导入后回链仍指向生产站，点分割线打开的是另一个环境的分享。
    // 改用与 API base 同一个环境解析器（buildRuntimeMesacodeApiUrl 也走它），保证同环境。
    // 优先级不变：显式 option > MESACODE_CONVERSATION_SHARE_WEB_URL > 按环境推导。
    this.shareWebUrl = (
      options.shareWebUrl ??
      process.env.MESACODE_CONVERSATION_SHARE_WEB_URL ??
      `${resolveRuntimeMesacodeEndpointOrigin(process.env)}/cn/share`
    ).replace(/\/+$/u, "");
    this.importIndexPath = join(this.conversationWorkspaceRoot, ".mesacode-share-imports.json");
    ...
    if (this.mesacodeSessionService) {
  ...
  async preflight(
    ...
    try {
      return await this.preflightWithAgent(input, this.mesacodeAgentService);
    } catch (error) {
  ...
  private async importShareInternal(
    ...
    const shareRoot = join(workspacePath, ".mesacode-share");
    const importRoot = join(shareRoot, sanitizeFileSegment(continuation.share.share_id));
    const markerPath = join(importRoot, ".mesacode-share-import.json");
    ...
    try {
      ...
      if (
        ...
        const sessions = await this.mesacodeSessionService.listSessions({ workspacePath, limit: 100 });
    ...
    try {
      ...
      for (const artifact of continuation.artifacts) {
        ...
        installedArtifacts.push({
          ...
          workspaceRelativePath: `.mesacode-share/${sanitizeFileSegment(continuation.share.share_id)}/shared-artifacts/${fileName}`,
      ...
      const snapshot = await this.mesacodeSessionService.createSession({
        workspacePath,
        ...(workspaceIdentity ? { workspaceIdentity } : {}),
        sessionId,
   ...
   *
   * 目录名用的是 share_id 而不是 contextId（二者不等价），所以扫 .mesacode-share/ 下各
  ...
  async getImportedConversation(input: {
    ...
    const shareRoot = join(input.workspacePath, ".mesacode-share");
    let entries: Dirent[];
    try {
  ...
  async publish(input: PublishTextConversationInput, operationId: string) {
    return this.publishWithAgent(input, operationId, this.mesacodeAgentService);
  }
  ...
  private async publishInternal(
    input: PublishTextConversationInput,
    operationId?: string,
    agentService: ConversationShareAgentService = this.mesacodeAgentService,
    ...
    const artifactSnapshot = await buildConversationShareArtifactSnapshot({
      mesacodeAgentService: agentService,
  ...
  private async cleanupAbandonedImports(): Promise<void> {
    ...
    for (const entry of imports) {
      ...
      if (typeof marker.sessionId !== "string" || !marker.sessionId.startsWith("share-import-"))
        continue;
      try {
        const sessions = await this.mesacodeSessionService.listSessions({
