...
import type { ConversationShareArtifactDescriptor } from "@mesacode/shared";
import type { ArtifactRow, ConversationRow } from "@mesacode/shared/mesacode-protocol-v4";
...
function assertSafeString(value: string): void {
  if (/^mesacode-artifact:\/\//iu.test(value)) {
...
function isPublicArtifactRef(value: string): boolean {
  return /^mesacode-artifact:\/\/share\/[A-Za-z0-9._~-]+$/u.test(value);
}
  ...
  switch (row.kind) {
    ...
    case "artifact": {
      ...
      return {
        ...
        sha256: row.sha256,
        ref: `mesacode-artifact://share/${artifactVersionId}`,
        state: row.state,
      };
    }
    case "timelineMarker":
  ...
  for (const [index, row] of input.rows.entries()) {
    ...
    if (row.kind === "turnHeader") {
      ...
      if (
        ...
        row.ref !== `mesacode-artifact://share/${row.artifactVersionId}`
      ) {
        throwProjectionError(
          "invalid_conversation",
