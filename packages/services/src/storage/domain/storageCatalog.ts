/**
 * 存储分类目录：把 .mesacode 根下的相对路径映射到类别、聚合 key 与可清理性。
 * 纯函数、单一事实源。
 * 匹配顺序：根级特例 → 文件规则（精确）→ 前缀规则（最长前缀优先）→ 其他。
 */
import type { StorageCategoryId, StorageCleanability, StorageRootId } from "@mesacode/shared";
