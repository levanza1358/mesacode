...
import { readLegacyMesacodeConfigProviders } from "../src/model-provider/legacyMesacodeConfigProviderReader.js";
...
async function setup() {
  const dir = await mkdtemp(join(tmpdir(), "mesacode-provider-migration-"));
  setDataBaseDir(dir);
  ...
  const runtime = createProviderConfigRuntime({
    ...
    readLegacyProviders: async () => {
      reads += 1;
      return readLegacyMesacodeConfigProviders();
    },
...
test("startup migrates published Mesacode config into personal config without changing the source", async () => {
