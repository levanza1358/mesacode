...
for (const clientMode of ["desktop-continuous", "web-remote-replayable"] as const) {
  test(`previously imported Claude history becomes a real session for ${clientMode}`, async () => {
    const dir = await mkdtemp(join(tmpdir(), "mesacode-import-recovery-"));
    setDataBaseDir(dir);
    const meta = {
    ...
    type CreateInput = Parameters<Options["mesacodeAgentService"]["createSession"]>[0];
    const created: CreateInput[] = [];
    let session: MesacodeSessionStateSnapshot | undefined;
    ...
    const service = createMesacodeTaskServiceAdapter({
      ...
      mesacodeAgentService: {
        ...
        async createSession(input: CreateInput) {
          created.push(input);
          session = mesacodeSessionStateSnapshotSchema.parse({
