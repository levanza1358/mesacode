...
import type { completeModelConfigDataSchema } from "@mesacode/shared/model-config";
import type {
  ...
  resolve(input: ProviderConfigResolverInput): ProviderConfigResolution {
    const accountProviders = new ProviderConfigMap(
      input.accountProviders
        .entries()
        .filter(([providerId]) => input.mesacodeBuiltinProviders.has(providerId))
        .map(([providerId, config]) => [providerId, config.withoutGroup()] as const),
    );
    const concreteBuiltinProviders = input.mesacodeBuiltinProviders.overlay(accountProviders);
    const providerTemplates = input.mesacodeBuiltinProviderTemplates;
    ...
    const effectiveModelRules = ModelConfigRules.composeEffective(
      input.mesacodeBuiltinModelRules,
    ...
    for (const providerId of resolveProviderOrder(input, effectiveProviders)) {
      ...
      const models = orderedModelIds.map((modelId): ResolvedProviderModel => {
        ...
        const effectiveBuiltinConfig = input.mesacodeBuiltinModelRules.resolve({
          providerId,
          templateId,
          modelId,
          apiType: config.api?.type,
...
function resolveProviderOrder(
  ...
  const builtinIds = input.mesacodeBuiltinProviders
    .keys()
