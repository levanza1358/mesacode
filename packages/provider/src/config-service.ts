...
import type { ModelSelection } from "@mesacode/shared/model-selection";
...
export interface ProviderConfigServiceDependencies {
  readonly mesacodeBuiltinSource: ProviderSource<ProviderConfigLayerSnapshot>;
...
export class ProviderConfigService implements ProviderSource<ProviderConfigSnapshot> {
  ...
  constructor(dependencies: ProviderConfigServiceDependencies) {
    this.#mesacodeBuiltinSource = dependencies.mesacodeBuiltinSource;
    this.#personalRepository = dependencies.personalRepository;
    this.#sourceDisposers = [
      this.#mesacodeBuiltinSource.onDidChange((reason) => this.#emit(`mesacodeBuiltin:${reason}`)),
  ...
  async read(): Promise<ProviderConfigSnapshot> {
    ...
    const [mesacodeBuiltin, personal] = await Promise.all([
      this.#mesacodeBuiltinSource.read(),
      this.#personalRepository.read(),
    ]);
    return Object.freeze({
      revision: JSON.stringify([mesacodeBuiltin.revision, personal.revision]),
      mesacodeBuiltinRevision: mesacodeBuiltin.revision,
      personalRevision: personal.revision,
      mesacodeBuiltinProviders: mesacodeBuiltin.providers,
      mesacodeBuiltinProviderTemplates: mesacodeBuiltin.providerTemplates ?? ProviderTemplateMap.empty(),
      personalProviders: personal.providers,
      mesacodeBuiltinModelRules: mesacodeBuiltin.models,
  ...
  async savePersonalProviderOverlay(
    ...
    const mesacodeBuiltin = await this.#mesacodeBuiltinSource.read();
    return this.#updatePersonal((current) => {
      assertMembershipCurrent(membership, providerId, current);
      const builtin = mesacodeBuiltin.providers.get(providerId);
      const currentPersonal = current.providers.get(providerId);
      const currentEffectiveProviders = mesacodeBuiltin.providers.overlay(current.providers);
      ...
      if (builtin?.access?.type === "zhipu-account" && config.access !== undefined) {
        ...
        throw new Error(
          `固定 Account Provider 的 Access 只能由 Mesacode Built-in Config 声明: ${providerId}`,
        );
      }
      ...
      const membershipBaseline =
        builtin ?? resolveTemplateBaseline(mesacodeBuiltin, current.providers, providerId);
      ...
      const nextEffectiveProviders = mesacodeBuiltin.providers.overlay(providers);
  ...
  async createPersonalProvider(
    ...
    const template = templateId ? mesacodeBuiltin.providerTemplates?.get(templateId) : undefined;
    ...
    await this.#updatePersonal((current) => {
      const occupied = new Set([...mesacodeBuiltin.providers.keys(), ...current.providers.keys()]);
      const providerId = nextPersonalProviderId(occupied, templateId);
      createdProviderId = providerId;
      const effectiveProviders = resolvePersonalProviderBaselines(mesacodeBuiltin, current.providers);
  ...
  async reorderPersonalProviders(
    ...
    return this.#updatePersonal((current) => ({
      ...
      models: current.models,
      providerOrder: normalizeProviderOrder(mesacodeBuiltin.providers, current.providers, providerIds),
    }));
  }
  ...
  async reorderPersonalModels(
    ...
    return this.#updatePersonal((current) => {
      ...
      const builtinProvider =
        mesacodeBuiltin.providers.get(providerId) ??
        resolveTemplateBaseline(mesacodeBuiltin, current.providers, providerId);
      const provider = writableProviderOverlay(mesacodeBuiltin, current, providerId);
  ...
  async addPersonalModel(
    ...
    return this.#updatePersonal((current) => {
      ...
      const provider = writableProviderOverlay(mesacodeBuiltin, current, normalizedProviderId);
      const builtinModelIds =
        membership?.inheritedModelIds ??
        resolveProviderBuiltinModelIds(mesacodeBuiltin, current.providers, normalizedProviderId);
  ...
  async setPersonalModelEnabled(
    ...
    const builtin = await this.#mesacodeBuiltinSource.read();
    return this.#updatePersonal((current) => {
...
function normalizeProviderOrder(
  _mesacodeBuiltinProviders: ProviderConfigMap,
...
function appendCurrentProviderOrder(
  mesacodeBuiltinProviders: ProviderConfigMap,
  personalProviders: ProviderConfigMap,
  currentOrder: readonly ProviderId[] | undefined,
  addedProviderId: ProviderId,
): ProviderId[] {
  const current = normalizeProviderOrder(
    mesacodeBuiltinProviders,
    personalProviders,
    currentOrder ?? [],
  );
  return normalizeProviderOrder(mesacodeBuiltinProviders, personalProviders, [
...
function resolvePersonalProviderBaselines(
  ...
  const personal = personalProviders.mapConfigs((config, _id, rule) => {
    const template = rule.templateId
      ? mesacodeBuiltin.providerTemplates?.get(rule.templateId)
      : undefined;
    return template ? template.config.overlay(config) : config;
  });
  return mesacodeBuiltin.providers.overlay(personal);
}
