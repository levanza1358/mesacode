...
export interface ProviderConfigSnapshot {
  ...
  readonly mesacodeBuiltinProviders: ProviderConfigMap;
  readonly mesacodeBuiltinProviderTemplates: ProviderTemplateMap;
  readonly personalProviders: ProviderConfigMap;
  readonly mesacodeBuiltinModelRules: ModelConfigRules;
...
export function createFailClosedAccountProviderConfigSnapshot(
  ...
  const unentitledProviders = new ProviderConfigMap(
    config.mesacodeBuiltinProviders.entries().flatMap(([providerId, provider]) =>
  ...
  return createAccountProviderConfigSnapshot(config.mesacodeBuiltinRevision, unentitledProviders);
}

export function createAccountProviderConfigSnapshot(
  basedOnMesacodeBuiltinRevision: string,
  providers: ProviderConfigMap,
  states?: AccountProviderStates,
): AccountProviderConfigSnapshot {
  return Object.freeze({
    revision: `account:${JSON.stringify([basedOnMesacodeBuiltinRevision, providers.toJSON(), states])}`,
...
const EMPTY_ACCOUNT_PROVIDER_CONFIG_SNAPSHOT: AccountProviderConfigSnapshot = Object.freeze({
  ...
  basedOnMesacodeBuiltinRevision: "uninitialized",
