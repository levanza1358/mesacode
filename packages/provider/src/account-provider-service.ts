  ...
  async #runRefreshLoop(): Promise<AccountProviderConfigSnapshot> {
    ...
    while (this.#pendingReasons.size > 0) {
      ...
      try {
        ...
        const configuredProviders = config.mesacodeBuiltinProviders;
        const { providers, states } = await this.#resolve({
          configRevision: config.mesacodeBuiltinRevision,
          configuredProviders,
        ...
        if (
          ...
          currentConfig.mesacodeBuiltinRevision !== config.mesacodeBuiltinRevision
        ) {
          this.#pendingReasons.add("superseded-resolution");
          continue;
        }
        const basedOnMesacodeBuiltinRevision = config.mesacodeBuiltinRevision;
