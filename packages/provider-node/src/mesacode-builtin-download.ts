import { z } from "zod";
import { decodeMesacodeBuiltinRelease, type MesacodeBuiltinRelease } from "./mesacode-builtin-release.js";
...
export interface MesacodeBuiltinDownloadOptions {
...
export async function downloadMesacodeBuiltinRelease(
  options: MesacodeBuiltinDownloadOptions,
  ...
  try {
    ...
    stage = "cdn";
    return decodeMesacodeBuiltinRelease(await readJson(new URL(downloadUrl)));
  } catch (error) {
    ...
    const reason = signal.aborted
      ...
      : error instanceof DownloadBoundaryError
        ...
        : error instanceof z.ZodError
          ...
          : "invalid response";
    throw new Error(`Mesacode Built-in ${stage}: ${reason}`);
  } finally {
    clearTimeout(timer);
  }
