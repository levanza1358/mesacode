...
export interface MaterializeMesacodeBuiltinProviderConfigOptions {
 ...
 */
export async function materializeMesacodeBuiltinProviderConfig(
  options: MaterializeMesacodeBuiltinProviderConfigOptions,
): Promise<string> {
  const content = `${serializeMesacodeBuiltinRelease(
    decodeMesacodeBuiltinRelease(JSON.parse(options.content)),
  )}\n`;
  const filePath = join(
