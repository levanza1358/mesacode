...
import {
  normalizeMesacodeBuiltinEndpointOrigin,
  resolveMesacodeBuiltinCachePaths,
} from "./mesacode-builtin-cache-paths.js";
...
export interface EndpointScopedMesacodeBuiltinSourceOptions {
  readonly bundledFilePath: string;
  readonly environmentConfigRoot: string;
  readonly platform: string;
  readonly appVersion: string;
  readonly resolveEndpointOrigin: () => string | Promise<string>;
  readonly fetchRelease: MesacodeBuiltinRemoteSynchronizerOptions["fetchRelease"];
  readonly onRefreshResult?: MesacodeBuiltinRemoteSynchronizerOptions["onRefreshResult"];
  readonly watch?: boolean;
}

/**
 * 让 Environment 的 Mesacode 控制面 Endpoint 同时决定 Active/LKG 与刷新控制路径。
 * Endpoint 切换只替换当前 Source，不读取上一 Endpoint 的缓存。
 */
export class EndpointScopedMesacodeBuiltinSource implements ProviderSource<ProviderConfigLayerSnapshot> {
  readonly #options: EndpointScopedMesacodeBuiltinSourceOptions;
  readonly #listeners = new Set<(reason: string) => void>();
  #current: CurrentEndpointSource | null = null;
  #ensureInFlight: Promise<CurrentEndpointSource> | null = null;
  #disposed = false;

  constructor(options: EndpointScopedMesacodeBuiltinSourceOptions) {
  ...
  async refresh(options?: { readonly force?: boolean }): Promise<MesacodeBuiltinRefreshResult> {
  ...
  async #resolveCurrent(): Promise<CurrentEndpointSource> {
    const endpointOrigin = normalizeMesacodeBuiltinEndpointOrigin(
      await this.#options.resolveEndpointOrigin(),
    );
    const paths = resolveMesacodeBuiltinCachePaths({
      environmentConfigRoot: this.#options.environmentConfigRoot,
      platform: this.#options.platform,
      appVersion: this.#options.appVersion,
      mesacodeEndpointOrigin: endpointOrigin,
    });
    ...
    const synchronizer = new MesacodeBuiltinRemoteSynchronizer({
      source,
      controlFilePath: paths.controlFilePath,
      resolveEndpointKey: async () =>
        normalizeMesacodeBuiltinEndpointOrigin(await this.#options.resolveEndpointOrigin()),
  ...
  #assertNotDisposed(): void {
    if (this.#disposed) throw new Error("EndpointScopedMesacodeBuiltinSource 已 dispose");
  }
}
...
class CurrentEndpointSource {
  constructor(
    ...
    readonly source: NodeMesacodeBuiltinProviderConfigSource,
    readonly synchronizer: MesacodeBuiltinRemoteSynchronizer,
