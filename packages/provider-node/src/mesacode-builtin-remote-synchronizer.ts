...
import type { NodeMesacodeBuiltinProviderConfigSource } from "./mesacode-builtin-provider-config-source.js";
import type { MesacodeBuiltinRelease } from "./mesacode-builtin-release.js";

const HOUR_MS = 60 * 60 * 1_000;

export type MesacodeBuiltinRefreshResult =
  | "updated"
  | "unchanged"
  | "stale"
  | "missing"
...
export interface MesacodeBuiltinRemoteSynchronizerOptions {
  ...
  readonly onRefreshResult?: (event: MesacodeBuiltinRefreshEvent) => void;
  readonly now?: () => number;
...
export interface MesacodeBuiltinRefreshEvent {
  readonly result: MesacodeBuiltinRefreshResult;
...
export class MesacodeBuiltinRemoteSynchronizer {
  readonly #options: MesacodeBuiltinRemoteSynchronizerOptions;
  readonly #now: () => number;
  #inFlight: Promise<MesacodeBuiltinRefreshResult> | null = null;
  #disposed = false;
  #abortController: AbortController | null = null;

  constructor(options: MesacodeBuiltinRemoteSynchronizerOptions) {
    this.#options = options;
    this.#now = options.now ?? Date.now;
  }

  refresh(options: { readonly force?: boolean } = {}): Promise<MesacodeBuiltinRefreshResult> {
  ...
  async #refresh(force: boolean, signal: AbortSignal): Promise<MesacodeBuiltinRefreshResult> {
    const endpointKey = (await this.#options.resolveEndpointKey()).trim();
    if (this.#disposed) return "disposed";
    if (!endpointKey) throw new Error("Mesacode Built-in 远端 Endpoint 不能为空");
  ...
  #report(event: MesacodeBuiltinRefreshEvent): void {
