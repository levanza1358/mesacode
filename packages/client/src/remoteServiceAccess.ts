import { ProxyChannel, type IChannelClient } from "@mesacode/rpc";
import {
  IFileService,
...
export class RemoteServiceAccess implements IServiceAccessor {
  ...
  readonly mesacodeAgentService: IMesacodeAgentService;
  readonly mesacodeSessionService: IMesacodeSessionService;
  ...
  constructor(channelClient: IChannelClient) {
    ...
    this.mesacodeTaskService = ProxyChannel.toService<IMesacodeTaskService>(
      channelClient.getChannel(IMesacodeTaskService.channelName),
    );
    ...
    this.mesacodeAgentService = ProxyChannel.toService<IMesacodeAgentService>(
      channelClient.getChannel(IMesacodeAgentService.channelName),
    );
    this.mesacodeSessionService = ProxyChannel.toService<IMesacodeSessionService>(
      channelClient.getChannel(IMesacodeSessionService.channelName),
    );
