import { pickProductEndpointEnv } from "@mesacode/shared/mesacodeEndpoint";
...
function loadEnvFiles(): Record<string, string> {
  ...
  // 真实环境变量优先级最高
  if (process.env.MESACODE_ENV) vars.MESACODE_ENV = process.env.MESACODE_ENV;
  if (process.env.MESACODE_BASE_URL) vars.MESACODE_BASE_URL = process.env.MESACODE_BASE_URL;
  if (process.env.VITE_MESACODE_BASE_URL) vars.VITE_MESACODE_BASE_URL = process.env.VITE_MESACODE_BASE_URL;
...
const { environment: mesacodeEnv } = await loadBuiltinProviderConfig();
// 安装包身份与后端环境分轴：MESACODE_PREVIEW_IDENTITY=1 让生产后端的构建仍以 Mesacode Preview 身份打包运行。
const mesacodeProductFlavor = resolveDesktopProductFlavor({ ...process.env, MESACODE_ENV: mesacodeEnv });
console.log(`[tsup] MESACODE_ENV=${mesacodeEnv} MESACODE_PRODUCT_FLAVOR=${mesacodeProductFlavor}`);

export function resolveDesktopTsupBundleSecurityOptions(
  runtimeEnv: Record<string, string | undefined> = process.env,
) {
  const isProduction = runtimeEnv.NODE_ENV === "production";
  const isE2ECoverageBuild = runtimeEnv.MESACODE_E2E_COVERAGE === "1";
  return {
...
function createSharedDefines() {
  return {
    ...
    // 缺失时 installer 抛 "Packaged Mesacode is missing its embedded Computer Use Helper build identity"。
    // CI 构建时通过 MESACODE_CUA_HELPER_BUILD_ID env 注入；dev 为空串走兜底（dev helper 不走下载）。
    __MESACODE_CUA_HELPER_BUILD_ID__: JSON.stringify(
      process.env.MESACODE_CUA_HELPER_BUILD_ID?.trim() ?? "",
    ),
    // 客户端只有一个 CDN 配置，与发布端 OSS 目标列表分离。
    __MESACODE_CDN_BASE_URL__: JSON.stringify(env.MESACODE_CDN_BASE_URL?.trim() || ""),
  };
}
...
export default defineConfig([
  {
    ...
    entry: {
      ...
      "main/mesacodeDataSizeWorker": "src/main/mesacodeDataSizeWorker.ts",
    ...
    noExternal: [
      ...
      "@mesacode/provider",
      "@mesacode/provider-node",
      // services 已内联进 main，但其 producer import 曾被保留为裸包引用；
      // electron-builder 又会排除 node_modules/@mesacode，导致安装包启动即 ERR_MODULE_NOT_FOUND。
  ...
  },
  {
    ...
    platform: "node",
    target: "node22",
    // 与 host 同构：常驻 cron scheduler 进程复用 @mesacode/services（tasks-index + cron），
