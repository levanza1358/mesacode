...
const desktopProductIdentity = resolveDesktopProductIdentity({
  ...process.env,
  MESACODE_ENV: builtinProviderConfig.environment,
});
...
const shouldEnableMacSigning =
  process.env.MESACODE_ENABLE_MAC_SIGN === "1" && Boolean(macSigningIdentity);
...
const REQUIRED_ASAR_RUNTIME_MODULES = [
  ...
  "pngjs",
  // @mesacode/services 的代理连通性探测会动态 require("undici") 取 ProxyAgent。
...
const WINDOWS_INSTALL_MANIFEST_NAME = ".mesacode-install-manifest";
...
function resolveElectronDownloadMirror(env = process.env) {
  const existingMirror =
    env.MESACODE_ELECTRON_RUNTIME_MIRROR ||
...
if (
  ...
  process.env.MESACODE_ENABLE_MAC_SIGN === "1" &&
  !macSigningIdentity
) {
  throw new Error(
    "Mesacode Preview macOS packaging requires APPLE_SIGNING_IDENTITY or CSC_NAME when MESACODE_ENABLE_MAC_SIGN=1",
  );
}
...
function resolveAppAsarPath(context) {
  if (context.electronPlatformName === "darwin") {
    const appName = `${context.packager?.appInfo?.productFilename ?? "Mesacode"}.app`;
...
async function injectHoistedRuntimeModulesIntoAsar(context) {
  ...
  const stagingDir = mkdtempSync(resolve(tmpdir(), "mesacode-app-asar-"));
  try {
...
export default {
  ...
  extraMetadata: {
    ...
    mesacodeProductFlavor: desktopProductIdentity.flavor,
    homepage: "https://mesacode.z.ai",
    author: {
      name: "Mesa Code",
      email: "dev@mesacode.z.ai",
    },
  },
  ...
  executableName: desktopProductIdentity.executableName,
  directories: {
  ...
  extraResources: [
      ...
      ? [
          {
            ...
            // 只是不吸附，所以这里不做存在性断言。
            from: "resources/macos-window-bounds/mesacode-window-bounds",
            to: "macos-window-bounds/mesacode-window-bounds",
          },
        ]
      : []),
    {
    ...
    {
      // Provider Registry 的 Mesacode Built-in Config 是静态 Provider/Model 事实的唯一内置来源。
      // 显式随包发布，避免正式 Host 回退到旧 Catalog/Preset hardcode。
      from: builtinProviderConfig.sourcePath,
      to: "config/provider/mesacode-builtin.json",
    },
    {
      // 应用图标：打包后放入 resources 目录，主进程通过 process.resourcesPath 加载
      from: "../../public/logo/icons/1024x1024.png",
      to: "icon.png",
    },
      ...
      ? [
          {
            ...
            from: "../../public/logo/icons/icon.ico",
            to: "icon_512x512.png",
          },
        ]
      : []),
    {
      ...
      from: "../../public/logo/icons/512x512.png",
      to: "icon_windows.png",
    },
      ...
      : []),
    {
      ...
      // 桌面端内置的是 agent 的 JS bundle（glm/mesacode.cjs，由 prepare:agent-bundle 生成），
      // Host 进程用 app 自带的 Electron Node runtime（ELECTRON_RUN_AS_NODE）执行 `mesacode.cjs app-server --stdio`，
  ...
  protocols: [
    {
      ...
      // 展示名跟随安装包身份；scheme 仍保持 mesacode，因此两个应用中最后注册者会成为默认 handler。
  ...
  linux: {
    ...
    // desktop 包名是 scoped package（@mesacode/desktop），electron-builder 默认会把
    // Linux executable/Icon 推成 @mesacodedesktop。部分桌面环境无法按这个 icon name 命中
    // hicolor 图标，最终回退成系统齿轮。这里固定成稳定的小写名称，让 Icon=mesacode
    // 与 /usr/share/icons/hicolor/*/apps/mesacode.png 保持一致。
    executableName: desktopProductIdentity.linuxExecutableName,
    category: "Development",
    maintainer: "Mesacode <dev@mesacode.z.ai>",
  },
  deb: {
  ...
  dmg: {
    ...
    icon: "../../public/logo/icons/icon.icns",
    contents: [
  ...
  nsis: {
    ...
    installerIcon: "../../public/logo/icons/icon.ico",
    uninstallerIcon: "../../public/logo/icons/icon.ico",
    installerHeaderIcon: "../../public/logo/icons/icon.ico",
  },
