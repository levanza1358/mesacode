...
if (process.platform === "darwin" && existsSync(electronBinary)) {
  ...
  // mesacode:// 交给一个没有项目入口的 Electron 默认壳。给本地启动副本补齐产品
  ...
  const devBundle = await prepareDevElectronAppBundle({
    electronAppPath,
    runtimeRoot: resolve(root, "../../.mesacode-runtime/desktop-dev"),
    electronVersion: electronPackage.version,
    arch: process.arch,
  });
  electronCommand = devBundle.executablePath;
  console.log(`[dev] Prepared macOS Mesacode Dev bundle: ${devBundle.appPath}`);
}
