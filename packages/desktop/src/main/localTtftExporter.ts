...
import { LOCAL_TTFT_BUCKETS_MS, localTtftBatchSchema, type LocalTtftBatch } from "@mesacode/shared";
import {
...
export function createLocalTtftExporter(options: {
  ...
  const resource = resourceFromAttributes({
    "service.name": "mesacode-local-ttft",
    "service.version": options.version,
    "os.type": process.platform,
    "mesacode.telemetry.schema_version": 1,
  });
  const meterProvider = new MeterProvider({
    ...
    views: [
      {
        instrumentName: "mesacode.local_ttft.*",
        aggregation: {
  ...
  const meter = meterProvider.getMeter("@mesacode/local-ttft", "1");
  const duration = meter.createHistogram("mesacode.local_ttft.duration", { unit: "ms" });
  const stageDuration = meter.createHistogram("mesacode.local_ttft.stage.duration", { unit: "ms" });
  const stageObservation = meter.createHistogram("mesacode.local_ttft.stage.observation.duration", {
    unit: "ms",
  });
  const preparationObservation = meter.createHistogram(
    "mesacode.local_ttft.preparation.observation.duration",
    { unit: "ms" },
  );
  const systemDuration = meter.createHistogram("mesacode.local_ttft.system.duration", { unit: "ms" });
  const preparationDuration = meter.createHistogram("mesacode.local_ttft.preparation.duration", {
    unit: "ms",
  });
  const attemptDuration = meter.createHistogram("mesacode.local_ttft.attempt.duration", {
    unit: "ms",
  });
  const executionDuration = meter.createHistogram("mesacode.local_ttft.execution.duration", {
    unit: "ms",
  });
  const failureWait = meter.createHistogram("mesacode.local_ttft.no_output.wait", { unit: "ms" });
  const textDuration = meter.createHistogram("mesacode.local_ttft.first_text.duration", {
    unit: "ms",
  });
  const outcomes = meter.createCounter("mesacode.local_ttft.records");
  const drops = meter.createCounter("mesacode.local_ttft.dropped");
