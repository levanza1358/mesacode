...
import { formatTimestamp } from "@mesacode/shared";
import { cleanupExpiredLogFiles, LOG_RETENTION_DAYS } from "./logRetention.js";
import { getAppConfigDir, maybeThrowInjectedFsFault } from "@mesacode/services/node";

function getLogDir() {
  const e2eLogDir =
    process.env.MESACODE_ENV === "test" ? process.env.MESACODE_E2E_RUNTIME_LOG_DIR?.trim() : undefined;
  if (e2eLogDir) {
    return e2eLogDir;
  }
...
/**
 * main 进程日志，默认写入 ~/.mesacode/v2/logs/YYYY-MM-DD.log；E2E 测试使用 worker 专属目录。
