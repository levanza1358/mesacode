...
import { formatMesacodeRendererProcessName } from "@mesacode/shared";
...
export function buildAuxiliaryRendererName(contents: WebContents): string {
  ...
  if (url?.startsWith("devtools://")) {
    return formatMesacodeRendererProcessName(
      `devtools-${contentsId}-${hostTitle ?? ownerLabel ?? "unknown"}`,
    );
  }

  if (type === "window") {
    return formatMesacodeRendererProcessName(title ?? `window-${contentsId}`);
  }

  return formatMesacodeRendererProcessName(`${type}-${contentsId}-${pageLabel}`);
}
