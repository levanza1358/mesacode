...
import {
  ...
  updateMesacodeStdioTapDevMenuState,
...
import { resolveMesacodeBuiltinProviderConfigFilePath } from "./desktopProviderConfig.js";
import {
  getCredentialsDir,
  ...
  resolveMesacodeEndpointEnvBaseOrigin,
  desktopRuntimeEnv,
...
import {
  registerDesktopMesacodeDataSizeTelemetry,
  stopDesktopMesacodeDataSizeTelemetry,
} from "./desktopMesacodeDataSizeTelemetry.js";
...
// 会和开发态已打开的 Mesacode Dev 抢端口，导致 WebDriver session 创建前白屏超时。
// 仅本地开发运行默认开启远程调试端口，并允许 e2e 通过环境变量交给 Chromedriver 接管。
if (!app.isPackaged && process.env.MESACODE_DISABLE_FIXED_REMOTE_DEBUGGING_PORT !== "1") {
...
const settingsFile = join(homedir(), ".mesacode", "v2", "setting.json");
...
async function resolveCurrentMesacodeEndpointOrigin() {
  return resolveMesacodeEndpointOrigin({
    env: MESACODE_ENV,
    envBaseOrigin: resolveMesacodeEndpointEnvBaseOrigin(hostProcessLocalEnv),
    overrideOrigin: (await mainSettingService.get()).mesacodeEndpointOrigin,
  });
}
...
app.on("browser-window-focus", (_event, win) => {
  ...
  rebuildMenu();
  // 设置/更新等无 Host 的 Mesacode 窗口也算前台：router 会先把旧 workspace Host 清成 null，
...
const readHelpConfig = createDesktopHelpConfigReader({
  appVersion: MESACODE_VERSION || app.getVersion(),
  deviceMid,
...
const localTtftExporter = createLocalTtftExporter({
  ...
  version: MESACODE_VERSION || app.getVersion(),
  logger,
});
...
const armsUserIdentitySync = createArmsUserIdentitySync({
  ...
  setUser:
    MESACODE_TELEMETRY_ENABLED && MESACODE_ARMS_RUM_ENDPOINT
...
async function prepareAppQuit(reason: string, kind: AppShutdownKind = "normal"): Promise<void> {
  ...
  appQuitPreparationInFlight = Promise.all([
    ...
    .finally(() => {
      ...
      // host 还没等到 agent 进程树的 SIGTERM/SIGKILL 兜底完成就被带走，mesacode-cli 会被 init 接管成残留进程。
...
function exitPreparedApp(reason: string): never | void {
  ...
  if (process.env.MESACODE_E2E_RUN_ID?.trim()) {
...
function shouldConfirmAppQuit() {
  ...
  return MESACODE_ENV === "production" && getRunningAgentSessionCount() > 0;
}

function confirmAppQuit(originWindow?: BrowserWindow | null) {
  if (!shouldConfirmAppQuit()) {
    logger.info(`[app-quit] quit confirmation skipped in ${MESACODE_ENV}`);
    return true;
  }
...
async function executeDesktopCommandForApp(
  ...
  return executeDesktopCommand({
    ...
    onMesacodeEndpointChanged: handleMesacodeEndpointChanged,
    mesacodeEndpointEnvBaseOrigin: resolveMesacodeEndpointEnvBaseOrigin(hostProcessLocalEnv),
    onRelaunchApp: async () => {
...
async function resolveMesacodeEndpointSelection(): Promise<"production" | "test" | "custom"> {
  if (MESACODE_ENV === "production") {
    return "production";
  }
  const origin = await resolveCurrentMesacodeEndpointOrigin();
  if (origin === DEFAULT_MESACODE_ENDPOINT_ORIGIN) {
    return "production";
  }
  return "custom";
}

async function handleMesacodeEndpointChanged() {
  rebuildMenu();
}
...
function rebuildMenu() {
  void Promise.all([resolveMesacodeEndpointSelection(), mainSettingService.get()]).then(
    ([mesacodeEndpointSelection, settings]) => {
      rebuildApplicationMenu({
        currentApplicationLocale,
        mesacodeEndpointSelection,
  ...
  const win = createWindow({
    ...
    spawnHostProcess: (win, label, initMessage) =>
      spawnHostProcess(
        ...
        {
          ...initMessage,
          mesacodeBuiltinProviderConfigFilePath: resolveMesacodeBuiltinProviderConfigFilePath({
            env: { ...hostProcessLocalEnv, ...process.env },
          }),
        },
        {
...
app.whenReady().then(async () => {
  ...
  if (process.platform === "win32") {
    ...
    app.setAppUserModelId(
      resolveWindowsAppUserModelIdForFlavor(MESACODE_PRODUCT_FLAVOR, { isPackaged: app.isPackaged }),
    );
  }
  ...
  // Preview 身份无论连接哪个后端都不自动更新：stable feed 上只分发正式 Mesacode 安装包，
  // 不向 Preview 渠道提供更新。
  void initAutoUpdater({
    enabled: MESACODE_PRODUCT_FLAVOR === "production",
  ...
  registerRemoteIpcHandlers({
    ...
    armsCustomContext: {
      ...
      armsEnv: mapMesacodeEnvToArmsRumEnv(desktopRuntimeEnv),
    },
  ...
  if (MESACODE_TELEMETRY_ENABLED && MESACODE_ARMS_RUM_ENDPOINT) {
    configureDesktopStabilityTelemetry({
      deviceMid,
      platform: process.platform,
      appVersion: MESACODE_VERSION,
  ...
  registerDesktopMesacodeDataSizeTelemetry({
    ...
    isMesacodeBusy: () => getRunningAgentSessionCount() > 0,
    logger,
    rootPath: getMesacodeDataRootDir(),
    stateFile: join(app.getPath("userData"), "mesacode-data-size-telemetry.json"),
  });
  registerDesktopNetworkTelemetry(logger);

  // 本地未打包 dev 构建（app.isPackaged === false）必须跳过远端强制升级 gate。
  // 原因：force-update gate 只看 MESACODE_ENV === "production"，但 dev 构建（如 dev:desktop:cua
  ...
  const forceUpdateGuardResult =
    MESACODE_PRODUCT_FLAVOR === "production" && !skipForceUpdateForLocalDevRuntime
