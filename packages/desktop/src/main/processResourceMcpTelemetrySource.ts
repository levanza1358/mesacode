...
interface StoredSample {
  sample: MesacodeMcpResourceSample;
...
function purgeExpired(now: number): void {
  for (const [key, entry] of latest) {
    if (now - entry.receivedAt > MESACODE_MCP_RESOURCE_SAMPLE_INTERVAL_MS * 2) latest.delete(key);
  }
}
...
export function ingestMcpResourceSamples(
  ...
  const parsed = mesacodeMcpResourceSamplesSchema.safeParse(raw);
  ...
  sample(context) {
    ...
    for (const [key, group] of groupSamples()) {
      recordExternalAppResourceSample({
        ...
        intervalMs: MESACODE_MCP_RESOURCE_SAMPLE_INTERVAL_MS,
