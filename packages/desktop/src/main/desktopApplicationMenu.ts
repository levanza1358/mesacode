...
import { readMesacodeStdioTapDevState } from "@mesacode/services/node";
...
import {
  HELP_TOGGLE_MESACODE_STDIO_TAP_MENU_ID,
  HELP_TOGGLE_DEV_TOOLS_MENU_ID,
} from "./desktopCommandHandlers.js";

const HELP_MESACODE_ENDPOINT_PRODUCTION_MENU_ID = "help.mesacode-endpoint.production";
...
export function updateMesacodeStdioTapDevMenuState() {
  const menu = Menu.getApplicationMenu();
  const item = menu?.getMenuItemById(HELP_TOGGLE_MESACODE_STDIO_TAP_MENU_ID);
  if (!item) {
    return;
  }
  const state = readMesacodeStdioTapDevState();
...
function buildApplicationMenuTemplate(options: {
  ...
  mesacodeEndpointSelection?: "production" | "test" | "custom";
  ...
  const stdioTapState = readMesacodeStdioTapDevState();
      ...
      ? [
          {
            ...
            submenu: [
              ...
              ...(MESACODE_PRODUCT_FLAVOR === "production"
                ? [
                    {
    ...
    },
    {
      ...
      submenu: [
        ...
        ...(isLocalDevelopmentRuntime && stdioTapState.visible
          ? [
              {
                id: HELP_TOGGLE_MESACODE_STDIO_TAP_MENU_ID,
                label: getLabel(desktopMenuMessageIds.helpToggleMesacodeStdioTap),
                type: "checkbox" as const,
                checked: stdioTapState.enabled,
                click: () =>
                  void options.executeDesktopCommand(DesktopCommandIds.ToggleMesacodeStdioTapDevProxy),
              },
              { type: "separator" as const },
            ]
          : []),
        ...(MESACODE_ENV === "test"
          ? [
              {
                label: getLabel(desktopMenuMessageIds.helpMesacodeEndpoint),
                submenu: [
                  {
                    id: HELP_MESACODE_ENDPOINT_PRODUCTION_MENU_ID,
                    label: getLabel(desktopMenuMessageIds.helpMesacodeEndpointProduction),
                    type: "radio" as const,
                    checked: (options.mesacodeEndpointSelection ?? "production") === "production",
                    click: () =>
                      void options.executeDesktopCommand(
                        DesktopCommandIds.SetMesacodeEndpointProduction,
                      ),
                  },
                  { type: "separator" as const },
                  {
                    label: getLabel(desktopMenuMessageIds.helpMesacodeEndpointCustom),
                    click: () =>
                      void options.executeDesktopCommand(DesktopCommandIds.SetMesacodeEndpointCustom),
                  },
                  {
                    label: getLabel(desktopMenuMessageIds.helpMesacodeEndpointReset),
                    click: () =>
                      void options.executeDesktopCommand(DesktopCommandIds.ResetMesacodeEndpoint),
                  },
                ],
              },
              { type: "separator" as const },
  ...
  Menu.setApplicationMenu(
    Menu.buildFromTemplate(
      buildApplicationMenuTemplate({
        ...
        mesacodeEndpointSelection: options.mesacodeEndpointSelection,
