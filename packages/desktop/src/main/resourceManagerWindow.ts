...
import {
  formatMesacodeAgentProcessName,
  formatMesacodeGpuProcessName,
  formatMesacodeHostProcessName,
  formatMesacodeMainProcessName,
  formatMesacodeRendererProcessName,
  formatMesacodeUtilityProcessName,
...
function collectElectronProcesses(): ResourceUsageProcess[] {
  ...
  for (const m of metrics) {
    if (m.type === "GPU") {
      ...
      push(baseProcess(m.pid, formatMesacodeGpuProcessName(), "gpu", cpuPercent, memoryBytes));
    }
  }
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    push(
      baseProcess(
        rendererPid,
        formatMesacodeRendererProcessName(win.getTitle() || `Window ${win.id}`),
        "renderer",
        cpuPercent,
        memoryBytes,
      ),
  ...
  for (const [label, child] of hostProcesses) {
    ...
    push(
      baseProcess(child.pid, formatMesacodeHostProcessName(label), "host", cpuPercent, memoryBytes),
    );
  }
  ...
  for (const m of metrics) {
    ...
    push(
      baseProcess(
        m.pid,
        m.type === "Utility"
          ? formatMesacodeUtilityProcessName(m.name || String(m.pid))
          : formatMesacodeUtilityProcessName(m.type, "process"),
        "utility",
        cpuPercent,
        memoryBytes,
      ),
...
function collectUnsampledAgentProcesses(sampledPids: Set<number>): ResourceUsageProcess[] {
  ...
  for (const processes of hostAgentProcesses.values()) {
    for (const agent of [...processes.values()].sort(
      ...
      rows.push({
        pid: agent.pid,
        name: formatMesacodeAgentProcessName(agent.provider, agent.workspacePath),
        category: "base",
        groupKey: "cli",
        groupLabel: "cli",
        cpuPercent: 0,
