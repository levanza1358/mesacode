...
import {
  ...
  resolveMesacodeEndpointOrigin,
} from "@mesacode/shared";
import { readMesacodeStdioTapDevState, setMesacodeStdioTapDevEnabled } from "@mesacode/services/node";
...
export const HELP_TOGGLE_MESACODE_STDIO_TAP_MENU_ID = "help.toggle-mesacode-stdio-tap";
const MESACODE_ENDPOINT_PROMPT_WIDTH = 460;
const MESACODE_ENDPOINT_PROMPT_HEIGHT = 210;
const CODING_PLAN_WEBVIEW_PARTITION = "persist:mesacode-coding-plan";
...
async function clearAllDataAndRelaunch(options: {
  ...
  const { response } = await dialog.showMessageBox({
    ...
    message: "确定要清除所有数据吗？",
    detail:
      "将删除 ~/.mesacode/v2（配置、凭据、日志）和浏览器缓存（localStorage）。操作不可恢复，清除后应用将自动重启。",
  });
  if (response !== 1) {
    return;
  }
  ...
  try {
    ...
    options.logger.info("[clear-all-data] deleted ~/.mesacode/v2");
  } catch (error) {
    options.logger.error("[clear-all-data] failed to delete ~/.mesacode/v2:", error);
  }
...
async function promptCustomMesacodeEndpoint(
  targetWindow: BrowserWindow | null | undefined,
  currentValue: string,
): Promise<string | undefined> {
  return showMesacodeEndpointPromptWindow({
    currentValue,
...
function buildMesacodeEndpointPromptHtml(currentValue: string): string {
  const value = escapeHtmlAttribute(currentValue);
  return `<!doctype html>
<html>
  <head>
    <meta charset="utf-8" />
    <title>Mesacode Endpoint</title>
    <style>
  ...
  <body>
    <form id="form">
      <label for="endpoint">Mesacode endpoint origin</label>
    ...
    <script>
      ...
      const submit = (value) => { document.title = "mesacode-endpoint-submit:" + encodeURIComponent(value); };
      document.getElementById("form").addEventListener("submit", (event) => {
        event.preventDefault();
        submit(input.value);
      });
      document.getElementById("cancel").addEventListener("click", () => {
        document.title = "mesacode-endpoint-cancel";
      });
      input.focus();
      input.select();
    </script>
...
function showMesacodeEndpointPromptWindow(options: {
  currentValue: string;
  ...
  return new Promise((resolve) => {
    ...
    const promptWindow = new BrowserWindow({
      width: MESACODE_ENDPOINT_PROMPT_WIDTH,
      height: MESACODE_ENDPOINT_PROMPT_HEIGHT,
      parent: options.parentWindow,
      modal: Boolean(options.parentWindow),
      resizable: false,
      minimizable: false,
      maximizable: false,
      title: "Mesacode Endpoint",
      webPreferences: {
    ...
    promptWindow.on("page-title-updated", (event, title) => {
      if (title === "mesacode-endpoint-cancel") {
        event.preventDefault();
        finish(undefined);
        return;
      }
      if (!title.startsWith("mesacode-endpoint-submit:")) {
        return;
      }
      event.preventDefault();
      finish(decodeURIComponent(title.slice("mesacode-endpoint-submit:".length)));
    });
    ...
    void promptWindow.loadURL(
      `data:text/html;charset=utf-8,${encodeURIComponent(
        buildMesacodeEndpointPromptHtml(options.currentValue),
      )}`,
    );
  });
}

async function setMesacodeEndpointOverride(options: {
  value: string | undefined;
  settingService: { update(patch: { mesacodeEndpointOrigin?: string | undefined }): Promise<void> };
  onMesacodeEndpointChanged: () => Promise<void> | void;
  logger: { warn: (...args: unknown[]) => void };
}) {
  if (MESACODE_ENV === "production") {
    return;
  }
  const normalized = options.value ? normalizeMesacodeEndpointOrigin(options.value) : undefined;
  await options.settingService.update({ mesacodeEndpointOrigin: normalized });
  await options.onMesacodeEndpointChanged();
}
...
function toggleMesacodeStdioTapDevProxy(options: {
  logger: { info: (...args: unknown[]) => void };
  updateMesacodeStdioTapDevMenuState: () => void;
}) {
  const current = readMesacodeStdioTapDevState();
  const next = setMesacodeStdioTapDevEnabled(!current.enabled);
  options.updateMesacodeStdioTapDevMenuState();
...
async function resolveCurrentMesacodeEndpointOrigin(settingService: {
  get(): Promise<{ mesacodeEndpointOrigin?: string }>;
  ...
  return resolveMesacodeEndpointOrigin({
    ...
    overrideOrigin: settings.mesacodeEndpointOrigin,
  });
}
...
export async function executeDesktopCommand(options: {
  ...
  settingService: {
    get(): Promise<Pick<AppSettings, "mesacodeEndpointOrigin" | "desktopZoomLevel">>;
    update(
      patch: Partial<Pick<AppSettings, "mesacodeEndpointOrigin" | "desktopZoomLevel">>,
    ): Promise<void>;
  };
  mesacodeEndpointEnvBaseOrigin?: string | null;
  credentialsDir: string;
  ...
  switch (options.command) {
    ...
    case DesktopCommandIds.OpenChangelog:
      await openChangelog(
        ...
        await resolveCurrentMesacodeEndpointOrigin({
          ...options.settingService,
          envBaseOrigin: options.mesacodeEndpointEnvBaseOrigin,
        }),
      );
      return;
    case DesktopCommandIds.CheckForUpdates:
      // 按产品身份而不是后端环境放行：生产后端的 Preview 同样没有更新器。
      if (MESACODE_PRODUCT_FLAVOR === "production") {
    ...
    case DesktopCommandIds.OpenResourceManager:
      openResourceManager();
      return;
    case DesktopCommandIds.ToggleMesacodeStdioTapDevProxy:
      toggleMesacodeStdioTapDevProxy({
        logger: options.logger,
        updateMesacodeStdioTapDevMenuState: options.updateMesacodeStdioTapDevMenuState,
      });
      return;
    case DesktopCommandIds.SetMesacodeEndpointProduction:
      await setMesacodeEndpointOverride({
        value: DEFAULT_MESACODE_ENDPOINT_ORIGIN,
        settingService: options.settingService,
        onMesacodeEndpointChanged: options.onMesacodeEndpointChanged,
        logger: options.logger,
      });
      return;
    case DesktopCommandIds.SetMesacodeEndpointTest:
      await setMesacodeEndpointOverride({
        value: options.mesacodeEndpointEnvBaseOrigin ?? resolveRuntimeMesacodeEndpointOrigin(),
        settingService: options.settingService,
        onMesacodeEndpointChanged: options.onMesacodeEndpointChanged,
        logger: options.logger,
      });
      return;
    case DesktopCommandIds.SetMesacodeEndpointCustom: {
      const current =
        (await options.settingService.get()).mesacodeEndpointOrigin ?? DEFAULT_MESACODE_ENDPOINT_ORIGIN;
      const value = await promptCustomMesacodeEndpoint(targetWindow, current);
      if (!value) {
        return;
      }
      try {
    ...
    case DesktopCommandIds.ResetMesacodeEndpoint:
      await setMesacodeEndpointOverride({
        value: undefined,
        settingService: options.settingService,
        onMesacodeEndpointChanged: options.onMesacodeEndpointChanged,
