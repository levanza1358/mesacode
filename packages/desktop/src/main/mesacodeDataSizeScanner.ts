...
export type MesacodeDataSizePartialReason = "file_limit" | "io_error" | "time_limit";

export type MesacodeDataSizeScanResult = {
  bytes: number;
  ...
  | { status: "partial"; partialReason: MesacodeDataSizePartialReason }
);

export interface MesacodeDataSizeScanRequest {
  rootPath: string;
  maxDurationMs: number;
  maxFiles: number;
}

interface MesacodeDataSizeScanOptions extends MesacodeDataSizeScanRequest {
  signal?: AbortSignal;
...
function createAbortError(): DOMException {
  return new DOMException("Mesacode data size scan aborted", "AbortError");
}
 ...
 */
export async function scanMesacodeDataDirectory(
  options: MesacodeDataSizeScanOptions,
  ...
  let terminalPartialReason: Exclude<MesacodeDataSizePartialReason, "io_error"> | null = null;
...
export function isMesacodeDataSizeScanResult(value: unknown): value is MesacodeDataSizeScanResult {
  if (typeof value !== "object" || value === null) {
    return false;
  }
  const candidate = value as Partial<MesacodeDataSizeScanResult>;
  const finiteNumbers = [
    candidate.bytes,
