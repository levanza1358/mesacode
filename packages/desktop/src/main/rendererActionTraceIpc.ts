...
import { PlatformChannels, type RendererActionTraceConfigV1 } from "@mesacode/shared";
...
function resolveRuntimeConfig(
  ...
  if (isTruthy(env.MESACODE_LOCAL_TTFT_ENABLED)) config = { ...config, localTtftEnabled: true };
  if (!isTruthy(env.MESACODE_RENDERER_ACTION_TRACE_ENABLED)) return config;
  return {
    ...config,
    enabled: true,
    sampleRatio: 1,
