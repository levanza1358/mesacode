...
function buildElectronManifestUrl(options: {
  ...
  const url = options.manifestUrl?.trim()
    ...
    : new URL(ELECTRON_MANIFEST_API_PATH, normalizeMesacodeEndpointOrigin(options.endpointOrigin));
...
export class ManifestUpdateProvider extends Provider<UpdateInfo> {
  ...
  private resolveBaseUrl = new URL(DEFAULT_MESACODE_ENDPOINT_ORIGIN);
  ...
  constructor(
    ...
    this.resolveBaseUrl = new URL(
      normalizeMesacodeEndpointOrigin(options.endpointOrigin ?? DEFAULT_MESACODE_ENDPOINT_ORIGIN),
    );
  }
  ...
  override async getLatestVersion(): Promise<UpdateInfo> {
    ...
    return {
      ...
      mesacodeReleaseChannel: releaseChannel,
    } as UpdateInfo;
  }
  ...
  private async resolveEndpointOrigin(): Promise<string> {
    const resolved =
      ...
      DEFAULT_MESACODE_ENDPOINT_ORIGIN;
    return normalizeMesacodeEndpointOrigin(resolved);
  }
