...
import { MESACODE_AGENT_LIFECYCLE_LOG_MARKER } from "@mesacode/shared/process-diagnostic";
import {
...
function hasProductExecutable(
  ...
  // Linux Preview 使用 mesacode-preview。两者都必须精确匹配，不能放宽成前缀以免混入 helper dump。
...
const armsRumEnv = mapMesacodeEnvToArmsRumEnv(desktopRuntimeEnv);
...
function startArmsRum(): Promise<void> {
  return armsRum
          ...
          (payload?.events ?? []).filter(
            (event) =>
              !(
                ...
                event.message.includes(MESACODE_AGENT_LIFECYCLE_LOG_MARKER)
              ),
          ),
    ...
    .then(() => {
      logger.info(`[arms] electron initialized env=${armsRumEnv} version=${MESACODE_VERSION}`);
    })
    .catch((error) => {
...
export const armsInitPromise: Promise<void> =
  MESACODE_TELEMETRY_ENABLED && MESACODE_ARMS_RUM_ENDPOINT ? startArmsRum() : Promise.resolve();
