...
const RECORDER_PORT_CHANNEL = "mesacode-browser-video-recorder:port";
...
export async function createElectronBrowserWebmRecorder(
  ...
  const recorderSession = session.fromPartition(`mesacode-browser-video-recorder-${randomUUID()}`);
