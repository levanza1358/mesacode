...
import type { SaveFileRequest, SaveFileResult } from "@mesacode/shared";
import { PlatformChannels } from "@mesacode/shared";
...
async function downloadRemoteFile(sourceUrl: URL, destinationPath: string): Promise<void> {
  ...
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "mesacode-save-file-"));
