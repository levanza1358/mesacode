...
function resolveScheduledAt(automation: MesacodeAutomation, now: number): number {
...
async function handleClaimed(automation: MesacodeAutomation, now: number): Promise<void> {
...
function postDispatchRequest(
  ...
  runId: string,
  fixedSelection?: MesacodeAutomationRun["modelSelection"],
): void {
 ...
 */
async function handleOffPeakClaimed(task: MesacodeOffPeakTask, now: number): Promise<void> {
...
async function settleDispatchResult(
  ...
  const trigger: MesacodeAutomationTrigger =
