...
type PatchedIpcRenderer = IpcRenderer & { __mesacodeArmsIpcPatched?: boolean };
...
type ArmsEventBridgeLike = {
  ...
  __mesacodeArmsBridgeForwardPatched?: boolean;
};
...
function patchArmsEventBridgeSend(bridge: ArmsEventBridgeLike): void {
  if (bridge.__mesacodeArmsBridgeForwardPatched) {
    return;
  }
  ...
  bridge.__mesacodeArmsBridgeForwardPatched = true;
}
...
export function installArmsRumBridgeIpcForward(ipc: IpcRenderer): void {
  ...
  if (patched.__mesacodeArmsIpcPatched) {
    return;
  }
  ...
  patched.__mesacodeArmsIpcPatched = true;
}
