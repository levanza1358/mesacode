...
import {
  ...
  type MesacodeAgentV4ClientMode,
...
import {
  ...
  type MesacodeAutomationRunOutcome,
  type ModelSelection,
...
import type {
  ...
  RemoteConnection,
} from "@mesacode/server/remote";
import type { RemoteTarget } from "@mesacode/shared";
...
import { createRemoteConnectionProgressContext } from "@mesacode/server/remote/remoteConnectionProgressContext.js";
...
const remoteMediaRangePreviewEnabled =
  process.env["MESACODE_REMOTE_MEDIA_RANGE_PREVIEW_ENABLED"] !== "0";
...
// 这里根据 main 传入的窗口 label 补一层稳定的 mesacode-* title，方便系统进程列表过滤。
process.title = formatMesacodeHostProcessName(process.env["MESACODE_PROCESS_LABEL"]);
...
function writeHostLog(level: HostLogLevel, ...args: unknown[]): void {
  const prefix = formatLogPrefix("mesacode-host", process.pid);
  const consoleFn =
...
function createFullFeedbackLogArchiveViaMain(
  ...
  return new Promise((resolve, reject) => {
    ...
    // 收集范围和“导出日志”不一致，缺少 mesacode-cli 日志、rollout/debug 以及导出链路脱敏。
...
async function resolveOffPeakFilesChanged(params: {
  mesacodeTaskService: IMesacodeTaskService;
  taskId: string;
  workspacePath: string;
  workspaceIdentity?: string;
}): Promise<number | undefined> {
  try {
    const snapshot = await params.mesacodeTaskService.getTaskSnapshot({
...
async function finalizeOffPeakRun(params: {
  ...
  workspacePath: string;
  workspaceIdentity?: string;
  outcome: MesacodeAutomationRunOutcome;
  error?: string;
}): Promise<void> {
  ...
  void params.mesacodeTaskService.setTaskUnread({
    taskId: params.taskId,
    workspacePath: params.workspacePath,
    ...(params.workspaceIdentity ? { workspaceIdentity: params.workspaceIdentity } : {}),
    unread: true,
...
function trackOffPeakRunOutcome(params: {
  ...
  const disposable = params.mesacodeTaskService.onDynamicTaskTerminalOutcome(params.taskId)(
    (result) => {
      if (result.inputId !== params.traceId) return;
      disposeOffPeakRunSubscription(key);
      void finalizeOffPeakRun({
        mesacodeTaskService: params.mesacodeTaskService,
...
async function dispatchOffPeakRun(request: OffPeakRunDispatchRequest): Promise<{
  ...
  const mesacodeTaskService = activeServices?.getOptional(IMesacodeTaskService);
  if (!mesacodeTaskService) {
    throw new Error("Mesacode task service is not initialized.");
  }
  const runtime = await ensureOffPeakRuntime();
  if (!runtime) {
  ...
  try {
    ...
    if (dispatchKind === "bound-first-run") {
      ...
      const [deletedIds, tasks] = await Promise.all([
        mesacodeTaskService.listDeletedTaskIds(workspaceScope),
        mesacodeTaskService.listTasks(workspaceScope),
      ]);
      assertBoundSessionDispatchable({
        sessionId: taskId,
      ...
      const task = await mesacodeTaskService.createTask({
        workspacePath: request.workspacePath,
        workspaceIdentity: request.workspaceIdentity,
        // 空 Session 沿用普通初始化；idle Selection 只在下方执行中注入。
        // 在此写入会让闲时轮结束后的普通消息继续使用无票的隐藏 Provider。
        mode: request.permissionMode as MesacodeTaskMode,
    ...
    await mesacodeTaskService.sendPrompt({
      taskId,
      traceId,
...
async function dispatchCronRun(request: CronRunDispatchRequest): Promise<{
  ...
  const mesacodeTaskService = targetServices.getOptional(IMesacodeTaskService);
  if (!mesacodeTaskService) {
    throw new Error("Mesacode task service is not initialized.");
  }
  ...
  try {
    const task = request.targetTaskId
      ...
      : await mesacodeTaskService.createTask({
          workspacePath: request.workspacePath,
          workspaceIdentity: request.workspaceIdentity,
...
function isMesacodeTaskMeta(value: unknown): value is MesacodeTaskMeta {
  return (
    typeof value === "object" &&
    value !== null &&
    "taskId" in value &&
...
function createReportingRemoteMesacodeTaskService<T extends object>(
  ...
  options?: {
    ...
    }) => Promise<{ content: string; attachments?: MesacodePromptAttachment[] }>;
  },
): T {
  ...
  function subscribeSessionMessageRequests(target: T, meta: MesacodeTaskMeta): void {
  ...
  async function mirrorRemotePrompt(
    ...
    // mesacodeSessionStore 的整条消费链词表都是 MesacodeStreamEvent。两个方案的评估结论：
    // a) relay 直接转发 v4 帧、手机端消费 v4 store（正解）：需要重做 relay stream-op
    //    协议 + 手机端 store；
    // b) 帧→MesacodeStreamEvent 薄映射：等价复刻 adapter mapSessionEvent，
    //    否决。
    // 结论：本镜像保持 legacy 源不动。
    ...
    try {
      ...
      // 远端 mesacode-server 没有 desktop realtime port；由窗口 Host 内的
  ...
  function finishWorkspaceTask(taskId: string, meta: MesacodeTaskMeta): void {
    workspaceProxyState.disposeTaskReadySubscription(taskId);
    workspaceTaskTracker.finish(taskId, meta);
  }

  function beginWorkspaceTask(target: T, taskId: string, meta: MesacodeTaskMeta): boolean {
    ...
    if (typeof onDynamicTaskReady !== "function") {
      workspaceTaskTracker.finish(taskId, meta);
      throw new Error("remote Mesacode task service does not expose onDynamicTaskReady");
    }
    const subscribe = onDynamicTaskReady.call(target, taskId);
    if (typeof subscribe !== "function") {
      workspaceTaskTracker.finish(taskId, meta);
      throw new Error("remote Mesacode task ready event is not subscribable");
    }
  ...
  // remote workspace 的 Mesacode Agent manager 跑在远端 server，desktop main 不能直接看到
  ...
  return new Proxy(service, {
    get(target, property, receiver) {
      ...
      return async (...args: unknown[]) => {
        let trackedTask: { taskId: string; meta: MesacodeTaskMeta; started: boolean } | undefined;
...
function warmUpMesacodeAgent(
  ...
  const mesacodeSessionService = services.getOptional(IMesacodeSessionService);
  if (!mesacodeSessionService) {
    return;
  }
  void mesacodeSessionService
    .initializeWorkspace({
      workspacePath,
    ...
    .then((result) => {
      if (!result.available) {
        if (result.reasonCode === "provider_not_ready") {
          logger.info(
            `Mesacode agent warmup waiting for provider/model (${reason}) workspace=${workspacePath}`,
          );
          return;
        }
        logger.warn(
          `Mesacode agent warmup unavailable (${reason}) workspace=${workspacePath} reason=${result.reason ?? "unknown"}`,
        );
        return;
      }
      ...
      logger.info(
        `Mesacode agent warmup ready (${reason}) workspace=${workspacePath} transport=${result.transportKind ?? "unknown"}`,
      );
    })
    .catch((error) => {
      logger.warn(`Mesacode agent warmup failed (${reason}) workspace=${workspacePath}:`, error);
    });
}
...
const hostResourceUsageResponder = createHostResourceUsageResponder({
  getAgentService: () => activeServices?.getOptional(IMesacodeAgentService),
...
async function createWindowRemoteConnectionHandle(params: {
  ...
  const services = createRemoteWorkspaceServiceCollection({
    ...
    createReportingRemoteMesacodeTaskService: (service) =>
      createReportingRemoteMesacodeTaskService(service, {
  ...
  let disposed = false;
  // 远端 workspace 的 CLI 与 MCP 样本走与本地同一条路径：远端 mesacode-server → 本地 Host → main。
...
>({
  ...
  releaseWorkspace: async (services, context) => {
    await services.get(IMesacodeTaskService).releaseWorkspacePreparation({
  ...
  resolveSource: (scope) => {
    ...
    if (remoteSession?.workspacePath && remoteSession.workspaceIdentity) {
      ...
      return {
        ...
        taskService: services.get(IMesacodeTaskService),
        agentService: services.getOptional(IMesacodeAgentService),
    ...
    const taskService = activeServices?.getOptional(IMesacodeTaskService);
    if (!taskService) {
      return null;
    }
    return {
      ...
      taskService,
      agentService: activeServices?.getOptional(IMesacodeAgentService),
      sourceAvailability: "online" as const,
    };
  },
});
  ...
  return new Proxy(base, {
    get(target, property, receiver) {
      if (property === "setTaskPinned") {
        return async (params: Parameters<IMesacodeTaskService["setTaskPinned"]>[0]) => {
      ...
      if (property === "archiveTask" || property === "unarchiveTask") {
        return async (
          params:
            | Parameters<IMesacodeTaskService["archiveTask"]>[0]
            | Parameters<IMesacodeTaskService["unarchiveTask"]>[0],
        ) => {
      ...
      if (property === "deleteTask") {
        return async (params: Parameters<IMesacodeTaskService["deleteTask"]>[0]) => {
          await route(params, { kind: "delete" });
        };
      }
      if (property === "deleteArchivedTasks") {
        return async (params: Parameters<IMesacodeTaskService["deleteArchivedTasks"]>[0]) => {
      ...
      if (property === "deleteArchivedTask") {
        return async (params: Parameters<IMesacodeTaskService["deleteArchivedTask"]>[0]) =>
      ...
      if (property === "setTaskUnread") {
        return async (params: Parameters<IMesacodeTaskService["setTaskUnread"]>[0]) => {
...
function exposeServicesOnMessagePort(
  ...
  services: ServiceCollection,
  deferInit: boolean,
  clientMode: MesacodeAgentV4ClientMode = "desktop-continuous",
  ...
  const agentService = services.getOptional(IMesacodeAgentService);
  const connectionScope = agentService
    ? createMesacodeAgentConnectionScope(agentService, {
  ...
  const taskService = services.getOptional(IMesacodeTaskService);
  if (taskService) {
    overrides.set(
      IMesacodeTaskService.channelName,
      createControllerRoutedTaskService(taskService, attachmentScope),
    );
  }
  if (connectionScope) {
    overrides.set(IMesacodeAgentService.channelName, connectionScope.service);
  }
...
parentPort.on("message", async (e: Electron.MessageEvent) => {
    ...
    void (async () => {
      try {
        const dispatchResult = await dispatchCronRun({
          ...msg,
          mode: msg.mode as MesacodeTaskMode | undefined,
        });
        parentPort.postMessage({
          type: HostResponseTypes.CronRunResult,
          runId: msg.runId,
  ...
  if (msg.type === HostMessageTypes.SessionMessageDeliver) {
    ...
    if (!mesacodeTaskService) {
      parentPort.postMessage({
        ...
        result: {
          error: "Mesacode task service is not initialized.",
  ...
  if (msg.type === HostMessageTypes.SessionMessageDeliveryResult) {
    const mesacodeTaskService = activeServices?.getOptional(IMesacodeTaskService);
    if (!mesacodeTaskService) {
      logger.warn("session message delivery result received before Mesacode task service initialized");
      return;
    }
    void mesacodeTaskService.sendSessionMessageDeliveryResult(msg.result).catch((error) => {
  ...
  if (msg.type === HostMessageTypes.InitLocal) {
    ...
    databaseStartup = createHostDatabaseStartup({
      ...
      initializeServices: async () => {
        ...
        const services = await initializeHostApiNetworkTransportOwner({
          ...
          establishOwner: () => {
            const initializedServices = createLocalServices({
              ...
              mesacodeAgentSpawnFallbackCwd: msg.agentSpawnFallbackCwd,
              mesacodeBuiltinProviderConfigFilePath: msg.mesacodeBuiltinProviderConfigFilePath,
              ...
              // browser-use：agent 的 interaction/browserExecute 经 mesacodeAgentService 转到这个 executor，
        ...
        const mesacodeTaskService = services.getOptional(IMesacodeTaskService);
        if (mesacodeTaskService) {
          const reportingMesacodeTaskService = createReportingRemoteMesacodeTaskService(
            mesacodeTaskService,
            {
              reportRunningPromptCount: false,
            },
          );
          services.register(IMesacodeTaskService, reportingMesacodeTaskService);
        }
...
async function setupRemoteConnection(
  ...
  const { createRemoteBackend, connectRemote, pickRemoteRuntimeEnv } =
    await import("@mesacode/server/remote");
  ...
  const connection = await connectRemote(backend, {
    ...
    // 这里显式透传编译期版本，避免漏导入后生成裸 MESACODE_VERSION 引用导致 SSH 初始化直接 ReferenceError。
    appVersion: MESACODE_VERSION,
    // 远端 mesacode-server/agent 是独立进程，不能继承 host 里的测试/生产 endpoint 选择。
