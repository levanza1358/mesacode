import type { IRemoteBackend, RemoteUploadOptions } from "@mesacode/server/remote";
import { quotePosixPathArg } from "@mesacode/server/remote/posixShell.js";
import type { TraceId, MesacodePromptAttachment } from "@mesacode/shared";
import { randomUUID } from "node:crypto";

const REMOTE_PROMPT_ATTACHMENT_ROOT = "~/.mesacode/tmp/prompt-attachments";
const REMOTE_PROMPT_ATTACHMENT_RELATIVE_ROOT = ".mesacode/tmp/prompt-attachments";
...
export async function materializeRemotePromptAttachments(
  ...
  const nextAttachments: MesacodePromptAttachment[] = [];
...
function getAttachmentLocalPath(attachment: MesacodePromptAttachment): string | undefined {
      ...
      return async (...args: unknown[]) => {
        ...
        const prepared = await options.materializePromptAttachments({
          ...
          attachments: Array.isArray((params as { attachments?: unknown }).attachments)
            ? (params as { attachments?: MesacodePromptAttachment[] }).attachments
            : undefined,
        });
