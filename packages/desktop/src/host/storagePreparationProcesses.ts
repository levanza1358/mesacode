...
import {
  ...
  mesacodeStoragePreparationFrameSchema,
  type DatabaseStartupState,
} from "@mesacode/shared";
import { resolveDefaultMesacodeAgentCommand } from "@mesacode/services/storage-startup";
...
export async function prepareSessionStorage(options: {
  ...
  const command = resolveDefaultMesacodeAgentCommand({
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    lines.on("line", (line) => {
      try {
        ...
        const frame = mesacodeStoragePreparationFrameSchema.parse(JSON.parse(line));
        clearTimeout(firstStateTimer);
