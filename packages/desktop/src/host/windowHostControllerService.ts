...
import { matchesTaskListMembershipKind } from "@mesacode/shared/mesacode-protocol-v4";
import type {
...
interface ResolvedWindowHostControllerSource {
  ...
  taskService?: IMesacodeTaskService;
...
function taskKey(task: Pick<MesacodeTaskMeta, "taskId" | "workspacePath" | "workspaceIdentity">) {
...
function compareItems(
  ...
  sortBy: MesacodeTaskListQuery["sortBy"],
): number {
...
function sessionOverlay(
  summary: import("@mesacode/shared/mesacode-protocol-v4").SessionSummary,
...
function liveStatusFromMeta(meta: MesacodeTaskMeta): WindowHostControllerTaskRow["liveStatus"] {
...
export function createWindowHostControllerRuntime(options: {
  createId: () => string;
  resolveSource: (scope: MesacodeTaskListWorkspaceScope) => ResolvedWindowHostControllerSource | null;
  onSourceError?: (
    scope: WindowHostControllerSourceScope,
  ...
  const sourceTaskServices = new Map<string, IMesacodeTaskService>();
  const sourceSnapshotTaskServices = new Map<string, IMesacodeTaskService>();
  const sourceEventSubscriptions = new Map<string, { dispose(): void }>();
  const sourceRefreshFlights = new Map<
    string,
    { taskService: IMesacodeTaskService; promise: Promise<void> }
  >();
  ...
  const sourceSessionObservers = new Map<
    string,
    { agentService: IMesacodeAgentService; observer: WindowHostSessionsIndexObserver }
  >();
  ...
  function resolveQuerySources(
    scopes: MesacodeTaskListWorkspaceScope[],
  ...
  return {
    ...
    async resolveTaskAddress(params: {
      ...
      attachmentScope?: import("@mesacode/shared").WindowHostAttachmentScope;
