import type { MesacodeAutomationRunOutcome, MesacodeAutomationTrigger } from "@mesacode/shared";
...
interface CronRunLifecycleRepo {
  ...
  markRunOutcome(runId: string, outcome: MesacodeAutomationRunOutcome, error?: string): Promise<void>;
...
export async function settleCronRunTerminalOutcome(
  params: CronRunLifecycleIdentity & {
    repo: CronRunLifecycleRepo;
    outcome: Exclude<MesacodeAutomationRunOutcome, "running">;
    error?: string;
    logWarn: LogWarn;
  },
): Promise<void> {
