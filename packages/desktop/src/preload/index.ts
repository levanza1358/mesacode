...
contextBridge.exposeInMainWorld("__MESACODE_DEVICE_ID__", parseDeviceIdFromArgs());
...
import {
  InternalChannels,
  PlatformChannels,
  formatMesacodeRendererProcessName,
...
if (shouldEnableE2ETestBridge(process.env)) {
  contextBridge.exposeInMainWorld("__mesacodeFinalArmsCustomEventsE2E", {
...
function updateRendererProcessTitle(): void {
  process.title = formatMesacodeRendererProcessName(document.title);
}
...
// 这里在 preload 阶段补上 mesacode-* title，便于按窗口角色筛选。
 ...
 */
contextBridge.exposeInMainWorld("mesacode", {
  connectRemote: (
    ...
    context?: {
      ...
      connectTrigger?: import("@mesacode/shared").RemoteWorkspaceConnectTrigger;
    },
  ) =>
  ...
  /** 打开 Mesacode Computer Use 完整权限引导 */
  ...
  reportLocalTtftBatch: (batch: import("@mesacode/shared").LocalTtftBatch): void =>
  ...
  /** 导出日志：打包 ~/.mesacode/v2 及外部 agent 日志为 zip 并在 Finder 中显示 */
  exportLogs: (): Promise<{
    success: boolean;
    path?: string;
    error?: string;
  ...
  importChromeBrowserData: (options?: import("@mesacode/shared").ChromeBrowserDataImportOptions) =>
  ...
  getMesacodeStdioTapDevState: (): Promise<MesacodeStdioTapDevState> =>
    ipcRenderer.invoke(PlatformChannels.GetMesacodeStdioTapDevState),
