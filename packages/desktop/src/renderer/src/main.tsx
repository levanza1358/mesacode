...
import { connectViaMessagePort, createMessagePortServiceConnection } from "@mesacode/client";
import {
  InternalChannels,
...
).__MESACODE_RENDERER_START__ = rendererStartedAt;
(window as Window & { __MESACODE_LAUNCH_MARKS__?: LaunchMarks | null }).__MESACODE_LAUNCH_MARKS__ =
  launchMarks;
...
function registerE2EStoreBridgesIfEnabled() {
  ...
  if (env.VITE_MESACODE_E2E_STORE_BRIDGE !== "1") {
    return;
  }

  void import("@mesacode/ui/e2e-store-bridge").then(({ registerE2EStoreBridges }) => {
...
const isLocalDevelopmentRuntime =
  (globalThis as typeof globalThis & { __MESACODE_LOCAL_DEVELOPMENT_RUNTIME__?: boolean })
    .__MESACODE_LOCAL_DEVELOPMENT_RUNTIME__ === true;
...
function StartupReadyNotifier() {
  useEffect(() => {
    ...
    (window as Window & { __MESACODE_REACT_COMMIT_AT__?: number }).__MESACODE_REACT_COMMIT_AT__ =
      Date.now();
    // HTML 启动壳的弹出动画结束时，React 首屏可能还没 commit，直接移除壳会露出空白。
    // 这里在 React commit 后通知 index.html，再由启动壳统一判断动画和 React ready 两个条件后退场。
    window.dispatchEvent(new Event("mesacode-react-startup-ready"));
  }, []);
...
function initializeBusinessRoot(port: MessagePort): void {
  ...
  syncAppTelemetryContext({
    bridge: {
      syncTelemetryContext: (context) => window.mesacode.syncTelemetryContext(context),
    },
