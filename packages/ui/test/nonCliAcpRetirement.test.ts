...
import { mesacodeTaskMetaSchema } from "../../shared/src/validation.js";
...
test("current task metadata is accepted without upgrading third-party Agent identities", () => {
  assert.equal(mesacodeTaskMetaSchema.parse(meta).provider, "glm");
  for (const provider of ["claude", "codex", "gemini", "opencode"]) {
    assert.equal(mesacodeTaskMetaSchema.safeParse({ ...meta, provider }).success, false, provider);
  }
});
...
test("Mesacode subagent identity wins over retired Codex nicknames", () => {
  const tool = {
    id: "tool-example",
    kind: "Agent",
    title: "Agent",
