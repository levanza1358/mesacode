...
import type { MesacodeGroupedTaskViewNode } from "@mesacode/services";
...
function getTopLevelNodeKey(node: MesacodeGroupedTaskViewNode | undefined, index: number): string {
  if (!node) {
...
function estimateTopLevelNodeSize(
  node: MesacodeGroupedTaskViewNode | undefined,
...
function VirtualizedGroupedTopLevelList({
  ...
  isGroupCollapsed: (groupId: string) => boolean;
  renderNode: (node: MesacodeGroupedTaskViewNode, index: number) => ReactNode;
}) {
