...
import type { EmbeddedBrowserOpenUrlRequest, IPlatformService } from "@mesacode/shared";
...
export function useAppPanels(options: {
  ...
  const { mesacodeAgentService, mesacodeSessionService } = useServices();
    ...
    (request: OpenScopedWorkflowArtifactSideTabRequest) => {
      ...
      void (async () => {
        try {
          ...
          // 这里只带 workspacePath。
          const result = await mesacodeAgentService.conversationWorkflowRunArtifactsV4({
            workspacePath: request.workspacePath,
