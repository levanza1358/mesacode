...
import { isRemoteWorkspaceIdentity, MESACODE_CUA_OFFICIAL_PLUGIN_ID } from "@mesacode/shared";
...
import { getVisibleTaskMetas, getWorkspaceState } from "@/store/mesacodeSessionStoreSelectors.js";
import { useOptionalTabStore } from "@/store/TabStoreProvider.js";

/** 与 mesacodeSessionStoreTaskSlice 的 isRunningStatus 同口径。 */
...
export function useCuaComposerEntry({
  ...
  // 失败都写）。只有失败操作的目标是 mesacode-cua 时才映射为本按钮的错误态，
  ...
  const workspaceSessionBusy = useMesacodeSessionStore((state) => {
  ...
  const view = useMemo(
    () =>
      resolveCuaComposerEntryView({
        ...
        pluginEnabled,
        pluginToggling: togglingPluginId === MESACODE_CUA_OFFICIAL_PLUGIN_ID,
        pluginError:
          Boolean(pluginStoreError) && lastFailedPluginId === MESACODE_CUA_OFFICIAL_PLUGIN_ID,
