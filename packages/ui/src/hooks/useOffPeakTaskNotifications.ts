...
import type { MesacodeOffPeakTaskStatus } from "@mesacode/shared";
import type { IOffPeakTaskService } from "@mesacode/services";
...
function notifiableStatus(status: MesacodeOffPeakTaskStatus): "completed" | "failed" | null {
...
export function useOffPeakTaskNotifications(params: {
  ...
  const seenStatusRef = useRef(new Map<string, MesacodeOffPeakTaskStatus>());
