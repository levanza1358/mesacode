...
import { APP_RUNTIME_PREFERENCES_CHANGED_BROADCAST_CHANNEL, type AppSettings } from "@mesacode/shared";
...
export function useSettings() {
  const { broadcastService, settingService, mesacodeAgentService } = useServices();
  ...
  const update = useCallback(
    async (patch: Partial<AppSettings>) => {
      ...
      if (
        ...
        const syncResults = await Promise.allSettled([
          mesacodeAgentService.syncAppRuntimePreferences(preferences),
        ]);
    ...
    [broadcastService, settingService, settingsStore, mesacodeAgentService, platform, refresh],
  );
