...
function createLastTurnChange(
  workspacePath: string,
  snapshotByPath: Map<string, MesacodePersistedFileSnapshot>,
  file: MesacodeTaskChangeSummary["files"][number],
): GitPaneFileChange {
...
function buildLastTurnDataset(options: {
  workspacePath: string;
  turnIndex: number | null;
  fileChange: MesacodePersistedFileChange | null;
  summary: MesacodeTaskChangeSummary | null;
}): GitPaneDataset {
  const snapshotByPath = new Map<string, MesacodePersistedFileSnapshot>(
  ...
  return {
    ...
    // 关键业务逻辑：上一轮更改继续优先复用 Mesacode Agent 已持久化的单轮文件快照，
