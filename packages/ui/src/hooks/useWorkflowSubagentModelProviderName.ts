 ...
 * （见 mesacodeSessionSettingsToConfigOptions），那种「名字」在这里就当作没有——屏幕上绝不出现
 * providerId（团队套餐的它是一个 UUID）。
 */
function workflowSubagentProviderNameLookup(
  configOptions: readonly MesacodeConfigOption[] | null | undefined,
...
export function useWorkflowSubagentModelProviderName(
  ...
  const configOptions = useMesacodeSessionStore((state) =>
    workspacePath === undefined
      ? undefined
      : selectWorkspaceMesacodeState(state, workspacePath, workspaceIdentity).configOptions,
  );
