...
interface PluginReferenceCatalogState {
  entries: MesacodePluginReferenceCatalogEntry[];
...
let sessionCatalogRequests = new WeakMap<
  object,
  Map<string, Promise<MesacodePluginsReferenceCatalogResult>>
>();

function releaseSessionCatalogRequest(
  services: object,
  serviceCache: Map<string, Promise<MesacodePluginsReferenceCatalogResult>>,
  requestKey: string,
  request: Promise<MesacodePluginsReferenceCatalogResult>,
): void {
...
export function usePluginReferenceCatalog(
  ...
  useEffect(() => {
    ...
    const subscription = services.mesacodeAgentService.onAgentRuntimeRestarted((event) => {
  ...
  useEffect(() => {
    ...
    let request: Promise<MesacodePluginsReferenceCatalogResult>;
    let requestServiceCache: Map<string, Promise<MesacodePluginsReferenceCatalogResult>> | undefined;
