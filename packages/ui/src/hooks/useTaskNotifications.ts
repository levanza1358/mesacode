...
import type { IPlatformService, TaskNotificationPayload } from "@mesacode/shared";
import type { ConversationSnapshot, SessionSummary } from "@mesacode/shared/mesacode-protocol-v4";
...
export function useWorkspaceTerminalTaskNotifications({
  ...
  useEffect(() => {
    ...
    const store = acquireSessionsIndex(registryScope, mesacodeAgentService);
    const syncState = () => {
      setIndexState({
        signature,
