...
import { mergeTaskWithOptimisticMeta } from "@/lib/mesacodeTaskMetaMerge.js";
import {
...
function collectViewWorkspaceScopes(
  ...
  const addTask = (task: MesacodeTaskMeta) => {
...
function mergeGroupedTaskViewWithOptimistic(params: {
  ...
  const optimisticTaskByKey = new Map<string, MesacodeTaskMeta>();
  const placementTaskByKey = new Map<string, MesacodeTaskMeta>();
  ...
  const groupMissingTasksByGroupId = new Map<string, MesacodeTaskMeta[]>();
  const topMissingTasks: MesacodeTaskMeta[] = [];
...
function collectGroupedViewTaskKeys(view: MesacodeGroupedTaskView): Set<string> {
...
function groupedNodeIdentityKey(node: MesacodeGroupedTaskView["nodes"][number]): string {
  if (node.type === "group") {
...
function areGroupedNodesEquivalent(
  previous: MesacodeGroupedTaskView["nodes"][number],
  next: MesacodeGroupedTaskView["nodes"][number],
): boolean {
...
function stabilizeGroupedView(
  previous: MesacodeGroupedTaskView,
  next: MesacodeGroupedTaskView,
): MesacodeGroupedTaskView {
...
function nodeToTopLevelRef(
  node: MesacodeGroupedTaskView["nodes"][number],
): MesacodeGroupedTaskViewTopLevelNodeRef {
  if (node.type === "group") {
...
function viewToOrderInput(params: { view: MesacodeGroupedTaskView }): MesacodeGroupedTaskViewOrderInput {
  return {
...
const groupedViewCacheBySignature = new Map<string, MesacodeGroupedTaskView>();

function readCachedGroupedView(signature: string): MesacodeGroupedTaskView | undefined {
  return groupedViewCacheBySignature.get(signature);
}

function writeCachedGroupedView(signature: string, view: MesacodeGroupedTaskView): void {
...
export function useGroupedTaskView(params: { workspaceTabs: WorkspaceTabState[] }) {
  ...
  const [view, setView] = useState<MesacodeGroupedTaskView>(
  ...
  const taskListVersionSignature = useMesacodeSessionStore((state) =>
    JSON.stringify(
      params.workspaceTabs.map((tab) => {
        ...
        const workspaceState = selectWorkspaceMesacodeState(
          state,
          tab.workspacePath,
  ...
  const clearPromotedGroupedDraftTask = useMesacodeSessionStore(
  ...
  const displayedViewRef = useRef<MesacodeGroupedTaskView>({ nodes: [] });
  ...
  const [remoteDataLoader] = useState(
    () =>
      new GroupedRemoteDataSingleFlight<{
        structure: MesacodeGroupedTaskViewStructure;
    ...
    try {
      ...
      const remoteData = await remoteDataLoader.load(remoteDataKey, async () => {
        const [structureResult, membershipResult] = await Promise.all([
          services.mesacodeTaskService.listGroupedTaskViewStructure({
            workspaceScopes: scopes,
          }),
          fetchTaskListMembershipSets({
            service: services.mesacodeTaskService,
  ...
  useEffect(() => {
    ...
    const promotedGroupTasks: MesacodeTaskMeta[] = [];
    const settledRootTasks: MesacodeTaskMeta[] = [];
    ...
    void services.mesacodeTaskService
  ...
  useEffect(() => {
    const disposables = scopes.map((scope) =>
      services.mesacodeTaskService.onDynamicWorkspaceEvent(scope)((event) => {
  ...
  }, [invalidateRemoteData, scopes, services.mesacodeTaskService]);
  ...
  const createGroup = useCallback(async (): Promise<MesacodeTaskGroup> => {
    setSaving(true);
    try {
      const group = await services.mesacodeTaskService.createTaskGroup();
  ...
  }, [invalidateRemoteData, refresh, services.mesacodeTaskService]);
  ...
  const renameGroup = useCallback(
    async (groupId: string, title: string) => {
      ...
      const optimisticView: MesacodeGroupedTaskView = {
      ...
      setSaving(true);
      try {
        const renamedGroup = await services.mesacodeTaskService.renameTaskGroup({
          groupId,
          title: nextTitle,
    ...
    [invalidateRemoteData, scopes, services.mesacodeTaskService, view],
  );

  const updateGroupColor = useCallback(
    async (groupId: string, color: MesacodeTaskGroupColor) => {
      const previousView = view;
      const groupNode = view.nodes.find(
        (node) => node.type === "group" && node.group.id === groupId,
      );
      ...
      setSaving(true);
      try {
        const updatedGroup = await services.mesacodeTaskService.updateTaskGroupColor({
          groupId,
          color,
          workspaceScopes: collectViewWorkspaceScopes(optimisticView),
        });
  ...
  const applyOrder = useCallback(
    async (nextView: MesacodeGroupedTaskView) => {
      const previousView = view;
      setView(nextView);
      setSaving(true);
      try {
        ...
        await services.mesacodeTaskService.applyGroupedTaskViewOrder(
          viewToOrderInput({
            view: nextView,
          }),
        );
    ...
    [invalidateRemoteData, refresh, services.mesacodeTaskService, view],
  );
  ...
  const ungroupGroup = useCallback(
    async (groupId: string) => {
      ...
      const nextView: MesacodeGroupedTaskView = {
      ...
      try {
        ...
        await services.mesacodeTaskService.deleteTaskGroup({
          groupId,
    ...
    [applyOrder, invalidateRemoteData, refresh, scopes, services.mesacodeTaskService, view],
  );
