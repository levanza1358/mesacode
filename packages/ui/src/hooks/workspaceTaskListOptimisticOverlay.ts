...
export function mergeWorkspaceTaskListItemsWithOptimistic(params: {
  items: readonly MesacodeTaskMeta[];
  optimisticTasks: readonly MesacodeTaskMeta[];
  activeTaskId: string | null;
  ...
  const sortedItems = [...taskByKey.values()].sort((left, right) =>
    compareMesacodeTaskListItems(left, right, params.sortBy),
  );
...
export function useWorkspaceTaskOptimisticOverlayByWorkspaceKey(
  ...
  const optimisticTaskListSignature = useMesacodeSessionStore((state) =>
    JSON.stringify(
  ...
  return useMemo(() => {
    const state = useMesacodeSessionStore.getState();
