...
import { mesacodeSessionSnapshotToTaskMeta } from "@/lib/mesacodeSessionProjection.js";

function resolveImmediateActiveTaskSnapshotMeta(
  previousSnapshotMeta: MesacodeTaskMeta | null,
  taskId: string | null,
  taskMetaFromLists?: MesacodeTaskMeta | null,
) {
...
export function useActiveTaskSnapshotMeta(
  ...
  const [snapshotMeta, setSnapshotMeta] = useState<MesacodeTaskMeta | null>(null);
  ...
  useEffect(() => {
    ...
    void mesacodeSessionService
      // active header 只需要 session meta/标题兜底，走 Mesacode Protocol 的轻量读取，
      ...
      .then((snapshot) => {
        ...
        setSnapshotMeta(mesacodeSessionSnapshotToTaskMeta(snapshot));
      })
      .catch(() => {
        if (cancelled) {
          return;
  ...
  }, [mesacodeSessionService, taskId, taskMetaFromLists, workspaceIdentity, workspacePath]);
