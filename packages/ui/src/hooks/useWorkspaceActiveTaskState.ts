...
function areResolvedTaskMetasEqual(left: MesacodeTaskMeta | null, right: MesacodeTaskMeta | null) {
  if (left === right) {
    return true;
  }
...
function useStableResolvedActiveTaskMeta(taskMeta: MesacodeTaskMeta | null) {
  const stableTaskMetaRef = useRef<MesacodeTaskMeta | null>(null);
...
export function useWorkspaceActiveTaskState({
  ...
  const workspaceState = useMesacodeSessionStore((state) =>
    selectWorkspaceMesacodeState(state, workspaceAbsPath, workspaceIdentity),
  );
  ...
  const activeTaskMeta = useMemo(() => {
    ...
    // App 以前依赖 mesacodeTaskMetaMerge 同时拉普通列表和 pinned 列表，只是为了给当前激活 task
