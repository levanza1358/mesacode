...
function supportsTaskNativeSessionLogFile(_provider: MesacodeProvider | null | undefined): boolean {
 ...
 *
 * 路径规则统一通过 mesacodeTaskService 解析，避免 UI 层猜 provider 自己的目录结构。
 */
export function useTaskNativeSessionLogFile(
  workspacePath: string,
  taskId: string | null,
  providerHint?: MesacodeProvider | null,
  workspaceIdentity?: string,
  options: { enabled?: boolean } = {},
) {
  const mesacodeTaskService = useMesacodeTaskService(workspacePath, undefined, workspaceIdentity);
  ...
  }, [enabled, mesacodeTaskService, providerHint, taskId, workspaceIdentity, workspacePath]);
