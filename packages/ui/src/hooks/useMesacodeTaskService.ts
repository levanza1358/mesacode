...
import type { IMesacodeTaskService } from "@mesacode/services";
import type { MesacodeTaskSnapshot } from "@mesacode/shared";
import { uiMemoryDiagnosticsRegistry } from "@/lib/memoryDiagnostics.js";

type GetTaskSnapshotParams = Parameters<IMesacodeTaskService["getTaskSnapshot"]>[0];
type GetTaskSnapshotResult = Promise<MesacodeTaskSnapshot | null>;
type GetTaskSnapshotWithEtagParams = Parameters<IMesacodeTaskService["getTaskSnapshotWithEtag"]>[0];

const mesacodeTaskServiceProxyCache = new WeakMap<IMesacodeTaskService, IMesacodeTaskService>();
...
const snapshotCacheByService = new WeakMap<
  IMesacodeTaskService,
  Map<string, { etag: string; snapshot: MesacodeTaskSnapshot }>
>();
const SNAPSHOT_CACHE_STORAGE_KEY = "mesacode-task-snapshot-cache:v1";
...
let latestSnapshotCache: Map<string, { etag: string; snapshot: MesacodeTaskSnapshot }> | undefined;
...
function getOrCreateSnapshotCacheMap(
  ...
  const created = new Map<string, { etag: string; snapshot: MesacodeTaskSnapshot }>();
...
function writePersistedSnapshotEntry(key: string, etag: string, snapshot: MesacodeTaskSnapshot): void {
  ensurePersistedSnapshotCacheLoaded();
...
function createMesacodeTaskServiceProxy(service: IMesacodeTaskService): IMesacodeTaskService {
...
/** 获取 Mesacode task wrapper 服务实例 */
export function useMesacodeTaskService(
  workspacePath?: string,
  preferredRemoteSessionId?: string | null,
  workspaceIdentity?: string | null,
): IMesacodeTaskService {
  // Mesacode task 服务按 workspace 身份解析，保证所有 task RPC 都落到对应的 host。
  const services = workspacePath
    ? useWorkspaceServices(workspacePath, preferredRemoteSessionId, workspaceIdentity)
    : useServices();
  const rawService = services.mesacodeTaskService;
  if (!rawService || typeof rawService !== "object") {
    return rawService;
  }
  const cachedProxy = mesacodeTaskServiceProxyCache.get(rawService);
  if (cachedProxy) {
    return cachedProxy;
  }
  const nextProxy = createMesacodeTaskServiceProxy(rawService);
  mesacodeTaskServiceProxyCache.set(rawService, nextProxy);
  return nextProxy;
}
