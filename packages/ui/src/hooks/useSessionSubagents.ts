...
import type { MesacodeSessionEndedSubagent, MesacodeSessionSubagentsResult } from "@mesacode/shared";
...
interface SessionSubagentsViewState {
  revision: number;
  ended: MesacodeSessionSubagentsResult["ended"];
  error: string | null;
  loading: boolean;
}
...
function mergeEndedSubagentPages(
  current: readonly MesacodeSessionEndedSubagent[],
  incoming: readonly MesacodeSessionEndedSubagent[],
): MesacodeSessionEndedSubagent[] {
...
export function useSessionSubagents(options: {
  ...
  const mesacodeAgentService = services.mesacodeAgentService;
  ...
  const requestPage = useCallback(
    (endedLimit: number, endedCursor?: string): Promise<MesacodeSessionSubagentsResult> => {
      if (!options.sessionId) return Promise.reject(new Error("session_id_missing"));
      if (typeof mesacodeAgentService?.listSessionSubagents !== "function") {
      ...
      return mesacodeAgentService.listSessionSubagents({
