...
export function useWorkspaceTaskNavigation({
  ...
  const setActiveTaskId = useMesacodeSessionStore((s) => s.setActiveTaskId);
  const taskNavHistory = useMesacodeSessionStore((s) => s.taskNavHistory);
  const taskNavPushAutomations = useMesacodeSessionStore((s) => s.taskNavPushAutomations);
  const taskNavPushPluginStore = useMesacodeSessionStore((s) => s.taskNavPushPluginStore);
  const taskNavGoBack = useMesacodeSessionStore((s) => s.taskNavGoBack);
  const taskNavGoForward = useMesacodeSessionStore((s) => s.taskNavGoForward);
  const removeTaskFromNavHistory = useMesacodeSessionStore((s) => s.removeTaskFromNavHistory);
  ...
  const handleSelectTask = useCallback(
    (
      ...
      const targetWorkspaceState = useMesacodeSessionStore
        .getState()
      ...
      if (shouldClearUnread) {
        ...
        if (!targetServices) {
          ...
          void targetServices.mesacodeTaskService
            .setTaskUnread({
              ...targetTask,
              unread: false,
              expectedUnreadAt,
  ...
  const handleTaskNavBack = useCallback(() => {
    const currentWorkspaceState = useMesacodeSessionStore
      .getState()
    ...
    while (entry) {
      ...
      const navWorkspaceState = useMesacodeSessionStore
        .getState()
  ...
  const currentWorkspaceState = useMesacodeSessionStore.getState().getWorkspaceState(workspaceAbsPath);
