...
export function useWorkspaceSessionReload({
  ...
  const handleReloadSession = useCallback(
    async (options?: { resumeTaskId?: string | null; provider?: MesacodeProvider | null }) => {
      ...
      const mesacodeSessionStore = useMesacodeSessionStore.getState();
      const latestWorkspaceState = mesacodeSessionStore.getWorkspaceState(
        workspaceAbsPath,
        workspaceIdentity,
      );
      const actionScope = resolveWorkspaceModelConfigSyncScope(latestWorkspaceState);
      const provider: MesacodeProvider = options?.provider ?? actionScope.provider;
      const resumeTaskId =
      ...
      mesacodeSessionStore.setWorkspaceInitState(
        workspaceAbsPath,
        "initializing",
        null,
        workspaceIdentity,
      ...
      if (shouldPrepareWorkspace) {
        mesacodeSessionStore.setConfigOptionsStatus(workspaceAbsPath, "loading", workspaceIdentity);
        // 草稿态点击 reload 后若不清空旧错误，输入区会继续显示上一轮失败提示，
        // 用户会误判本次重建仍失败。这里在新一轮重建开始时先清空草稿错误。
        mesacodeSessionStore.setDraftError(workspaceAbsPath, null, workspaceIdentity);
        mesacodeSessionStore.setTaskState(workspaceAbsPath, "idle", null, workspaceIdentity);
      }

      try {
        await services.mesacodeTaskService.restartWorkspaceProcess({
        ...
        if (shouldPrepareWorkspace) {
          const prepareResult = await prepareWorkspaceWithMesacodeSessionService({
            ...
            workspaceIdentity,
            provider,
            mesacodeSessionService: services.mesacodeSessionService,
          });

          const latestAfterPrepare = mesacodeSessionStore.getWorkspaceState(
            workspaceAbsPath,
            workspaceIdentity,
          );
          if (latestAfterPrepare.selectedProvider === provider) {
            const latestAfterResolve = mesacodeSessionStore.getWorkspaceState(
              workspaceAbsPath,
              workspaceIdentity,
            );
            if (latestAfterResolve.selectedProvider === provider) {
              mesacodeSessionStore.setConfigOptions(
                workspaceAbsPath,
                prepareResult.configOptions ?? [],
                workspaceIdentity,
              );
              mesacodeSessionStore.setConfigOptionsStatus(
                workspaceAbsPath,
                "ready",
                workspaceIdentity,
              );
              mesacodeSessionStore.setSlashCommands(
                workspaceAbsPath,
        ...
        mesacodeSessionStore.setWorkspaceInitAttempts(workspaceAbsPath, 0, workspaceIdentity);
        mesacodeSessionStore.setWorkspaceInitState(workspaceAbsPath, "ready", null, workspaceIdentity);
        ...
        if (shouldPrepareWorkspace) {
          mesacodeSessionStore.setConfigOptionsStatus(workspaceAbsPath, "error", workspaceIdentity);
          // 草稿态下 reload 前会先清空旧错误；如果失败后不回填 draftError，
          // 聊天区只剩 toast，用户看不到可重试的详细报错。这里统一回填标准化错误到输入区。
          mesacodeSessionStore.setDraftError(workspaceAbsPath, reloadDraftError, workspaceIdentity);
        }
    ...
    [
      ...
      services.mesacodeSessionService,
      workspaceAbsPath,
      workspaceIdentity,
    ],
  );
