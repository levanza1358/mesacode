...
import { getDefaultWorkspaceState } from "@/store/mesacodeSessionStoreTypes.js";
...
function sortTasksByUpdatedAt(tasks: readonly MesacodeTaskMeta[]): MesacodeTaskMeta[] {
...
function arePermissionOptionsEqual(
  left: MesacodePermissionRequest["options"],
  right: MesacodePermissionRequest["options"],
): boolean {
...
function arePermissionRequestsEquivalent(
  left: MesacodePermissionRequest,
  right: MesacodePermissionRequest,
): boolean {
  if (left === right) {
    return true;
  }
          ...
          (current) => ({
            ...
            taskUiByTaskId: {
              ...
              [taskId]: {
                ...
                // 完整错误对象之前只放在 ChatView/useMesacodeChat 的本地 state。
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          (current) => {
            ...
            return {
              ...current,
              selectedProvider: normalizeAgentProviderToMesacodeAgent(params.provider),
