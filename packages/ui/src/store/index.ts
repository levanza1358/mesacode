...
import type { IBroadcastService, BroadcastMessage } from "@mesacode/services";
import type { OAuthProviderId, UserInfo } from "@mesacode/shared";
import type { CodingPlanResetType } from "@mesacode/shared";
...
const CODE_PREVIEW_SETTINGS_KEY = "mesacode-code-preview-settings";
const PERFORMANCE_MODE_STORAGE_KEY = "mesacode-performance-mode";
...
export interface MesacodeState {
 ...
 */
export function createMesacodeStore(
  ...
  const useStore = create<MesacodeState>()((set, get) => ({
    ...
    locale: readSafeLocalStorage("mesacode-locale") || "zh-CN",
    setLocale: (locale: string) => {
      writeSafeLocalStorage("mesacode-locale", locale);
      set({ locale });
    },
...
export type MesacodeStore = ReturnType<typeof createMesacodeStore>;
