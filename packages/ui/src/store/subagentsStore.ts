...
import type { ISubagentsService } from "@mesacode/services";
...
interface SubagentsStoreState {
  ...
  loadedProvider: MesacodeProvider | null;
  agents: AgentSummary[];
  ...
  initialize: (
    ...
    providerOrSubagentsService: MesacodeProvider | ISubagentsService,
...
export const useSubagentsStore = create<SubagentsStoreState>((set, get) => ({
  ...
  async initialize(
    ...
    const provider = normalizeAgentProviderToMesacodeAgent(
      hasProvider ? providerOrSubagentsService : MESACODE_AGENT_PROVIDER,
    );
  ...
  async refresh(subagentsService: ISubagentsService, workspaceIdentity?: string) {
    ...
    const provider = normalizeAgentProviderToMesacodeAgent(get().provider);
  ...
  async createAgent(
    ...
    try {
      const result = await subagentsService.createAgent({
        config,
        provider: normalizeAgentProviderToMesacodeAgent(provider),
      });
      await get().refresh(subagentsService, workspaceIdentityFromState);
      return result.agent;
    } catch (error) {
...
export async function refreshLoadedSubagentsStoreForWorkspace(params: {
  ...
  const contextKey = getSubagentsContextKey(workspacePath, MESACODE_AGENT_PROVIDER, workspaceIdentity);
