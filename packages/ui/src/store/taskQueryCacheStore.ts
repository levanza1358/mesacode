...
import type { MesacodeTaskListItem } from "@mesacode/services";
...
import { mergeTaskMetaCandidates } from "@/lib/mesacodeTaskMetaMerge.js";
import { compareMesacodeTaskListItems } from "@/lib/taskListOrdering.js";
...
interface TaskQueryCacheState {
  ...
  upsertTaskMeta: (task: MesacodeTaskMeta) => void;
  updateTaskMetaPreservingMembership: (task: MesacodeTaskMeta) => void;
  applyTaskMutation: (params: {
    previousTask: MesacodeTaskMeta;
    nextTask: MesacodeTaskMeta;
    previousState: TaskListMembershipState;
    nextState: TaskListMembershipState;
  }) => void;
  setTaskUnreadOverlay: (
    task: Pick<MesacodeTaskMeta, "taskId" | "workspacePath" | "workspaceIdentity">,
...
type CachedTaskListItem = MesacodeTaskListItem & { searchSnippets?: string[] };
...
function matchesTaskSearch(descriptor: TaskListCacheDescriptor, task: MesacodeTaskMeta): boolean {
...
function sortTaskKeysByDescriptor(params: {
  taskKeys: TaskEntityKey[];
  taskMetaByEntityKey: Record<TaskEntityKey, MesacodeTaskListItem>;
  descriptor: TaskListCacheDescriptor;
}): TaskEntityKey[] {
  ...
  return uniqueTaskKeys.sort((leftKey, rightKey) => {
    ...
    return compareMesacodeTaskListItems(leftTask, rightTask, params.descriptor.sortBy);
  });
}
