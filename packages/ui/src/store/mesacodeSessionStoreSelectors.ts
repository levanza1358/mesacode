/**
 * Mesacode Session Store 选择器与内部辅助函数
 *
 * 从 mesacodeSessionStore.ts 拆分出来，包含 workspace 状态读取/更新辅助函数，
 * 以及所有按 task 粒度的只读访问器和独立选择器。
 */
import type { MesacodeTaskRuntimeStatus, MesacodeTaskMeta } from "@mesacode/shared";
...
import {
  ...
  type MesacodeSessionStoreState,
  type WorkspaceInitState,
  type TaskRuntimeState,
  type WorkspaceMesacodeUIState,
} from "./mesacodeSessionStoreTypes.js";
...
function collectIdentityTaskIds(
  baseState: WorkspaceMesacodeUIState,
  workspaceIdentity: string,
): Set<string> {
...
function createIdentityWorkspaceStateSeed(
  baseState: WorkspaceMesacodeUIState | undefined,
  workspaceIdentity?: string,
...
export function updateWorkspaceState(
  state: MesacodeSessionStoreState,
  workspacePath: string,
  updater: (current: WorkspaceMesacodeUIState) => WorkspaceMesacodeUIState,
  workspaceIdentity?: string,
  ...
  if (nextWorkspaceState === current) {
    // 单 Mesacode Agent 迁移后旧 provider 选择都会归一为 glm，很多调用实际不会改变状态。
...
export function getTaskRuntimeState(
  workspaceState: WorkspaceMesacodeUIState,
  taskId: string,
): TaskRuntimeState {
...
interface WorkspaceDisplayedTaskState {
  taskStatus: MesacodeTaskRuntimeStatus;
...
export function getTaskUiState(workspaceState: WorkspaceMesacodeUIState, taskId: string) {
...
export function getTaskMeta(
  workspaceState:
    | WorkspaceMesacodeUIState
    | Partial<Pick<WorkspaceMesacodeUIState, "optimisticTaskListByTaskId" | "taskListCache">>,
  taskId: string,
...
export function getVisibleTaskMetas(
  ...
  const taskById = new Map<string, MesacodeTaskMeta>();
...
export function getTaskUnreadIndicator(
  workspaceState:
    ...
    | Partial<
      ...
      >,
  taskId: string,
  fallbackTask?: Pick<MesacodeTaskMeta, "unreadAt">,
): boolean {
...
export function selectWorkspaceMesacodeState(
  state: MesacodeSessionStoreState,
  workspacePath: string,
  workspaceIdentity?: string,
) {
...
export function getWorkspaceInitState(
  workspaceState: Pick<WorkspaceMesacodeUIState, "workspaceInit">,
): WorkspaceInitState {
