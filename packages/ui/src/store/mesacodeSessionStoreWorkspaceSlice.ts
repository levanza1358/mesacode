...
import {
  ...
  normalizeAgentProviderToMesacodeAgent,
...
} from "@/store/mesacodeSessionStoreTypes.js";
import {
  getTaskMeta,
  getWorkspaceState,
  getWorkspaceInitState,
  updateWorkspaceState,
} from "@/store/mesacodeSessionStoreSelectors.js";

type SetFn = (
  partial:
    | MesacodeSessionStoreState
    | Partial<MesacodeSessionStoreState>
    | ((state: MesacodeSessionStoreState) => MesacodeSessionStoreState | Partial<MesacodeSessionStoreState>),
) => void;
...
function normalizeThoughtLevelConfigOption(option: MesacodeConfigOption): MesacodeConfigOption {
  if (
    option.type !== "select" ||
...
function normalizeConfigOptions(options: MesacodeConfigOption[]): MesacodeConfigOption[] {
  return options.map(normalizeThoughtLevelConfigOption);
}

function cloneConfigOptions(options: readonly MesacodeConfigOption[]): MesacodeConfigOption[] {
...
function isModeConfigOption(option: MesacodeConfigOption): boolean {
  return option.category === "mode" && option.type === "select";
}

function createFallbackModeOption(params: {
  currentModeId: string;
  options?: NonNullable<MesacodeConfigOption["options"]>;
...
function replaceModeConfigOption(
  currentOptions: MesacodeConfigOption[] | null,
  nextModeOption: MesacodeConfigOption | null,
): MesacodeConfigOption[] | null {
...
function updateActiveTaskConfigOptions(
  current: WorkspaceMesacodeUIState,
  updater: (options: MesacodeConfigOption[] | null) => MesacodeConfigOption[] | null,
): Pick<WorkspaceMesacodeUIState, "taskConfigOptionsByTaskId"> | null {
...
function resolveActiveTaskConfigOptionsOnSwitch(
  current: WorkspaceMesacodeUIState,
  taskId: string,
): { configOptions: MesacodeConfigOption[]; status: ConfigOptionsStatus } | null {
  ...
  return {
    ...
    promoteGroupedDraftTask: (
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          workspacePath,
          (current) => {
            const optimisticTask: MesacodeTaskMeta = {
              taskId,
              traceId: `session-${taskId}` as MesacodeTaskMeta["traceId"],
              title: "",
              workspacePath,
    ...
    startDraft: (
      ...
      const normalizedProvider = provider
        ? normalizeAgentProviderToMesacodeAgent(provider)
        : undefined;
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          workspacePath,
          (current) => {
            const nextSelectedProvider = normalizeAgentProviderToMesacodeAgent(
    ...
    bindRuntimeProvider: (
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          (current) => ({
            ...current,
            selectedProvider: normalizeAgentProviderToMesacodeAgent(provider),
          }),
          workspaceIdentity,
        ),
      ),
