...
import type { IBroadcastService } from "@mesacode/services";
import { createMesacodeStore, type MesacodeStore, type MesacodeState } from "./index.js";

// 导出 Context 供测试直接注入已构造的 store 实例（如跨窗口广播抑制用例）。
const StoreContext = createContext<MesacodeStore | null>(null);
...
export function StoreProvider({
  ...
  const storeRef = useRef<MesacodeStore | null>(null);
  if (!storeRef.current) {
    storeRef.current = createMesacodeStore(broadcastService, {
 ...
 */
export function useMesacodeStore<T>(selector: (state: MesacodeState) => T): T {
  const store = useContext(StoreContext);
  if (!store) {
    throw new Error("useMesacodeStore 必须在 StoreProvider 内使用");
  }
  return useStore(store, selector);
}

/**
 * 带默认值的容错版 useMesacodeStore（store 耦合剥离配套）。
 *
 ...
 */
export function useMesacodeStoreWithDefault<T>(
  selector: (state: MesacodeState) => T,
  defaultValue: T,
): T {
  const store = useContext(StoreContext);
