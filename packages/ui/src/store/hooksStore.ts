...
import type { Hook, HookConfig } from "@mesacode/shared";
import type { IHooksService } from "@mesacode/services";
...
function buildMesacodeHookLocation(
...
function hookFromConfig(config: HookConfig, workspacePath: string): Hook {
  return {
    ...
    location: buildMesacodeHookLocation(workspacePath, config.storageLevel),
  };
}
...
export const useHooksStore = create<HooksStoreState>((set, get) => ({
  ...
  importHook: async (id, hooksService) => {
    ...
    if (!source || source.location?.source === "mesacode") throw new Error("Hook is not importable");
    const imported: Hook = {
      ...source,
      id: `hook-${crypto.randomUUID()}`,
      enabled: true,
      location: buildMesacodeHookLocation(workspacePath, source.location?.scope ?? "user"),
    };
