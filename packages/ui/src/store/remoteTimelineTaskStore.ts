...
import type { MesacodeTaskListSortBy, IMesacodeTaskService } from "@mesacode/services";
...
interface RemoteTimelineTaskState {
  itemsByWorkspaceKey: Record<string, MesacodeTaskMeta[]>;
  ...
  refreshWorkspace: (params: {
    ...
    mesacodeTaskService: Pick<IMesacodeTaskService, "listTaskList">;
    sortBy: MesacodeTaskListSortBy;
    limit?: number;
  }) => Promise<void>;
  upsertTask: (task: MesacodeTaskMeta, sortBy?: MesacodeTaskListSortBy) => void;
...
function sortTimelineTasks(
  ...
  sortBy: MesacodeTaskListSortBy = "updated",
): MesacodeTaskMeta[] {
...
export const useRemoteTimelineTaskStore = create<RemoteTimelineTaskState>()((set) => ({
  ...
  errorByWorkspaceKey: {},
  async refreshWorkspace({ workspacePath, workspaceIdentity, mesacodeTaskService, sortBy, limit }) {
    ...
    try {
      const result = await mesacodeTaskService.listTaskList({
        kind: "timeline",
        workspaceScopes: [
          {
            workspacePath,
...
export async function refreshRemoteTimelineTasksForSession({
  ...
  await useRemoteTimelineTaskStore.getState().refreshWorkspace({
    workspacePath,
    workspaceIdentity,
    mesacodeTaskService: session.services.mesacodeTaskService,
    sortBy,
    limit,
  });
}
