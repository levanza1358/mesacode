...
declare global {
  interface Window {
    __mesacodeComposerAttachmentUploadStoreE2E?: typeof useComposerAttachmentUploadStore;
    __mesacodeCurrentComposerAttachmentScopeKeyE2E?: string;
  }
}

if (shouldExposeE2EStoreBridge()) {
  // E2E 只暴露当前唯一附件 owner，供 scope 切换用例准备状态；不再把附件塞回旧 Session Store。
  window.__mesacodeComposerAttachmentUploadStoreE2E = useComposerAttachmentUploadStore;
}

export function exposeComposerAttachmentScopeKeyForE2E(scopeKey: string): void {
  if (window.__mesacodeComposerAttachmentUploadStoreE2E) {
    window.__mesacodeCurrentComposerAttachmentScopeKeyE2E = scopeKey;
  }
}
