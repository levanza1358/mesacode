...
export function getSubagentsContextKey(
  ...
  return `${workspaceIdentity?.trim() || workspacePath}::${normalizeAgentProviderToMesacodeAgent(provider)}`;
}
...
async function loadContext(
  ...
  const provider = normalizeAgentProviderToMesacodeAgent(params.provider);
