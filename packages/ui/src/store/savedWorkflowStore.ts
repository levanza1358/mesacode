...
import {
  ...
  type MesacodeSavedWorkflowInvalidEntry,
...
export interface SavedWorkflowWorkspaceState {
  entries: MesacodeSavedWorkflowEntry[];
  invalid: MesacodeSavedWorkflowInvalidEntry[];
  runs: MesacodeSavedWorkflowRun[];
 ...
 */
function savedWorkflowStoreKey(target: MesacodeAgentSavedWorkflowTarget): string {
...
async function fetchWorkspace(
  target: MesacodeAgentSavedWorkflowTarget,
  agentService: IMesacodeAgentService,
): Promise<{ list: MesacodeWorkflowsListResult; runs: MesacodeSavedWorkflowRun[] }> {
  ...
  const [list, runs] = await Promise.all([
    ...
    agentService
      ...
      .catch((error: unknown) => {
        ...
        return [] as MesacodeSavedWorkflowRun[];
      }),
  ]);
  return { list, runs };
}
...
export function selectSavedWorkflowState(
  ...
  target: MesacodeAgentSavedWorkflowTarget | null,
