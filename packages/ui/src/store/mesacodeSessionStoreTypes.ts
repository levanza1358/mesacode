/* oxlint-disable eslint(max-lines) -- Mesacode Agent store 类型和默认状态集中导出，避免切片间重复定义共享结构。 */
/**
 * Mesacode Session Store 类型定义、接口、常量与默认值工厂
 *
 * 从 mesacodeSessionStore.ts 拆分出来，供 store 本体和 selectors / navigation 等子模块共享。
 */
import {
...
export interface WorkspaceInitState {
  status: MesacodeWorkspaceInitStatus;
  error: string | null;
  attempts: number;
}
...
export interface TaskRuntimeState {
  status: MesacodeTaskRuntimeStatus;
  error: string | null;
  /** 该 task 当前运行态绑定的 Mesacode Agent 进程 provider，用于 workspace 级进程重建 busy lock。 */
  ...
  backgroundTaskControls: MesacodeBackgroundTaskControlItem[];
...
export interface TaskUiState {
  permissionRequest: MesacodePermissionRequest | null;
  pendingPermissionRequests: MesacodePermissionRequest[];
  elicitationRequest: MesacodeElicitationRequest | null;
  pendingElicitationRequests: MesacodeElicitationRequest[];
  elicitationFormDraftsByRequestId: Record<string, ElicitationFormDraft>;
  error: MesacodeUiError | null;
}
...
export interface WorkspaceMesacodeUIState {
  /** 当前 workspace 激活中的 task */
  activeTaskId: string | null;
  /**
   * Mesacode Agent 的 workspace 初始化状态。
   *
   * 单 Mesacode Agent 迁移后继续按 provider 分桶会保留多份已经不会再被真实运行时更新的旧状态，
  ...
  draftError: MesacodeUiError | null;
  ...
  taskConfigOptionsByTaskId: Record<string, MesacodeConfigOption[]>;
  ...
  optimisticTaskListByTaskId: Record<string, MesacodeTaskMeta>;
  ...
  /** 当前 workspace 选中的 Mesacode Agent provider */
  ...
  /** Mesacode Agent configOptions（模型、模式、思考级别等） */
  configOptions: MesacodeConfigOption[] | null;
  /** configOptions 加载状态 */
  ...
  taskListCache: MesacodeTaskMeta[] | null;
  /** 新建草稿时递增，驱动输入框在切到草稿态后主动聚焦 */
  draftFocusVersion: number;
}

export interface MesacodeSessionStoreState {
  /** 按 workspace 维护聊天相关 UI 状态 */
  workspaces: Record<string, WorkspaceMesacodeUIState>;
  getWorkspaceState: (workspacePath: string, workspaceIdentity?: string) => WorkspaceMesacodeUIState;
  ...
  setTaskPermissionRequest: (
    workspacePath: string,
    taskId: string,
    request: MesacodePermissionRequest | null,
    workspaceIdentity?: string,
  ) => void;
  ...
  setTaskElicitationRequest: (
    workspacePath: string,
    taskId: string,
    request: MesacodeElicitationRequest | null,
    workspaceIdentity?: string,
  ) => void;
  ...
  /** 任务列表变更时调用（创建/删除任务），驱动 mesacodeTaskMetaMerge 重新拉取列表 */
  bumpTaskListVersion: (workspacePath: string, workspaceIdentity?: string) => void;
  /** 更新已拉取的任务列表缓存 */
  setTaskListCache: (
    workspacePath: string,
    tasks: MesacodeTaskMeta[],
    workspaceIdentity?: string,
  ) => void;
...
const FALLBACK_PROVIDER: MesacodeProvider = MESACODE_AGENT_PROVIDER;
...
export function getDefaultWorkspaceState(): WorkspaceMesacodeUIState {
  // 单 Mesacode Agent 迁移后默认 provider 必须收敛到 glm。
