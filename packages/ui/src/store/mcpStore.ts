...
import { convertToMesacodeAgentMcpServer } from "@mesacode/shared";
...
import {
  importLegacyCommonServersToMesacodeAgent,
  migrateStoredCommonMcpToMesacodeAgent,
...
interface McpStoreState {
  ...
  addMesacodeAgentMcpServer: (name: string, config: McpServerConfig, projectPath?: string) => void;
  updateMesacodeAgentMcpServer: (name: string, config: McpServerConfig, projectPath?: string) => void;
  deleteMesacodeAgentMcpServer: (name: string, projectPath?: string) => void;
  ...
  beginServerStatusListRefresh: (mode?: MesacodeMcpListMode) => number;
  ...
  getServer: (id: string) => MesacodeMcpServer | undefined;
  setCurrentProjectPath: (
    path: string,
    workspaceIdentity?: string,
    directoryService?: McpDirectoryService | null,
  ) => void;
  getEnabledMcpServersForMesacode: (provider: string) => MesacodeAgentMcpServer[];
...
export const useMcpStore = create<McpStoreState>((set, get) => {
  ...
  const statusListEpochs: Record<MesacodeMcpListMode, number> = {
    connect: 0,
    status: 0,
  };
  ...
  function resolveMcpDirectoryService(
    ...
    if (!effectiveWorkspaceIdentity?.trim()) {
      ...
      // 到用户级 Mesacode Agent MCP 的迁移；目录服务只用于远端 workspace 覆盖路由。
      return null;
    }
  ...
  return {
    ...
    loadConfig: () => {
      ...
      // MCP 启停状态已经迁移到 ~/.mesacode/cli/config.json，不能再读取旧 localStorage，
      // 否则旧的本地开关会覆盖新的 Mesacode Agent 配置来源。
      ...
      loadMcpPromise = (async () => {
        try {
          ...
          if (!activeDirectoryService) {
            servers = await migrateStoredCommonMcpToMesacodeAgent(
              mcpPlatformService,
              servers,
              latestWorkspacePath,
            );
    ...
    addMcpServer: (name, config) => get().addMesacodeAgentMcpServer(name, config),
    updateMcpServer: (name, config) => get().updateMesacodeAgentMcpServer(name, config),
    deleteMcpServer: (name) => get().deleteMesacodeAgentMcpServer(name),
    addScopedMcpServer: async (source, name, config, projectPath) => {
      invalidateStatusListRequests();
      const targetSource: CliMcpSource = source === "mcp" ? "mesacodeagentmcp" : source;
    ...
    addMesacodeAgentMcpServer: (name, config, projectPath) =>
      get().addScopedMcpServer("mesacodeagentmcp", name, config, projectPath),
    updateMesacodeAgentMcpServer: (name, config, projectPath) =>
      get().updateScopedMcpServer("mesacodeagentmcp", name, config, projectPath),
    deleteMesacodeAgentMcpServer: (name, projectPath) =>
      get().deleteScopedMcpServer("mesacodeagentmcp", name, projectPath),
    ...
    markServerStatusListRefreshFailed: (error, requestEpoch, mode = "connect") => {
      ...
      set((state) => ({
        ...
        servers: state.servers.map((server) => {
          const enabled = state.enabledStates[server.id] ?? server.enabled;
          if (server.source !== "mesacodeagentmcp" || !enabled || server.status !== "connecting") {
            return server;
          }
          return {
            ...server,
    ...
    getEnabledMcpServersForMesacode: (_provider) => {
      const { servers, enabledStates, currentProjectPath } = get();
      const candidates = new Map<string, MesacodeMcpServer>();

      for (const server of servers) {
        const isEnabled = enabledStates[server.id] ?? server.enabled;
        if (!isEnabled) continue;
        if (server.source !== "mesacodeagentmcp") continue;
      ...
      const result: MesacodeAgentMcpServer[] = [];
      for (const server of candidates.values()) {
        const mesacodeAgentServer = convertToMesacodeAgentMcpServer(server.name, server.config);
        if (mesacodeAgentServer) result.push(mesacodeAgentServer);
      }
      return result;
    },
    ...
    mergePreloadedMcpServers: (preloaded) => {
      ...
      const targetSource: CliMcpSource = "mesacodeagentmcp";
