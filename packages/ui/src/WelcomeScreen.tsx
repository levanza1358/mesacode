...
function LoginPanel({ active, onComplete }: LoginPanelProps) {
  ...
  const user = useMesacodeStore((s) => s.user);
  const oauthError = useMesacodeStore((s) => s.oauthError);
  const setOAuthError = useMesacodeStore((s) => s.setOAuthError);
  const oauthSuccessSeq = useMesacodeStore((s) => s.oauthSuccessSeq);
  const lastOAuthSuccessProvider = useMesacodeStore((s) => s.lastOAuthSuccessProvider);
  const loginEntryRequest = useMesacodeStore((s) => s.loginEntryRequest);
  const clearLoginEntryRequest = useMesacodeStore((s) => s.clearLoginEntryRequest);
  const markLoginEntryAttemptStatus = useMesacodeStore((s) => s.markLoginEntryAttemptStatus);
