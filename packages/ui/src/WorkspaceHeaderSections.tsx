...
export function WorkspaceHeaderTitleSection({
  ...
  const buildHeaderTaskSnapshot = (
    fallbackTask: MesacodeTaskMeta,
    overrides?: Partial<MesacodeTaskMeta>,
  ): MesacodeTaskMeta => {
    if (activeTaskMeta) {
      return {
        ...activeTaskMeta,
  ...
  const handleSubmitRenameTask = async () => {
    ...
    try {
      const renamedTask = await services.mesacodeTaskService.renameTask({
  ...
  const handleArchiveTask = async () => {
    ...
    void services.mesacodeTaskService
      ...
      .then((meta) => {
        // Header 更多菜单不能只依赖 mesacodeTaskMetaMerge：归档后只有旧列表状态被更新。
