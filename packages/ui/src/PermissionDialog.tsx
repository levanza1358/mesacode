...
import { MAX_PERMISSION_FEEDBACK_CHARS } from "@mesacode/shared/mesacode-protocol-v4";
...
function readPermissionRuleScopes(option: MesacodePermissionOption): PermissionRuleScope[] {
...
function isOfficialCuaProjectPermission(option: MesacodePermissionOption): boolean {
 ...
 */
function isWorkflowRefineOption(option: MesacodePermissionOption): boolean {
...
const PROVIDER_PERMISSION_OPTION_NAME_LABELS: Partial<
  Record<MesacodeProvider, Record<string, string>>
> = {
  glm: {
    // GLM/Mesacode Agent 通过 Mesacode Agent 发来的项目级记忆授权文案是英文原文。
...
function getProviderOptionNameMessageIds(
  provider: MesacodeProvider | undefined,
  name: string,
...
function getPermissionDisplayReason(request: MesacodePermissionRequest): string | null {
...
function buildPermissionToolCall(
  request: MesacodePermissionRequest,
  preview: ReturnType<typeof getPermissionRequestPreview>,
): TaskChatToolCall {
  const rawInput = readRawToolCallInput(request.raw);
...
export function PermissionDialog({
  ...
  onRespond: (requestId: string, option: MesacodePermissionOption, feedback?: string) => void;
  workspacePath: string;
  ...
  const respondWithOption = useCallback(
    (option: MesacodePermissionOption) => {
  ...
  // 当前 Mesacode Agent 的 ExitPlanMode 权限请求不再传 legacy switch_mode。
            ...
            >
              {orderedOptions.map((option, index) => {
                ...
                const knownNameLabel = nameMessageIds
                  ...
                  : null;
                // Mesacode Agent 协议里 option.name 才是给用户看的真实选项文案，kind 只表示按钮语义。
