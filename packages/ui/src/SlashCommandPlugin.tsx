 ...
 * 1. `/` 直接展示 Mesacode Agent 广播的真实 slash commands，并补充可用 subagents
...
export function SlashCommandPlugin({
  ...
  appCommands,
}: SlashCommandPluginProps & { provider: MesacodeProvider }) {
  ...
  // 这里只按 workspacePath 读取会落到 path 桶，表现为 Mesacode Agent 已收到 available_commands_update 但 / 面板为空。
