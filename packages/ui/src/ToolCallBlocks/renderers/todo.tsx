...
import { extractPlanStepsFromToolInput, extractPlanStepsFromToolOutput } from "@mesacode/shared";
import type { MesacodePlanStep } from "@mesacode/shared";
...
const todoTextClasses: Record<MesacodePlanStep["status"], string> = {
  pending: "text-foreground-subtle",
  in_progress: "text-foreground",
  completed: "text-foreground-subtlest line-through",
};

function readTodoPlan(context: ToolCallBlockRenderContext): MesacodePlanStep[] | null {
...
function TodoStatusIcon({ status }: { status: MesacodePlanStep["status"] }) {
  if (status === "completed") {
