import type { useMesacodeIntl } from "@/i18n/IntlProvider.js";
import type { ToolCallBlockRenderContext } from "@/ToolCallBlocks/shared.js";
import { isSubagentColor } from "@/lib/subagentColors.js";

type AgentIntl = ReturnType<typeof useMesacodeIntl>["intl"];
...
export function getAgentKindLabel(
  ...
  const rawName =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "mesacode", "agentType"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "mesacode", "agent_type"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "mesacode", "subagent_type"]);
...
export function getAgentColor(toolCall: AgentToolCall) {
  ...
  const inputRecord = isPlainRecord(toolCall.input) ? toolCall.input : null;
  const rawColor =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "mesacode", "color"]) ??
...
export function readBackgroundAgentInfo(toolCall: AgentToolCall) {
  ...
  const mesacode = meta && isPlainRecord(meta.mesacode) ? meta.mesacode : null;
  const mesacodeBackgroundAgent =
    mesacode && isPlainRecord(mesacode.backgroundAgent) ? mesacode.backgroundAgent : null;
  const taskNotification =
    mesacode && isPlainRecord(mesacode.taskNotification) ? mesacode.taskNotification : null;
  const backgroundAgent = mesacodeBackgroundAgent;
...
export function getAgentActivityContent(toolCall: AgentToolCall) {
  const taskNotificationResult =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "mesacode", "taskNotification", "result"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "mesacode", "taskNotification", "summary"]);
