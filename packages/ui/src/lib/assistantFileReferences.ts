...
import { extractConversationPreviewFileReferences } from "@mesacode/shared";
import { decodeFilePathUriEscapes, joinFilePath } from "@/lib/path.js";
import { MEDIA_PREVIEW_FORMATS } from "@mesacode/shared";
import {
  extractMesacodeFileCitationDirectives,
  resolveMesacodeFileCitationPreviewKind,
} from "@/lib/mesacodeFileCitation.js";
import {
...
export function extractAssistantFileReferences(
  ...
  for (const citation of extractMesacodeFileCitationDirectives(content)) {
  ...
  for (const match of content.matchAll(FILE_PATH_RE)) {
    ...
    // 明确的 mesacode-file-citation 仍在上面的专用分支处理。
    if (/^~[\\/]/.test(raw)) {
      protectedRanges.push([start, end]);
      continue;
    }
