...
import { type MesacodeProviderAccountAccess, mesacodeProviderAccountAccessSchema } from "@mesacode/shared";
...
export function resolveEntitledAccountProviderAccess(
  ...
  const parsed = mesacodeProviderAccountAccessSchema.safeParse(provider.effectiveConfig.access);
