import type { MesacodeWorkspaceTaskListChanged } from "@mesacode/shared";
...
const TASK_LIST_MEMBERSHIP_REASONS = new Set<MesacodeWorkspaceTaskListChanged["reason"]>([
  "task_archived",
  "task_unarchived",
  "task_pinned",
  "task_unpinned",
...
function isTaskListMembershipWorkspaceEvent<
  T extends Pick<MesacodeWorkspaceTaskListChanged, "reason">,
>(
  event: T,
): event is T & {
...
export function shouldRefetchTaskListMembershipForWorkspaceEvent(
  event: Pick<MesacodeWorkspaceTaskListChanged, "reason">,
): boolean {
