// re-home 产物（为删除 mesacodeChatMessages 铺路，模式同 mesacode-task-types-core）。
// 本文件承载旧消息转换层（mesacodeChatMessages/mesacodeChatMessageHelpers）中仍被存活栈
...
// 旧消息拼装运行时（mesacodeChatMessages.ts）删除后，这里是该类型面的唯一事实源。
import type {
  MesacodeAssistantMessagePart,
  MesacodeAssistantMessageFeedback,
  MesacodePromptAttachment,
  MesacodeTimelineMeta,
  MesacodeTaskSnapshotBodyRef,
  MesacodeTaskSnapshotToolFieldRef,
  MesacodeTaskSnapshotToolSlice,
...
export interface TaskChatToolCall {
  ...
  /** Mesacode 固定工具名；kind 仍保留给旧 Mesacode Agent 快照和 UI 聚合分类兼容。 */
  toolName?: string;
  kind: string;
  title?: string;
  input: unknown;
...
export type TaskChatMessagePart = MesacodeAssistantMessagePart;
