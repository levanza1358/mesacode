import type {
  ...
  MesacodeTaskChangedFileSummary,
  MesacodeTaskMeta,
} from "@mesacode/shared";
import { computeLineChangeStat } from "@mesacode/shared";
...
export function getTaskChangeSummary(
  task: Pick<MesacodeTaskMeta, "changeSummary"> | null | undefined,
): MesacodeTaskChangeSummary | null {
...
function formatTaskChangeStats(
  summary: MesacodeTaskChangeSummary,
...
export function buildTaskChangeSummary(
  fileChanges: readonly MesacodePersistedFileChange[] | undefined,
): MesacodeTaskChangeSummary | null {
  ...
  let added = 0;
  let removed = 0;
  const files: MesacodeTaskChangedFileSummary[] = Array.from(changedFileMap.values())
    .map((file) => {
...
export function buildTurnChangeSummary(
  turn: MesacodePersistedFileChange | null | undefined,
): MesacodeTaskChangeSummary | null {
  ...
  let added = 0;
  let removed = 0;
  const files: MesacodeTaskChangedFileSummary[] = Array.from(filesByPath.entries())
    .map(([path, snapshot]) => {
...
export function buildPerTurnChangeSummaries(
  fileChanges: readonly MesacodePersistedFileChange[] | undefined,
): Map<number, MesacodeTaskChangeSummary> {
  const result = new Map<number, MesacodeTaskChangeSummary>();
  if (!fileChanges || fileChanges.length === 0) {
    return result;
  }
