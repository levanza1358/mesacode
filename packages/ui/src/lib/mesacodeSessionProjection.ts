/* oxlint-disable eslint(max-lines) -- Mesacode session 到当前聊天 projection 的迁移桥需要同时保持 snapshot 和 event 映射一致。 */
import {
...
const MESACODE_AGENT_MODE_OPTIONS = [
  {
    id: "build",
...
] as const satisfies readonly MesacodeTaskModeInfo[];
const MESACODE_AGENT_MODE_ID_SET = new Set<string>(MESACODE_AGENT_MODE_OPTIONS.map((mode) => mode.id));
...
export function parseModelPickerValue(value: string): ModelSelection {
  ...
  if (customModel?.providerId && customModel.modelName) {
    ...
    // Mesacode Protocol 必须收到严格的 providerId/modelId 结构。
    return {
...
export function mesacodeSessionSettingsToConfigOptions(
  settings: MesacodeSessionSettingsState,
): MesacodeConfigOption[] {
  const configOptions: MesacodeConfigOption[] = [
    {
      id: MODEL_CONFIG_ID,
      name: "Model",
      category: "model",
    ...
    },
    {
      ...
      category: "mode",
      type: "select",
      currentValue: normalizeAvailableMesacodeMode(settings.mode.current),
      options: getMesacodeAgentModeSelectOptions(),
    },
  ];
...
export function mesacodeWorkspacePresentationToConfigOptions(
  ...
  return [
    {
      ...
      category: "mode",
      type: "select",
      currentValue: normalizeAvailableMesacodeMode(mode),
      options: getMesacodeAgentModeSelectOptions(),
    },
  ];
}
...
function resolveSettingsThoughtLevelCurrentValue(
  thoughtLevel: MesacodeSessionSettingsState["thoughtLevel"],
): string | undefined {
  ...
  const defaultThoughtLevel =
    thoughtLevel.defaultLevel && thoughtLevelValues.has(thoughtLevel.defaultLevel)
      ...
      : undefined;
  // Mesacode Protocol 的 defaultLevel 是模型事实，current 为空时表示用户尚未显式修改。
...
export function mesacodeSessionSnapshotToTaskMeta(snapshot: MesacodeSessionStateSnapshot): MesacodeTaskMeta {
  return {
...
function normalizeAvailableMesacodeMode(mode: MesacodeSessionMode): string {
  return MESACODE_AGENT_MODE_ID_SET.has(mode) ? mode : "build";
}

function getMesacodeAgentModeSelectOptions(): NonNullable<MesacodeConfigOption["options"]> {
  return MESACODE_AGENT_MODE_OPTIONS.map((mode) => ({
    value: mode.id,
    name: mode.name,
