import type { MesacodeError, TraceId } from "@mesacode/shared";
import { errorAttributionSchema, type ErrorAttribution } from "@mesacode/shared/mesacode-protocol-v4";

export interface MesacodeUiError extends MesacodeError {
...
interface NormalizeMesacodeUiErrorOptions {
  fallbackCode?: string;
...
const GENERIC_MESACODE_UI_ERROR_MESSAGES = new Set([
  "Internal error",
  "Turn execution failed",
  "Compact failed",
  "Rewind failed",
  "Mesacode session failed",
]);
...
function collectMessageCandidatesFromRecord(record: Record<string, unknown>): string[] {
  ...
  const messagePaths: Array<readonly string[]> = [
    ...
    ["data", "message"],
    ["data", "detail"],
    // Mesacode Agent 常把可读原因放在 data.details（复数）里；之前只识别 detail，
    ...
    ["data", "error", "detail"],
    ["data", "error", "details"],
    // mesacode-cli 会把模型/网络错误摘要放在 data.mesacode.error 下。
    // 之前 UI 只读 data.error，导致已经结构化好的 provider 根因仍被 “Internal error” 盖住。
    ["data", "mesacode", "error", "message"],
    ["data", "mesacode", "error", "detail"],
    ["data", "mesacode", "error", "details"],
  ];
...
function readFirstAttributionFromPaths(error: unknown): ErrorAttribution | undefined {
  ...
  const paths: Array<readonly string[]> = [
    ...
    ["data", "mesacode", "error", "attribution"],
  ];
  for (const path of paths) {
...
export function normalizeMesacodeUiError(
  error: unknown,
  options: NormalizeMesacodeUiErrorOptions = {},
): MesacodeUiError {
  const candidates = collectMessageCandidates(error);
  // mesacode-cli 已经把 provider/network 根因放进 detail 或 data.mesacode.error，
  // 外层仍可能保留 "Internal error" 这类包装文案。主提示优先选非泛化候选，避免根因被盖住。
  const primaryMessage =
    candidates.find((candidate) => !GENERIC_MESACODE_UI_ERROR_MESSAGES.has(candidate)) ??
    candidates[0] ??
    options.fallbackMessage ??
    "Internal error";
  const detailMessage = candidates.find(
    (candidate) => candidate !== primaryMessage && !GENERIC_MESACODE_UI_ERROR_MESSAGES.has(candidate),
  );
  ...
  const underlyingErrorMessage = readFirstStringFromPaths(error, [
    ...
    ["data", "mesacode", "error", "underlyingErrorMessage"],
  ]);
  const underlyingErrorDetail = readFirstStringFromPaths(error, [
    ...
    ["data", "mesacode", "error", "underlyingErrorDetail"],
  ]);
