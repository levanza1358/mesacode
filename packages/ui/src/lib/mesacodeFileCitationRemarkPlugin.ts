...
import { extractMesacodeFileCitations } from "@/lib/mesacodeFileCitation.js";
...
function projectCitationTextNode(
  ...
  const citations = extractMesacodeFileCitations(value);
...
export function createMesacodeFileCitationRemarkPlugin(
  workspacePath: string,
  homePath?: string,
): Plugin {
  return function mesacodeFileCitationRemarkPlugin() {
    return (tree: unknown) => {
