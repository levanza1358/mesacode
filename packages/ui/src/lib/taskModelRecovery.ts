import type { MesacodeConfigOption, MesacodeTaskMeta } from "@mesacode/shared";
import { getMesacodeAgentModeSelectOptions } from "@mesacode/shared";
...
function resolveRecoveredTaskModelValue(
  taskMeta: Pick<MesacodeTaskMeta, "provider" | "model">,
): string | null {
...
function ensureModelOptionValue(option: MesacodeConfigOption, modelValue: string): MesacodeConfigOption {
...
function createRecoveredModelOption(modelValue: string): MesacodeConfigOption {
  return {
    category: "model",
    currentValue: modelValue,
    id: "model",
...
function createRecoveredModeOption(modeValue: string): MesacodeConfigOption {
  return {
    category: "mode",
...
function createRecoveredThoughtLevelOption(thoughtLevel: string): MesacodeConfigOption {
  return {
...
export function resolveTaskRestorePreloadConfigOptions({
  ...
  taskMeta: Pick<MesacodeTaskMeta, "provider" | "model"> &
    Partial<Pick<MesacodeTaskMeta, "mode" | "thoughtLevel">>;
  cachedTaskConfigOptions?: readonly MesacodeConfigOption[];
}): MesacodeConfigOption[] {
