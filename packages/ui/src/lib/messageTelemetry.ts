...
import type {
  ...
  MesacodePersistedFileChange,
  MesacodeProvider,
  MesacodeStreamEvent,
  MesacodeTimelineStatus,
  MesacodeUsage,
...
interface FinalizePromptTelemetryInput {
  ...
  status: string;
  finishedAt: number;
  fileChanges?: readonly MesacodePersistedFileChange[];
  usage?: MesacodeUsage;
  errorType?: string;
  errorMsg?: string;
...
function resolvePromptTelemetryModelProvider(params: {
  ...
  // ghost supplier 代表尚未解析到稳定 custom provider 的临时隔离态，provider 维度统一回退到当前 Mesacode Agent provider。
...
export function buildPromptTelemetryExtraDetail(params: {
  ...
  return {
    ...
    // agent 字段取 Mesacode Agent provider；本仓库没有独立 session.agentId。
...
const COMPACTION_TERMINAL_STATUSES: readonly MesacodeTimelineStatus[] = [
  "completed",
  "failed",
  "interrupted",
];

function isCompactionTerminalStatus(status: MesacodeTimelineStatus): boolean {
...
export function buildCompactionTelemetryExtraDetail(params: {
  timeline: MesacodeContextCompactionTimelineMeta;
  provider?: MesacodeProvider;
...
function buildPromptUsageTelemetryExtraDetail(
  ...
  return {
    ...
    cache_write_input_tokens: String(usage.cachedWriteInputTokens ?? 0),
    // Mesacode Agent 链路未透出 tool use prompt token，成功态显式补 0 保持 extraDetail 字段集合完整。
...
function mergePromptUsage(left: MesacodeUsage | null, right: MesacodeUsage): MesacodeUsage {
  if (!left) {
    return {
...
function collectFileChangeMetrics(fileChanges: readonly MesacodePersistedFileChange[] | undefined) {
...
export function recordPromptModelRequestStarted(
  taskId: string,
  event: Extract<MesacodeStreamEvent, { type: "task_network_debug_status" }>,
...
export function recordAgentStepTelemetryEvent(input: {
  ...
  switch (input.event.type) {
    ...
    case "agent_message_chunk":
      if (input.event.mesacodeTimeline) {
        return finalized;
      }
