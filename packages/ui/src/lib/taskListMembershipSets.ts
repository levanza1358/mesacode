...
> &
  Partial<Pick<IMesacodeTaskService, "listDeletedTaskIds">>;
...
interface TaskListMembershipSets {
  ...
  terminalStatusByTaskId: Map<string, Extract<MesacodeTaskMeta["status"], "completed" | "error">>;
  /**
...
function collectUnreadAt(unreadAtByTaskId: Map<string, number>, tasks: MesacodeTaskMeta[]): void {
...
function mergeTaskIndexItems(lists: MesacodeTaskMeta[][]): MesacodeTaskMeta[] {
  const itemByEntityKey = new Map<string, MesacodeTaskMeta>();
...
async function fetchTaskListMembershipSetsForEndpoints(
  ...
  const taskIndexItemByEntityKey = new Map<string, MesacodeTaskMeta>();
