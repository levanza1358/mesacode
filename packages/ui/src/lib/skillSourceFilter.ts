...
function resolveSkillSourceType(skillPath: string): SkillSourceType {
  ...
  if (normalized.includes("/.mesacode/skills/")) {
    return "glm";
  }
  if (normalized.includes("/.mesacode/cli/plugins/cache/")) {
    return "glm";
  }
  return "unknown";
}
...
function isZcodeSkill(skill: { id?: string; path: string; scope?: string }): boolean {
  return (
    // plugin skill 的真实路径在 CLI plugin cache 下，不在 `.mesacode/skills`。
...
export function filterSkillsForProvider<T extends { path: string; id?: string; scope?: string }>(
  skills: T[],
  _legacyProvider: MesacodeProvider,
): T[] {
