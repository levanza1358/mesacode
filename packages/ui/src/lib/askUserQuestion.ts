type MesacodeUserQuestionAnswers = Record<string, unknown>;
...
interface AskUserQuestionData {
  ...
  answers?: MesacodeUserQuestionAnswers;
}
...
function readQuestions(input: unknown): unknown[] {
  ...
  // Mesacode Agent 的单题输入与交互请求的多题输入共用展示管线。
  return typeof input.question === "string" && Array.isArray(input.options) ? [input] : [];
}

function readAnswers(input: unknown): MesacodeUserQuestionAnswers | undefined {
  if (!isPlainRecord(input)) {
    return undefined;
  }
...
function readNestedAskUserQuestionAnswers(input: unknown): MesacodeUserQuestionAnswers | undefined {
...
function parseMesacodeAskUserQuestionOutput(
  output: unknown,
  input: unknown,
): MesacodeUserQuestionAnswers | undefined {
...
export function readAskUserQuestionAnswers(value: {
  ...
  const parsedMesacodeOutputAnswers = parseMesacodeAskUserQuestionOutput(
    value.output,
    readAskUserQuestionInput(value),
  );
  if (parsedMesacodeOutputAnswers) {
    return parsedMesacodeOutputAnswers;
  }
  ...
  if (isPlainRecord(value.raw)) {
    ...
    const parsedRawMesacodeOutputAnswers = parseMesacodeAskUserQuestionOutput(
      value.raw.rawOutput,
      readAskUserQuestionInput(value),
    );
    if (parsedRawMesacodeOutputAnswers) {
      return parsedRawMesacodeOutputAnswers;
    }
    const nestedRawAnswers = readNestedAskUserQuestionAnswers(value.raw.output);
    if (nestedRawAnswers) {
      return nestedRawAnswers;
    }
    const parsedRawMesacodeAnswers = parseMesacodeAskUserQuestionOutput(
      value.raw.output,
      readAskUserQuestionInput(value),
    );
    if (parsedRawMesacodeAnswers) {
      return parsedRawMesacodeAnswers;
    }
...
export function getAskUserQuestionAnswerText(
  ...
  answers: MesacodeUserQuestionAnswers | undefined,
  noAnswerText: string,
) {
