...
// sessions-index 只补实时 activity/detail，最终拼出旧 MesacodeGroupedTaskView 同形结果。
...
function taskKeyOf(task: MesacodeTaskMeta): string {
...
function taskOrderMapKey(task: MesacodeTaskMeta): string {
...
function nodeMapKey(node: MesacodeGroupedTaskViewNode): string {
...
function sortGroupTasks(
  ...
  membersByTaskKey: Map<string, MesacodeGroupedTaskViewStructureMember>,
): MesacodeTaskListItem[] {
  const resolvedOrderByTaskKey = new Map<string, number>();
  let maxOrder = 0;
  const missing: Array<{ task: MesacodeTaskListItem; addedAt: number; key: string }> = [];
...
interface BuildGroupedTaskViewParams {
  ...
  taskIndexItems: MesacodeTaskMeta[];
  /** sessions-index 派生的会话 activity/detail，只 enrich 命中的持久行。 */
  sessions: MesacodeTaskMeta[];
...
export function buildGroupedTaskViewFromSessions(
  ...
  const activeTaskByKey = new Map<string, MesacodeTaskListItem>();
  ...
  const membersByGroupId = new Map<string, MesacodeGroupedTaskViewStructureMember[]>();
  ...
  const nodes: MesacodeGroupedTaskViewNode[] = structure.groups.map((group) => {
