...
export async function refreshSharedSkillStoreForWorkspace(params: {
  ...
  const normalizedProvider = normalizeAgentProviderToMesacodeAgent(
    params.provider ?? MESACODE_AGENT_PROVIDER,
  );
  ...
  if (
    ...
    normalizeAgentProviderToMesacodeAgent(skillStore.provider) === normalizedProvider &&
