...
import { resolveWorkspaceStateKey } from "@/store/mesacodeSessionStoreSelectors.js";

export type TaskEntityKey = string;
export type TaskListCacheKey = string;
export type TaskListQueryKind = MesacodeTaskListKind | "workspace";
...
function normalizeTaskListWorkspaceScopes(
  ...
  const uniqueScopes = new Map<string, MesacodeTaskListWorkspaceScope>();
