...
import type { CuaAccessibilitySettingsResult, CuaPermissionKind } from "@mesacode/shared";
import { requiredCuaPermissionsForRequestAccessStatus } from "@mesacode/shared/mesacode-protocol-v4";
import {
  isCuaPermissionStatusAvailable,
  type CuaPermissionRestartOptions,
  type CuaPermissionStatusResult,
  type MesacodeAgentCuaPermissionObservation,
...
function cuaPermissionObservationKey(observation: MesacodeAgentCuaPermissionObservation): string {
...
export function CuaPermissionObservationAttachment() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      typeof services.mesacodeAgentService.onDynamicCuaPermissionObservation !== "function"
    ) {
      return;
    }
    ...
    let disposed = false;
    const subscription = services.mesacodeAgentService.onDynamicCuaPermissionObservation()(
      (observation) => {
        const missing = requiredCuaPermissionsForRequestAccessStatus(observation.permissionStatus);
