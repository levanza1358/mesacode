...
import { RENDERER_MESACODE_ENDPOINT_URLS } from "@/lib/rendererMesacodeEndpoint.js";
...
interface CodingPlanEmbeddedWebviewDialogProps {
   ...
   * 官网页购买成功后的回调。
   *
   * 官网页通过 preload 注入的 window.mesacodeBridge.notifyPurchaseComplete({ provider })
   * 发送 mesacode:coding-plan-purchase-complete 频道消息，本组件在 webview 的
...
interface CodingPlanWebviewImportMetaEnv {
  ...
  VITE_MESACODE_E2E_STORE_BRIDGE?: string;
}
...
export function CodingPlanEmbeddedWebviewDialog({
  ...
  const userId = useMesacodeStoreWithDefault((state) => state.user?.id ?? null, null);
  ...
  const embeddedTheme: CodingPlanEmbeddedTheme = resolveTheme(theme);
  const computedWebviewUrl = useMemo(() => {
    const env = readCodingPlanWebviewImportMetaEnv();
    const origin = resolveCodingPlanEmbeddedOrigin({
      endpointOrigin: RENDERER_MESACODE_ENDPOINT_URLS.origin,
      e2eStoreBridgeEnabled: env.VITE_MESACODE_E2E_STORE_BRIDGE === "1",
        ...
        const credentials: CodingPlanEmbeddedCredentials =
          provider === "zai"
            ? {
                ...
                mesacodeJwtToken: values[1],
              }
            : {
                bigmodelAccessToken: values[0],
                // BigModel OAuth callback 同样会落盘 mesacode JWT；官网用它在
                // mesacode-plan 域查 billing/balance 判定 Start Plan 状态。
                mesacodeJwtToken: values[1],
              };
  ...
  const handleWebviewRef = useCallback(
    (element: ElectronWebviewTag | null) => {
      ...
      // ipc-message: 官网页通过 preload 的 window.mesacodeBridge.notifyPurchaseComplete
      // 发回购买完成信号（mesacode:coding-plan-purchase-complete）。
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        >
          ...
          <webview
            ...
            partition="persist:mesacode-coding-plan"
            src={webviewUrl}
            className={cn(
