 ...
 * Supports the unified Mesacode Agent MCP source backed by settings directories.
 */
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import {
  convertToMesacodeAgentMcpServer,
...
import { isMesacodeAgentMcpStatusModeUnsupportedError, type IMcpSyncService } from "@mesacode/services";
...
const DEFAULT_MCP_SOURCE: ServerScope = "mesacodeagentmcp";
...
async function refreshMcpServerStatusList({
  ...
  // mcp/list 收敛到 IMcpSyncService——UI 不直接触达 mesacodeAgentService。
  ...
  try {
    ...
    if (refreshMode === "status" && isMesacodeAgentMcpStatusModeUnsupportedError(error)) {
...
function buildMcpOAuthAuthorizationStatusRefreshServers(
  ...
  for (const server of servers) {
    if (server.source !== "mesacodeagentmcp" || !server.enabled) {
      continue;
    }
    ...
    if (!mesacodeAgentServer) {
      continue;
    }
    seenNames.add(server.name);
    result.push(mesacodeAgentServer);
  }

  return result;
}

function buildMcpOAuthAuthorizationStatusRefreshOptions(mcpServers: MesacodeAgentMcpServer[]): {
  mode: "status";
  mcpServers: MesacodeAgentMcpServer[];
} {
  return {
...
function buildMcpServerStatusListKey(servers: MesacodeMcpServer[]): string {
  return servers
    .filter((server) => server.source === "mesacodeagentmcp")
    .map((server) => {
...
function buildPluginMcpServerStatusListKey(plugins: MesacodePluginInfo[]): string {
  return plugins
    .map((plugin) => {
...
function PluginMcpServerList({
  ...
  pluginListingById: ReadonlyMap<string, MesacodeAvailablePluginSummary["listing"]>;
...
export function McpSettingsSection({
  ...
  const [editingServer, setEditingServer] = useState<MesacodeMcpServer | null>(null);
  ...
  const requestMcpServerStatusList = useCallback(
    async (options?: {
      ...
      const requestedMcpServers =
        options?.mcpServers ?? useMcpStore.getState().getEnabledMcpServersForMesacode("mesacode");
  ...
  async function handleSave(form: FormState, prev?: MesacodeMcpServer) {
    if (!activeWorkspacePath) {
      return;
    }
    const loaded = await ensureLoadedForWorkspace(
  ...
  async function handleDelete(server: MesacodeMcpServer) {
  ...
  function handleEdit(server: MesacodeMcpServer) {
  ...
  function handleOpenAuthorization(server: MesacodeMcpServer) {
