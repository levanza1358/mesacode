...
export function SavedWorkflowProjectGroup({
  ...
  const agentService = services.mesacodeAgentService;
  ...
  const launch = useCallback(
    async (entry: MesacodeSavedWorkflowEntry, args: Record<string, unknown>) => {
  ...
  const handleCardPromote = useCallback(
    (entry: MesacodeSavedWorkflowEntry) => void handlePromote(entry),
    [handlePromote],
  );
