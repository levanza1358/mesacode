...
import {
  ...
  type MesacodeSavedWorkflowEntry,
  type MesacodeSavedWorkflowMeta,
  type MesacodeSavedWorkflowRun,
  type MesacodeWorkflowsGetResult,
} from "@mesacode/shared";
import type { IMesacodeAgentService, MesacodeAgentSavedWorkflowTarget } from "@mesacode/services";
...
type DetailLoad =
  | { status: "loading" }
  | { status: "ready"; detail: Extract<MesacodeWorkflowsGetResult, { ok: true }> }
  | { status: "failed"; reason: string };

type MetaDraft = SavedWorkflowMetaDraft;

function draftFromMeta(meta: MesacodeSavedWorkflowMeta): MetaDraft {
  return {
...
interface SavedWorkflowDetailViewProps {
  ...
  target: MesacodeAgentSavedWorkflowTarget;
  agentService: IMesacodeAgentService;
  name: string;
  /** 面包屑「自动化 › <项目名> › <工作流名>」里的项目一级。 */
  projectLabel: string;
  entry: MesacodeSavedWorkflowEntry | undefined;
  runs: readonly MesacodeSavedWorkflowRun[];
  now: number;
  busy: boolean;
  canOpenRun: boolean;
  onBack: () => void;
  ...
  onMove?: () => void;
  onDelete: () => void;
  onOpenRun: (run: MesacodeSavedWorkflowRun) => void;
  /** 产物 chip → `workflow-artifact` tab；缺席即 chips 只读。 */
  onOpenArtifact?: (run: MesacodeSavedWorkflowRun, artifactId: string) => void;
  onMetaSaved: () => void;
  /** 传入即在运行历史每行渲染项目列（全局工作流跨项目历史）。 */
  resolveRunProject?: (run: MesacodeSavedWorkflowRun) => SavedWorkflowRunProject | null;
...
export function SavedWorkflowDetailView({
  ...
  const save = useCallback(async () => {
    ...
    const meta: MesacodeSavedWorkflowMeta = {
      description,
