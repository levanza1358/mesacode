...
// 也便于单测这些无 React 的判定。
import { resolveWorkspaceKey, type MesacodeSavedWorkflowRun } from "@mesacode/shared";
import type { MesacodeAgentSavedWorkflowTarget } from "@mesacode/services";
...
export const GLOBAL_SAVED_WORKFLOW_TARGET: MesacodeAgentSavedWorkflowTarget = { scope: "global" };
...
export function buildGlobalRunProjectResolver(
  localProjects: readonly AutomationWorkspaceOption[],
): (run: MesacodeSavedWorkflowRun) => SavedWorkflowRunProject | null {
  return (run) => {
