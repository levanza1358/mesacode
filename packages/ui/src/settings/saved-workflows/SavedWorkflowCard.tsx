...
function SavedWorkflowLastRunBadge({
  ...
  run: MesacodeSavedWorkflowRun | undefined;
  now: number;
}) {
...
interface SavedWorkflowCardProps {
  ...
  lastRun: MesacodeSavedWorkflowRun | undefined;
  now: number;
  busy?: boolean;
  onOpen: (entry: MesacodeSavedWorkflowEntry) => void;
  onRun: (entry: MesacodeSavedWorkflowEntry) => void;
  onRevise: (entry: MesacodeSavedWorkflowEntry) => void;
  onCopyPath: (entry: MesacodeSavedWorkflowEntry) => void;
  /** 作用域动作：项目档「提升为全局」（AI 概括）/ 全局档「移到项目…」；仅在传入时出现。 */
  onMove?: (entry: MesacodeSavedWorkflowEntry) => void;
  onDelete: (entry: MesacodeSavedWorkflowEntry) => void;
}
