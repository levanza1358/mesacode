...
// max-lines 400；载体是 `useServices().mesacodeAgentService`，RPC 一律带 `{ scope: "global" }`。
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import type { MesacodeSavedWorkflowEntry, MesacodeSavedWorkflowRun } from "@mesacode/shared";
...
export function useSavedWorkflowGlobalGroup({
  ...
  const { mesacodeAgentService: agentService, fileWatcherService } = useServices();
  ...
  const [launchEntry, setLaunchEntry] = useState<MesacodeSavedWorkflowEntry | null>(null);
  const [moveEntry, setMoveEntry] = useState<MesacodeSavedWorkflowEntry | null>(null);
  ...
  // 目录监听：list 回的绝对目录（`~/.mesacode/workflows`）本机可直接 watch。
  ...
  const handleRun = useCallback(
    (entry: MesacodeSavedWorkflowEntry) => {
      launcher.clearError();
  ...
  const handleDelete = useCallback(
    async (entry: MesacodeSavedWorkflowEntry) => {
  ...
  const handleCardDelete = useCallback(
    (entry: MesacodeSavedWorkflowEntry) => void handleDelete(entry),
    [handleDelete],
  );
  const handleCardMove = useCallback((entry: MesacodeSavedWorkflowEntry) => setMoveEntry(entry), []);
  const handleOpen = useCallback(
    (entry: MesacodeSavedWorkflowEntry) => onOpenDetail(entry.name),
    [onOpenDetail],
  );
  // 全局档的目标项目按 `run.cwd` 反查已打开项目；没打开就两个入口都关掉
  // （实例必须开在发起它的项目）。门与实参构造与项目档共用（产物不需要 toolCallId）。
  const resolveRunTarget = useCallback(
    (run: MesacodeSavedWorkflowRun) => findProjectByCwd(localProjects, run.cwd) ?? null,
    [localProjects],
  );
