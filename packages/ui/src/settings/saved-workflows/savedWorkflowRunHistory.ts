...
import type { MesacodeSavedWorkflowRun, MesacodeSavedWorkflowRunStatus } from "@mesacode/shared";
...
): Map<string, MesacodeSavedWorkflowRun> {
  const byName = new Map<string, MesacodeSavedWorkflowRun>();
...
export function savedWorkflowRunBadgeKind(
  status: MesacodeSavedWorkflowRunStatus | "errored" | "stopped" | "failed" | "cancelled" | undefined,
...
export function savedWorkflowRunDurationMs(run: MesacodeSavedWorkflowRun, now: number): number {
