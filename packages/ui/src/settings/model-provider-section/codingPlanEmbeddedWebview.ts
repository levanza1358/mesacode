...
import type { CodingPlanWebviewLocale } from "@mesacode/shared";
...
const CODING_PLAN_REPORT_CONTEXT_STORAGE_KEY = "mesacode:coding-plan:report-context";
...
export function resolveCodingPlanEmbeddedOrigin({
  ...
  if (
    ...
    return normalizeMesacodeEndpointOrigin(normalizedOverride);
  }
  const normalizedEndpointOrigin = normalizeMesacodeEndpointOrigin(endpointOrigin);
...
export function buildCodingPlanEmbeddedWebviewUrl({
  ...
  const url = new URL("/coding-plan", normalizeMesacodeEndpointOrigin(origin));
...
export function createCodingPlanAuthInjectionScript({
  ...
  // App 当前 locale，写入 window.__mesacodeLang__ 供 mesacodeBridge.getLang() 读取，
  ...
  const values: Record<string, string | null> =
    provider === "zai"
      ? {
          ...
          mesacodejwttoken: credentials.mesacodeJwtToken?.trim() || null,
          "oauth:bigmodel:access_token": null,
        }
      : {
          "oauth:zai:access_token": null,
          // mesacodejwttoken 是 mesacode-plan 域通用凭证（BigModel OAuth callback 同样落盘），
  ...
  window.__mesacodeLang__ = ${JSON.stringify(resolvedLocale)};
  const mesacodeReportContext = ${JSON.stringify(normalizedReportContext)};
  window.__mesacodeReportContext__ = mesacodeReportContext;
  localStorage.setItem(${JSON.stringify(CODING_PLAN_REPORT_CONTEXT_STORAGE_KEY)}, JSON.stringify(mesacodeReportContext));
  window.dispatchEvent(new CustomEvent("mesacode-coding-plan-auth-ready", {
    detail: { ...${JSON.stringify({ provider, locale: resolvedLocale })}, reportContext: mesacodeReportContext },
  }));
})()`;
}
...
export function createCodingPlanCredentialClearScript(): string {
  ...
  delete window.__mesacodeReportContext__;
})()`;
}

export function createCodingPlanScrollbarHideScript(): string {
  return `(() => {
  const styleId = "mesacode-coding-plan-hide-scrollbar";
 ...
 * 重写 window.__mesacodeLang__ 并派发 mesacode-coding-plan-lang-change 事件，
 * 官网侧（mesacodeBridge.onLangChange 或 window 监听）据此无感切换语言。
 */
export function createCodingPlanLangInjectionScript(locale: CodingPlanWebviewLocale): string {
  ...
  window.__mesacodeLang__ = ${JSON.stringify(resolvedLocale)};
  window.dispatchEvent(new CustomEvent("mesacode-coding-plan-lang-change", {
...
export function getCodingPlanCredentialKeys(provider: CodingPlanWebsiteProvider): string[] {
  // mesacodejwttoken 对两个 provider 都加载：它是 mesacode-plan 域通用凭证，
  // BigModel OAuth callback 同样落盘（见 resolveBigModelStartPlanZcodeJwt）。
  return provider === "zai"
    ? [`oauth:${ZAI_PROVIDER_ID}:access_token`, "mesacodejwttoken"]
    : [`oauth:${BIGMODEL_PROVIDER_ID}:access_token`, "mesacodejwttoken"];
}
