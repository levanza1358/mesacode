...
interface OffPeakTaskListProps {
  ...
  onOpen: (task: MesacodeOffPeakTask) => void;
  onPause: (task: MesacodeOffPeakTask) => void;
  onContinue: (task: MesacodeOffPeakTask) => void;
  onCancel: (task: MesacodeOffPeakTask) => void;
  onDelete: (task: MesacodeOffPeakTask) => void;
  onOpenSession: (task: MesacodeOffPeakTask) => void;
}

// 按状态分组会让任务在运行和终态切换时跳位，破坏用户对已有卡片位置的预期；
// 列表只按不可变的创建时间倒序，状态变化不再影响顺序。
function sortOffPeakTasksByCreatedAt(tasks: readonly MesacodeOffPeakTask[]): MesacodeOffPeakTask[] {
