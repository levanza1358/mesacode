...
import { compareDocumentPluginPriority, isPluginCommand, isUserCommand } from "@mesacode/shared";
...
export function selectBuiltInPlugins(
  ...
  installedPlugins: readonly MesacodeInstalledPluginSummary[],
): MesacodePluginInfo[] {
...
interface PluginSettingsGroups {
  installed: MesacodePluginInfo[];
  builtIn: MesacodePluginInfo[];
}
...
export function selectPluginsForScope(
  plugins: readonly MesacodePluginInfo[],
  _installedPlugins: readonly MesacodeInstalledPluginSummary[],
  _scope: MesacodePluginScope,
): MesacodePluginInfo[] {
...
export function selectSkillsForScope(
  ...
  scopedPlugins: readonly Pick<MesacodePluginInfo, "id" | "name" | "enabled">[],
...
export function selectCommandsForScope(
  commands: readonly MesacodeCommand[],
  scopedPlugins: readonly Pick<MesacodePluginInfo, "id" | "name" | "enabled">[],
  scope: MesacodePluginScope,
): MesacodeCommand[] {
  const enabledPlugins = scopedPlugins.filter((plugin) => plugin.enabled);
