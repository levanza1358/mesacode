...
import {
  ...
  type MesacodeAutomationScheduleRule,
...
interface AutomationEditViewProps {
  /** null = 新建；否则编辑。 */
  editing: MesacodeAutomation | null;
  ...
  onBack: () => void;
  /** 编辑态:立即运行 / 启停 / 删除。 */
  onRunNow?: (automation: MesacodeAutomation) => Promise<void> | void;
...
function resolveRunStatus(run: MesacodeAutomationRun): RunStatusKind {
...
function normalizeAutomationScheduleRule(
  rule: MesacodeAutomationScheduleRule | null | undefined,
...
export function AutomationEditView({
  ...
  const modelSelectGroups = useMemo(() => {
    ...
    return buildAutomationModelSelectGroups({
      selectedProvider: MESACODE_AGENT_PROVIDER,
      labels: {
  ...
  const currentScheduleRule = useMemo<MesacodeAutomationScheduleRule | undefined>(
    () =>
