...
import { TID_PLUGIN_STORE_BROWSE } from "@mesacode/shared";
import type { MesacodePluginInfo, MesacodePluginScope, MesacodePluginUserConfigOption } from "@mesacode/shared";
...
function PluginList({
  ...
  const getPluginOptionValue = (
    ...
    key: string,
    option: MesacodePluginUserConfigOption,
  ...
  const savePluginOptions = async (plugin: MesacodePluginInfo) => {
  ...
  const renderPluginRows = (items: MesacodePluginInfo[]) => (
  ...
  return (
    <section className="space-y-6" aria-labelledby="plugin-installed-title">
      ...
      <RemoteSyncDialogs
        ...
        remoteMesacodeAgentService={targetServiceResolution.services.mesacodeAgentService}
