...
import { isCronTask, isOffPeakTask, type MesacodeTaskMeta } from "@mesacode/shared";
import { TID_TASK_ARCHIVE, TID_TASK_ITEM, testId } from "@mesacode/shared";
...
function getTaskAutomationIdentity(task: MesacodeTaskMeta): string | undefined {
  return task.cronAutomationId ?? (task as MesacodeTaskMeta & { automationId?: string }).automationId;
}

function areTaskListItemTaskFieldsEqual(left: MesacodeTaskMeta, right: MesacodeTaskMeta) {
  if (left === right) {
    return true;
  }
...
export const MemoTaskItem = memo(function TaskListItem({
  ...
  const handleDragStart = useCallback(
    (event: React.DragEvent<HTMLLIElement>) => {
      ...
      const payload = {
        kind: "mesacode/session" as const,
        workspacePath,
        ...(task.workspaceIdentity?.trim() ? { workspaceIdentity: task.workspaceIdentity } : {}),
        ...(remoteSessionId ? { remoteSessionId } : {}),
        sessionId: task.taskId,
