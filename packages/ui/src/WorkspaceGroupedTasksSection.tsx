...
import type { MesacodeGroupedTaskView, MesacodeTaskGroupColor } from "@mesacode/services";
import { OFF_PEAK_DEFAULT_GROUP_ID, type MesacodeTaskMeta } from "@mesacode/shared";
...
function getGroupedTaskViewSignature(view: MesacodeGroupedTaskView): string {
  return view.nodes
    .map((node) =>
      node.type === "task"
        ? `t:${taskKey(node.task)}`
...
export function WorkspaceGroupedTasksSection({
  ...
  const removeTaskState = useMesacodeSessionStore((state) => state.removeTaskState);
  const upsertOptimisticTaskListItem = useMesacodeSessionStore(
    (state) => state.upsertOptimisticTaskListItem,
  );
  const setTaskUnreadIndicator = useMesacodeSessionStore((state) => state.setTaskUnreadIndicator);
  const groupedDraftTask = useMesacodeSessionStore(
    (state) =>
      selectWorkspaceMesacodeState(state, activeWorkspacePath, activeWorkspaceIdentity)
        .groupedDraftTask,
  );
  const groupedDraftFocusVersion = useMesacodeSessionStore(
    (state) =>
      selectWorkspaceMesacodeState(state, activeWorkspacePath, activeWorkspaceIdentity)
        .draftFocusVersion,
  );
  const clearGroupedDraftTask = useMesacodeSessionStore((state) => state.clearGroupedDraftTask);
  const {
  ...
  const dragOriginViewRef = useRef<MesacodeGroupedTaskView | null>(null);
  const dragPreviewViewRef = useRef<MesacodeGroupedTaskView | null>(null);
  ...
  const handleStartRenameTask = useCallback((task: MesacodeTaskMeta) => {
    setRenamingTaskKey(taskKey(task));
    setRenameDraft(task.title ?? "");
  }, []);

  const handleMoveTaskToGroup = useCallback(
    (task: MesacodeTaskMeta, groupId: string | null) => {
  ...
  const handleSubmitRenameTask = useCallback(async () => {
    ...
    try {
      const meta = await workspaceServices.services.mesacodeTaskService.renameTask({
        taskId: task.taskId,
  ...
  const handleMarkTaskAsUnread = useCallback(
    (task: MesacodeTaskMeta) => {
      ...
      void workspaceServices.services.mesacodeTaskService
        .setTaskUnread({
          taskId: task.taskId,
          workspacePath: task.workspacePath,
          unread: true,
  ...
  const handleUpdateGroupColor = useCallback(
    (groupId: string, color: MesacodeTaskGroupColor) => {
  ...
  const setViewWithGroupedTaskAnimation = useCallback(
    (nextView: MesacodeGroupedTaskView) => {
