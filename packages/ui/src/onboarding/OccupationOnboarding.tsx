...
import type { OnboardingRecordEntry } from "@mesacode/shared";
...
export function OccupationOnboarding({
  ...
  const requested = useMesacodeStore((state) => state.newUserOnboardingOpen);
  const setRequested = useMesacodeStore((state) => state.setNewUserOnboardingOpen);
  // 登录态变化（useRootOAuthEffects 登录成功后 setUser）时按 userId 重新判定是否触发引导。
  const userId = useMesacodeStore((state) => state.user?.id) ?? null;
  ...
  const savedInterfaceMode = useMesacodeStore((state) => state.interfaceMode);
  ...
  const requestOnboardingDialog = useMesacodeStore((state) => state.requestOnboardingDialog);
