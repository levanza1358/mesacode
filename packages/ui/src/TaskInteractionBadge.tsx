...
import type { MesacodeTaskPendingInteraction } from "@mesacode/shared";
import { cn } from "@/components/lib/utils.js";

interface TaskInteractionBadgeProps {
  interaction?: MesacodeTaskPendingInteraction;
...
function isAskUserQuestionInteraction(
  interaction: MesacodeTaskPendingInteraction | undefined,
): boolean {
...
export function TaskInteractionBadge({
  ...
  const style =
    progress !== undefined
      ? ({
          "--mesacode-interaction-progress": String(progress),
          "--mesacode-interaction-remaining-ms": `${remainingMs}ms`,
        } as CSSProperties)
      : undefined;
  ...
  const badgeContent = (
    <>
      {progress !== undefined ? (
        <span
          aria-hidden="true"
          className="mesacode-task-interaction-countdown-fill absolute inset-0 origin-left bg-interaction-ask-fill transition-opacity group-hover/interaction-badge:opacity-0 group-focus-visible/interaction-badge:opacity-0 motion-reduce:transition-none"
        />
      ) : null}
      {canSnooze ? (
