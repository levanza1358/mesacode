...
import type { GitChangeSourceId, WorkspacePurpose } from "@mesacode/shared";
import { useMesacodeStore } from "@/store/StoreProvider.js";
import { getVisibleTaskMetas, useMesacodeSessionStore } from "@/store/mesacodeSessionStore.js";
...
import { MESACODE_PRODUCT_DOCS_URL } from "@/lib/productDocs.js";
import appLogoUrl from "@/assets/provider-icons/logo-mesa-code.svg";
...
import { useWorkspaceShellMesacodeState } from "@/app-shell/useWorkspaceShellMesacodeState.js";
...
export function App({
  ...
  const { intl, locale, setLocale } = useMesacodeIntl();
  ...
  const workspaceReadOnlyReason = workspaceReadOnly
    ...
    : undefined;
  const { workspaceShellMesacodeState, reloadSessionDisabled } = useWorkspaceShellMesacodeState(
    workspaceAbsPath,
    workspaceIdentity,
  );
  const activeTaskId = workspaceShellMesacodeState.activeTaskId;
  ...
  const draftSessionId = useMesacodeSessionStore(
  ...
  const draftFocusVersion = workspaceShellMesacodeState.draftFocusVersion;
  const {
    isTerminalOpen,
    setIsTerminalOpen,
    sidePaneState,
  ...
  const notificationEnabled = useMesacodeStore((s) => s.notificationEnabled);
  ...
  const theme = useMesacodeStore((s) => s.theme);
  const setTheme = useMesacodeStore((s) => s.setTheme);
  const {
    isMacFullscreen,
  ...
  const {
    ...
    workspaceRemoteSessionId,
    workspaceIdentity,
    selectedProvider: workspaceShellMesacodeState.selectedProvider,
    intl,
  });
  ...
  const handleOpenProductDocs = useCallback(() => {
    platform.openExternal(MESACODE_PRODUCT_DOCS_URL);
  }, [platform]);
  ...
  const handleStartDraftInWorkspace = useCallback(
    (
      ...
      const targetSelectedProvider = resolveWorkspaceSwitchDraftProvider({
        currentSelectedProvider: workspaceShellMesacodeState.selectedProvider,
        targetWorkspacePath,
    ...
    [
      ...
      workspaceAbsPath,
      workspaceIdentity,
      workspaceShellMesacodeState.selectedProvider,
    ],
  );
  ...
  const testActions = useMemo<TestActions>(
    () => ({
      ...
      getPluginsOverview: (params) => services.mesacodeAgentService.getPluginsOverview(params),
      addPluginMarketplace: (params) => services.mesacodeAgentService.addPluginMarketplace(params),
      updatePluginMarketplace: (params) =>
        services.mesacodeAgentService.updatePluginMarketplace(params),
      installPlugin: (params) => services.mesacodeAgentService.installPlugin(params),
      listPlugins: (params) => services.mesacodeAgentService.listPlugins(params),
      getPluginReferenceCatalog: (params) =>
        services.mesacodeAgentService.getPluginReferenceCatalog(params),
    }),
    [locale, services.mesacodeAgentService, setLocale, theme, setTheme, testMessages],
  );
  ...
  const handleSelectAdjacentConversation = useCallback(
    (direction: "previous" | "next") => {
      runVisibleWorkspaceCommand(() => {
        const sessionState = useMesacodeSessionStore.getState();
  ...
  return (
    <>
      ...
      <WorkspaceShellLayout
        ...
        workspaceShellMesacodeState={workspaceShellMesacodeState}
        theme={theme}
