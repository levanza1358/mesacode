import { formatLogPrefix } from "@mesacode/shared";
  ...
  return (
    (
      globalThis as typeof globalThis & {
        __MESACODE_RENDERER_DISABLE_LOGGING__?: boolean;
      }
    ).__MESACODE_RENDERER_DISABLE_LOGGING__ === true
  );
}
...
function log(level: LogLevel, ...args: unknown[]) {
  ...
  // ui 包单独 typecheck 时拿不到 desktop renderer 注入的 window.mesacode 声明，
  ...
  if (level !== "debug" && typeof window !== "undefined") {
    ...
    (window as DesktopLogBridgeWindow).mesacode?.log?.(level, args);
  }
}
...
export function logMemoryDiagnostics(line: string): void {
  ...
  if (typeof window !== "undefined") {
    const bridge = (window as DesktopLogBridgeWindow).mesacode?.log;
    if (bridge) {
      bridge("info", [line]);
      return;
    }
