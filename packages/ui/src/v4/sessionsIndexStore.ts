...
import { isMesacodeFileLockTimeoutError } from "@mesacode/shared";
import { MESACODE_AGENT_RUNTIME_UNAVAILABLE_CODE } from "@mesacode/services";
...
function isTransientSubscribeError(error: unknown): boolean {
  if (isMesacodeFileLockTimeoutError(error)) {
    return true;
  }
  const code =
