...
import { MESACODE_AGENT_PROVIDER, resolveExecutionState } from "@mesacode/shared";
...
import type { SessionConfigState } from "@mesacode/shared/mesacode-protocol-v4";
import type { IModelSelectionService } from "@mesacode/services";
...
import { submissionModeSchema } from "@mesacode/shared/mesacode-protocol-v4";
...
function shouldHydrateWorkspaceCatalog(params: {
  configOptions: readonly MesacodeConfigOption[];
  sessionId: string | null;
  slashCommands: readonly MesacodeSlashCommand[];
}): boolean {
...
export function useDraftConfigControl(params: {
  ...
  const mesacodeSessionService = useMesacodeSessionService(workspacePath, null, workspaceIdentity);
  ...
  useEffect(() => {
    ...
    const flight = prepareWorkspaceWithMesacodeSessionService({
      workspacePath,
      workspaceIdentity,
      ...
      .then((prepareResult) => {
        ...
        const latest = useMesacodeSessionStore.getState();
