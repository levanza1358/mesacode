...
import type { ConversationTelemetryFact } from "@mesacode/shared/mesacode-protocol-v4";
...
export interface ConversationPromptTelemetrySeed {
  localTtft?: import("@mesacode/shared").LocalTtftContext;
...
function toLegacyNetworkEvent(
...
): Extract<MesacodeStreamEvent, { type: "task_network_debug_status" }> {
  return {
  ...
  } as Extract<MesacodeStreamEvent, { type: "task_network_debug_status" }>;
}
...
function toToolLifecycleEvent(input: {
  ...
  if (!input.hasStarted && fact.phase !== "completed" && fact.phase !== "failed") {
    ...
    } as Extract<MesacodeStreamEvent, { type: "tool_call" }>;
  }
  return {
    ...
    raw: {},
    ...(skillMetadata ? { skillMetadata } : {}),
  } as Extract<MesacodeStreamEvent, { type: "tool_call_update" }>;
}

function toUsage(fact: Extract<ConversationTelemetryFact, { kind: "usage.delta" }>): MesacodeUsage {
  return {
...
function mergeUsage(left: MesacodeUsage | null, right: MesacodeUsage): MesacodeUsage {
...
export class ConversationTelemetrySupervisor {
  ...
  private processPromptFact(
    ...
    switch (fact.kind) {
      ...
      case "stream.chunk": {
        ...
        } as Extract<MesacodeStreamEvent, { type: "agent_thought_chunk" | "agent_message_chunk" }>;
  ...
  private handleTerminal(
    ...
    const terminalEvent = // 修复原因：旧 adapter 把所有 TurnComplete（包括 cancelled）先投影成
      ...
      ) as Extract<MesacodeStreamEvent, { type: "task_complete" | "task_error" }>;
    this.reportFinalizedSteps(
      lifecycle,
      recordAgentStepTelemetryEvent({
        taskId: lifecycle.taskKey,
  ...
  private handleCompaction(
    ...
    const timeline: MesacodeContextCompactionTimelineMeta = {
      version: 1,
      kind: "synthetic",
