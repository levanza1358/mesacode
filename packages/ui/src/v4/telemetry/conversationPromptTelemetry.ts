import { MESACODE_AGENT_PROVIDER, type PlanIdentitySnapshot, type MesacodeProvider } from "@mesacode/shared";
import { buildPromptTelemetryExtraDetail } from "@/lib/messageTelemetry.js";
import { encodeCustomModelValue } from "@/lib/mesacodeCustomModelValue.js";
import {
...
function resolveLegacyConversationModelValue(params: {
  ...
  if (!configProvider || !modelName || configProvider === MESACODE_AGENT_PROVIDER) {
...
/** V4 config.provider 是实际模型 provider id；agentProvider 表示 Mesacode 运行时。 */
export function buildV4ConversationPromptTelemetryExtraDetail(params: {
  agentProvider?: MesacodeProvider;
  configProvider?: string | null;
  modelName?: string | null;
  askMode?: string | null;
  ...
  const agentProvider = params.agentProvider ?? MESACODE_AGENT_PROVIDER;
  ...
  return {
    ...
    // agent 表示 Mesacode 运行时，不能用模型 provider id 替代。
    agent: agentProvider,
  };
}
