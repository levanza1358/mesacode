...
import { resolveWorkspaceTelemetryDetail, type IPlatformService } from "@mesacode/shared";
import { createConversationTelemetryService, type IServiceAccessor } from "@mesacode/services";
...
function acquireSupervisor(
  ...
  const key = attachmentScopeKey(scope, services.mesacodeAgentService);
...
function ensureSupervisorSubscription(
  ...
  const telemetryService = createConversationTelemetryService(services.mesacodeAgentService);
...
export function ConversationTelemetryWorkspaceAttachment({
  ...
  const lease = useMemo(() => {
    const agentService = services.mesacodeAgentService as object | null | undefined;
