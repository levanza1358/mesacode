...
const RUNTIME_RECYCLED_ERROR = "Mesacode agent runtime 已被回收，重连未成功";
 ...
 *
 * 三种文案都来自同一次回收：transport 关闭时 MesacodeProtocolClient.rejectAll 打断在途请求
...
function isRuntimeRecycleError(error: unknown): boolean {
  ...
  return (
    ...
    message.includes("Mesacode agent transport closed") ||
    message.includes("Mesacode Protocol client disposed") ||
    message.includes("Mesacode Protocol client is disposed") ||
    message.includes("Mesacode Agent runtime is not running")
  );
}
...
export class ConversationProjectionStore {
  ...
  /**
   * 有界退避重连。重订阅走 start-if-needed（mesacodeAgentService.subscribeConversationV4），
