...
interface BuildTaskListParams {
  ...
  terminalStatusByTaskId?: ReadonlyMap<
    string,
    Extract<MesacodeTaskMeta["status"], "completed" | "error">
  >;
...
function mergeTaskIndexRowWithSession(
  taskIndexTask: MesacodeTaskMeta,
  sessionTask: MesacodeTaskMeta,
): MesacodeTaskMeta {
...
export function buildTaskListResult(params: BuildTaskListParams): BuildTaskListResult {
  ...
  filtered.sort((a, b) => compareMesacodeTaskListItems(a, b, params.sortBy));
