import { BufferWriter, serialize } from "@mesacode/rpc";
...
interface AttachmentUploadAgent {
  ...
  attachmentCommitV4(params: MesacodeAgentAttachmentTerminalParams): Promise<V4AttachmentPutResult>;
...
function measureAttachmentChannelRequestBytes(method: string, params: unknown): number {
  ...
  serialize(writer, [100, 2_147_483_647, ServiceChannels.MesacodeAgent, method]);
...
export async function uploadAttachmentTransaction(
  ...
  const beginParams: MesacodeAgentAttachmentBeginParams = {
    ...common,
  ...
  try {
    ...
    for (let chunkIndex = begin.nextChunkIndex; chunkIndex < totalChunks; chunkIndex += 1) {
      ...
      const chunkParams: MesacodeAgentAttachmentChunkParams = {
        ...common,
        chunkIndex,
    ...
    const terminal = common satisfies MesacodeAgentAttachmentTerminalParams;
