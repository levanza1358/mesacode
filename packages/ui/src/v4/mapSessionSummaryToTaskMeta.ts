// sessions-index 的 SessionSummary → 侧栏实时 detail MesacodeTaskMeta 映射。
...
import type { TraceId, MesacodeProvider, MesacodeTaskMeta } from "@mesacode/shared";
import type { SessionSummary } from "@mesacode/shared/mesacode-protocol-v4";
import {
...
function phaseToStatus(phase: SessionSummary["phase"]): MesacodeTaskMeta["status"] {
  switch (phase) {
    case "running":
    case "prewarming":
      return "running";
...
interface MapSessionSummaryOptions {
  ...
  previous?: MesacodeTaskMeta;
}

/**
 * SessionSummary → 实时 detail MesacodeTaskMeta。sessions-index 不携带的字段
  ...
  return attachTaskListRowActivity(
    {
      ...
      ...(previous?.provider ? { provider: previous.provider as MesacodeProvider } : {}),
      ...(status ? { status } : {}),
