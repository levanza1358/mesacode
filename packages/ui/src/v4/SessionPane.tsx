...
import type { SessionCreateSource } from "@mesacode/shared";
...
import { localizeConversationShareUrl } from "@mesacode/shared";
import type {
...
import { prepareWorkspaceWithMesacodeSessionService } from "@/hooks/useWorkspacePrepare.js";
import {
  createCodingPlanFunnelContext,
  resolveCodingPlanEntryPlanState,
} from "@/lib/codingPlanFunnelTelemetry.js";
import { decodeCustomModelValue, encodeCustomModelValue } from "@/lib/mesacodeCustomModelValue.js";
import { parseModelPickerValue } from "@/lib/mesacodeSessionProjection.js";
...
import type { GroupedDraftTaskState } from "@/store/mesacodeSessionStoreTypes.js";
import {
...
export function SessionPane({
  ...
  const { conversationShareService, modelSelectionService, mesacodeSessionService, mesacodeTaskService } =
    useServices();
  ...
  const [sendSubmissionError, setSendSubmissionError] = useState<MesacodeUiError | null>(null);
  ...
  const workspaceConfigOptions = useMesacodeSessionStore(
  ...
  useEffect(() => {
    ...
    useMesacodeSessionStore.getState().invalidateDraftRuntime(workspacePath, workspaceIdentity);
  ...
  const promoteGroupedDraftTask = useMesacodeSessionStore((state) => state.promoteGroupedDraftTask);
  ...
  const dispatchCommand = useCallback(
    async (
      ...
      const groupedDraftTask =
        type === "createSession"
          ? useMesacodeSessionStore.getState().getWorkspaceState(workspacePath, workspaceIdentity)
              .groupedDraftTask
          : null;
    ...
    async (reference?: ConversationSelectionReference, forceNew = false) => {
      ...
      try {
        ...
        if (targetChildSessionId) {
          try {
            await mesacodeSessionService.readSession({
              workspacePath,
              ...(workspaceIdentity ? { workspaceIdentity } : {}),
  ...
  const dispatchSendText = useCallback(
    (text: string, options?: ConversationComposerSendOptions) => {
      const createSource = useMesacodeSessionStore
        .getState()
    ...
    async (
      ...
      try {
        ...
        setSendSubmissionError({
          code: runtimeModelUnavailable ? "MESACODE_RUNTIME_MODEL_UNAVAILABLE" : "SEND_FAILED",
  ...
  const handleRecoverCustomModelSelection = useCallback(
    async (value: string, sourceModel: ModelSelectionSource | null) => {
      ...
      const displayProvider = provider ?? MESACODE_AGENT_PROVIDER;
      ...
      const store = useMesacodeSessionStore.getState();
      ...
      try {
        await mesacodeTaskService.restartWorkspaceProcess({
          workspacePath,
          workspaceIdentity,
        ...
        const prepareResult = await prepareWorkspaceWithMesacodeSessionService({
          workspacePath,
          workspaceIdentity,
          provider: displayProvider,
          mesacodeSessionService,
      ...
      >
        ...
        {readOnly && controlLastError ? (
          <div
            ...
            className="mx-4 mt-3 rounded-lg border border-(--color-border) bg-surface-hover px-3 py-2 text-ui-base text-(--color-danger)"
          >
          ...
          >
            <ConversationTimeline
              ...
              headerSlot={
                ...
                // rows 为空，但只读块必须留下来显示「需要更新 Mesacode」，不能整块消失。
                importedShare &&
