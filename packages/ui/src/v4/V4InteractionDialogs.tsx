...
import type { MesacodeElicitationRequest, MesacodePermissionOption, MesacodeProvider } from "@mesacode/shared";
...
import type { ElicitationFormDraft } from "@/store/mesacodeSessionStoreTypes.js";
...
function buildV4ElicitationProgressKey(request: MesacodeElicitationRequest): string {
...
export function V4InteractionDialogs({
  ...
  const notificationEnabled = useMesacodeStoreWithDefault((state) => state.notificationEnabled, true);
  const localElicitationDraft = useMesacodeSessionStore((state) => {
  ...
  useEffect(() => {
    ...
    const taskUiState = getTaskUiState(
      getWorkspaceState(useMesacodeSessionStore.getState(), workspacePath, workspaceIdentity),
      sessionId,
    );
  ...
  if (pending.payload.kind === "permission") {
    ...
    return (
      ...
      <PermissionDialog
        ...
        onRespond={(_requestId, option: MesacodePermissionOption, feedback?: string) => {
