  ...
  const revalidateAndPrependPlugin = useCallback(
    async (current: DraftSuggestedPluginFlow, requestVersion: number) => {
      await invalidateDeferredDraftSessionForSkillChange({
        mesacodeSessionService: resolution.services.mesacodeSessionService,
        workspacePath,
        workspaceIdentity,
    ...
    [
      ...
      resolution.services.mesacodeSessionService,
      targetParams,
      workspaceIdentity,
      workspacePath,
    ],
      ...
      if (
        ...
        (kind === "install" &&
          (!flow.result?.pluginName ||
            flow.result.marketplace !== MESACODE_OFFICIAL_PLUGIN_MARKETPLACE_ID ||
