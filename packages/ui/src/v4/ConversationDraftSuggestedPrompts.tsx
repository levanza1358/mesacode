...
export function ConversationDraftSuggestedPrompts({
  ...
  const { locale, intl } = useMesacodeIntl();
    ...
    return (
      ...
      >
        <div className="flex items-center justify-between gap-2 px-3 pb-2 text-ui-sm text-foreground-subtle">
          <span>{intl.formatMessage({ id: "chat.officeSuggestions.setting" })}</span>
            ...
            <Popover key={item.id} open={isPopoverAnchor} modal={false}>
              ...
              >
                <Button
                  ...
                  className="mesacode-draft-prompt-waterfall h-8 min-w-0 justify-start gap-1.5 overflow-hidden rounded-lg px-3 text-left text-ui-caption font-normal leading-4.5 text-foreground"
                  style={
                    {
                      "--mesacode-draft-prompt-waterfall-delay": `${index * 65}ms`,
                    } as CSSProperties
                  }
                >
                  <span
