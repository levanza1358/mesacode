...
// 复用 treemappingActivity 的既有解析规则，不复活 mesacodeChatMessages。
//
...
import type { ConversationRow, ToolCallRow } from "@mesacode/shared/mesacode-protocol-v4";
  ...
  const layer = useMemo(
    () =>
      new SessionDataLayer({
        transport: createAgentConversationTransport(mesacodeAgentService, {
          workspacePath,
          workspaceIdentity,
        }),
      }),
    [mesacodeAgentService, workspacePath, workspaceIdentity],
  );
  useEffect(() => {
