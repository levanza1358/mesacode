...
export function trajectoryToolOutputs(message: MesacodeModelTrajectoryMessage): string[] {
  return message.parts.flatMap((part) =>
    part.kind === "tool-result" ? [formatTrajectoryToolPayload(part.output)] : [],
  );
}

export function trajectoryToolCallInputs(message: MesacodeModelTrajectoryMessage): string[] {
  return message.parts.flatMap((part) =>
    part.kind === "tool-call" ? [formatTrajectoryToolPayload(part.input)] : [],
  );
}

export function trajectoryToolHasError(message: MesacodeModelTrajectoryMessage): boolean {
...
export function trajectoryToolMetadata(message: MesacodeModelTrajectoryMessage): {
  names: string;
  ids: string;
} {
