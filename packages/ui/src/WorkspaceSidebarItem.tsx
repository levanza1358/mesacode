...
const EMPTY_PINNED_TASKS: MesacodeTaskMeta[] = [];
...
export const WorkspaceSidebarItem = memo(function WorkspaceSidebarItem({
  ...
  const workspaceMesacodeState = useMesacodeSessionStore((state) =>
    selectWorkspaceMesacodeState(state, tab.workspacePath, tab.workspaceIdentity),
  );
  const activeTaskId = workspaceMesacodeState.activeTaskId;
  ...
  const removeOptimisticTaskListItem = useMesacodeSessionStore(
  ...
  const mesacodeTaskService = services.mesacodeTaskService;
  const taskItemsRef = useRef(taskItems);
  taskItemsRef.current = taskItems;
  const workspaceMesacodeStateRef = useRef(workspaceMesacodeState);
  workspaceMesacodeStateRef.current = workspaceMesacodeState;
  ...
  const handleRemoveWorkspace = useCallback(async () => {
    ...
    if (
      hasRunningWorkspaceChat({
        workspaceState: workspaceMesacodeStateRef.current,
  ...
  const handleRenameTask = useCallback(
    async (taskId: string, title: string) => {
      ...
      let meta: MesacodeTaskMeta;
      try {
        meta = await mesacodeTaskService.renameTask({
          taskId,
          workspacePath: tab.workspacePath,
          title,
          ...(tab.workspaceIdentity ? { workspaceIdentity: tab.workspaceIdentity } : {}),
  ...
  const handleSetTaskPinned = useCallback(
    async (taskId: string, pinned: boolean) => {
      ...
      try {
        const meta = await mesacodeTaskService.setTaskPinned({
          taskId,
          workspacePath: tab.workspacePath,
          pinned,
          ...(tab.workspaceIdentity ? { workspaceIdentity: tab.workspaceIdentity } : {}),
  ...
  const handleArchiveTask = useCallback(
    async (taskId: string) => {
      ...
      const previousTask = findCurrentTaskItem(taskId);
      const meta = await mesacodeTaskService.archiveTask({
        taskId,
        workspacePath: tab.workspacePath,
        ...(tab.workspaceIdentity ? { workspaceIdentity: tab.workspaceIdentity } : {}),
      });
  ...
  const handleSetTaskUnread = useCallback(
    async (taskId: string, unread: boolean) => {
      ...
      const previousTask = findCurrentTaskItem(taskId);
      const meta = await mesacodeTaskService.setTaskUnread({
        taskId,
        workspacePath: tab.workspacePath,
        unread,
        ...(tab.workspaceIdentity ? { workspaceIdentity: tab.workspaceIdentity } : {}),
  ...
  return (
    <li ref={itemRef} style={itemStyle} className="space-y-2">
      ...
      <RemoteSyncDialogs
        ...
        localMesacodeAgentService={baseServices.mesacodeAgentService}
        remoteMesacodeAgentService={services.mesacodeAgentService}
        remoteTarget={tab.remoteTarget}
        ...
        onPluginsSynced={async () => {
          await refreshWorkspacePluginCapabilitiesAfterRemoteSync({
            ...
            workspaceIdentity: tab.workspaceIdentity,
            workspacePath: tab.workspacePath,
            mesacodeAgentService: services.mesacodeAgentService,
            mesacodeSessionService: services.mesacodeSessionService,
          });
        }}
      />
