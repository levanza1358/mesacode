...
import { markZcodeJwtInvalidRestart } from "@/root/mesacodeJwtInvalidRestartMarker.js";
...
export function useRootOAuthEffects({
  ...
  useEffect(() => {
    ...
    const disposable = services.broadcastService.onMessage((message) => {
      if (message.channel !== MESACODE_JWT_INVALID_BROADCAST_CHANNEL || disposed) {
        return;
      }
      void (async () => {
        ...
        if (typeof window !== "undefined" && !("mesacode" in window)) {
