// composer parity：`#` 会话候选的数据源从旧 mesacodeSessionStore/taskQueryCache/remote*
...
import type { MesacodeProvider, MesacodeTaskMeta } from "@mesacode/shared";
...
function compareSessionTasks(
  left: MesacodeTaskMeta,
  right: MesacodeTaskMeta,
...
function getSessionLabel(task: MesacodeTaskMeta): string {
  const title = task.title.replace(/^#sess_[a-zA-Z0-9._-]+\s*/, "").trim();
  return title || "Untitled session";
}

function getWorkspaceLabel(task: MesacodeTaskMeta): string {
  const raw = task.workspacePath.trim() || task.workspaceIdentity?.trim() || "";
  return raw.split(/[\\/]/).filter(Boolean).at(-1) ?? raw;
}

function mapTaskToMentionItem(task: MesacodeTaskMeta, provider: MesacodeProvider): SessionMentionItem {
...
function collectSessionMentionItems(
  ...
  const currentWorkspaceKey = options.workspacePath
    ...
    : undefined;
  const taskBySessionId = new Map<string, MesacodeTaskMeta>();
...
function buildSessionMentionScopes(params: {
  ...
  const currentAgentService = params.currentServices.mesacodeAgentService;
  ...
  for (const candidate of candidates) {
    ...
    if (!resolved || resolved.services.mesacodeAgentService !== currentAgentService) {
      continue;
    }
    ...
    scopes.push({
      ...
      agentService: resolved.services.mesacodeAgentService,
    });
  }
      ...
      return [
        {
          ...
          agentService: workspaceServices.mesacodeAgentService,
        },
      ];
    }
