...
import { TID_TASK_LIST, TID_TASK_EMPTY, TID_TASK_SETTINGS_BUTTON } from "@mesacode/shared";
...
export const TaskList = memo(function TaskList({
  ...
  hasMore?: boolean;
  onShowMore?: () => void;
  onRenameTask: (taskId: string, title: string) => Promise<MesacodeTaskMeta | null>;
  onSetTaskPinned: (taskId: string, pinned: boolean) => Promise<MesacodeTaskMeta | null>;
  onArchiveTask: (taskId: string) => Promise<MesacodeTaskMeta | null>;
  onSetTaskUnread: (taskId: string, unread: boolean) => Promise<MesacodeTaskMeta | null>;
  ...
  const startDraft = useMesacodeSessionStore((state) => state.startDraft);
  ...
  const handleSubmitRenameTask = useCallback(
    async (taskId: string) => {
      ...
      let renamedTask: MesacodeTaskMeta | null;
      try {
  ...
  const visibleSourceTasks = useMemo(() => {
    const orderedTasks =
      sortBy === "manual"
        ? tasks
        : [...tasks].sort((left, right) => compareMesacodeTaskListItems(left, right, sortBy));
    return orderedTasks;
  }, [sortBy, tasks]);
  useEffect(() => {
