...
import { StoreProvider, useMesacodeStore } from "@/store/StoreProvider.js";
...
import { consumeZcodeJwtInvalidRestartMarker } from "@/root/mesacodeJwtInvalidRestartMarker.js";
...
function RootInner({
  ...
  const user = useMesacodeStore((state) => state.user);
  const isRestoringOAuthSession = useMesacodeStore((state) => state.isRestoringOAuthSession);
  const setUser = useMesacodeStore((state) => state.setUser);
  const setIsRestoringOAuthSession = useMesacodeStore((state) => state.setIsRestoringOAuthSession);
  const setOAuthError = useMesacodeStore((state) => state.setOAuthError);
  const oauthPollingActive = useMesacodeStore((state) => state.oauthPollingActive);
  const setOAuthPollingActive = useMesacodeStore((state) => state.setOAuthPollingActive);
  const markOAuthSuccess = useMesacodeStore((state) => state.markOAuthSuccess);
  const {
  ...
  const [providerFamilyDomainMigrationComplete, setProviderFamilyDomainMigrationComplete] =
    useState(false);
  const loginEntryRequest = useMesacodeStore((state) => state.loginEntryRequest);
  ...
  useEffect(() => {
    const disposable = services.broadcastService.onMessage((message) => {
      if (message.channel === APP_RUNTIME_PREFERENCES_CHANGED_BROADCAST_CHANNEL) {
        ...
        void services.mesacodeAgentService.syncAppRuntimePreferences(parsed.data).catch((error) => {
  ...
  }, [refreshAppSettings, services.broadcastService, services.mesacodeAgentService]);

  useEffect(() => {
    if (!appSettings) {
      return;
    }
    void services.mesacodeAgentService
  ...
  const totalUnreadTaskCount = useMesacodeSessionStore((state) =>
  ...
  // 这里把 provider 启动检查纳入 workspace 恢复门禁，避免未连接账号前启动 Mesacode session。
  ...
  if (isStartupRenderBlocked) {
    ...
    return (
      <RootShell>
        ...
        {/* HTML 启动壳已经展示 Mesacode SVG，但 React 接管 root 后旧壳会被整棵替换。
