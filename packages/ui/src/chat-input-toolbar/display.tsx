...
import {
  ...
  type MesacodeConfigSelectValue,
...
import { MESACODE_MODE_OPTION_DESCRIPTION_IDS, MESACODE_MODE_OPTION_LABEL_IDS } from "./display-help.js";
...
function isConfigSelectValueInOptions(option: MesacodeConfigOption, value: string): boolean {
  return option.options?.some((entry) => String(entry.value) === value) ?? false;
}

function getConfigSelectTriggerTestId(option: MesacodeConfigOption): string | undefined {
...
function getConfigSelectItemTestId(
  ...
  entry: MesacodeConfigSelectValue,
): string | undefined {
  if (option.category === "thought_level") {
...
export function ChatApiRetryStatus({
  ...
  const retryLabel = useMemo(() => {
    ...
    // 当前 Mesacode Agent 只会推送某一刻的 retryDelayMs 快照，不会每秒递减。
...
export function getModeOptionDisplayLabel(
  ...
  entry: Pick<MesacodeConfigSelectValue, "name" | "value">,
): string {
...
function getModeOptionLabelMessageId(
  provider: MesacodeProvider | undefined,
  entry: Pick<MesacodeConfigSelectValue, "value">,
): string | null {
  if (!provider) {
    return null;
  }

  return MESACODE_MODE_OPTION_LABEL_IDS[provider]?.[entry.value] ?? null;
}
...
export function getModeOptionDescriptionMessageId(
  ...
  return MESACODE_MODE_OPTION_DESCRIPTION_IDS[provider]?.[entry.value] ?? null;
}
