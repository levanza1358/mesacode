...
import { connectViaWebSocket } from "@mesacode/client";
...
import type { IPlatformService, RemoteTarget, ServerRemoteInfo } from "@mesacode/shared";
import { WEB_DEFAULT_THEME, resolveWebInitialTheme } from "./webThemeSeed.js";

function resolveWebThemePreference(defaultTheme: Theme = WEB_DEFAULT_THEME): Theme {
  const saved = localStorage.getItem("mesacode-theme");
...
function renderWebAuthCallbackPage(): void {
  document.title = "Mesacode - Sign In";
...
async function renderConversationSharePage(): Promise<void> {
  ...
  document.title = routeLocale === "zh-CN" ? "Mesacode 会话分享" : "Mesacode Conversation Share";
  ...
  const endpointOrigin =
    import.meta.env.VITE_MESACODE_BASE_URL?.trim().replace(/\/+$/u, "") || window.location.origin;
  const mockMode =
  ...
  const getMockToken = () =>
    mockMode && window.sessionStorage.getItem("mesacode:share:mock-auth") === "owner"
      ? "mock-owner-token"
      : null;
  const onLogout = () => {
    if (mockMode) {
      window.sessionStorage.removeItem("mesacode:share:mock-auth");
  ...
  root.render(
    <ConversationShareLandingLoader
      shareCode={shareCode}
      client={client}
      getAccessToken={() => getMockToken() ?? webAuthService.getMesacodeJwtToken()}
      onLogin={(provider) => {
        if (mockMode) {
          window.sessionStorage.setItem("mesacode:share:mock-auth", "owner");
          window.location.reload();
          return;
        }
...
async function bootstrapWebApp() {
  ...
  try {
    ...
    document.title = "Mesacode - Web + Server";
