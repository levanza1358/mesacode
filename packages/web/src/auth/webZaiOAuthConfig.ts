...
function createWebZaiOAuthConfig(env: WebImportMetaEnv = {}): WebZaiOAuthConfig {
  ...
  const mesacodeEndpointUrls = buildMesacodeEndpointUrls(
  ...
  return {
    ...
    // BigModel 用 appId 而不是 client_id，且默认值就是桌面端在用的 "mesacode"。
    bigmodelAppId: env.VITE_BIGMODEL_OAUTH_APP_ID?.trim() || "mesacode",
    redirectUri: mesacodeEndpointUrls.webShareCallbackUrl,
    shareRedirectUri: mesacodeEndpointUrls.webShareCallbackUrl,
    ...(devOrigin ? { devOrigin } : {}),
