...
export class BrowserOAuthCredentialRepo {
  ...
  saveTokenSet(tokenSet: WebZaiTokenSet | OAuthTokenSet, provider: WebOAuthProviderId): void {
    ...
    const mesacodeJwtToken = tokenSet.mesacodeJwtToken;

    this.localStorage.setItem(providerKeys(provider).accessToken, accessToken);
    if (mesacodeJwtToken) {
      this.localStorage.setItem(MESACODE_JWT_TOKEN_KEY, mesacodeJwtToken);
    } else {
      this.localStorage.removeItem(MESACODE_JWT_TOKEN_KEY);
    }
  }
  ...
  loadCachedSessionState(): OAuthCachedSessionRestoreResult {
    ...
    const mesacodeJwtToken = this.localStorage.getItem(MESACODE_JWT_TOKEN_KEY);
    ...
    if (!keys || !hasText(mesacodeJwtToken) || !hasText(accessToken) || !hasText(rawUserInfo)) {
    ...
    if (resolveJwtExpiration(mesacodeJwtToken, this.now()).kind === "expired") {
  ...
  loadMesacodeJwtToken(): string | null {
    const session = this.loadCachedSessionState();
    if (session.status !== "authenticated") return null;
    return this.localStorage.getItem(MESACODE_JWT_TOKEN_KEY)?.trim() || null;
  }

  private hasAnyStoredCredential(): boolean {
    return Boolean(
      this.localStorage.getItem(ACTIVE_PROVIDER_KEY) ||
      this.localStorage.getItem(MESACODE_JWT_TOKEN_KEY) ||
