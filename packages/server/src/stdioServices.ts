import { createLocalServices, type MesacodeAgentCommandResolver } from "@mesacode/services/node";
import {
...
function resolveRemoteAgentNetworkFromEnv(
  ...
  if (env[MESACODE_REMOTE_RUNTIME_NETWORK_AUTHORITY_ENV_KEY]?.trim() !== "1") {
    return undefined;
  }
  return {
    httpProxy: env[MESACODE_REMOTE_HTTP_PROXY_ENV_KEY]?.trim() || undefined,
    noProxy: env[MESACODE_REMOTE_NO_PROXY_ENV_KEY]?.trim() || undefined,
  };
}
...
export function createStdioServices(options: CreateStdioServicesOptions) {
  ...
  const services = createLocalServices({
    mesacodeBuiltinProviderConfigFilePath: options.mesacodeBuiltinProviderConfigFilePath,
    serviceAuthorityMode: authorityModeParseResult.mode,
    mesacodeAgentCommandResolver: options.mesacodeAgentCommandResolver,
    remoteAgentNetwork,
  });
