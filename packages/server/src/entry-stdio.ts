import { disposeServiceResourcesAndWait, getAppConfigDir } from "@mesacode/services/node";
import {
  MESACODE_VERSION,
...
const log = (...args: unknown[]) =>
  console.error(formatLogPrefix("mesacode-server:stdio", process.pid), ...args);
...
if (process.argv.includes("--version")) {
  process.stdout.write(MESACODE_VERSION + "\n");
  process.exit(0);
}
...
async function main() {
  ...
  // 远端主机没有 Desktop main，没人写 telemetry-state.json，services 发往 Mesacode endpoint
