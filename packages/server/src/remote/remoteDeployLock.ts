...
import { REMOTE_BASE } from "@mesacode/server/remote/deployShared.js";
...
function buildRemoteDeployLockScript(lockDir: string, ownerToken: string): string {
  const acquiredMarker = `mesacode-deploy-lock-acquired:${ownerToken}`;
  const releaseMarker = `mesacode-deploy-lock-release:${ownerToken}`;
  return [
    "set -eu",
