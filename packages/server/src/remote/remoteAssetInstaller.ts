...
import type { RemoteAssetInstallMode } from "@mesacode/shared";
import type { IRemoteBackend, StdioStream } from "@mesacode/server/remote/backend.js";
import {
  REMOTE_BASE,
...
import { quotePosixPathArg, quotePosixShellArg } from "@mesacode/server/remote/posixShell.js";
import {
  buildComponentArtifactUrlCandidates,
  buildReleaseAssetUrlCandidates,
  buildReleaseBaseCandidates,
  resolveRemoteCdnBaseUrls,
} from "@mesacode/server/remote/remoteAssetCdn.js";
import {
...
import type {
  ...
  RemoteDownloadTool,
  RemoteSha256Tool,
} from "@mesacode/server/remote/remoteAssetPreflight.js";
import {
  resolveRemoteAssetFetch,
  type RemoteAssetNetworkPort,
} from "@mesacode/server/remote/remoteAssetNetwork.js";
...
function buildStaleRemoteStagingCleanupCommand(parentDir: string, patterns: string[]): string {
  ...
  return [
    ...
    // 新连接只回收超过 24 小时的 Mesacode owner staging，避免误删当前 owner 或正常时长内的活跃部署。
...
export class LocalUploadAssetInstaller implements RemoteAssetInstaller {
  ...
  async installDirectory(params: {
    ...
    const localTarPath = join(
      tmpdir(),
      `mesacode-remote-${params.componentId}-${Date.now()}-${randomUUID()}.tar.gz`,
    );
