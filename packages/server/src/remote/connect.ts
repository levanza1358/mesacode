import { SocketProtocol, ChannelClient } from "@mesacode/rpc";
import type { IServiceAccessor } from "@mesacode/services";
import { RemoteServiceAccess } from "@mesacode/client";
import {
...
/**
 * Connect to a remote mesacode server via an IRemoteBackend.
 *
 * Steps:
...
async function connectRemoteUnchecked(
  ...
  stream.stderr.on("data", (chunk: Buffer) => {
    // 远端 mesacode-server 的服务日志走 stderr，直接写 host stderr 时可能被结构化日志中继吞掉。
...
function buildRemoteServerCommand(
  ...
  const envParts = [
    ...
    'MESACODE_SERVER_RUNTIME_ROOT="$HOME/.mesacode/server"',
  ];
  ...
  if (appVersion) {
    ...
    envParts.push(`${MESACODE_APP_VERSION_ENV}=${quotePosixShellArg(appVersion)}`);
  }
  if (remoteRuntimeNetwork?.authoritative) {
    envParts.push(`${MESACODE_REMOTE_RUNTIME_NETWORK_AUTHORITY_ENV_KEY}='1'`);
    if (remoteRuntimeNetwork.httpProxy !== undefined) {
      envParts.push(
        `${MESACODE_REMOTE_HTTP_PROXY_ENV_KEY}=${quotePosixShellArg(remoteRuntimeNetwork.httpProxy)}`,
      );
    }
    if (remoteRuntimeNetwork.noProxy !== undefined) {
      envParts.push(
        `${MESACODE_REMOTE_NO_PROXY_ENV_KEY}=${quotePosixShellArg(remoteRuntimeNetwork.noProxy)}`,
      );
    }
  }
  return `${envParts.join(" ")} ~/.mesacode/server/node ~/.mesacode/server/mesacode-server.cjs`;
}
