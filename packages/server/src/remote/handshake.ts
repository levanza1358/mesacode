import type { HelloMessage, HelloAckMessage } from "@mesacode/shared";
import { MESACODE_VERSION, formatZodError, helloMessageSchema } from "@mesacode/shared";
 ...
 * 1. Read lines from stdout until we find a mesacode-hello JSON
 *    (skip SSH banner/motd lines)
 * 2. Send a mesacode-hello-ack to stdin
...
export function performHandshake(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      rejectWithDiagnostics("Handshake timeout: no mesacode-hello received within timeout");
    }, timeoutMs);
        ...
        if (line.startsWith("{")) {
          try {
            ...
            if (
              ...
              "type" in rawValue &&
              (rawValue as { type?: unknown }).type === "mesacode-hello"
            ) {
              rejectWithDiagnostics(`Invalid mesacode-hello: ${formatZodError(result.error)}`);
              return;
            }
          } catch {
