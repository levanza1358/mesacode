...
import { deployMesacodeAgentRuntime } from "./mesacodeAgentDeploy.js";
import {
  deployNodePtyPrebuilds,
  deployNodeRuntime,
  createRemoteComponentVersionResolver,
  logDeployRequired,
} from "@mesacode/server/remote/remoteAssetDeployDecision.js";
import {
  REMOTE_BASE,
  fileExists,
  formatOptionalValue,
...
import { quotePosixPathArg } from "@mesacode/server/remote/posixShell.js";
import { checkServerBundleRequiredMarkers } from "@mesacode/server/remote/serverBundleDeployCheck.js";
import { deployRuntimeTools } from "@mesacode/server/remote/runtimeToolDeploy.js";
import { REMOTE_AGENT_OFFICIAL_PLUGIN_REQUIRED_RELATIVE_PATHS } from "@mesacode/server/remote/mesacodeAgentOfficialPluginAssets.js";
import {
  ensureRemoteReleaseDirFromCdn,
  selectRemoteAssetManifestComponents,
  type RemoteAssetManifestRef,
} from "@mesacode/server/remote/remoteAssetCache.js";
import {
...
} from "@mesacode/server/remote/remoteAssetInstaller.js";
import {
...
import { detectRemoteAssetTools } from "@mesacode/server/remote/remoteAssetPreflight.js";
import { assertSupportedRemoteEnvironment } from "@mesacode/server/remote/remotePlatformSupport.js";
import { acquireRemoteDeployLock } from "@mesacode/server/remote/remoteDeployLock.js";
import type { RemoteAssetNetworkPort } from "@mesacode/server/remote/remoteAssetNetwork.js";
...
/**
 * Deploy the mesacode server to the remote machine.
...
export async function deployServer(
  ...
  const deployWithDecision = async (
    ...
    if (!serverDeployDecision.shouldDeploy) {
      ...
      // 主 server 版本相同只证明 node/mesacode-server.cjs 可启动，不代表随包工具仍存在。
      ...
      await deployMesacodeAgentRuntime(
        backend,
        env,
        {
          ...assetDeployOptions,
    ...
    await installer.installFile({
      componentId: SERVER_BUNDLE_COMPONENT_ID,
      sourceRelativePath: "server/mesacode-server.cjs",
      remotePath: `${REMOTE_BASE}/mesacode-server.cjs`,
    ...
    // 部署 Mesacode Agent runtime 到远程，历史资源包选择已在入口统一忽略。
    await deployMesacodeAgentRuntime(
      backend,
      env,
      {
...
async function checkServerDeployDecision(
  ...
  try {
    ...
    const serverPath = `${REMOTE_BASE}/mesacode-server.cjs`;
    ...
    log("remote version:", JSON.stringify(version), "local:", MESACODE_VERSION);
    if (version !== MESACODE_VERSION) {
      return {
        shouldDeploy: true,
        reason: `remote server version mismatch remote=${version} expected=${MESACODE_VERSION}`,
...
function resolveMockCdnReleaseDir(mockCdnDir?: string): string | null {
  ...
  return join(mockCdnDir, "releases", MESACODE_VERSION);
}
  ...
  for (const componentId of ids) {
    switch (componentId) {
      case SERVER_BUNDLE_COMPONENT_ID:
        requiredPaths.add("server/mesacode-server.cjs");
        break;
      case "node-runtime":
      ...
      case "node-pty":
        ...
        break;
      case "glm":
        requiredPaths.add(`glm/${platformArch}/mesacode.cjs`);
