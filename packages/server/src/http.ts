...
import {
  ...
  MESACODE_RPC_HOST_CAPABILITY_HEADER,
  MESACODE_VERSION,
...
const log = (...args: unknown[]) =>
  console.log(formatLogPrefix("mesacode-server:http", process.pid), ...args);
...
function resolveServerId(options: HttpServerOptions): string {
  return (
    options.serverId?.trim() || readTrimmedEnv("MESACODE_SERVER_ID") || hostname() || "mesacode-server"
  );
}
...
function resolveServerWorkspaces(options: HttpServerOptions): ServerRemoteWorkspaceInfo[] {
  ...
  const workspacePath = readTrimmedEnv("MESACODE_SERVER_WORKSPACE") || process.cwd();
  return [
    {
      path: workspacePath,
...
function createServerInfo(options: HttpServerOptions): ServerRemoteInfo {
  return {
    ...
    ...(options.name?.trim() || readTrimmedEnv("MESACODE_SERVER_NAME")
      ? { name: options.name?.trim() || readTrimmedEnv("MESACODE_SERVER_NAME") }
      : {}),
    version: MESACODE_VERSION,
    protocolVersion: SERVER_REMOTE_PROTOCOL_VERSION,
    authRequired: options.authRequired ?? Boolean(readTrimmedEnv("MESACODE_SERVER_TOKEN")),
...
const mesacodeLiteTokenCookieName = "mesacode_lite_token";
...
function hasValidLiteToken(c: Context, token: string): boolean {
  ...
  if (url.searchParams.get("token") === token) {
    c.header(
      "Set-Cookie",
      `${mesacodeLiteTokenCookieName}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax`,
    );
    return true;
  }
  return parseCookieHeader(c.req.header("cookie")).get(mesacodeLiteTokenCookieName) === token;
}
...
export function createHttpServer(
  ...
  app.use("/ws/host", async (c, next) => {
    const capability = c.req.header(MESACODE_RPC_HOST_CAPABILITY_HEADER);
