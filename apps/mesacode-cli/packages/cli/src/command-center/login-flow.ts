import type { TuiSelection, TuiSubmitPrompt } from "@mesacode/tui";
...
export function buildLoginSelection(locale?: string): TuiSelection {
  const copy = getMesacodeCopy(locale).tui.loginSetup;
  return {
...
export function loginSetupResponse(locale?: string): string {
  return getMesacodeCopy(locale).tui.loginSetup.response;
}
