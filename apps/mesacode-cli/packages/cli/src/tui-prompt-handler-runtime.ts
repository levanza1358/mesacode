...
// 品牌类型（SessionId）和 createMesacodeApp 的联合签名收窄错。
export async function prepareTuiAppRuntime(
  ...
  const createAppFactory = deps.createMesacodeApp ?? bootstrapModule?.createMesacodeApp;
  if (!createAppFactory) throw new Error("Mesacode app factory is unavailable.");
  ...
  if (prepareTelemetry) {
    state.shutdownTelemetry =
      deps.shutdownMesacodeTelemetry ?? bootstrapModule?.shutdownMesacodeTelemetry;
  }
