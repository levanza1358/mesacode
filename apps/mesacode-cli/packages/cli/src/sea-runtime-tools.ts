...
import { getRuntimeToolRuntime, type RuntimeToolId } from "@mesacode/shared/runtime-tool-runtime";
...
const assetPrefix = "mesacode-runtime-tools/";
const manifestAssetKey = `${assetPrefix}manifest.json`;
const markerFileName = ".mesacode-runtime-tool.json";
...
export async function ensureSeaRuntimeTools(
  ...
  const configuredStorageRoot = options.storageRoot ?? env.MESACODE_STORAGE_DIR?.trim();
  const storageRoot = configuredStorageRoot || join(homedir(), ".mesacode");
