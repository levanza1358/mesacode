...
import type { TuiClipboardImage, TuiImageMediaType, TuiReadClipboardImage } from "@mesacode/tui";
...
function resolveDefaultClipboardDirectory(processEnv: NodeJS.ProcessEnv = process.env): string {
  const storageRoot = processEnv.MESACODE_STORAGE_DIR?.trim() || join(homedir(), ".mesacode");
...
async function readMacClipboardImage(options: {
  ...
  const directory = await mkdtemp(join(options.tempDirectory, "mesacode-clipboard-"));
