...
const USAGE = `Usage:
  mesacode hooks trust status [--workspace <path-or-identity>] [--json]
  mesacode hooks trust review [--workspace <path-or-identity>] [--json]
  mesacode hooks trust grant --workspace <path-or-identity> --hook-digest <sha256> [--hook-digest <sha256> ...]
  mesacode hooks trust grant --workspace <path-or-identity> --all-current --bundle-digest <sha256>
  mesacode hooks trust revoke --workspace <path-or-identity> [--hook-digest <sha256> ... | --all]
`;
...
type HooksCommandDependencies = RunDependencies & {
  loadBootstrapModule?: () => Promise<typeof import("@mesacode/bootstrap")>;
...
function formatHuman(status: WorkspaceHookTrustCliStatus, action: string): string {
  ...
  if (status.reasonCode === "workspace_hooks_pending_trust" && status.bundleDigest) {
    lines.push(
      ...
      `  mesacode hooks trust grant --workspace ${quote(status.workspaceIdentity)} --hook-digest <sha256>`,
      "Or trust every currently enabled declaration in this exact bundle with:",
      `  mesacode hooks trust grant --workspace ${quote(status.workspaceIdentity)} --all-current --bundle-digest ${status.bundleDigest}`,
    );
  }
