...
import type { PlaywrightChromiumModule } from "@mesacode/adapters/browser";
...
const ASSET_PREFIX = "mesacode-playwright-runtime/";
...
export async function loadCliPlaywrightChromium(): Promise<PlaywrightChromiumModule> {
  ...
  const require = createRequire(join(runtimeDirectory, "mesacode-playwright-loader.cjs"));
...
function cacheBaseDirectory(): string {
  ...
  if (platform() === "darwin" && home) {
    return join(home, "Library", "Caches", "mesacode", "sea-assets");
  }
  if (platform() === "win32") {
    return join(
