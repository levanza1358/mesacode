...
import { formatJson, type PresentationSurface } from "@mesacode/core";
import type { RunContext, GlobalOptions } from "@mesacode/shared-types";
...
export const runPrompt = async (
  ...
  let app:
    | Awaited<ReturnType<Awaited<ReturnType<typeof loadBootstrapModule>>["createMesacodeApp"]>>
    | undefined;
  ...
  try {
    ...
    const bootstrapModule = deps.createMesacodeApp ? undefined : await loadBootstrapModule();
    const createApp = deps.createMesacodeApp ?? bootstrapModule?.createMesacodeApp;
    if (!createApp) throw new Error("Mesacode app factory is unavailable.");
    ...
    const prepareTelemetry =
      deps.prepareMesacodeTelemetryEnv ?? bootstrapModule?.prepareMesacodeTelemetryEnv;
    if (prepareTelemetry) {
      shutdownTelemetry = deps.shutdownMesacodeTelemetry ?? bootstrapModule?.shutdownMesacodeTelemetry;
    }
    const appEnv = prepareTelemetry
      ? await prepareTelemetry(env, {
          cliVersion: version,
          productVersion: env.MESACODE_APP_VERSION,
        })
      : env;
    ...
    // 回合。结果是 `mesacode -p "/workflow ..."` 在第一个回合后就退出，把在飞的 run 孤儿化
...
async function isResolvableCustomCommand(deps: RunDependencies, name: string): Promise<boolean> {
  ...
  //
  // 探测刻意用「保留名检查 + load」这一对，而不是直接调 resolveMesacodeCustomCommandPrompt：
...
async function isReservedSlashCommandName(deps: RunDependencies, name: string): Promise<boolean> {
  ...
  return bootstrap.isReservedMesacodeSlashCommandName(name);
}
...
async function listCustomCommandsForPrompt(deps: RunDependencies) {
  ...
  const bootstrap = await loadBootstrapModule();
  return await bootstrap.listMesacodeCustomCommands({ env, logger: deps.logger, workingDirectory });
}
...
async function loadCustomCommandForPrompt(deps: RunDependencies, name: string) {
  ...
  const bootstrap = await loadBootstrapModule();
  return await bootstrap.loadMesacodeCustomCommand({
    env,
    logger: deps.logger,
    name,
    workingDirectory,
...
function writeHeadlessWorkspaceHookTrustDiagnostic(
  ...
  ctx.stderr.write(
    [
      ...
      `Review with: mesacode hooks trust review --workspace ${JSON.stringify(status.workspaceIdentity)}`,
    ].join("\n") + "\n",
  );
}
