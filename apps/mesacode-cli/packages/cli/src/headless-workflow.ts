...
import { createDenyPermissionBroker } from "@mesacode/core";
...
export const createHeadlessPermissionBroker = (): NonNullable<
  MesacodeAppOptions["permissionBroker"]
> => {
 ...
 *
 * 刻意**不**进 `mesacodeSessionEventTypeSchema`（`shared/src/mesacode-protocol/index.ts`）：
 * 那是个闭集 `z.enum` 且喂给 `mesacodeSessionEventSchema` 的 discriminated union，加值等于让
 ...
 * `RunDependencies.createMesacodeApp` 注入点边界）。
 *
