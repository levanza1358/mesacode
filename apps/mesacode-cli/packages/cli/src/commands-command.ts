...
type CustomCommandListOutcome = Awaited<ReturnType<typeof listMesacodeCustomCommands>>;
...
const COMMANDS_COMMAND_USAGE = "Usage: mesacode commands [list|inspect <name>]";
...
async function resolveListCustomCommands(deps: CommandsCommandDependencies) {
  ...
  return (await bootstrap()).listMesacodeCustomCommands;
}
...
async function resolveInspectCustomCommand(deps: CommandsCommandDependencies) {
  ...
  const bootstrap = deps.loadBootstrapModule ?? (() => import("@mesacode/bootstrap"));
  return (await bootstrap()).inspectMesacodeCustomCommand;
}
...
function formatHumanCommandInspection(
  inspection: MesacodeCustomCommandInspection,
  options: GlobalOptions,
): string {
