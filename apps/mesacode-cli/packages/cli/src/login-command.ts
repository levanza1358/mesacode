...
export async function runLoginCommand(
  ...
  try {
    ...
    if (args.length > 1 || (providerId !== "zai" && providerId !== "bigmodel")) {
      throw new Error("Usage: mesacode login [zai|bigmodel] [--no-browser]");
    }
    ...
    const login = deps.loginMesacodeCli ?? (await loadBootstrapModule()).loginMesacodeCli;
...
export async function runLogoutCommand(
  ...
  try {
    ...
    const logout = deps.logoutMesacodeCli ?? (await loadBootstrapModule()).logoutMesacodeCli;
