import type { TuiSessionMetadata } from "@mesacode/tui";
...
export async function listSkillsForTui(deps: RunDependencies) {
  const env = deps.env ?? process.env;
  const workingDirectory = (deps.cwd ?? process.cwd)();
  const listMesacodeSkillsForTui = deps.listSkills ?? (await loadBootstrapModule()).listMesacodeSkills;
  return await listMesacodeSkillsForTui({
    env,
    logger: deps.logger,
    workingDirectory,
  });
...
export async function listSessionsForTui(deps: RunDependencies) {
  const env = deps.env ?? process.env;
  const workingDirectory = (deps.cwd ?? process.cwd)();
  const listMesacodeSessionsForTui =
    deps.listSessions ?? (await loadBootstrapModule()).listMesacodeSessions;
  const sessions = await listMesacodeSessionsForTui({
