...
import type { SetMesacodePluginEnabledResult, MesacodePluginInstallData } from "@mesacode/bootstrap";
import {
...
const PLUGINS_COMMAND_USAGE = `Usage: mesacode plugins <command> [options]
...
Scopes: user (default), project. \`mesacode plugin\` is an alias of \`mesacode plugins\`.`;
...
async function runPluginsUpdateCommand(
  ...
  ctx.stdout.write(
    version && version === result.previousVersion
      ...
      : `Updated plugin ${pluginId} from ${result.previousVersion || "unknown"} to ${version || "unknown"}. Restart mesacode to apply.\n`,
  );
...
function reportInstallOutcome(
  ctx: RunContext,
  options: GlobalOptions,
  result: MesacodePluginInstallData,
  pluginId: string,
): number {
  const failed = hasErrors(result.diagnostics);
  if (options.json) {
...
async function runPluginsDisableCommand(
  ...
  const entries: Array<{ plugin: PluginListItem; result?: SetMesacodePluginEnabledResult; error?: string }> = [];
