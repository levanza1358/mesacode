import { getDefaultConfigPath, updateUiLocaleInFileConfig } from "@mesacode/adapters/config";
import type { SessionEvent } from "@mesacode/contracts";
import type { MesacodeAppOptions } from "@mesacode/bootstrap";
import { DEFAULT_LOCALE, type SupportedLocale } from "@mesacode/i18n";
import type { TuiRequestPermission } from "@mesacode/tui";
...
export function createTuiSubmitPrompt(
  ...
  let app: Awaited<ReturnType<NonNullable<RunDependencies["createMesacodeApp"]>>> | undefined;
  ...
  const permissionBroker: NonNullable<MesacodeAppOptions["permissionBroker"]> = {
  ...
  const replaceApp = async (
    factory: () => Promise<Awaited<ReturnType<NonNullable<RunDependencies["createMesacodeApp"]>>>>,
  ...
  const createApp = async (request: CliResumeRequest) => {
    ...
    let createdApp: Awaited<ReturnType<NonNullable<RunDependencies["createMesacodeApp"]>>>;
    try {
