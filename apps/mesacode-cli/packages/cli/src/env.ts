...
import {
  ...
  normalizeMesacodeRuntimeEnv,
  sanitizeMesacodeRuntimeEnv,
  sanitizeMesacodeRuntimeEnvInPlace,
...
export function prepareCliRuntimeEnv(
  ...
  const prepared = {
    ...sanitizeMesacodeRuntimeEnv(env),
    ...buildMesacodeToolEnvPassthroughEnv(env),
  };
...
export function applyCliRuntimeEnvSanitization(
  ...
  const toolEnvPassthrough = buildMesacodeToolEnvPassthroughEnv(env);
  sanitizeMesacodeRuntimeEnvInPlace(env);
...
function applyCliRuntimeEnvDefaults(env: CliEnv, argv: readonly string[]): void {
  env[MESACODE_RUNTIME_ENV_KEY] = resolveCliRuntimeEnv(env, argv);
  applyBetaStorageDefault(env, argv);
}

function resolveCliRuntimeEnv(env: CliEnv, argv: readonly string[]): MesacodeRuntimeEnv {
  const explicit = normalizeMesacodeRuntimeEnv(env[MESACODE_RUNTIME_ENV_KEY]);
  if (explicit) {
    return explicit;
  }
...
function applyBetaStorageDefault(env: CliEnv, argv: readonly string[]): void {
  if (env.MESACODE_STORAGE_DIR?.trim()) return;
  const explicitBeta = env.MESACODE_BETA === "1" || env.MESACODE_ENV === "beta";
  const invokedAsBeta = argv.some((arg) => /(^|[/\\])mesacode-beta(?:$|\.)/u.test(arg));
  if (!explicitBeta && !invokedAsBeta) return;
  env.MESACODE_STORAGE_DIR = join(homedir(), ".mesacode-beta");
}
