...
import { createNodeLoggerFactory } from "@mesacode/adapters";
import { getRuntimeInfo, type PresentationSurface } from "@mesacode/core";
import { color, formatJson, supportsColor } from "@mesacode/core";
import { getMesacodeCopy, isUiLocale, type UiLocale } from "@mesacode/i18n";
import type { RunContext, GlobalOptions, GlobalOutputFormat } from "@mesacode/shared-types";
import {
...
const normalizeLocaleOption = (value: string | undefined): UiLocale | undefined => {
  ...
  throw new Error(getMesacodeCopy().cli.errors.localeUnsupported(value));
};
...
const normalizePresentationSurface = (value: string | undefined): PresentationSurface => {
  ...
  if (value.toLowerCase() === "desktop") return "mesacode_desktop";
...
const runDoctor = (ctx: RunContext, options: GlobalOptions, workingDirectory: string): number => {
  ...
  ctx.stdout.write(`${color.bold("mesacode doctor", colors)}\n`);
...
const runMesacodeProtocolCommand = async (
  ctx: RunContext,
  ...
  try {
    ...
    // 直接退出，外层只能看到 Mesacode agent transport closed。
    ...
    const runProtocolAgent =
      deps.runMesacodeProtocolAgent ?? (await loadBootstrapModule()).runMesacodeProtocolAgent;
...
export const run = async (ctx: RunContext, deps: RunDependencies = {}): Promise<number> => {
  ...
  const commandDeps: RunDependencies = {
    ...
    logger:
      deps.logger ??
      createNodeLoggerFactory({ env }).createLogger("mesacode").child({ module: "cli" }),
  ...
  switch (commandName(parsed.positionals)) {
    ...
    case "agent-server":
    case "app-server":
      return await runMesacodeProtocolCommand(
        ctx,
        options,
        commandDeps,
        presentationSurface,
