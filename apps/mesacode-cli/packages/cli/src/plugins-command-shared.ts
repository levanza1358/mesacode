...
import type { listMesacodePlugins } from "@mesacode/bootstrap";
import { createInterface } from "node:readline";
import type { GlobalOptions, RunContext } from "@mesacode/shared-types";
import type { CliEnv } from "./env.js";

export type BootstrapModule = typeof import("@mesacode/bootstrap");
export type PluginListOutcome = ReturnType<typeof listMesacodePlugins>;
 ...
 * 避免 `mesacode plugins list` 这类轻命令把整个 bootstrap 图提前拉起来。
 */
export const BOOTSTRAP_EXPORTS = {
  addMarketplace: "addMesacodePluginMarketplace",
  getPluginsOverview: "getMesacodePluginsOverview",
  installPlugin: "installMesacodeMarketplacePlugin",
  listPlugins: "listMesacodePlugins",
  removeMarketplace: "removeMesacodePluginMarketplace",
  setPluginEnabled: "setMesacodePluginEnabled",
  uninstallPlugin: "uninstallMesacodeMarketplacePlugin",
  updateMarketplace: "updateMesacodePluginMarketplace",
  updatePlugin: "updateMesacodeMarketplacePlugin",
  validatePluginPath: "validateMesacodePluginPath",
...
/** `mesacode plugins` 子命令专属旗标；由 run.ts 的全局解析器收集后原样透传。 */
...
export async function resolveDep<K extends PluginDepName>(
  ...
  const bootstrap = deps.loadBootstrapModule ?? (() => import("@mesacode/bootstrap"));
...
export function resolveScope(value: string | undefined): PluginScope | undefined {
  ...
  if (value === "local") {
    throw new PluginsUsageError("Scope 'local' is not supported by mesacode. Use: user, project");
  }
