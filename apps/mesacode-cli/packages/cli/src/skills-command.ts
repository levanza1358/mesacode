...
type BootstrapModule = typeof import("@mesacode/bootstrap");
type CliSkillListOutcome = Awaited<ReturnType<typeof listMesacodeSkills>>;
...
const SKILLS_COMMAND_USAGE = "Usage: mesacode skills [list|inspect <name>]";
...
async function runSkillsListCommand(
  ...
  try {
    ...
    const listSkills = deps.listSkills ?? (await bootstrap()).listMesacodeSkills;
...
async function runSkillsInspectCommand(
  ...
  try {
    ...
    const workingDirectory = (deps.cwd ?? process.cwd)();
    const bootstrap = deps.loadBootstrapModule ?? (() => import("@mesacode/bootstrap"));
    const inspectSkill = deps.inspectSkill ?? (await bootstrap()).inspectMesacodeSkill;
...
function formatHumanSkillInspection(
  inspection: MesacodeSkillInspection,
  options: GlobalOptions,
): string {
...
const formatSkillInspectionJson = (inspection: MesacodeSkillInspection, cwd: string): string =>
  formatJson({
    cwd,
