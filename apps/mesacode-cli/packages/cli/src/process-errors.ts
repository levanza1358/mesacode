...
import {
  MESACODE_PROCESS_DIAGNOSTIC_PREFIX,
  ...
  type MesacodeProcessDiagnostic,
...
function writeProcessErrorDiagnostic(
  ...
  try {
    const detail = formatProcessError(reason).slice(0, MESACODE_PROCESS_DIAGNOSTIC_STACK_MAX_CHARS);
    const diagnostic: MesacodeProcessDiagnostic = {
      version: 1,
      errorId: randomUUID(),
      kind,
    ...
    stderr.write(
      `${MESACODE_PROCESS_DIAGNOSTIC_PREFIX}${JSON.stringify(diagnostic)}\n[mesacode] process error kind=${kind} origin=${origin}\n${detail}\n`,
    );
  } catch {
    // 诊断输出不能再次击穿进程级异常边界。
  }
