...
const cliBundle = resolve(dist, "mesacode.cjs");
const seaBlobForTarget = (target) => resolve(dist, `mesacode-${target}.sea.blob`);
    ...
    JSON.stringify(
      {
        assets: {
          ...
          "mesacode-node-license": nodeLicensePath,
        },
        disableExperimentalSEAWarning: true,
        main: "dist/mesacode.cjs",
        output: `dist/mesacode-${target}.sea.blob`,
        useCodeCache: false,
        useSnapshot: false,
      },
      null,
...
const smokeTestHostTarget = async (target, binaryPath) => {
  if (isHostTarget(target)) {
    const storageRoot = await mkdtemp(join(tmpdir(), "mesacode-sea-smoke-"));
    try {
      const env = {
        ...process.env,
        MESACODE_STORAGE_DIR: storageRoot,
      };
      delete env.MESACODE_BFS_BINARY;
      delete env.MESACODE_RG_BINARY;
      delete env.MESACODE_UGREP_BINARY;
...
export const main = async (argv = process.argv.slice(2)) => {
  ...
  if (!existsSync(cliBundle)) {
    throw new Error("Missing dist/mesacode.cjs. Run `pnpm build` first.");
  }
