...
export const seaTuiAssetPrefix = "mesacode-tui-runtime/";
...
const workspacePackageParentDirectories = (root) => {
  ...
  return [
    ...
    // TUI runtime 闭包会经由 @mesacode/contracts 依赖仓库根的 @mesacode/shared；
    // 只扫描 apps/mesacode-cli 子 workspace 会把这个合法 workspace 误判为缺失。
    resolve(repositoryRoot, "packages"),
  ];
};

const runtimePackageNames = async ({ root, target, workspacePackageDirectories }) => {
  const tuiDirectory = workspacePackageDirectories.get("@mesacode/tui");
  if (!tuiDirectory) {
    throw new Error("Missing @mesacode/tui workspace package.");
  }
  const queue = [
    { fromDirectory: tuiDirectory, packageName: "@mesacode/tui" },
  ...
  while (queue.length > 0) {
    ...
    const assetPackagePath = placeRuntimePackage({
      ...
      packageDirectory,
      placements,
      fromAssetPath: entry.fromAssetPath ?? "node_modules/@mesacode/tui",
    });
    ...
    // SEA 缓存目录无法解析 pnpm workspace 链接；旧的 @mesacode 白名单漏掉 i18n
