...
export const DEFAULT_DESTINATION_ROOT = resolve(DEFAULT_MOUNT_ROOT, "mesacode", "deps");
export const RELEASE_DIRECTORY_PREFIX = "mesacode-cli-";
...
const usage = `Usage:
  ...
  node packages/cli/scripts/upload-sea-smb.mjs --dest-root /Volumes/shared/mesacode/deps
...
Options:
  ...
  --dest-root <path>  Mounted SMB release root. Defaults to /Volumes/shared/mesacode/deps.
  ...
  if (missing.length > 0) {
    throw new Error(
      `Missing SEA binaries in ${distDir}: ${missing.join(", ")}. ` +
        "Run `pnpm --filter @mesacode/cli build:sea` first.",
    );
  }
