...
import {
  MESACODE_CLI_RESOURCE_SAMPLE_INTERVAL_MS,
  type MesacodeProcessResourceSample,
} from "@mesacode/shared";

/** 采样周期与 app 侧聚合共用 shared 的同一个常量，避免两侧节拍各自漂移。 */
const MESACODE_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS = MESACODE_CLI_RESOURCE_SAMPLE_INTERVAL_MS;
...
interface CreateMesacodeProcessResourceSamplerOptions {
  /**
   * 第二个参数是本周期完整的内存快照，供进程内本地诊断日志使用；
   * 协议样本自身只带 rss 与 heapUsed 两项内存字段。
   */
  onSample(sample: MesacodeProcessResourceSample, memoryUsage: ProcessMemoryUsageSnapshot): void;
  platform?: MesacodeProcessResourceSample["platform"];
  arch?: MesacodeProcessResourceSample["arch"];
...
export interface MesacodeProcessResourceSampler {
  start(): void;
  stop(): void;
}
...
export function createMesacodeProcessResourceSampler(
  options: CreateMesacodeProcessResourceSamplerOptions,
): MesacodeProcessResourceSampler {
  const platform = options.platform ?? (process.platform as MesacodeProcessResourceSample["platform"]);
  const arch = options.arch ?? (process.arch as MesacodeProcessResourceSample["arch"]);
  ...
  const sample = (): void => {
    try {
      ...
      baseline = nextBaseline;
      const resourceSample: MesacodeProcessResourceSample = {
        platform,
        arch,
        logicalCpuCount,
        intervalMs,
  ...
  return {
    start() {
      ...
      try {
        timerHandle = timer.setInterval(sample, MESACODE_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS);
      } catch {
