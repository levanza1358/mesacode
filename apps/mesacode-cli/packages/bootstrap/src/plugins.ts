...
} from "@mesacode/adapters/plugins";
import type {
  Logger,
  PluginHookDetail,
  PluginLoadOutcome,
...
import { MESACODE_OFFICIAL_PLUGIN_MARKETPLACE, isOfficialMarketplaceId } from "@mesacode/contracts";
import { MESACODE_CUA_OFFICIAL_PLUGIN_ID, isMesacodeCuaInternalFeatureEnabled } from "@mesacode/shared";
...
export interface ResolveMesacodePluginsOptions {
...
export interface ListMesacodePluginsOptions extends ResolveMesacodePluginsOptions {}

export interface SetMesacodePluginEnabledOptions extends ResolveMesacodePluginsOptions {
  enabled: boolean;
  plugin: string;
  scope?: "user" | "workspace";
}

export interface SetMesacodePluginEnabledResult {
  enabled: boolean;
  path: string;
  plugin: PluginMetadata;
}

export interface MesacodeMarketplaceSummaryData {
  id: string;
  name: string;
...
export interface MesacodeAvailablePluginData {
  id: string;
  name: string;
  marketplace: string;
  description?: string;
...
export interface MesacodeInstalledPluginData {
  id: string;
  name: string;
  marketplace: string;
  description?: string;
...
export interface MesacodePluginsOverviewData {
  marketplaces: MesacodeMarketplaceSummaryData[];
  availablePlugins: MesacodeAvailablePluginData[];
  installedPlugins: MesacodeInstalledPluginData[];
  restorableBuiltins: MesacodeAvailablePluginData[];
  diagnostics: PluginLoadOutcome["diagnostics"];
}

export interface MesacodeMarketplaceUpdateData {
  diagnostics: PluginLoadOutcome["diagnostics"];
  marketplaces: MesacodeMarketplaceSummaryData[];
}

export interface AddMesacodeMarketplaceOptions extends ResolveMesacodePluginsOptions {
...
export interface RemoveMesacodeMarketplaceOptions extends ResolveMesacodePluginsOptions {
  marketplace: string;
}

export interface UpdateMesacodeMarketplaceOptions extends ResolveMesacodePluginsOptions {
  abortSignal?: AbortSignal;
  marketplace?: string;
}

export interface InstallMesacodeMarketplacePluginOptions extends ResolveMesacodePluginsOptions {
  abortSignal?: AbortSignal;
  dryRun?: boolean;
  marketplace: string;
  pluginName: string;
...
export interface UninstallMesacodeMarketplacePluginOptions extends ResolveMesacodePluginsOptions {
  pluginId?: string;
  pluginName?: string;
  marketplace?: string;
  removeCache?: boolean;
  /** 保留 data/<plugin-id> 用户数据目录（`mesacode plugins uninstall --keep-data`）。 */
  keepData?: boolean;
}

export interface UpdateMesacodeMarketplacePluginOptions extends ResolveMesacodePluginsOptions {
  abortSignal?: AbortSignal;
  pluginId: string;
}

export interface ValidateMesacodePluginPathOptions extends ResolveMesacodePluginsOptions {
  abortSignal?: AbortSignal;
  path: string;
}

export interface MesacodePluginUpdateData extends MesacodePluginInstallData {
  previousVersion: string;
}

interface RestoreBuiltinPluginOptions extends ResolveMesacodePluginsOptions {
  pluginId: string;
}

interface ConfigureMesacodePluginOptions extends ResolveMesacodePluginsOptions {
...
interface ResetMesacodePluginConfigOptions extends ResolveMesacodePluginsOptions {
  pluginId: string;
  scope?: "user" | "workspace";
}

interface ValidateMesacodePluginOptions extends ResolveMesacodePluginsOptions {
  marketplace?: string;
  pluginName?: string;
  source?: string;
}

interface DescribeMesacodePluginOptions extends ResolveMesacodePluginsOptions {
  marketplace: string;
  pluginName: string;
}

export interface MesacodePluginInstallData {
...
function countVisibleMarketplacePlugins(
  ...
  if (marketplaceId !== MESACODE_OFFICIAL_PLUGIN_MARKETPLACE) return plugins.length;
  return plugins.filter((entry) => entry.name !== OFFICIAL_NODE_REPL_HOST_PLUGIN_NAME).length;
}

export function resolveMesacodePlugins(options: ResolveMesacodePluginsOptions = {}): PluginLoadOutcome {
...
export function getMesacodePluginsOverview(
  options: ResolveMesacodePluginsOptions = {},
): MesacodePluginsOverviewData {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  const outcome = resolveMesacodePlugins({
    ...options,
    configResult,
    pluginStorageRoot,
  });
  ...
  // mesacode-plugins-official 的内置与 CDN 分片已在 adapter 层合并为唯一 canonical manifest。
  const catalogs: Array<{
    summary: MesacodeMarketplaceSummaryData;
  ...
  const restorableBuiltins: MesacodeAvailablePluginData[] = OFFICIAL_PLUGIN_DEFINITIONS.filter(
    (def) =>
      suppressed.has(`${def.name}@${MESACODE_OFFICIAL_PLUGIN_MARKETPLACE}`) &&
      // computer-use 的恢复入口需要 internal 特性开启（与 restoreBuiltinPluginCore 同口径）。
      (def.name !== "computer-use" || isMesacodeCuaInternalFeatureEnabled(options.env ?? process.env)),
  ).map((def) => {
    const listing = def.listing
      ? parseEntryStoreListing({ name: def.name, ...def.listing })
      : undefined;
    return {
      id: `${def.name}@${MESACODE_OFFICIAL_PLUGIN_MARKETPLACE}`,
      name: def.name,
      marketplace: MESACODE_OFFICIAL_PLUGIN_MARKETPLACE,
      version: def.version,
      installed: false,
      ...(listing ? { listing } : {}),
    };
...
export function listMesacodePlugins(options: ListMesacodePluginsOptions = {}): PluginLoadOutcome {
  const outcome = resolveMesacodePlugins(options);
...
function loadPluginListingsById(storageRoot: string): Record<string, PluginStoreListing> {
  ...
  for (const definition of OFFICIAL_PLUGIN_DEFINITIONS) {
    ...
    if (listing) {
      listings.set(`${definition.name}@${MESACODE_OFFICIAL_PLUGIN_MARKETPLACE}`, listing);
    }
  }
...
export async function setMesacodePluginEnabled(
  options: SetMesacodePluginEnabledOptions,
): Promise<SetMesacodePluginEnabledResult> {
...
export async function addMesacodePluginMarketplace(
  options: AddMesacodeMarketplaceOptions,
): Promise<MesacodeMarketplaceSummaryData> {
  const { pluginStorageRoot } = resolvePluginContext(options);
...
export async function removeMesacodePluginMarketplace(
  options: RemoveMesacodeMarketplaceOptions,
): Promise<void> {
  const { pluginStorageRoot } = resolvePluginContext(options);
...
export async function updateMesacodePluginMarketplace(
  options: UpdateMesacodeMarketplaceOptions,
): Promise<MesacodeMarketplaceUpdateData> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
...
export async function installMesacodeMarketplacePlugin(
  options: InstallMesacodeMarketplacePluginOptions,
): Promise<MesacodePluginInstallData> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  if (options.dryRun === true) {
  ...
  const isSuppressedBundledOfficial =
    options.marketplace === MESACODE_OFFICIAL_PLUGIN_MARKETPLACE &&
  ...
  if (options.marketplace === MESACODE_OFFICIAL_PLUGIN_MARKETPLACE) {
...
export async function uninstallMesacodeMarketplacePlugin(
  options: UninstallMesacodeMarketplacePluginOptions,
): Promise<MesacodeInstalledPluginData | null> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  return withPluginStorageLock(pluginStorageRoot, async () => {
    const pluginId = resolvePluginIdForMutation(options);

    // 官方 CDN marketplace 与内置插件共享 mesacode-plugins-official id 空间，且其缓存
...
/**
 * `mesacode plugins update <plugin>`：先刷新所属 marketplace 目录，再按同一条目重装。
 * cacheMarketplacePlugin 对已存在的安装记录做原地覆盖并保留 installedAt；启用态只会给
 * 用户配置里尚未显式声明的 id 补默认值，因此更新不会改变用户已经做过的开关选择。
 */
export async function updateMesacodeMarketplacePlugin(
  options: UpdateMesacodeMarketplacePluginOptions,
): Promise<MesacodePluginUpdateData> {
  const { pluginStorageRoot } = resolvePluginContext(options);
  ...
  const refreshed = await updateMesacodePluginMarketplace({
    ...options,
  ...
  const installed = await installMesacodeMarketplacePlugin({
    ...options,
    marketplace: record.marketplace,
...
/** `mesacode plugins validate <path>`：只读校验本地插件目录或 marketplace 目录。 */
export async function validateMesacodePluginPath(
  options: ValidateMesacodePluginPathOptions,
...
async function restoreBuiltinPluginCore(options: RestoreBuiltinPluginOptions): Promise<void> {
  const mesacodeCuaPluginId = MESACODE_CUA_OFFICIAL_PLUGIN_ID;
  if (
    options.pluginId === mesacodeCuaPluginId &&
    !isMesacodeCuaInternalFeatureEnabled(options.env ?? process.env)
  ) {
    // overview 虽然隐藏了恢复入口，但协议调用仍可绕过 UI 写用户配置。
    // 功能开关关闭时在写盘前失败，确保用户配置与插件缓存都保持零痕迹。
    throw new Error("computer-use built-in plugin requires MESACODE_CUA_PRODUCT_HELPER to be enabled");
  }
...
export async function configureMesacodePlugin(options: ConfigureMesacodePluginOptions): Promise<void> {
...
export async function resetMesacodePluginConfig(
  options: ResetMesacodePluginConfigOptions,
...
export async function validateMesacodePlugin(
  options: ValidateMesacodePluginOptions,
): Promise<PluginLoadOutcome["diagnostics"]> {
  const { pluginStorageRoot } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  if (options.source) {
...
export async function describeMesacodePlugin(
  options: DescribeMesacodePluginOptions,
...
function resolvePluginContext(options: ResolveMesacodePluginsOptions): {
  configResult: ConfigResult;
  pluginStorageRoot: string;
  workingDirectory: string;
} {
...
function resolvePluginIdForMutation(options: UninstallMesacodeMarketplacePluginOptions): string {
...
function resolvePluginConfigPath(
  options: ResolveMesacodePluginsOptions & { scope?: "user" | "workspace" },
  configResult: ConfigResult,
  workingDirectory: string,
): string {
  if (options.scope !== "workspace") {
  ...
  // Workspace Plugin 配置固定落在当前 `<workspace>/.mesacode/config.json`。嵌套 workspace
  // 可能同时发现仓库根与自身的配置，读取端 innermost 优先；写入端也必须锁定当前
  // workspace，不能用 project discovery 的第一个 outermost 文件。
  const workspaceConfigPath = join(workingDirectory, ".mesacode", "config.json");
