...
export function createRuntimeAiSdkModelExecutionConfig(
  ...
  return {
    defaultHeaders: buildCliMesacodeSourceHeaders(env, options),
    env,
    ...(network ? { network } : {}),
  };
}
...
function buildCliMesacodeSourceHeaders(
  env: EnvRecord,
...
function resolveAppVersionForHeaders(
  ...
  return normalizePrintableHeaderValue(env[MESACODE_APP_VERSION_ENV] ?? options.appVersion);
}
