...
import { migrateUserSubagentMarkdown, migrateSubagentStateFile } from "@mesacode/shared/node";
import {
  parseAgentProfileFromMarkdown,
  type AgentProfile,
  type AgentProfileParseDiagnostic,
} from "@mesacode/core";
import type { Logger, PluginMetadata } from "@mesacode/contracts";
import {
  createAgentStateId,
...
interface LoadMesacodeAgentProfilesInput {
  logger?: Logger;
  storageRoot: string;
  workingDirectory: string;
}

interface LoadMesacodeAgentProfilesResult {
...
export async function loadMesacodeAgentProfiles(
  input: LoadMesacodeAgentProfilesInput,
): Promise<LoadMesacodeAgentProfilesResult> {
  ...
  const roots = [
    ...
    { path: join(input.workingDirectory, ".mesacode", "agents"), source: "project" as const },
  ];
...
function sanitizeProjectAgentProfile(profile: AgentProfile): AgentProfile {
  ...
  // 项目级 .mesacode/agents/*.md 是仓库内容，不能通过 frontmatter
...
): LoadMesacodeAgentProfilesResult {
  const diagnostics: AgentProfileParseDiagnostic[] = [];
