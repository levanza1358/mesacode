...
import { openStartupSqliteSessionStore } from "@mesacode/adapters/storage";
import type { SessionInfo } from "@mesacode/contracts";
import type { ListMesacodeSessionsOptions, ResolveLatestSessionOptions } from "./app/types.js";
...
export async function listMesacodeSessions(
  options: ListMesacodeSessionsOptions = {},
): Promise<SessionInfo[]> {
  const ownsStore = options.sessionStore === undefined;
  let sessionStore = options.sessionStore;
  if (!sessionStore) {
