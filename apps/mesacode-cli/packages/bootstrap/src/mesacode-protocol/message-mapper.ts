import type { MesacodeMessagePart, MesacodeMessageWithParts, MesacodeToolState } from "@mesacode/shared";
import type { MessagePart, MessageWithParts, ToolState } from "@mesacode/contracts";
import { shouldHideInvalidToolCallFromProduct } from "../tool-call-product-visibility.js";

export function mapMessageWithParts(message: MessageWithParts): MesacodeMessageWithParts {
  return {
    info:
...
function mapMessagePart(part: MessagePart): MesacodeMessagePart {
  const base = {
...
function mapToolState(state: ToolState): MesacodeToolState {
  switch (state.status) {
    case "pending":
...
function mapCompletedToolStateMetadata(
  ...
  // Mesacode protocol 的 completed tool state 要求 metadata 必须是 object。
  return visibleMetadata;
}
