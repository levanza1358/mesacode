...
import { workspaceHookPolicySchema } from "@mesacode/contracts";
import type { WorkspaceHookPolicyProvider } from "@mesacode/core";
import {
  mesacodeWorkspaceHookTrustGrantParamsSchema,
  mesacodeWorkspaceHookTrustGrantReasonCodeSchema,
  mesacodeWorkspaceHookTrustGrantResultSchema,
  type MesacodeWorkspaceHookTrustGrantReasonCode,
  type MesacodeWorkspaceHookTrustGrantResult,
...
export async function grantWorkspaceHookTrustForProtocol(
  ...
  const params = mesacodeWorkspaceHookTrustGrantParamsSchema.parse(rawParams);
  ...
  if (policyRejection) {
    return mesacodeWorkspaceHookTrustGrantResultSchema.parse({
      accepted: false,
...
): MesacodeWorkspaceHookTrustGrantReasonCode | undefined {
  try {
...
function toPublicReasonCode(error: unknown): MesacodeWorkspaceHookTrustGrantReasonCode {
  const candidate = error instanceof Error ? error.message : error;
  const parsed = mesacodeWorkspaceHookTrustGrantReasonCodeSchema.safeParse(candidate);
