/* eslint-disable max-lines -- Mesacode Protocol 的 session/workspace 方法共享同一个 server context 与 snapshot helpers，迁移期先集中维护。 */
import { observeSessionDebug } from "./session-debug.js";
import {
  TASK_LIST_SESSION_TYPES,
  isTaskListSessionType,
} from "../mesacode-protocol-v4/task-list-session-membership.js";
import { resolveEffectiveBashShellSelection } from "@mesacode/adapters/exec";
import { inputIntentMetadata } from "../mesacode-protocol-v4/commands/input-intent.js";
import { createModelExecutionContext } from "./model-execution.js";
import type { SendInputOptions } from "../app/types.js";
import { repairPersistedRemoteSessionPaths, type TurnAttachment } from "@mesacode/core";
import {
  CoreErrorType,
...
import {
  ...
  mesacodeProtocolErrorCodes,
  mesacodeProtocolMethods,
  mesacodeSessionCancelBackgroundTaskParamsSchema,
  mesacodeSessionCompactParamsSchema,
  mesacodeSessionCloseParamsSchema,
  mesacodeSessionCreateParamsSchema,
  mesacodeSessionEventsParamsSchema,
  mesacodeSessionForkParamsSchema,
  mesacodeSessionGoalParamsSchema,
  mesacodeSessionListParamsSchema,
  mesacodeSessionMessagesParamsSchema,
  mesacodeSessionReadParamsSchema,
  mesacodeSessionRuntimePreferencesResultSchema,
  mesacodeSessionResumeParamsSchema,
  mesacodeSessionSendParamsSchema,
  mesacodeSessionSetModeParamsSchema,
  mesacodeSessionSetModelParamsSchema,
  mesacodeSessionSetThoughtLevelParamsSchema,
  mesacodeSessionStopParamsSchema,
  mesacodeSessionSubscribeParamsSchema,
  mesacodeSessionSubagentsParamsSchema,
  mesacodeTaskTokenUsageParamsSchema,
  mesacodeUsageStatsParamsSchema,
  mesacodeWorkspaceGenerateTextParamsSchema,
  getConversationMessageProjectionPolicy,
  parseRemoteWorkspaceIdentity,
  type MesacodeSessionCreateParams,
  type MesacodeDeliveryKind,
  type IntegratedTerminalShellSelection,
  type MesacodeSessionRuntimePreferencesScope,
  type MesacodeSessionRuntimePreferencesResult,
  type MesacodeModelContextBudgetStrategy,
  type MesacodeProtocolTrace,
  type MesacodeSessionEvent,
  type MesacodeSessionHistoryTarget,
  type MesacodeSessionResumeParams,
  type MesacodeSessionPersistence,
...
import {
  ...
  parseParams,
  requireSession,
  type MesacodeProtocolAgentServerContext,
  type MesacodeProtocolSessionRecord,
  type MesacodeProtocolToolInputTransmissionState,
} from "./server-types.js";
import { createWorkspaceMesacodeApp, ensureSessionModelAvailable } from "./workspace-model-runtime.js";
...
import { runSessionModelConfigMutation } from "../mesacode-protocol-v4/model-config-mutation.js";
...
type ProtocolGoalTarget = NonNullable<
  Awaited<ReturnType<NonNullable<MesacodeProtocolSessionRecord["app"]["readTarget"]>>>
>;

type MesacodeSessionRecordParams = (
  | MesacodeSessionCreateParams
  | (MesacodeSessionResumeParams & {
      mode?: MesacodeSessionCreateParams["mode"];
      model?: MesacodeSessionCreateParams["model"];
      parentSessionId?: MesacodeSessionCreateParams["parentSessionId"];
      thoughtLevel?: MesacodeSessionCreateParams["thoughtLevel"];
      // 自动化会话会在创建期关闭标题二次生成，而 create/resume
      // 共用 record 初始化函数；resume 兼容分支也必须声明该策略字段。
      titleGenerationEnabled?: MesacodeSessionCreateParams["titleGenerationEnabled"];
    })
) & { taskType?: SessionTaskType };

interface SessionStartupPreferences {
  memoryEnabled: boolean;
  modelContextBudgetStrategy: MesacodeModelContextBudgetStrategy;
...
type SessionStartupPreferencesSource =
  | { kind: "host" }
  | { kind: "inherit"; parent: MesacodeProtocolSessionRecord };

function resolveSupportedAppThoughtLevel(
  app: Pick<MesacodeProtocolSessionRecord["app"], "listThoughtLevels">,
...
const liveProtocolStreamingDeltaBatches = new WeakMap<
  MesacodeProtocolSessionRecord,
  Map<string, LiveProtocolStreamingDeltaBatch>
>();

function protocolEventSequenceKey(deliveryKind?: MesacodeDeliveryKind): string {
  return deliveryKind ?? DEFAULT_PROTOCOL_EVENT_SEQUENCE_KEY;
}

function getProtocolEventSequenceState(
  record: MesacodeProtocolSessionRecord,
...
function getProtocolToolInputTransmissionState(
  record: MesacodeProtocolSessionRecord,
  deliveryKind?: MesacodeDeliveryKind,
): MesacodeProtocolToolInputTransmissionState {
  const key = protocolEventSequenceKey(deliveryKind);
  const existing = record.protocolToolInputTransmissions.get(key);
  if (existing) {
    return existing;
  }
  const created: MesacodeProtocolToolInputTransmissionState = {
...
function readProtocolStreamingParentToolUseId(
  ...
  const mesacode = asRecord(meta.mesacode);
  return (
    stringValue(meta.parentToolUseId) ??
    stringValue(meta.parentToolCallId) ??
    stringValue(mesacode.parentToolUseId) ??
    stringValue(mesacode.parentToolCallId)
  );
}
...
function markStreamedToolInputIfPresent(
  mappedEvent: MesacodeSessionEvent,
  toolInputTransmissions: MesacodeProtocolToolInputTransmissionState,
): void {
...
function mapProtocolSessionEvent(
  context: MesacodeProtocolAgentServerContext,
  record: MesacodeProtocolSessionRecord,
  event: SessionEvent,
  deliveryKind?: MesacodeDeliveryKind,
  options: {
...
): MesacodeSessionEvent | null {
  if (shouldHideProtocolSessionEvent(record, event)) {
    return null;
  }
  if (!shouldExposeSessionEventToProtocol(event)) {
    const payload = asRecord(event.payload);
    context.logger?.debug("Mesacode Protocol session event filtered", {
      event: "mesacode_protocol.session_event.filtered",
  ...
  // tool_input_delta 等高频模型中间态。Mesacode Protocol 的 seq 是 UI/replay 的恢复键，
...
): Promise<MesacodeSessionEvent[]> {
  const events = await record.eventStore.getEvents(record.app.sessionId as SessionId);
  const visibleEvents: MesacodeSessionEvent[] = [];
  const toolInputTransmissions: MesacodeProtocolToolInputTransmissionState = {
    streamedToolCallIdsWithInput: new Set(),
  };
...
function logProtocolSessionEventSent(
  ...
  context.logger?.debug("Mesacode Protocol session event sent", {
    deliveryKind: mappedEvent.deliveryKind,
    event: "mesacode_protocol.session_event.sent",
...
function isImportedHistoryMessage(
  ...
  source: NonNullable<MesacodeSessionCreateParams["importedHistory"]>["source"],
): boolean {
...
async function removePreviousImportedSessionHistory(params: {
  sessionStore: NonNullable<MesacodeProtocolAgentServerContext["deps"]["sessionStore"]>;
  sessionId: SessionId;
  source: NonNullable<MesacodeSessionCreateParams["importedHistory"]>["source"];
}): Promise<number> {
...
async function persistImportedSessionHistory(params: {
  ...
  record: MesacodeProtocolSessionRecord;
  sessionId: SessionId;
  createParams: MesacodeSessionCreateParams;
}): Promise<void> {
  ...
  params.context.logger?.info("Mesacode Protocol imported history persisted", {
    event: "mesacode_protocol.session_import.persisted",
...
async function createSessionWithProjection<T>(
  context: MesacodeProtocolAgentServerContext,
  rawParams: unknown,
  trace: MesacodeProtocolTrace | undefined,
  project: (
    record: MesacodeProtocolSessionRecord,
  ) => Promise<{ value: T; phaseDurationsMs?: SnapshotPhaseDurationsMs; messageCount?: number }>,
): Promise<T> {
  const params = parseParams(mesacodeSessionCreateParamsSchema, rawParams);
  ...
  context.logger?.info("Mesacode Protocol session/create started", {
    event: "mesacode_protocol.session_create.started",
  ...
  try {
    await runSessionModelConfigMutation(record.app, async () => {
      ...
      if (supportedInitialThoughtLevel) {
        ...
        context.logger?.warn("Mesacode Protocol session/create skipped unsupported thought level", {
          event: "mesacode_protocol.session_create.thought_level_skipped",
          module: "bootstrap.mesacode_protocol",
    ...
    if (params.importedHistory) {
      ...
      // 让导入结果从第一刻起就是可续聊、可切模型的 Mesacode session。
    ...
    context.logger?.info("Mesacode Protocol session/create completed", {
      durationMs: Date.now() - startedAt,
      event: "mesacode_protocol.session_create.completed",
    ...
    context.logger?.warn("Mesacode Protocol session/create failed", {
      durationMs: Date.now() - startedAt,
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "mesacode_protocol.session_create.failed",
      module: "bootstrap.mesacode_protocol",
      persistence: params.persistence,
      recordCreateDurationMs,
      rootTraceId: record.traceContext.traceId,
...
export async function activateSessionForResume(
  context: MesacodeProtocolAgentServerContext,
  params: MesacodeSessionResumeParams,
  ...
  context.logger?.debug("Mesacode Protocol session resume started", {
    activeBeforeWait,
    event: "mesacode_protocol.session.resume_started",
  ...
  if (!session) {
    ...
    context.logger?.warn("Mesacode Protocol session resume found no persisted record", {
      activeBeforeWait,
      activeSessionCount: context.sessions.size,
      durationMs: Math.max(0, Date.now() - resumeStartedAt),
      event: "mesacode_protocol.session.resume_persisted_missing",
      hasSessionStore: Boolean(context.deps.sessionStore),
      module: "bootstrap.mesacode_protocol",
      sessionId: params.sessionId,
    });
    throw new ProtocolRequestError(
      mesacodeProtocolErrorCodes.sessionUnavailable,
  ...
  context.logger?.info("Mesacode Protocol session resume completed", {
    activeSessionCount: context.sessions.size,
    durationMs: Math.max(0, Date.now() - resumeStartedAt),
    event: "mesacode_protocol.session.resume_completed",
    module: "bootstrap.mesacode_protocol",
...
export async function resumeSession(context: MesacodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(mesacodeSessionResumeParamsSchema, rawParams);
...
async function repairLegacyRemoteSessionWorkspaceForResume(
  ...
  if (!repairPersisted) {
    context.logger?.warn("legacy remote session workspace repair was not persisted", {
      event: "mesacode_protocol.session_resume.legacy_remote_workspace_repair_rejected",
      legacyWorkspaceDirectory,
  ...
  if (repaired) {
    context.logger?.info("legacy remote session workspace repaired", {
      event: "mesacode_protocol.session_resume.legacy_remote_workspace_repaired",
      legacyWorkspaceDirectory,
      legacyWorkspaceIdentity: legacyRemote.workspaceIdentity,
      module: "bootstrap.mesacode_protocol",
      sessionId: session.id,
...
export async function listSessions(context: MesacodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(mesacodeSessionListParamsSchema, rawParams ?? {});
...
export async function listSessionSubagents(
  ...
  const params = parseParams(mesacodeSessionSubagentsParamsSchema, rawParams ?? {});
  const store = context.deps.sessionStore;
  ...
  if (!parentSession) {
    ...
    context.logger?.warn("Mesacode Protocol session subagents has no persisted parent", {
      activeSessionCount: context.sessions.size,
      activeSession: Boolean(liveParent),
      event: "mesacode_protocol.session.persisted_missing",
      module: "bootstrap.mesacode_protocol",
...
export async function getUsageStats(context: MesacodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(mesacodeUsageStatsParamsSchema, rawParams ?? {});
...
export async function readSession(context: MesacodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(mesacodeSessionReadParamsSchema, rawParams);
...
export async function readMessages(context: MesacodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(mesacodeSessionMessagesParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
export async function readEvents(context: MesacodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(mesacodeSessionEventsParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  return {
...
export async function subscribeSession(
  ...
  const params = parseParams(mesacodeSessionSubscribeParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
export async function sendPrompt(context: MesacodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(mesacodeSessionSendParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  ...
  if (record.activeAbortController) {
    context.logger?.warn("Mesacode Protocol session/send rejected: active prompt exists", {
  ...
  context.logger?.info("Mesacode Protocol session/send accepted", {
...
export async function compactSession(context: MesacodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(mesacodeSessionCompactParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
async function runCompactTurnInBackground(
  ...
  context.logger?.info("Mesacode Protocol background compact started", {
    inputId: params.inputId,
    sessionId: record.app.sessionId,
    workspacePath: record.workspace.workspacePath,
  });
  try {
    ...
    context.logger?.info("Mesacode Protocol background compact completed", {
      durationMs: Date.now() - startedAt,
      inputId: params.inputId,
      sessionId: record.app.sessionId,
      workspacePath: record.workspace.workspacePath,
    ...
    context.logger?.warn("Mesacode Protocol background compact failed", {
      durationMs: Date.now() - startedAt,
...
export async function goalSession(context: MesacodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(mesacodeSessionGoalParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
  ...
  if (params.action === "pause") {
    ...
    if (target && activeAbortController) {
      context.logger?.info("Mesacode Protocol goal pause aborting active turn", {
        inputId: params.inputId,
        sessionId: params.sessionId,
        targetId: target.targetID,
        workspacePath: record.workspace.workspacePath,
      });
      activeAbortController.abort(new Error("Mesacode Protocol goal paused"));
    }
...
export async function forkSession(context: MesacodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(mesacodeSessionForkParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
export async function registerForkedSession(
  context: MesacodeProtocolAgentServerContext,
  record: MesacodeProtocolSessionRecord,
  fork: Awaited<ReturnType<MesacodeProtocolSessionRecord["app"]["forkFromCheckpoint"]>>,
  options: {
    runtimeConfig: {
...
async function inheritForkedSessionTarget(
  context: MesacodeProtocolAgentServerContext,
  parentRecord: MesacodeProtocolSessionRecord,
  forkRecord: MesacodeProtocolSessionRecord,
...
async function runPromptTurnInBackground(
  ...
  const startedAt = Date.now();
  context.logger?.info("Mesacode Protocol background turn started", {
    inputId: params.inputId,
    queryId: params.queryId,
    sessionId: record.app.sessionId,
    textLength: params.content.length,
  ...
  try {
    ...
    context.logger?.info("Mesacode Protocol background turn completed", {
      durationMs: Date.now() - startedAt,
      inputId: params.inputId,
      queryId: params.queryId,
      sessionId: record.app.sessionId,
    ...
    context.logger?.warn("Mesacode Protocol background turn failed", {
      durationMs: Date.now() - startedAt,
      error: error instanceof Error ? error.message : String(error),
      inputId: params.inputId,
      sessionId: record.app.sessionId,
    ...
    if (record.activeAbortController === params.abortController) {
      ...
      record.activeAbortController = undefined;
      context.logger?.info("Mesacode Protocol background turn cleared active controller", {
        durationMs: Date.now() - startedAt,
        inputId: params.inputId,
        sessionId: record.app.sessionId,
        workspacePath: record.workspace.workspacePath,
...
export async function stopSession(context: MesacodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(mesacodeSessionStopParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  const hadActivePrompt = Boolean(record.activeAbortController);
  let pausedTarget: ProtocolGoalTarget | null = null;
  context.logger?.info("Mesacode Protocol session/stop received", {
    hadActivePrompt,
    sessionId: params.sessionId,
    workspacePath: record.workspace.workspacePath,
  });
  ...
  record.activeAbortController?.abort(new Error("Mesacode Protocol session stopped"));
  if (pausedTarget) {
...
async function pauseActiveGoalForSessionStop(
  ...
  try {
    ...
    if (pausedTarget) {
      context.logger?.info("Mesacode Protocol session/stop paused active goal", {
        sessionId: params.sessionId,
    ...
    context.logger?.warn("Mesacode Protocol session/stop failed to pause active goal", {
      error: error instanceof Error ? error.message : String(error),
      sessionId: params.sessionId,
      targetId: target.targetID,
      workspacePath: record.workspace.workspacePath,
...
export async function cancelBackgroundTask(
  ...
  const params = parseParams(mesacodeSessionCancelBackgroundTaskParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  context.logger?.info("Mesacode Protocol session/cancelBackgroundTask received", {
    sessionId: params.sessionId,
...
export async function setModel(context: MesacodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(mesacodeSessionSetModelParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
  await runSessionModelConfigMutation(record.app, async () => {
...
export async function setThoughtLevel(
  ...
  const params = parseParams(mesacodeSessionSetThoughtLevelParamsSchema, rawParams);
...
export async function setMode(context: MesacodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(mesacodeSessionSetModeParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
export async function closeSession(context: MesacodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(mesacodeSessionCloseParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
function shouldCloseSessionForExpectedPersistence(
  currentPersistence: MesacodeSessionPersistence,
  expectedPersistence?: MesacodeSessionPersistence,
): boolean {
...
export async function generateWorkspaceText(
  ...
  const params = parseParams(mesacodeWorkspaceGenerateTextParamsSchema, rawParams);
  ...
  const app =
    active?.app ??
    (await createWorkspaceMesacodeApp(context, params.workspace, {
...
function ensureNoActiveTurn(record: MesacodeProtocolSessionRecord, message: string): void {
  if (!record.activeAbortController) {
    return;
  }
...
function assertExpectedProviderRevision(
  _context: MesacodeProtocolAgentServerContext,
  _record: MesacodeProtocolSessionRecord,
...
async function resolveForkTarget(
  context: MesacodeProtocolAgentServerContext,
  record: MesacodeProtocolSessionRecord,
  target: MesacodeSessionHistoryTarget,
...
export async function getTaskTokenUsage(
  ...
  const params = parseParams(mesacodeTaskTokenUsageParamsSchema, rawParams ?? {});
  const usageStore = context.deps.sessionStore as Partial<UsageStorePort> | undefined;
 ...
 */
function reconcileRecordPersistence(record: MesacodeProtocolSessionRecord): void {
...
export function onSessionEvent(
  ...
  if (computerUseOperationEvent) {
    try {
      ...
      context.notify({
        method: mesacodeProtocolMethods.computerUseOperationEvent,
      ...
      context.logger?.debug("Computer Use operation lifecycle notification sent", {
        event: "mesacode_protocol.computer-use.operation-event.sent",
      ...
      context.logger?.warn("Computer Use operation lifecycle notification failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "mesacode_protocol.computer-use.operation-event.failed",
        eventId: computerUseOperationEvent.eventId,
        sessionId: computerUseOperationEvent.sessionId,
        turnId:
          "turnId" in computerUseOperationEvent ? computerUseOperationEvent.turnId : undefined,
...
async function requestSessionRuntimePreferences(
  ...
  const startedAt = Date.now();
  // MesacodeProtocolTrace.traceId 是协议层的 string；LogContext 需要 branded TraceId，
  // 按本文件既有惯例断言收口，避免多处日志构造重复转换。
  const traceId = trace?.traceId as TraceId | undefined;
  context.logger?.debug("Mesacode Protocol runtime preferences request started", {
    event: "mesacode_protocol.runtime_preferences.request_started",
    module: "bootstrap.mesacode_protocol",
    scope,
    sessionId,
    traceId,
  ...
  try {
    const result = await context.requestClient(
      mesacodeProtocolMethods.sessionRequestRuntimePreferences,
      { sessionId, scope },
      mesacodeSessionRuntimePreferencesResultSchema,
      {
    ...
    context.logger?.debug("Mesacode Protocol runtime preferences response received", {
      durationMs: Math.max(0, Date.now() - startedAt),
      event: "mesacode_protocol.runtime_preferences.response_received",
      module: "bootstrap.mesacode_protocol",
      scope,
      sessionId,
      traceId,
    ...
    const diagnostic = {
      ...
      errorCode,
      errorMessage,
      event: "mesacode_protocol.runtime_preferences.request_failed",
      module: "bootstrap.mesacode_protocol",
      scope,
      sessionId,
      traceId,
    ...
    if (errorCode === -32022) {
      ...
      context.logger?.warn("Mesacode Protocol runtime preferences request timed out", diagnostic);
    } else if (errorCode !== -32601 && errorCode !== -32020) {
      context.logger?.warn("Mesacode Protocol runtime preferences request failed", diagnostic);
    } else {
      context.logger?.debug(
        "Mesacode Protocol runtime preferences compatibility fallback",
        diagnostic,
      );
    }
    if (error instanceof ProtocolRequestError && (error.code === -32601 || error.code === -32020)) {
      ...
      return {
        ...
        memoryEnabled: false,
        modelContextBudgetStrategy: DEFAULT_MESACODE_MODEL_CONTEXT_BUDGET_STRATEGY,
...
async function materializeSessionRecord(
  context: MesacodeProtocolAgentServerContext,
  params: MesacodeSessionRecordParams,
  sessionId: SessionId,
  resume: boolean,
  source: SessionStartupPreferencesSource,
  trace?: MesacodeProtocolTrace,
): Promise<MesacodeProtocolSessionRecord> {
...
async function createRecord(
  ...
  context.logger?.info("Mesacode Protocol createRecord MCP config", {
    event: "mesacode_protocol.create_record.mcp_config",
  ...
  let ownSessionRecord: MesacodeProtocolSessionRecord | undefined;
  const app = await createWorkspaceMesacodeApp(context, workspace, {
    env: context.deps.env,
    eventStore,
    resume,
    runtimeConfig: {
      ...
      // desktop-continuous session/create 由 UI 先解析 ~/.mesacode/.agents 的 enabled MCP，
      ...
      // Mesacode Protocol app-server 创建的 session（desktop/web/mobile）没有传，导致
    ...
    // Mesacode Protocol app-server 以前没有注入可等待的交互 broker，
  ...
  const now = Date.now();
  const record: MesacodeProtocolSessionRecord = {
    app,
    createdAt: now,
    eventStore,
...
): MesacodeSessionCreateParams["mode"] | undefined {
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    const info = messages[index]?.info;
    if (info?.role === "assistant" && isMesacodeSessionMode(info.mode)) return info.mode;
  }
  return undefined;
}

function isMesacodeSessionMode(
  value: unknown,
): value is NonNullable<MesacodeSessionCreateParams["mode"]> {
  return (
    value === "plan" ||
    value === "build" ||
    value === "edit" ||
...
async function snapshotWithDiagnostics(
  ...
  if (totalDurationMs >= SLOW_SNAPSHOT_LOG_THRESHOLD_MS) {
    ...
    context.logger?.warn("Mesacode Protocol session snapshot slow", {
      durationMs: totalDurationMs,
      event: "mesacode_protocol.session_snapshot.slow",
...
async function readSnapshotTarget(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted session goal for protocol snapshot", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "mesacode_protocol.snapshot.target_read_failed",
      sessionId: record.app.sessionId,
    });
    return undefined;
  }
...
async function readSnapshotTodos(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted session todos for protocol snapshot", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "mesacode_protocol.snapshot.todos_read_failed",
      sessionId: record.app.sessionId,
    });
    return [];
  }
...
async function readSessionTargetCompletionVerificationEvents(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted goal verification entries", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "mesacode_protocol.snapshot.goal_verification_entries_read_failed",
      sessionId: record.app.sessionId,
    });
    return [];
  }
  ...
  return await repairPersistedRemoteSessionPaths(sessionStore, session, {
    onPersistenceFailure: (error) => {
      context.logger?.warn("Session path repair persistence failed; using in-memory repair", {
        errorMessage: error instanceof Error ? error.message : String(error),
        event: "mesacode_protocol.session.path_repair.persist_failed",
        sessionId,
      });
    },
  });
...
function emitStateUpdated(
  ...
  const notification: MesacodeStateUpdatedNotification = {
    patch,
    reason,
