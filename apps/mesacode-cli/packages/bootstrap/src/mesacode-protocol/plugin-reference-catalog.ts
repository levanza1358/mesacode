...
import {
  MESACODE_OFFICIAL_PLUGIN_MARKETPLACE_ID,
  mesacodeProtocolNotifications,
  mesacodePluginsReferenceCatalogParamsSchema,
  mesacodePluginsResolveSuggestedReferenceParamsSchema,
  type MesacodePluginReferenceCatalogEntry,
  type MesacodePluginDiagnostic as SharedPluginDiagnostic,
  type MesacodePluginsReferenceCatalogResult,
  type MesacodePluginsResolveSuggestedReferenceResult,
} from "@mesacode/shared";
import type { PluginReferenceCatalogEntry } from "@mesacode/contracts";
import { buildPluginReferenceCatalog } from "@mesacode/core";
import {
...
export async function getPluginReferenceCatalog(
  ...
  const params = parseParams(mesacodePluginsReferenceCatalogParamsSchema, rawParams);
  if (params.sessionId) {
...
const SUGGESTED_PLUGIN_MARKETPLACE = MESACODE_OFFICIAL_PLUGIN_MARKETPLACE_ID;
...
export async function resolveSuggestedPluginReference(
  ...
  rawParams: unknown,
  signal?: AbortSignal,
): Promise<MesacodePluginsResolveSuggestedReferenceResult> {
  const params = parseParams(mesacodePluginsResolveSuggestedReferenceParamsSchema, rawParams);
  ...
  if (
    ...
    return unavailable(
      ...
      "推荐插件不是受信任的官方 mesacode-plugins-official 来源",
    );
  }

  const workingDirectory = params.workspace.workspacePath;
  const readState = () => {
    const outcome = resolveMesacodePlugins({ logger: context.logger, workingDirectory });
  ...
  context.notify({
    method: mesacodeProtocolNotifications.pluginOperationProgress,
  ...
  try {
    const refreshRequest = updateMesacodePluginMarketplace({
    ...
    const refreshTimeoutRequest = new Promise<never>((_, reject) => {
      ...
      refreshTimeout = setTimeout(() => {
        ...
        const timeoutError = new Error("刷新 mesacode-plugins-official 超时（10000 ms）");
  ...
  const overview = getMesacodePluginsOverview({ logger: context.logger, workingDirectory });
...
function resolveReferenceListingDisplayByPluginId(
  ...
  const overview = getMesacodePluginsOverview({ workingDirectory: workspacePath });
...
function toReferenceCatalogEntry(
  ...
  includeCategory = false,
): MesacodePluginReferenceCatalogEntry {
