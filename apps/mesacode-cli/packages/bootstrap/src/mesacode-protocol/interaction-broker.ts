...
import {
  ...
  mesacodeUserInputResponseSchema,
  type MesacodePermissionOption,
  type MesacodePermissionResponse,
  type MesacodeUserInputQuestion,
...
import type { MesacodeProtocolAgentServerContext } from "./server-types.js";
import {
  ...
  const response = await raceClientRequestWithV4Interaction(
    ...
    (signal) =>
      context.requestClient(
        mesacodeProtocolMethods.interactionRequestPermission,
        {
          input: request.input,
    ...
    // v4 answer → MesacodePermissionResponse：optionId 语义来自 v4 reducer 合成的
...
function v4AnswerToPermissionResponse(
  ...
  permissionOptions: MesacodePermissionOption[],
  toolName: string,
  ...
  if (exact) {
    ...
    // option.response——wire 上 mesacodePermissionUpdateSchema 是 strict，旧桌面多一个字段就丢事件。
...
async function requestUserInput(
  ...
  const response = await raceClientRequestWithV4Interaction(
    ...
    (signal) =>
      context.requestClient(
        mesacodeProtocolMethods.interactionRequestUserInput,
        {
          input: request.input,
...
function v4AnswerToUserInputResponse(answer: V4InteractionAnswer): MesacodeUserInputResponse {
...
function v4AnswerToPlanApprovalResponse(answer: V4InteractionAnswer): MesacodeUserInputResponse {
...
function createExitPlanModeApprovalQuestion(): MesacodeUserInputQuestion {
  return {
    header: "Plan",
    options: [
      {
...
function mapAskUserQuestion(question: AskUserQuestion): MesacodeUserInputQuestion {
  return {
...
async function readPersistedAutoResolution(
  ...
  try {
    ...
    context.logger?.warn("Failed to restore user input auto-resolution state", {
      ...
      event: "mesacode_protocol.user_input_auto_resolution_restore_failed",
      interactionId: request.requestId,
