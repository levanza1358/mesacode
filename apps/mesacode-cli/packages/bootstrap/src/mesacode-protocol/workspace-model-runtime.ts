...
import { createInMemorySessionEventStore } from "@mesacode/adapters/storage";
import type { ModelSelection } from "@mesacode/contracts";
import {
  mesacodeProviderDiscoverModelsParamsSchema,
  mesacodeProviderTestModelConnectivityParamsSchema,
  mesacodeWorkspaceReadPresentationParamsSchema,
...
export async function readWorkspacePresentation(
  ...
  const params = parseParams(mesacodeWorkspaceReadPresentationParamsSchema, rawParams);
  return {
...
export async function testProviderModelConnectivity(
  ...
  const params = parseParams(mesacodeProviderTestModelConnectivityParamsSchema, rawParams);
...
export async function discoverProviderModels(
  ...
  const params = parseParams(mesacodeProviderDiscoverModelsParamsSchema, rawParams);
...
export async function createWorkspaceMesacodeApp(
  context: MesacodeProtocolAgentServerContext,
  ...
  return context.deps.createMesacodeApp({
    ...options,
    platform: context.deps.platform,
    providerRuntimeHeadersPort,
    runtimeConfig: {
      ...options.runtimeConfig,
      // createMesacodeApp 会把 workingDirectory 规范化为执行 cwd。把协议入口的
...
export function hasSessionModelProvider(
  ...
  record: Pick<MesacodeProtocolSessionRecord, "app" | "workspace">,
  providerId: string,
): boolean {
...
export function resolveSessionModelContextWindow(
  _context: MesacodeProtocolAgentServerContext,
  record: Pick<MesacodeProtocolSessionRecord, "app" | "workspace" | "restoredModelSelection">,
): number | undefined {
