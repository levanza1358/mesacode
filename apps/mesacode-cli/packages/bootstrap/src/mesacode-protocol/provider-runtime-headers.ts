...
import {
  ...
  mesacodeProviderRuntimeHeadersResponseSchema,
  type MesacodeWorkspaceRef,
} from "@mesacode/shared";
import type { MesacodeAppOptions } from "../app/types.js";
import {
...
): NonNullable<MesacodeAppOptions["providerRuntimeHeadersPort"]> {
  return {
    ...
    async refreshBeforeModelRequest(input) {
      ...
      try {
        result = await context.requestClient(
          mesacodeProtocolMethods.interactionRequestProviderRuntimeHeaders,
          {
        ...
        if (input.abortSignal?.aborted || timedOut) {
          context.notify({
            method: mesacodeProtocolNotifications.providerRuntimeHeadersCancelled,
            trace: protocolTraceFromTraceContext(input.traceContext),
      ...
      // mesacode-plan 的账号鉴权材料是每次请求刷新的运行时配置，
