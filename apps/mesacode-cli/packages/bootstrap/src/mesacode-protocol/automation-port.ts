...
import {
  ...
  mesacodeAutomationCheckTaskBindingResultSchema,
  mesacodeAutomationCreateResultSchema,
  mesacodeAutomationDeleteResultSchema,
  mesacodeAutomationListResultSchema,
  mesacodeAutomationUpdateResultSchema,
  mesacodeProtocolMethods,
  type MesacodeAutomationProtocol,
...
export function createProtocolAutomationPort(
  ...
  resolveOwnSession?: () => MesacodeProtocolSessionRecord | undefined,
): AutomationPort {
  return {
    async create(input, createContext) {
        ...
        try {
          try {
            const result = await context.requestClient(
              mesacodeProtocolMethods.automationCheckTaskBinding,
            ...
            const legacyResult = await context.requestClient(
              mesacodeProtocolMethods.automationList,
              {},
              mesacodeAutomationListResultSchema,
            );
      ...
      try {
        result = await context.requestClient(
          mesacodeProtocolMethods.automationCreate,
          {
    ...
    async update(input) {
      const hasIntervalCarrier = input.intervalUnit !== undefined && input.interval !== undefined;
      const result = await context.requestClient(
        mesacodeProtocolMethods.automationUpdate,
        {
    ...
    async delete(input) {
      const result = await context.requestClient(
        mesacodeProtocolMethods.automationDelete,
        { automationId: input.id },
        mesacodeAutomationDeleteResultSchema,
      );
...
function normalizeCronAutomationMode(
  mode: MesacodeAutomationProtocol["mode"],
): CronAutomation["mode"] {
  switch (mode) {
    case undefined:
      return undefined;
...
function toCronAutomation(input: MesacodeAutomationProtocol): CronAutomation {
  return {
