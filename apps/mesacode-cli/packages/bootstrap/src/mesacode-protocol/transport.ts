...
import { mesacodeProtocolMessageSchema, mesacodeProtocolMethods } from "@mesacode/shared";
import type { Logger } from "@mesacode/contracts";

type MesacodeProtocolOutgoingMessage =
  | MesacodeProtocolError
  | MesacodeProtocolNotification
  | MesacodeProtocolRequest
  | MesacodeProtocolResponse;

type MesacodeProtocolMessageHandler = (
  message: MesacodeProtocolMessage,
) => Promise<MesacodeProtocolOutgoingMessage | undefined>;

const PROTOCOL_EOF_DRAIN_MS = 100;

interface MesacodeProtocolNdjsonConnectionOptions {
  signal?: AbortSignal;
  clearPostResponseMessages?: () => void;
  handleMessage: MesacodeProtocolMessageHandler;
  ...
  ) => readonly MesacodeProtocolOutgoingMessage[];
  takePostResponseBatch?: (requestId: MesacodeProtocolRequestId) => {
    readonly messages: readonly MesacodeProtocolOutgoingMessage[];
    commit(): boolean;
  } | null;
}

export class MesacodeProtocolNdjsonConnection {
  private buffer = "";
  ...
  constructor(private readonly options: MesacodeProtocolNdjsonConnectionOptions) {
  ...
  send(message: MesacodeProtocolOutgoingMessage): void {
  ...
  private readonly onClose = (): void => {
    ...
    this.draining = true;
    this.notifyTransportClosed(new Error("Mesacode Protocol client connection closed"));
  ...
  private readonly onAbort = (): void => {
    this.notifyTransportClosed(new Error("Mesacode Protocol runtime stopping"));
    this.finish();
  };
  ...
  private decodeLine(line: string): MesacodeProtocolMessage | null {
    let decoded: unknown;
    try {
      decoded = JSON.parse(line);
    } catch (error) {
      this.options.logger?.warn("Mesacode Protocol JSON parse failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "mesacode_protocol.parse.failed",
    ...
    const parsed = mesacodeProtocolMessageSchema.safeParse(decoded);
    if (!parsed.success) {
      this.sendError("invalid-message", -32600, "Invalid Mesacode Protocol message", {
  ...
  private shouldBypassProcessingQueue(message: MesacodeProtocolMessage): boolean {
    // 模型任务占住串行队列时，停止/取消请求必须仍能进入 server，
    // 才能把底层 AbortSignal 传给真实模型请求。控制面只旁路当前执行，普通请求仍保持串行。
    return (
      "id" in message &&
      "method" in message &&
      (message.method === mesacodeProtocolMethods.sessionStop ||
        message.method === mesacodeProtocolMethods.workspaceCancelGenerateText)
    );
  }

  private async handleMessage(message: MesacodeProtocolMessage): Promise<void> {
    if (this.terminal) return;
