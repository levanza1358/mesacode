...
import type { McpTelemetryTracker } from "@mesacode/adapters";
import type { WorkspaceHookPolicyProvider } from "@mesacode/core";
import type { AccountProviderConfigSnapshot } from "@mesacode/provider";
import {
  mesacodeProtocolErrorCodes,
  type MesacodeDeliveryKind,
  type ModelSelection,
  type MesacodeModelContextBudgetStrategy,
  type MesacodeProtocolMessage,
  type MesacodeProtocolMethod,
  type MesacodeProtocolNotification,
  type MesacodeProtocolRequest,
  type MesacodeProtocolRequestId,
...
import type { MesacodeApp, MesacodeAppOptions } from "../app/types.js";
import type { V4InteractionRegistry } from "../mesacode-protocol-v4/interaction-registry.js";
import type { ConversationV4Gateway } from "../mesacode-protocol-v4/v4-gateway.js";
...
export interface MesacodeProtocolAgentDependencies {
  createMesacodeApp(options?: Omit<MesacodeAppOptions, "providerRegistry">): MesacodeApp | Promise<MesacodeApp>;
  /**
...
export type MesacodeProtocolAgentResolvedDependencies = MesacodeProtocolAgentDependencies & {
  createSessionEventStore(sessionId: string): SessionEventStorePort;
  workspaceHookPolicyProvider: WorkspaceHookPolicyProvider;
};

export interface MesacodeProtocolEventSequenceState {
  lastSeq: number;
  seqBySourceEventKey: Map<string, number>;
}

export interface MesacodeProtocolToolInputTransmissionState {
  streamedToolCallIdsWithInput: Set<string>;
}

export interface MesacodeProtocolSessionRecord {
  app: MesacodeApp;
  ...
  protocolEventSequences: Map<string, MesacodeProtocolEventSequenceState>;
  protocolToolInputTransmissions: Map<string, MesacodeProtocolToolInputTransmissionState>;
...
export interface MesacodeProtocolClientRequestOptions {
  signal?: AbortSignal;
  timeoutMs?: number;
...
export interface MesacodeProtocolAgentServerContext {
  /** 进程退出期间，迟到的异步物化不能重新登记 session。 */
  assertServing?: () => void;
  deps: MesacodeProtocolAgentResolvedDependencies;
  logger?: Logger;
  ...
  notify(notification: MesacodeProtocolNotification): void;
  requestClient<T>(
...
export function isRequest(message: MesacodeProtocolMessage): message is MesacodeProtocolRequest {
  return "method" in message && "id" in message;
}

export function isNotification(
  message: MesacodeProtocolMessage,
): message is MesacodeProtocolNotification {
  return "method" in message && !("id" in message);
}

export function isResponse(
  message: MesacodeProtocolMessage,
): message is { id: MesacodeProtocolRequestId; result: unknown } {
  return "id" in message && "result" in message;
}

export function isErrorResponse(message: MesacodeProtocolMessage): message is {
...
export function protocolTraceFromTraceContext(context: TraceContext): MesacodeProtocolTrace {
  return {
...
export function requireSession(
  ...
  sessionId: string,
  options: { deliveryKind?: MesacodeDeliveryKind; operation?: string } = {},
  ...
  if (!record) {
    ...
    context.logger?.warn("Mesacode Protocol session runtime missing", {
      activeSessionCount: context.sessions.size,
      event: "mesacode_protocol.session.require_missing",
...
export function createProtocolLogger(deps: MesacodeProtocolAgentDependencies): Logger | undefined {
  return deps.loggerFactory?.createLogger("mesacode").child({
    module: "bootstrap.mesacode_protocol",
  });
}
