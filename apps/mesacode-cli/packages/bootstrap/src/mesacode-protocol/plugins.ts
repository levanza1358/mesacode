import {
  mesacodePluginsConfigureParamsSchema,
  mesacodePluginsResetConfigParamsSchema,
  mesacodePluginsInstallParamsSchema,
  mesacodePluginsMarketplaceAddParamsSchema,
  mesacodePluginsMarketplaceRemoveParamsSchema,
  mesacodePluginsMarketplaceUpdateParamsSchema,
  mesacodePluginsOverviewParamsSchema,
  mesacodePluginsListParamsSchema,
  mesacodePluginsSetEnabledParamsSchema,
  mesacodePluginsUninstallParamsSchema,
  mesacodePluginsUpdateParamsSchema,
  mesacodePluginsValidateParamsSchema,
  mesacodePluginsDescribeParamsSchema,
  mesacodePluginsRestoreBuiltinParamsSchema,
  type MesacodeAvailablePluginSummary,
  type MesacodeInstalledPluginSummary,
  type MesacodePluginComponentGroup,
  type MesacodePluginDiagnostic,
  type MesacodePluginInfo,
  type MesacodePluginMarketplaceSummary,
  type MesacodePluginsConfigureResult,
  type MesacodePluginsDescribeResult,
  type MesacodePluginsInstallResult,
  type MesacodePluginsListResult,
  type MesacodePluginsMarketplaceMutationResult,
  type MesacodePluginsOverviewResult,
  type MesacodePluginsRestoreBuiltinResult,
  type MesacodePluginsSetEnabledResult,
  type MesacodePluginsUninstallResult,
  type MesacodePluginsValidateResult,
} from "@mesacode/shared";
import type { PluginDiagnostic, PluginMetadata } from "@mesacode/contracts";
import {
  addMesacodePluginMarketplace,
  configureMesacodePlugin,
  describeMesacodePlugin,
  getMesacodePluginsOverview,
  installMesacodeMarketplacePlugin,
  removeMesacodePluginMarketplace,
  resolveMesacodePlugins,
  resetMesacodePluginConfig,
  ...
  validateMesacodePlugin,
} from "../plugins.js";
import { listInstalledPluginRecords } from "@mesacode/adapters/plugins";
...
import { createConfig, resolvePath, type ConfigResult } from "@mesacode/adapters/config";
import { parseParams, type MesacodeProtocolAgentServerContext } from "./server-types.js";

// 把 CLI 的 PluginMetadata 投影成协议可序列化的 MesacodePluginInfo (只保留 UI 需要的字段)。
function toPluginInfo(plugin: PluginMetadata, configResult?: ConfigResult): MesacodePluginInfo {
...
function toPluginDiagnostic(diagnostic: PluginDiagnostic): MesacodePluginDiagnostic {
  return {
...
export async function listPlugins(
  context: MesacodeProtocolAgentServerContext,
  rawParams: unknown,
): Promise<MesacodePluginsListResult> {
  const params = parseParams(mesacodePluginsListParamsSchema, rawParams);
...
): Promise<MesacodePluginsSetEnabledResult> {
  const params = parseParams(mesacodePluginsSetEnabledParamsSchema, rawParams);
  abortSignal?.throwIfAborted();
  const result = await setMesacodePluginEnabled({
...
export async function getPluginsOverview(
  context: MesacodeProtocolAgentServerContext,
  rawParams: unknown,
): Promise<MesacodePluginsOverviewResult> {
  const params = parseParams(mesacodePluginsOverviewParamsSchema, rawParams);
  const overview = getMesacodePluginsOverview({
...
export async function addPluginMarketplace(
  ...
  const params = parseParams(mesacodePluginsMarketplaceAddParamsSchema, rawParams);
...
export async function removePluginMarketplace(
  ...
  const params = parseParams(mesacodePluginsMarketplaceRemoveParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
...
export async function updatePluginMarketplace(
  ...
  const params = parseParams(mesacodePluginsMarketplaceUpdateParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
...
export async function installPlugin(
  ...
  rawParams: unknown,
  abortSignal?: AbortSignal,
): Promise<MesacodePluginsInstallResult> {
  const params = parseParams(mesacodePluginsInstallParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
  const result = await withPluginStorageLock(pluginStorageRoot, async () =>
    installMesacodeMarketplacePlugin({
      abortSignal,
...
export async function uninstallPlugin(
  context: MesacodeProtocolAgentServerContext,
  rawParams: unknown,
): Promise<MesacodePluginsUninstallResult> {
  const params = parseParams(mesacodePluginsUninstallParamsSchema, rawParams);
  const removed = await uninstallMesacodeMarketplacePlugin({
    logger: context.logger,
    marketplace: params.marketplace,
...
export async function updatePlugin(
  ...
  const params = parseParams(mesacodePluginsUpdateParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
  ...
  return withPluginStorageLock(pluginStorageRoot, async () => {
    const installedPlugins: MesacodeInstalledPluginSummary[] = [];
    const dependencyClosure: string[] = [];
    // 聚合每条记录重装产生的诊断：installMesacodeMarketplacePlugin 失败时不抛错，而是返回
    // CLI 形态的 PluginDiagnostic（见其错误分支的 toMarketplaceInstallDiagnostic），
    // 这里逐条经协议侧 toPluginDiagnostic 投影成 MesacodePluginDiagnostic 回传，
    // 让失败的重装显式暴露，而不是静默"成功"。
    const diagnostics: MesacodePluginDiagnostic[] = [];
    for (const record of installed) {
      const result = await installMesacodeMarketplacePlugin({
        logger: context.logger,
...
export async function restoreBuiltinPlugin(
  ...
  const params = parseParams(mesacodePluginsRestoreBuiltinParamsSchema, rawParams);
...
export async function configurePlugin(
  ...
  const params = parseParams(mesacodePluginsConfigureParamsSchema, rawParams);
  await configureMesacodePlugin({
...
export async function resetPluginConfig(
  ...
  const params = parseParams(mesacodePluginsResetConfigParamsSchema, rawParams);
  await resetMesacodePluginConfig({
    logger: context.logger,
    pluginId: params.pluginId,
    scope: params.scope,
    workingDirectory: params.workspace.workspacePath,
...
export async function validatePlugin(
  context: MesacodeProtocolAgentServerContext,
  rawParams: unknown,
): Promise<MesacodePluginsValidateResult> {
  const params = parseParams(mesacodePluginsValidateParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
...
export async function describePlugin(
  context: MesacodeProtocolAgentServerContext,
  rawParams: unknown,
): Promise<MesacodePluginsDescribeResult> {
  const params = parseParams(mesacodePluginsDescribeParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
  const result = await withPluginStorageLock(pluginStorageRoot, async () =>
    describeMesacodePlugin({
      logger: context.logger,
  ...
  const components: MesacodePluginComponentGroup[] = result.components.map((group) => ({
    kind: group.kind,
    items: group.items.map((item) => ({
      name: item.name,
      ...(item.description ? { description: item.description } : {}),
...
function toMarketplaceSummary(input: {
  ...
  isOfficial?: boolean;
  featured?: string[];
  refreshFailure?: MesacodePluginMarketplaceSummary["refreshFailure"];
...
function toAvailablePluginSummary(input: {
  ...
  listing?: MesacodeAvailablePluginSummary["listing"];
...
function toInstalledPluginSummary(input: {
  ...
  installedAt?: string;
  componentTypes?: string[];
  hookDetails?: MesacodeInstalledPluginSummary["hookDetails"];
  updateStatus?: "none" | "update-available" | "version-changed";
  latestVersion?: string;
  listing?: MesacodeInstalledPluginSummary["listing"];
