...
import {
  mesacodePluginsCancelOperationParamsSchema,
  mesacodeProtocolMethods,
  mesacodeWorkspaceCancelGenerateTextParamsSchema,
  mesacodeWorkspaceHookTrustGrantParamsSchema,
} from "@mesacode/shared";
import type { BrowserControlPort } from "@mesacode/contracts";
import { InMemoryWorkspaceHookPolicyProvider } from "@mesacode/core";
import {
  V4_METHODS,
  V4_NOTIFICATIONS,
...
import type {
  MesacodeProtocolError,
  MesacodeProtocolMessage,
  MesacodeProtocolMethod,
  MesacodeProtocolNotification,
  MesacodeProtocolRequest,
  MesacodeProtocolRequestId,
  MesacodeProtocolResponse,
} from "@mesacode/shared";
import {
...
} from "../mesacode-protocol-v4/interaction-registry.js";
...
import {
  ...
  parseParams,
  toProtocolError,
  type MesacodeProtocolClientRequestOptions,
  type MesacodeProtocolAgentDependencies,
  type MesacodeProtocolAgentServerContext,
  type MesacodeProtocolSessionRecord,
} from "./server-types.js";
import { createInMemorySessionEventStore } from "@mesacode/contracts";

export type { MesacodeProtocolAgentDependencies, MesacodeProtocolSessionRecord };

const MAX_CLIENT_REQUEST_REANNOUNCE_INTERVAL_MS = 10_000;

type MesacodeProtocolOutboundMessage = MesacodeProtocolNotification | MesacodeProtocolRequest;
...
async function notifyWorkspaceHookTrustGrantSessions(input: {
  ...
  sessions: Map<string, MesacodeProtocolSessionRecord>;
}): Promise<void> {
...
interface MesacodeProtocolPostResponseBatch {
  readonly messages: readonly MesacodeProtocolOutboundMessage[];
  commit(): boolean;
}
...
export class MesacodeProtocolAgentServer {
   ...
   */
  get officialMcpAuthRequestContext(): Pick<MesacodeProtocolAgentServerContext, "requestClient"> {
    return this.context;
  }

  private messageSink?: (message: MesacodeProtocolOutboundMessage) => void;
  private clientDisconnectError?: Error;
  private readonly context: MesacodeProtocolAgentServerContext;
  ...
  private readonly postResponseOutbox = new Map<
    MesacodeProtocolRequestId,
    MesacodeProtocolPostResponseBatch
  >();
  private nextClientRequestId = 1;

  constructor(deps: MesacodeProtocolAgentDependencies) {
    this.runtimeResources = new ProtocolRuntimeResources(deps.createMesacodeApp);
    const resolvedDeps = {
      ...deps,
      createMesacodeApp: this.runtimeResources.create,
      // 默认 turn 窗口保留策略。
    ...
    this.context = {
      ...
      sessions: new Map<string, MesacodeProtocolSessionRecord>(),
  ...
  setNotificationSink(sink: (message: MesacodeProtocolOutboundMessage) => void): void {
  ...
  shutdown(): Promise<void> {
    ...
    const error = new Error("Mesacode Protocol runtime stopping");
    ...
    for (const record of this.context.sessions.values()) {
      ...
      try {
        ...
        this.logger?.warn("Session unsubscribe failed during protocol shutdown", {
          event: "mesacode_protocol.session.unsubscribe.failed",
        });
      }
    }
  ...
  takePostResponseMessages(requestId: MesacodeProtocolRequestId): MesacodeProtocolOutboundMessage[] {
  ...
  takePostResponseBatch(requestId: MesacodeProtocolRequestId): MesacodeProtocolPostResponseBatch | null {
  ...
  async handleMessage(
    message: MesacodeProtocolMessage,
  ): Promise<MesacodeProtocolError | MesacodeProtocolResponse | undefined> {
    this.runtimeResources.assertServing();
    ...
    if (isNotification(message)) {
      this.logger?.debug("Mesacode Protocol notification ignored", {
        event: "mesacode_protocol.notification.ignored",
  ...
  private async handleRequest(
    request: MesacodeProtocolRequest,
  ): Promise<MesacodeProtocolError | MesacodeProtocolResponse> {
  ...
  private async dispatchRequest(request: MesacodeProtocolRequest) {
      ...
      case mesacodeProtocolMethods.workspaceHookTrustGrant: {
        ...
        if (grantResult.accepted) {
          await notifyWorkspaceHookTrustGrantSessions({
            ...
            grantedWorkspaceKey: mesacodeWorkspaceHookTrustGrantParamsSchema.safeParse(request.params)
              .success
              ? mesacodeWorkspaceHookTrustGrantParamsSchema.parse(request.params).workspace
                  .workspaceKey
              : undefined,
  ...
  private cancelPluginOperation(rawParams: unknown) {
    const params = parseParams(mesacodePluginsCancelOperationParamsSchema, rawParams);
  ...
  private cancelWorkspaceGenerateText(rawParams: unknown) {
    const params = parseParams(mesacodeWorkspaceCancelGenerateTextParamsSchema, rawParams);
  ...
  private ok(id: MesacodeProtocolRequestId, result: unknown): MesacodeProtocolResponse {
    return { id, result };
  }

  private fail(
    id: MesacodeProtocolRequestId,
    code: number,
    message: string,
    data?: unknown,
  ): MesacodeProtocolError {
  ...
  private requestClient<T>(
    method: MesacodeProtocolMethod,
    params: unknown,
    resultSchema: ParamsSchema<T>,
    options?: MesacodeProtocolClientRequestOptions,
  ): Promise<T> {
    if (this.clientDisconnectError) {
      throw this.clientDisconnectError;
    }
    if (!this.messageSink) {
      throw new ProtocolRequestError(-32020, `No Mesacode Protocol client is attached for ${method}`);
    }
  ...
  private resolveClientRequest(id: MesacodeProtocolRequestId, result: unknown): void {
    const key = String(id);
  ...
  private rejectClientRequest(id: MesacodeProtocolRequestId, error: Error): void {
    const key = String(id);
    const pending = this.pendingClientRequests.get(key);
    if (!pending) {
      return;
