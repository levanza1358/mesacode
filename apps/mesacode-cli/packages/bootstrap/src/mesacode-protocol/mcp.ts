...
import type { Logger, McpConnectionSnapshot, McpPort, McpServerStatus } from "@mesacode/contracts";
import {
  mesacodeMcpListParamsSchema,
  mesacodeMcpListResultSchema,
  type MesacodeMcpListResult,
...
export async function listMcpServers(
  ...
  rawParams: unknown,
): Promise<MesacodeMcpListResult> {
  const params = parseParams(mesacodeMcpListParamsSchema, rawParams);
  ...
  const startupTimer = new StartupTimer(
    ...
    {
      module: "bootstrap.mesacode_protocol.mcp",
  ...
  const configuredMcpServers = {
    ...
    // 设置页的本地 MCP 列表由 desktop main 解析 `.mesacode` / `.agents` fallback，
  ...
  if (mcpPort) {
    if (params.mode === "status") {
      ...
      return mesacodeMcpListResultSchema.parse({ statuses });
    }
    ...
    if (pendingAuthorizationSnapshot) {
      ...
      return mesacodeMcpListResultSchema.parse({
