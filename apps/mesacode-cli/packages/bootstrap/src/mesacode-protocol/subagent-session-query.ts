...
import type { MesacodeSessionEndedSubagent, MesacodeSessionRunningSubagent } from "@mesacode/shared";
...
interface SessionSubagentProjection {
  revision: number;
  running: MesacodeSessionRunningSubagent[];
  ended: MesacodeSessionEndedSubagent[];
}
...
function runningStatus(input: {
  ...
  parentProjection?: SessionProjection;
}): MesacodeSessionRunningSubagent["status"] | undefined {
...
): MesacodeSessionEndedSubagent["status"] | undefined {
  switch (task?.status) {
    case "completed":
      return "success";
    case "cancelled":
...
function endedStatus(input: {
  ...
  childOutcome: ReturnType<typeof lastChildOutcome>;
  childProjection?: SessionProjection;
}): MesacodeSessionEndedSubagent["status"] {
...
export function projectSessionSubagents(
  ...
  const running: MesacodeSessionRunningSubagent[] = [];
  const ended: MesacodeSessionEndedSubagent[] = [];
...
function encodeCursor(item: MesacodeSessionEndedSubagent): string {
  return Buffer.from(
...
export function paginateEndedSubagents(
  ended: readonly MesacodeSessionEndedSubagent[],
  options: { cursor?: string; limit: number },
): { items: MesacodeSessionEndedSubagent[]; nextCursor?: string } {
