...
import {
  createMesacodeProcessResourceSampler,
  type MesacodeProcessResourceSampler,
...
export function startProtocolResourceSampler(
  server: MesacodeProtocolAgentServer,
  send: (message: MesacodeProtocolNotification) => void,
  logger: Logger,
): MesacodeProcessResourceSampler | undefined {
  try {
    // 本地内存诊断日志：复用同一 60s 节拍，
    // 变化/心跳门控后才写一行，避免与消息流同频刷盘。
    const memoryDiagnosticsGate = createMemorySampleWriteGate();
    const sampler = createMesacodeProcessResourceSampler({
      onSample: (sample, memoryUsage) => {
        send({
          method: mesacodeProtocolNotifications.processResourceSample,
          params: sample,
        });
        ...
        try {
          ...
          if (reason) {
            ...
            logger.info("Process memory sample", {
              event: "mesacode_protocol.process.memory_sample",
              reason,
              ...memoryFields,
              counters,
            });
