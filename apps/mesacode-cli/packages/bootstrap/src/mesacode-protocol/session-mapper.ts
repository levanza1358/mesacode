import {
  ...
  type MesacodeActiveToolCall,
  type MesacodeContextUsageBreakdownItem,
  type MesacodeDeliveryKind,
  type MesacodePendingPermission,
  type MesacodeSessionContextUsage,
  type MesacodeSessionEvent,
  type MesacodeSessionGoal,
  type MesacodeSessionGoalVerification,
  type MesacodeSessionGoalVerificationTimeline,
  type MesacodeSessionGoalStats,
  type MesacodeSessionInfo,
  type MesacodeSessionKind,
  type MesacodeSessionProjection,
  type MesacodeSessionRuntimeState,
  type MesacodeSessionSettingsState,
  type MesacodeSessionStateSnapshot,
  type MesacodeSessionTodoGroup,
...
import type { MesacodeApp } from "../app/types.js";
...
export async function buildSessionSnapshot(input: {
  ...
  return {
    ...
    protocol: {
      name: MESACODE_PROTOCOL_NAME,
      version: MESACODE_PROTOCOL_VERSION,
    },
...
async function mapSnapshotMessages(
  app: Pick<MesacodeApp, "readToolResultArtifact">,
...
async function hydrateSnapshotFilePartUrl(
  ...
  // 历史图片附件持久化后只剩 mesacode-artifact:// 引用，UI/手机端不能直接渲染。
...
function snapshotFilePartArtifactUri(
  ...
  return artifactUri.startsWith("mesacode-artifact://") ? artifactUri : undefined;
}
...
): Promise<MesacodeSessionSettingsState> {
...
export function mapSessionInfo(input: {
  app?: Pick<MesacodeApp, "getMode" | "getModel" | "sessionId" | "traceId">;
  fallbackCreatedAt?: number;
  fallbackUpdatedAt?: number;
  ...
  return {
    ...
    sessionId,
    sessionKind: (input.session?.taskType ?? input.taskType ?? "interactive") as MesacodeSessionKind,
...
export function mapSessionEvents(
  ...
  return events
    ...
    .filter((event): event is MesacodeSessionEvent => event !== null);
}
...
function mapModelNetworkStatusPayload(payload: unknown): Record<string, unknown> {
  const record = asRecord(payload);
  const apiRetry = mesacodeApiRetryFromModelNetworkStatusPayload(record);
  if (apiRetry === undefined) {
    return record;
  }
  const meta = asRecord(record._meta);
  const mesacodeMeta = asRecord(meta.mesacode);
  return {
    ...record,
    _meta: {
      ...meta,
...
function mapStreamRecoveryPayload(payload: unknown): Record<string, unknown> {
  const record = asRecord(payload);
  const apiRetry = mesacodeApiRetryFromStreamRecoveryPayload(record);
  if (apiRetry === undefined) {
    return record;
  }
  const meta = asRecord(record._meta);
...
function mapSessionProjection(projection: SessionProjection): MesacodeSessionProjection {
  return {
...
function mapRuntimeState(input: {
  activeTurn?: ReturnType<MesacodeApp["runtime"]["getActiveTurnInfo"]>;
  deliveryKind?: MesacodeDeliveryKind;
  eventSeq: number;
  messages: MessageWithParts[];
...
export function resolveSessionContextUsage(input: {
  ...
  persistedContextUsageBreakdownEvents?: readonly SessionEvent[];
  projection: SessionProjection;
}): MesacodeSessionContextUsage | undefined {
...
): MesacodeSessionGoalVerification[] {
...
): MesacodeSessionGoalVerificationTimeline[] {
...
function contextUsageFromProjection(
  projection: SessionProjection,
  cache: MesacodeSessionContextUsage["cache"] | undefined,
): MesacodeSessionContextUsage | undefined {
...
function contextCacheUsageFromMessages(
  messages: readonly MessageWithParts[],
): MesacodeSessionContextUsage["cache"] | undefined {
  let inputTokens = 0;
...
function mapPendingPermission(permission: PendingPermission): MesacodePendingPermission {
  // display / optionsPolicy 刻意不进 legacy v3 输出。
  // 根因不是"扩 schema 只能单向兼容"，而是 strict schema 随 packages/shared 打进每个桌面端
  // 的产物：今天把 mesacodePendingPermissionSchema（shared/src/mesacode-protocol/index.ts:1139）和
  // mesacodePermissionRequestedEventPayloadSchema（同文件:1536）改成可选，也保护不了已经装出去
...
function mapPermissionRequestedPayload(payload: unknown): Record<string, unknown> {
  ...
  // 剔除，否则会直接漏进 strict 的 mesacodePermissionRequestedEventPayloadSchema。
...
function mapActiveToolCall(toolCall: ActiveToolCall): MesacodeActiveToolCall {
  return {
...
function mapSessionGoal(goal: SessionGoal | null | undefined): MesacodeSessionGoal | null | undefined {
...
function buildGoalStats(
  projection: SessionProjection,
  messages: readonly MessageWithParts[],
): MesacodeSessionGoalStats | undefined {
  const target = projection.target;
  if (!target) {
    return undefined;
  }
...
function buildTodoGroups(
  ...
  projection: SessionProjection,
): MesacodeSessionTodoGroup[] {
  const target = projection.target;
  const timeline = getTargetGoalVerificationTimeline(projection, target);
  const groups = new Map<string, MesacodeSessionTodoGroup>();
...
function ensureTodoGroup(
  groups: Map<string, MesacodeSessionTodoGroup>,
  input: {
  ...
  const group: MesacodeSessionTodoGroup = {
    id: input.groupId,
...
function addOrUpdateTodoInGroup(
  group: MesacodeSessionTodoGroup,
  fingerprint: string,
  todo: TodoItem,
): void {
...
function mapSessionEventType(type: SessionEvent["type"]): MesacodeSessionEvent["type"] {
  switch (type) {
