import type { Logger, McpPort } from "@mesacode/contracts";
import type { McpConnectionPool, McpTelemetryTracker } from "@mesacode/adapters/mcp";
import type { SqliteSessionStore } from "@mesacode/adapters/storage";
import { shutdownPreparedModelTelemetry } from "@mesacode/telemetry";
...
import type { MesacodeProtocolAgentServer } from "./server.js";
...
export async function cleanupProtocolRuntime(options: {
  logger: Logger;
  deadlineAt?: number;
  server?: Pick<MesacodeProtocolAgentServer, "shutdown" | "disposeProjections">;
  processResourceSampler?: Pick<MesacodeProcessResourceSampler, "stop">;
  ...
  const step = async (resource: string, cleanup: () => unknown | Promise<unknown>) => {
    ...
    try {
      ...
      options.logger.warn(`Mesacode Protocol ${resource} shutdown failed`, {
        errorType: error instanceof Error ? error.name : typeof error,
        event: `mesacode_protocol.${resource}.shutdown.failed`,
