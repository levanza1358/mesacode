...
import {
  mesacodeSkillsReferenceCatalogParamsSchema,
  type MesacodeSkillReferenceCatalogEntry,
  type MesacodeSkillsReferenceCatalogResult,
} from "@mesacode/shared";
import type { SkillLoadOutcome, SkillMetadata } from "@mesacode/contracts";
import { listMesacodeSkills } from "../skills.js";
import {
  parseParams,
  requireSession,
...
export async function getSkillReferenceCatalog(
  ...
  const params = parseParams(mesacodeSkillsReferenceCatalogParamsSchema, rawParams);
  if (params.sessionId) {
  ...
  const outcome = await listMesacodeSkills({
...
function toResult(
  authority: MesacodeSkillsReferenceCatalogResult["authority"],
...
function toReferenceCatalogEntry(skill: SkillMetadata): MesacodeSkillReferenceCatalogEntry {
  const scope =
