import type { OffPeakPort, OffPeakTaskSummary } from "@mesacode/contracts";
import {
  mesacodeOffPeakCreateResultSchema,
  mesacodeOffPeakListResultSchema,
  mesacodeProtocolMethods,
  type MesacodeOffPeakTaskProtocolSnapshot,
  ...
  return {
    async create(input, createContext) {
      ...
      if (boundSessionId) {
        ...
        try {
          const listed = await context.requestClient(
            mesacodeProtocolMethods.offPeakList,
            {},
            mesacodeOffPeakListResultSchema,
          );
      ...
      const result = await context.requestClient(
        mesacodeProtocolMethods.offPeakCreate,
        {
          title: input.title,
          prompt: input.prompt,
...
function toOffPeakTaskSummary(input: MesacodeOffPeakTaskProtocolSnapshot): OffPeakTaskSummary {
  return {
