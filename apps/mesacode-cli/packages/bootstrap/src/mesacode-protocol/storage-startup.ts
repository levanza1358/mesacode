...
import { SqliteSessionStore, type SqliteMigrationProgress } from "@mesacode/adapters/storage";
import {
  classifyDatabaseStartupError,
  databaseStartupErrorDetails,
  mesacodeProtocolNotifications,
  mesacodeStorageStartupStateSchema,
  type MesacodeStorageStartupState,
} from "@mesacode/shared";

export async function openProtocolStartupStorage(options: {
  dbPath: string;
  output: NodeJS.WritableStream;
  onProgress?: (progress: MesacodeStorageStartupState) => void;
  ...
  const report = async (progress: SqliteMigrationProgress) => {
    const params = mesacodeStorageStartupStateSchema.parse({
      schemaVersion: 1,
      attemptId,
      databaseId,
    ...
    await new Promise<void>((resolve, reject) => {
      options.output.write(
        `${JSON.stringify({ method: mesacodeProtocolNotifications.storageStartup, params })}\n`,
...
export async function prepareProtocolStartupStorage(options: {
  ...
  const { mesacodeStoragePathReadySchema, classifyDatabaseStartupError } =
    await import("@mesacode/shared");
  ...
  const acknowledgement = new Promise<boolean>((resolve, reject) => {
    ...
    lines.once("line", (line) => {
      try {
        ...
        const ack = mesacodeStoragePathReadySchema.parse(JSON.parse(line));
