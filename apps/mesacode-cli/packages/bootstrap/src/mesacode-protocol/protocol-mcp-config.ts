import type { McpServerConfig } from "@mesacode/contracts";
import type { MesacodeProtocolMcpServer } from "@mesacode/shared";

export function protocolMcpServersToRuntimeMcpConfig(
  servers: MesacodeProtocolMcpServer[] | undefined,
  ...
  for (const server of servers) {
    if ("command" in server) {
      runtimeServers[server.name] = {
        ...
        // Mesacode Protocol 的 mcpServers 是 session/runtime 和状态查询的完整覆盖配置。
