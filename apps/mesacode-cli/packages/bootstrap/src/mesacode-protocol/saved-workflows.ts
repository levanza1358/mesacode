...
// 解析器与序列化器只从 @mesacode/core 取：这里不解析 frontmatter，也不拼 YAML。
//
// 全局作用域：五个方法的 params 收可选 `scope`（缺省
// `project`）。`global` 时改按本机全局根（`~/.mesacode/workflows/`）操作，`workspace` 只是**载体**——
// 处理器对全局档不读它的路径。`workflows/move` 把全局档搬回 `workspace` 项目（只此一向）。
import { unlink, writeFile } from "node:fs/promises";
import { SavedWorkflowMetaSchema, isValidSavedWorkflowName } from "@mesacode/contracts";
import {
  listSavedWorkflows,
  moveSavedWorkflow,
...
import {
  MESACODE_WORKFLOWS_RUNS_MAX_LIMIT,
  mesacodeWorkflowsDeleteParamsSchema,
  mesacodeWorkflowsGetParamsSchema,
  mesacodeWorkflowsListParamsSchema,
  mesacodeWorkflowsMoveParamsSchema,
  mesacodeWorkflowsRunsParamsSchema,
  mesacodeWorkflowsUpdateMetaParamsSchema,
  type MesacodeSavedWorkflowRun,
  type MesacodeSavedWorkflowScope,
  type MesacodeWorkflowsDeleteResult,
  type MesacodeWorkflowsGetResult,
  type MesacodeWorkflowsListResult,
  type MesacodeWorkflowsMoveResult,
  type MesacodeWorkflowsRunsResult,
  type MesacodeWorkflowsUpdateMetaResult,
} from "@mesacode/shared";
import type { JournalStorePort } from "@mesacode/dynamic-workflow";
...
function scopeOf(params: { scope?: MesacodeSavedWorkflowScope }): MesacodeSavedWorkflowScope {
...
export async function listSavedWorkflowsOp(
  ...
  rawParams: unknown,
): Promise<MesacodeWorkflowsListResult> {
  const params = parseParams(mesacodeWorkflowsListParamsSchema, rawParams);
...
export async function getSavedWorkflowOp(
  _context: MesacodeProtocolAgentServerContext,
  rawParams: unknown,
): Promise<MesacodeWorkflowsGetResult> {
  const params = parseParams(mesacodeWorkflowsGetParamsSchema, rawParams);
...
export async function updateSavedWorkflowMetaOp(
  ...
  const params = parseParams(mesacodeWorkflowsUpdateMetaParamsSchema, rawParams);
...
export async function deleteSavedWorkflowOp(
  ...
  const params = parseParams(mesacodeWorkflowsDeleteParamsSchema, rawParams);
...
export async function listSavedWorkflowRunsOp(
  ...
  const params = parseParams(mesacodeWorkflowsRunsParamsSchema, rawParams);
  ...
  const limit = Math.min(MESACODE_WORKFLOWS_RUNS_MAX_LIMIT, params.limit);
  ...
  const runs: MesacodeSavedWorkflowRun[] = page.map((row) => ({
    runId: row.runId,
...
): Map<string, MesacodeSavedWorkflowRun["artifacts"]> {
  const byRun = new Map<string, MesacodeSavedWorkflowRun["artifacts"]>();
...
export async function moveSavedWorkflowOp(
  _context: MesacodeProtocolAgentServerContext,
  rawParams: unknown,
): Promise<MesacodeWorkflowsMoveResult> {
  const params = parseParams(mesacodeWorkflowsMoveParamsSchema, rawParams);
