...
import { detectLocale, resolveLocale } from "@mesacode/i18n";
import type { RuntimeConfigPatch, SupportedLocale, UiLocale } from "@mesacode/contracts";
import type { MesacodeAppOptions } from "./types.js";

export function isMessageEnabled(env: NodeJS.ProcessEnv): boolean {
  return env.MESACODE_MESSAGE_ENABLED === "1" || env.MESACODE_MESSAGE_ENABLED === "true";
}

export function createConfigCliOverrides(options: MesacodeAppOptions): RuntimeConfigPatch | undefined {
...
function detectAppLocale(options: MesacodeAppOptions): SupportedLocale | undefined {
