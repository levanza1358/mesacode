...
import type { PluginLoadOutcome } from "@mesacode/contracts";
...
import type {
  MesacodeApp,
  MesacodeAppOptions,
  MesacodePluginSetResult,
  MesacodePluginUninstallResult,
} from "./types.js";
...
function createPluginFacade(options: CreatePluginFacadeOptions): PluginFacade {
  ...
  return {
    listPlugins: async (): Promise<PluginLoadOutcome> => listMesacodePlugins(commonOptions()),
    setPluginEnabled: async (
      plugin: string,
      enabled: boolean,
    ): Promise<MesacodePluginSetResult> => {
    ...
    uninstallPlugin: async (plugin: string): Promise<MesacodePluginUninstallResult> => {
      ...
      if (removed) {
        ...
        if (removed.marketplace === MESACODE_OFFICIAL_PLUGIN_MARKETPLACE) {
