...
import {
  ...
  downloadMesacodeBuiltinRelease,
...
export interface ProcessProviderRegistryRuntimeOptions {
  ...
  readonly standalone?: {
    ...
    readonly onBuiltinRefreshResult?: (event: MesacodeBuiltinRefreshEvent) => void;
  };
}
...
export async function startProcessProviderRegistryRuntime(
  ...
  if (!paths) {
    throw new Error("缺少进程 Provider Registry 的 Mesacode Built-in / Personal Config 路径");
  }

  const accountSource = new MutableAccountProviderConfigSource();
  const credentialStore = options.standalone
    ? (options.standalone.credentialStore ?? createSharedMesacodeCredentialStore({ env: { ...env } }))
    : undefined;
  let standaloneAccount: AccountProviderService | undefined;
  const bundledFile = options.standalone
    ? env[MESACODE_BUILTIN_PROVIDER_BUNDLED_CONFIG_FILE_ENV]?.trim()
    : undefined;
  const runtime = new NodeProviderRegistryRuntime({
    ...paths,
    ...(bundledFile
      ? {
          mesacodeBuiltinFilePath: bundledFile,
          mesacodeBuiltinActiveFilePath: paths.mesacodeBuiltinFilePath,
          mesacodeBuiltinRemote: {
            controlFilePath: join(
              dirname(paths.mesacodeBuiltinFilePath),
              "mesacode-builtin-refresh.json",
            ),
            resolveEndpointKey: () => resolveRuntimeMesacodeEndpointOrigin(env),
            fetchRelease: (endpointOrigin, signal) =>
              downloadMesacodeBuiltinRelease({
                endpointOrigin,
                signal,
                appVersion: MESACODE_VERSION,
                platform: resolveMesacodeBuiltinClientPlatform(),
      ...
      : {}),
    onMesacodeBuiltinRefreshError: options.standalone?.onBuiltinRefreshError,
    accountSource,
    ...(credentialStore
      ? {
  ...
  const disposeRecovery = standaloneAccount
    ? runtime.onDidCheckMesacodeBuiltin(async () => {
        const [config, account] = await Promise.all([
          runtime.configService.read(),
          standaloneAccount!.read(),
        ]);
        if (config.mesacodeBuiltinRevision !== account.basedOnMesacodeBuiltinRevision)
...
export function parseProcessAccountProviderConfigSnapshot(input: {
  ...
  readonly basedOnMesacodeBuiltinRevision: string;
  ...
  const basedOnMesacodeBuiltinRevision = input.basedOnMesacodeBuiltinRevision.trim();
  if (!basedOnMesacodeBuiltinRevision) {
  ...
  return Object.freeze({
    revision,
    basedOnMesacodeBuiltinRevision,
    providers,
