import { updateUiLocaleInFileConfig, type ConfigResult } from "@mesacode/adapters/config";
import type { AgentRuntime } from "@mesacode/core";
import { resolveLocale } from "@mesacode/i18n";
import { normalizeModelSelection, type ModelSelection } from "@mesacode/provider";
import {
...
import type { PrepareUserExecutionBoundary, MesacodeApp } from "./types.js";
...
export function createSessionFacade(deps: CreateSessionFacadeDeps): SessionFacade {
  ...
  return {
    ...
    setTarget: async (input) =>
      (await setTargetStatus("set", input)) as Awaited<ReturnType<MesacodeApp["setTarget"]>>,
    updateTargetStatus: async (status) =>
      (await setTargetStatus("status_updated", { status })) as Awaited<
        ReturnType<MesacodeApp["updateTargetStatus"]>
      >,
