...
import { writeBundledOfficialMarketplacePartitionSync } from "@mesacode/adapters";
import { MESACODE_OFFICIAL_PLUGIN_MARKETPLACE, type Logger } from "@mesacode/contracts";
import { isMesacodeCuaInternalFeatureEnabled, MESACODE_CUA_OFFICIAL_PLUGIN_ID } from "@mesacode/shared";
import {
...
const OFFICIAL_PLUGIN_MARKETPLACE = MESACODE_OFFICIAL_PLUGIN_MARKETPLACE;
const SEA_PLUGIN_ASSET_PREFIX = "mesacode-official-plugins/";
const SEA_PLUGIN_MANIFEST_ASSET_KEY = `${SEA_PLUGIN_ASSET_PREFIX}manifest.json`;
const SEED_MARKER_FILE = ".mesacode-plugin-seed.json";
    ...
    if (plugin.missingSeedPaths.length > 0) {
      warnCacheDegraded(input.logger, {
        error: Object.assign(
          ...
          { code: "MESACODE_PLUGIN_SEED_INCOMPLETE" },
        ),
...
export function resolveOfficialPluginRoots(input: {
  ...
  // mesacode-cua 内置 plugin 默认不启用，由 feature flag 控制加载。在 seed/discovery 层门控
  // （而非只隐藏某个 UI 面），这样开关关闭时用户无法经 plugin 列表/marketplace/MCP 设置/CLI 命令看到它。
  if (!isMesacodeCuaInternalFeatureEnabled(input.env ?? process.env)) {
    suppressedBuiltins.add(MESACODE_CUA_OFFICIAL_PLUGIN_ID);
  }
  ...
  const fallbackRoots = failedSeeds.flatMap((definition) => {
    ...
    if (`${definition.name}@${OFFICIAL_PLUGIN_MARKETPLACE}` === MESACODE_CUA_OFFICIAL_PLUGIN_ID) {
      return [];
    }
...
function resolveFilesystemPluginRoot(definition: OfficialPluginDefinition): string | undefined {
  for (const baseDir of candidateBaseDirs()) {
    for (const relativePath of definition.rootCandidates) {
      ...
      if (existsSync(join(rootPath, ".mesacode-plugin", "plugin.json"))) return rootPath;
    }
  }
  return undefined;
}
...
function readSeedPluginDescription(
  ...
  const manifestFile = plugin.files.find((file) => file.path === ".mesacode-plugin/plugin.json");
...
function isSeedUsable(targetRoot: string, definition: OfficialPluginDefinition): boolean {
  try {
    const manifest = JSON.parse(
      readFileSync(join(targetRoot, ".mesacode-plugin", "plugin.json"), "utf8"),
    ) as { name?: unknown };
...
function candidateBaseDirs(): string[] {
  // Electron app-server 运行在 resources/glm/mesacode.cjs，官方插件资源也随桌面包
