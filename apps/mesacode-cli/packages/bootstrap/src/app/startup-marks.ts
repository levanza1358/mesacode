import type { ConfigResult } from "@mesacode/adapters/config";
import { discoverNodePluginsSync } from "@mesacode/adapters/plugins";
import type { AgentRuntimeConfig } from "@mesacode/core";
import type { Logger, McpServerConfig, PluginLoadOutcome } from "@mesacode/contracts";
...
import type { MesacodeAppOptions } from "./types.js";
...
export function resolveStartupPlugins(input: {
  ...
  logger?: Logger;
  options: Pick<MesacodeAppOptions, "officialPluginRoots" | "pluginStorageRoot">;
  ...
  const pluginOutcome = discoverNodePluginsSync({
    ...
    // (resolveMesacodePlugins) 和应用启动路径都要传，否则 `/skill skill-creator` 在会话里
    // 报 "Skill not found: skill-creator"，但 `mesacode plugins list` 却显示它是 enabled。
    officialPluginsEnabledByDefault: DEFAULT_ENABLED_OFFICIAL_PLUGIN_IDS,
    storageRoot: pluginStorageRoot,
    workingDirectory: input.workingDirectory,
  });
  input.startupTimer.mark("Mesacode plugins resolved", {
    context: {
...
export function startAppStartup(input: {
  ...
  input.startupTimer.start("Mesacode app startup started", {
    context: {
...
export function markConfigurationLoaded(input: {
  ...
  input.startupTimer.mark("Mesacode app configuration loaded", {
    context: {
...
export function markStorageAdaptersInitialized(input: {
  ...
  input.startupTimer.mark("Mesacode storage adapters initialized", {
    context: {
...
export function markMcpAdapterInitialized(input: {
  ...
  input.startupTimer.mark("Mesacode MCP adapter initialized", {
    context: {
...
export function markRuntimeConstructed(input: {
  ...
  input.startupTimer.mark("Mesacode runtime constructed", {
    context: {
      hasInjectedModelAdapter: input.hasInjectedModelAdapter,
...
export function completeAppStartup(input: {
  ...
  input.startupTimer.complete("Mesacode app startup completed", {
    context: {
      sessionId: input.sessionId,
      workingDirectory: input.workingDirectory,
    },
