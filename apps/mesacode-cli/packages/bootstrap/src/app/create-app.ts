...
} from "@mesacode/adapters/storage";
import { createNodeLoggerFactory } from "@mesacode/adapters/logging";
import { createConfig, resolvePath } from "@mesacode/adapters/config";
import {
  createNodeExecutionAdapter,
  resolveEffectiveBashShellSelection,
} from "@mesacode/adapters/exec";
import { createNodeFileSystemAdapter } from "@mesacode/adapters/fs";
import { createNodeWebFetchHttpClientAdapter } from "@mesacode/adapters/http";
import { createJimpImageProcessorAdapter } from "@mesacode/adapters/image";
import { createPopplerPdfDocumentAdapter } from "@mesacode/adapters/pdf";
import { createNodeSessionMailboxAdapter } from "@mesacode/adapters/mailbox";
import { createNodeContextSourceAdapter } from "@mesacode/adapters/context";
import { createNodeSkillAdapter } from "@mesacode/adapters/skills";
import { createMcpAdapter } from "@mesacode/adapters/mcp";
import {
  AgentRuntime,
  PermissionService,
...
import { createModelTelemetry } from "@mesacode/telemetry";
import {
...
import { isRemoteWorkspaceIdentity, resolveMesacodeRuntimeEnv } from "@mesacode/shared";
import {
...
import { resolveMesacodeCustomCommandPrompt } from "../custom-command-prompt.js";
import { resolveMesacodeBuiltinPromptCommand } from "../builtin-prompt-command.js";
import { collectDisabledPaths } from "../skill-command-overrides.js";
import { loadPluginAgentProfiles, loadMesacodeAgentProfiles } from "../subagents.js";
...
export async function createMesacodeApp(options: MesacodeAppOptions): Promise<MesacodeApp> {
  if (!options?.providerRegistry) {
    throw new Error("createMesacodeApp requires a Provider Registry");
  }
  ...
  const modelLogger = loggerFactory.createLogger("mesacode").child({
    ...traceContextToLogContext(traceContext),
  ...
  try {
    ...
    const modelIoDir = getModelIoDir(
      cliStorageRoot,
      resolveMesacodeRuntimeEnv(options.env ?? process.env) === "development",
    );
    const mesacodeSubagentProfileOutcome = await loadMesacodeAgentProfiles({
      logger,
      storageRoot,
      workingDirectory,
    });
    const mesacodeSubagentProfiles = mesacodeSubagentProfileOutcome.profiles;
    ...
    const pluginSubagentProfiles = loadPluginAgentProfiles({
      ...
      reservedProfileNames: mesacodeSubagentProfiles.map((profile) => profile.name),
      modelSelectionOverrides: mesacodeSubagentProfileOutcome.pluginAgentModelSelectionOverrides,
    }).profiles;
    ...
    const subagentProfiles = [...mesacodeSubagentProfiles, ...pluginSubagentProfiles];
    ...
    let { configuredMcpServers, runtimeConfig, untrustedProjectMcpServers } =
      resolveAppRuntimeConfig({
        ...
        builtInSubagentModelSelectionOverrides:
          mesacodeSubagentProfileOutcome.builtInModelSelectionOverrides,
    ...
    startupTimer.mark("Mesacode runtime configuration resolved", {
      ...
      ...(options.workspaceHookReviewHost
        ? {
            emitReviewEvent: async (event) => {
              if (!runtime) throw new Error("Mesacode runtime is not initialized yet.");
    ...
    const sessionMailboxPort =
      ...
      (messageEnabled
        ? createNodeSessionMailboxAdapter({
            rootDir: resolvePath(
              (options.env ?? process.env).MESACODE_MAILBOX_ROOT ?? "~/.mesacode/mailbox",
            ),
          })
        : undefined);
    markStorageAdaptersInitialized({
    ...
    const inputFacade = createInputFacade({
      ...
      customCommandPromptResolver: async (text, resolverOptions) => {
        const builtinPrompt = resolveMesacodeBuiltinPromptCommand(text, {
          workingDirectory,
        });
        if (builtinPrompt !== undefined) {
          return builtinPrompt;
        }
        return await resolveMesacodeCustomCommandPrompt(text, {
      ...
      if (input.messageId && input.attachmentIndex !== undefined) {
        ...
        if (persistedAttachment?.type === "file") {
          ...
          artifactUri =
            ...
            (persistedAttachment.url.startsWith("mesacode-artifact://")
    ...
    return {
      ...
      readPromptAttachment: async (input) => {
        ...
        if (ref.startsWith("mesacode-artifact://")) {
      ...
      statPromptAttachment: async (input) => {
        ...
        if (artifactUri) {
          if (!artifactStore.statToolResultArtifact) {
            throw new MesacodeAttachmentFaultError(MESACODE_ATTACHMENT_FAULT_CODES.statUnsupported);
          }
        ...
        if (result.kind !== "file") {
          ...
          throw new MesacodeAttachmentFaultError(MESACODE_ATTACHMENT_FAULT_CODES.statNotFile);
        }
        return {
    ...
    startupTimer.fail("Mesacode app startup failed", error, {
