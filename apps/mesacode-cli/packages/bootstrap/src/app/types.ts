import type { MesacodeToolExecResource, BackgroundBashOutputResult } from "@mesacode/shared";
import type { MesacodeProviderDiscoverModelsResult } from "@mesacode/shared";
import type { AiSdkModelAdapter } from "@mesacode/adapters/model";
import type {
  AgentRuntime,
  AgentRuntimeConfig,
  ExecuteTurnOptions,
...
import type { MesacodeModelOption } from "@mesacode/shared";
import type { EffectiveModelSelectionResult } from "@mesacode/shared/model-selection";
export type { MesacodeModelOption } from "@mesacode/shared";
import type { ModelProviderSourceTitle } from "../model-config.js";
import type { MesacodeInstalledPluginData } from "../plugins.js";
import type {
  AutomationPort,
  OffPeakPort,
...
import type { AgentTelemetryRuntimeOwner, WorkspaceHookPolicy } from "@mesacode/contracts";
...
export type MesacodeAppRuntimeConfigInput = AgentRuntimeConfig;

export interface MesacodeAppOptions {
  sessionId?: SessionId;
  resume?: boolean;
  version?: string;
  traceContext?: TraceContext;
  runtimeConfig?: MesacodeAppRuntimeConfigInput;
  /**
  ...
  onToolExecResource?: (sample: MesacodeToolExecResource) => void;
...
export type SubmitPromptOptions = SubmitPromptOptionsBase &
  import("@mesacode/contracts").TurnBackgroundAttribution;
...
export interface MesacodePluginSetResult {
  enabled: boolean;
  path: string;
  plugin: PluginMetadata;
}

export interface MesacodePluginUninstallResult {
  // null 表示该 plugin id 当前未安装（幂等 no-op），调用方据此提示"未安装"。
  removed: MesacodeInstalledPluginData | null;
}
...
export interface MesacodeApp {
  ...
  getCurrentModelOption?(): MesacodeModelOption | undefined;
  /** 只读 Registry 元数据；不要求选项完整，也不绑定执行模型。 */
  getModelOption?(selection: ModelSelection): MesacodeModelOption | undefined;
  ...
  readSubagents(input?: {
    endedCursor?: string;
    endedLimit?: number;
  }): Promise<import("@mesacode/shared").MesacodeSessionSubagentsResult>;
  ...
  listModels(): MesacodeModelOption[];
  listThoughtLevels(): string[];
  listPlugins(): Promise<PluginLoadOutcome>;
  setPluginEnabled(plugin: string, enabled: boolean): Promise<MesacodePluginSetResult>;
  uninstallPlugin(plugin: string): Promise<MesacodePluginUninstallResult>;
  /**
...
export interface RunMesacodeProtocolAgentOptions {
...
export interface ListMesacodeSessionsOptions {
  directory?: string;
  env?: NodeJS.ProcessEnv;
  limit?: number;
  sessionStore?: SessionStorePort;
