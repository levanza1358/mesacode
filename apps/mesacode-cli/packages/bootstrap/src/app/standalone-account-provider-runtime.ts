...
import type { SharedMesacodeCredentialStore } from "@mesacode/adapters/auth";
import type { ProviderRuntimeHeadersPort } from "@mesacode/core";
import {
...
async function readStandaloneCodingPlanCatalog(
  ...
  readonly mesacodeBuiltinRevision: string;
  readonly providers: readonly StandaloneCodingPlanProvider[];
}> {
  if (config)
    return {
      mesacodeBuiltinRevision: config.revision,
  ...
  const filePath = env[MESACODE_BUILTIN_PROVIDER_CONFIG_FILE_ENV]?.trim();
  if (!filePath) {
    throw new Error(`${MESACODE_BUILTIN_PROVIDER_CONFIG_FILE_ENV} is required for login`);
  }
...
export async function resolveStandaloneCodingPlanProvider(
  ...
  if (matches.length !== 1) {
    throw new Error(
      `Mesacode Built-in Config 必须为 ${family} 声明唯一 Individual Coding Plan Provider`,
    );
  }
  return matches[0]!;
}
...
export async function readStandaloneAccountProviderConfigSnapshot(
  credentialStore: Pick<SharedMesacodeCredentialStore, "loadMany">,
  env: Readonly<Record<string, string | undefined>>,
  config?: Pick<ProviderConfigLayerSnapshot, "revision" | "providers">,
  ...
  return createAccountProviderConfigSnapshot(catalog.mesacodeBuiltinRevision, providers);
}
...
export function createStandaloneProviderRuntimeHeadersPort(
  credentialStore: Pick<SharedMesacodeCredentialStore, "load" | "loadMany">,
  env: Readonly<Record<string, string | undefined>>,
