import { PERMISSION_FULL_ACCESS_OPTION_ID } from "@mesacode/shared/mesacode-protocol-v4";
...
import { verdictWorkspaceHookReviewRequest } from "@mesacode/shared/workspace-hook-review-monotonicity";
import {
  extractPlanStepsFromToolInput,
  extractPlanStepsFromToolOutput,
  isMesacodeModelRetryRecoveryProgressPayload,
  isMesacodeFileStreamingToolInputPreviewTool,
  parseMesacodeBackgroundTaskNotificationText,
  resolveMesacodeBackgroundTaskControlKind,
...
export class ProductProjection {
  ...
  private applyBackgroundTaskNotification(fact: CanonicalUserIntentFact): ConversationDelta[] {
    const parsed = parseMesacodeBackgroundTaskNotificationText(fact.input);
    if (!parsed) return [];
    const row = this.findToolRow(parsed.toolUseId);
    if (!row) return [];

    const notificationStatus = mesacodeBackgroundTaskNotificationToolUpdateStatus(
  ...
  private openToolRow(
    ...
    if (isMesacodeFileStreamingToolInputPreviewTool(row.toolName)) {
  ...
  private appendStreamingToolInput(
    ...
    if (
      state.lastPublishedAt !== null &&
      now - state.lastPublishedAt < MESACODE_FILE_STREAMING_TOOL_INPUT_PREVIEW_MIN_INTERVAL_MS
    ) {
      return [];
    }
  ...
  private onBackgroundTaskLifecycle(event: SessionEvent): ConversationDelta[] {
    ...
    const legacyKind = resolveMesacodeBackgroundTaskControlKind(payload);
  ...
  //
  // 归约本体在 @mesacode/shared 的 workflow-runs-reducer（与状态 schema 同居）：TUI 镜像要用
  // 同一份归约，两处各写一份就是两个时钟。
