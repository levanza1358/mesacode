...
import { getModelUsageTotalTokens, SessionEventType } from "@mesacode/contracts";
import { parseAutomationRunId } from "@mesacode/shared";
...
export function streamingParentToolCallId(payload: Record<string, unknown>): string | undefined {
  ...
  const mesacode = recordValue(meta.mesacode);
  return (
    ...
    optionalString(mesacode.parentToolCallId) ??
    optionalString(mesacode.parentToolUseId)
  );
}
