...
import { localTtftNow, localTtftFactsSchema } from "@mesacode/shared/mesacode-protocol-v4";
import {
...
import type { ConversationSnapshot } from "@mesacode/shared/mesacode-protocol-v4";
import { SessionEventType, isFileSystemPortError } from "@mesacode/contracts";
import type { MesacodeWorkspaceRef } from "@mesacode/shared";
import { extractMarkdownArtifactImageRefs } from "@mesacode/shared";
import type {
  CommandAck,
  AttachmentRef,
  CommandEnvelope,
...
export interface V4GatewayHost {
  ...
  emitLocalTtftFacts?(facts: import("@mesacode/shared").LocalTtftFacts): void;
...
function artifactRefBelongsToSession(ref: string, sessionId: string): boolean {
  return ref.startsWith(`mesacode-artifact://${encodeURIComponent(sessionId)}/`);
}
...
function toShareStatFault(error: unknown): unknown {
  if (readMesacodeAttachmentFaultCode(error)) return error;
  if (isFileSystemPortError(error) && MISSING_ATTACHMENT_FS_CODES.has(error.code)) {
    return new MesacodeAttachmentFaultError(MESACODE_ATTACHMENT_FAULT_CODES.shareStatNotFound, {
      cause: error,
    });
  }
  return error;
...
export class ConversationV4Gateway {
   ...
   * effect 之前，而 CLI 严格串行派发请求（`mesacode-protocol/transport.ts`，只有 session/stop
  ...
  async conversationAttachmentRead(
    ...
    if (!this.host.readSessionAttachment) {
      throw new MesacodeAttachmentFaultError(MESACODE_ATTACHMENT_FAULT_CODES.readUnsupported);
    }
    const existingReady = this.readyFlights.get(params.sessionId);
    const publisher = existingReady
      ? await existingReady
    ...
    if (row?.kind !== "userInput") {
      throw new MesacodeAttachmentFaultError(MESACODE_ATTACHMENT_FAULT_CODES.shareReadNotAuthorized);
    }
  ...
  async conversationAttachmentStat(
    ...
    if (row?.kind !== "userInput") {
      throw new MesacodeAttachmentFaultError(MESACODE_ATTACHMENT_FAULT_CODES.shareStatNotAuthorized);
    }
    const attachment = row.attachments?.[params.attachmentIndex];
    if (!attachment || (attachment.ref !== params.ref && attachment.previewRef !== params.ref)) {
      throw new MesacodeAttachmentFaultError(MESACODE_ATTACHMENT_FAULT_CODES.shareStatNotAuthorized);
    ...
    if (result.totalBytes > PROTOCOL_V4_LIMITS.attachmentStatMaxBytes) {
      throw new MesacodeAttachmentFaultError(MESACODE_ATTACHMENT_FAULT_CODES.shareStatTooLarge);
    }
    ...
    const payload = this.host.readSessionAttachment!(sessionId, {
      ...
      .then((result) => {
        ...
        if (
          ...
          throw new MesacodeAttachmentFaultError(MESACODE_ATTACHMENT_FAULT_CODES.previewNotMedia);
        }
        if (result.bytes.byteLength > maxBytes) {
          throw new MesacodeAttachmentFaultError(MESACODE_ATTACHMENT_FAULT_CODES.previewTooLarge);
        }
        const current = this.binaryReadCache.get(key);
        if (current) {
  ...
  private emitReservation<F extends RoutedTopicFrame>(
    ...
    if (
      ...
      const observations: import("@mesacode/shared").LocalTtftFacts[] = [];
