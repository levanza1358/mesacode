...
//   goal-pause barrier）在 handler 里直驱 core（MesacodeApp / runtime），不经旧协议 op。
// - 会话注册表仍归宿主：通过 V4CommandCoreHost 以「结构化窄视图」透传旧
//   MesacodeProtocolSessionRecord（同一对象引用，字段变更双向可见，不产生第二份注册表）。
...
/**
 * 旧 MesacodeProtocolSessionRecord 的结构化窄视图（只声明命令层需要的字段）。
...
export interface V4CommandCoreHost {
   ...
   * 见 mesacode-protocol/server-operations.ts）；v4 自持会话注册表后收编为原生实现。
   */
