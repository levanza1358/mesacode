...
const traceHeaderNames = ["x-mesacode-trace-id", "x-trace-id", "traceparent"];
...
export class NetworkCaptureService {
  ...
  getStatus(): NetworkCaptureStatus {
    ...
    const env: Record<string, string> = url
      ? {
          MESACODE_HTTP_PROXY: url,
          MESACODE_AGENT_CA_CERT: caCertPath,
        }
      : {};
  ...
  private recordForContext(ctx: IContext): NetworkRequestRecord | undefined {
    const id = stringValue(ctx.tags?.mesacodeCaptureId) ?? ctx.uuid;
...
export function createNetworkCaptureServiceFromEnv(
  ...
  if (isDisabled(env.MESACODE_DEBUG_NETWORK_CAPTURE)) return undefined;
  return new NetworkCaptureService({
    host: env.MESACODE_DEBUG_NETWORK_HOST || undefined,
    port: parsePositiveInteger(env.MESACODE_DEBUG_NETWORK_PORT) ?? DEFAULT_PORT,
    caDir: env.MESACODE_DEBUG_NETWORK_CA_DIR || undefined,
    maxEntries: parsePositiveInteger(env.MESACODE_DEBUG_NETWORK_MAX_ENTRIES) ?? DEFAULT_MAX_ENTRIES,
  });
}
