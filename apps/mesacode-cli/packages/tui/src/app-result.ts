import type { CollaborationMode, ModelUsageSummary, TodoItem, TurnId } from "@mesacode/contracts";
...
export function useTuiApplyResult(input: TuiApplyResultInput) {
  return React.useCallback(
    (result: TuiSubmitPromptResult, preserveTurnState = false) => {
      const resultCopy = getMesacodeCopy(result.locale ?? input.fallback.locale).tui;
