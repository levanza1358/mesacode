...
export class ContextBuilder {
  ...
  build(): ContextBuildResult {
    ...
    // 「You are Mesacode, an interactive coding agent」对一个
    ...
    if (!hasCustomSystemPrompt) {
      if (!isWorkflowActor && this.config.presentationSurface === "mesacode_desktop") {
  ...
  private assembleSystemMessages(sections: ContextSection[]): ModelInputMessage[] {
    ...
    if (dynamicSystemContent) {
      messages.push({
        role: "system",
        // Mesacode by design：Main Agent 的 dynamic system block 自带左边界，所有 provider 保持一致。
