...
import { WORKFLOW_DRAFTS_DIR } from "@mesacode/contracts";
...
/** `filePath` 是否落在 `<workingDirectory>/.mesacode/workflow-drafts/` 之内。 */
function isWorkflowDraftPath(input: WorkflowDraftPathInput): boolean {
  ...
  // 只看路径字符串，不看文件系统：`.mesacode/workflow-drafts-other/x.ts` 因此不会因为前缀
