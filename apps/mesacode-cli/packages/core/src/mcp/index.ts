...
import {
  ...
  MESACODE_MCP_ERROR_PRESENTATION_MESSAGE_ONLY,
  MESACODE_MCP_ERROR_PRESENTATION_META_KEY,
  type JsonSchema,
  type McpPort,
...
import { MESACODE_CUA_OFFICIAL_MCP_NAMESPACE_NAME as MESACODE_CUA_OFFICIAL_MCP_SERVER_NAME } from "@mesacode/shared";
import { OFFICIAL_CUA_FRAME_MODEL_CONTENT_PROTECTION } from "@mesacode/cua/frame-contract";
...
const MESACODE_CUA_CANONICAL_MODEL_PREFIX = "mcp__computer-use__";
const MESACODE_CUA_PROVIDER_SPELLING_ALIAS_PREFIX = "mcp__computer_use__";
...
function toRegisteredMcpToolName(
  ...
  if (
    ...
    descriptor.serverName === MESACODE_CUA_OFFICIAL_MCP_SERVER_NAME
  ) {
    // adapter 会把官方插件 serverName 命名空间化，descriptor.name 因而是
    // mcp__plugin_mesacode-cua_computer-use__*；直接沿用它会让 provider 约定的 computer-use
    // 工具永远不存在。可信门成立后仅投影模型可见名称，handler 仍用 descriptor 的原路由。
    return `${MESACODE_CUA_CANONICAL_MODEL_PREFIX}${toModelVisibleMcpNamePart(descriptor.toolName)}`;
  }
...
function createMcpToolEntry(
  ...
  const isCuaAppObservation = isMesacodeCuaGetAppState(descriptor);
...
function officialCuaProviderSpellingAliases(
  ...
  if (
    !officialCuaAuthorityVerified ||
    descriptor.serverName !== MESACODE_CUA_OFFICIAL_MCP_SERVER_NAME ||
    !name.startsWith(MESACODE_CUA_CANONICAL_MODEL_PREFIX)
  ) {
    return undefined;
  }
  const toolName = name.slice(MESACODE_CUA_CANONICAL_MODEL_PREFIX.length);
  return toolName.length > 0
    ? [`${MESACODE_CUA_PROVIDER_SPELLING_ALIAS_PREFIX}${toolName}`]
    : undefined;
}
...
function createModelFacingMcpInputSchema(
  ...
  // 原因：title 是 Mesacode 给用户看的意图摘要，不属于上游 mesacode-cua 参数。只在模型 contract
...
function isMesacodeCuaGetAppState(
  ...
  return (
    descriptor.serverName === MESACODE_CUA_OFFICIAL_MCP_SERVER_NAME ||
    serverName === "mesacode-cua" ||
    serverName === "computer-use" ||
    (serverName.includes("mesacode-cua") && serverName.includes("computer-use"))
  );
}
...
function formatMcpToolResult(output: unknown): ModelMessageContent {
  ...
  const errorPresentation = output._meta?.[MESACODE_MCP_ERROR_PRESENTATION_META_KEY];
  return typeof errorPresentation === "string" &&
    errorPresentation === MESACODE_MCP_ERROR_PRESENTATION_MESSAGE_ONLY
    ? content
