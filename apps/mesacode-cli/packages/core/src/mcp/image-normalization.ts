...
} from "@mesacode/cua/frame-contract";
...
import { preserveOfficialCuaFrameResult } from "@mesacode/cua/frame-contract";
...
function readBrowserScreenshotContentIndices(result: McpToolCallResult): Set<number> {
  const value = result._meta?.[MESACODE_MCP_BROWSER_SCREENSHOT_CONTENT_INDICES_META_KEY];
