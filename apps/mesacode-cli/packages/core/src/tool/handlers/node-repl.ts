...
import type { SessionId } from "@mesacode/contracts";
...
/**
 * 每个 mesacode session 一个持久 NodeReplSession，跨多次 js 调用保持内存状态（globalThis）。
...
function isBrowserSurfaceSideEffect(command: import("@mesacode/contracts").BrowserCommand): boolean {
...
function isAutoScreenshotTriggerCommand(
  command: import("@mesacode/contracts").BrowserCommand,
): boolean {
  if (
        ...
        execute: async (browserId, browserGeneration, command) => {
          ...
          if (result.meta) {
            ...
            onBrowserResponseMeta({
              "mesacode/browserUse": true,
              "mesacode/toolSurface": {
                kind: "browserUse",
              ...
              ...(result.ok && meta.tabId && isAutoScreenshotTriggerCommand(command)
                ? {
                    "mesacode/browserTurnScreenshot": {
