...
async function resolvePathSource(
  ...
  if (file.meta === undefined && model.args !== undefined) {
    return failure(
      `The workflow script file ${file.described} declares no arguments (it has no \`/* mesacode-workflow\` metadata block), so it takes none. Drop \`args\`, or add a block declaring them.`,
    );
  }
