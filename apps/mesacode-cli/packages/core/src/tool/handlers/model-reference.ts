...
import type { ModelCatalogEntry, ModelSelection } from "@mesacode/contracts";
import {
  MESACODE_MODEL_REASONING_SEPARATOR,
  formatModelPickerValue,
  parseModelPickerValue,
} from "@mesacode/shared/model-selection";
...
function splitReasoningLevel(text: string): { reference: string; level?: string } {
  ...
  const index = text.indexOf(MESACODE_MODEL_REASONING_SEPARATOR, searchFrom);
