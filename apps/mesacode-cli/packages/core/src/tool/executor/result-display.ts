...
import { parseOfficialMcpToolError, type OfficialMcpToolErrorCode } from "@mesacode/shared";
import {
  CUA_REQUEST_ACCESS_STATUS_META_KEY,
  cuaRequestAccessStatusSchema,
} from "@mesacode/cua/request-access-contract";
 ...
 * 响应来自已校验 origin 的 Mesacode 后端。stdio 官方 MCP 与第三方 MCP 塞同样的 payload 一律忽略：
...
function readCuaToolName(toolName: string): string | undefined {
  ...
  // 命名空间形式 `mcp__plugin_mesacode-cua_computer-use__<action>`。
 ...
 * 读取宿主写入的 CUA 目标应用身份。
 *
 * 只认 `mesacode/nodeReplCuaApp`：producer 自己的 `mesacode.cua/app-associations-v1` 也可能出现在
...
function readNodeReplCuaApp(output: Record<string, unknown>): NodeReplCuaAppDisplay | undefined {
  ...
  const parsed = nodeReplCuaAppDisplaySchema.safeParse(
    meta?.[MESACODE_MCP_NODE_REPL_CUA_APP_META_KEY],
  );
