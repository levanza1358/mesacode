...
export function computeOfficialCuaServerNames(
  ...
  const expectedAuthority = getCapturedMesacodeCuaBrokerCredentials().pluginAuthority;
  ...
  for (const [name, config] of Object.entries(servers)) {
    ...
    if (
      config.env?.[MESACODE_PLUGIN_ID_ENV_KEY]?.trim().toLowerCase() !==
        MESACODE_CUA_OFFICIAL_PLUGIN_ID ||
      config.env?.[MESACODE_CUA_PLUGIN_AUTHORITY_ENV_KEY]?.trim() !== expectedAuthority
    ) {
      continue;
    }
    names.add(name);
