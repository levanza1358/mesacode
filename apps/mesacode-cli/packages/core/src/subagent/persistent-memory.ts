...
import type { FileSystemPort, Logger, TraceContext } from "@mesacode/contracts";
...
function resolvePersistentAgentMemoryRoot(input: {
  ...
  return input.scope === "project"
    ? join(workspace, ".mesacode", "agent-memory", agentKey)
    : join(workspace, ".mesacode", "agent-memory-local", agentKey);
}
