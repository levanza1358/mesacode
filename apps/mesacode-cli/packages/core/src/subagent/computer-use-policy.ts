...
import {
  MESACODE_CUA_OFFICIAL_MCP_NAMESPACE_NAME as MESACODE_CUA_OFFICIAL_MCP_SERVER_NAME,
  MESACODE_CUA_OFFICIAL_PLUGIN_ID,
} from "@mesacode/shared";
import type { McpToolDescriptor, PluginReferenceCatalog, SkillMetadata } from "@mesacode/contracts";
...
const OFFICIAL_CUA_SERVER_PARTS = new Set([
  "computer-use",
  "computer_use",
  toModelVisibleMcpNamePart(MESACODE_CUA_OFFICIAL_MCP_SERVER_NAME),
]);
...
export function createOfficialCuaPolicy(
  ...
  for (const descriptor of descriptors) {
    ...
    if (descriptor.serverName === MESACODE_CUA_OFFICIAL_MCP_SERVER_NAME) {
  ...
  if (officialServerNames.has(MESACODE_CUA_OFFICIAL_MCP_SERVER_NAME)) {
  ...
  for (const serverName of officialServerNames) {
    if (serverName === MESACODE_CUA_OFFICIAL_MCP_SERVER_NAME) {
  ...
  const officialPlugin = catalog?.plugins.find(
    (plugin) => plugin.pluginId === MESACODE_CUA_OFFICIAL_PLUGIN_ID,
  );
