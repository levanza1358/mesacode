...
const ACTIVE_WRITER_CONTEXT_KEY = createContextKey("@mesacode/telemetry/active-writer-v4");
...
export function contextFromCausation(causation: AgentTelemetryCausation): Context {
  ...
  // 为底，否则会把另一个异步任务的 Baggage 或 Mesacode Writer 私有状态夹带进来。
...
export function executionProjection(
  ...
  return {
    "mesacode.execution.session_id": options.includeSession
      ? safeId(correlation.sessionId)
      : undefined,
    "mesacode.execution.parent_session_id": options.includeSession
      ? safeId(correlation.parentSessionId)
      : undefined,
    "mesacode.execution.turn_id": safeId(correlation.turnId),
    "mesacode.execution.parent_turn_id": options.includeSession
      ? safeId(correlation.parentTurnId)
      : undefined,
    "mesacode.execution.query_id": options.includeQuery ? safeId(correlation.queryId) : undefined,
    "mesacode.execution.actor_kind": options.includeActor
      ? safeEnum(correlation.actorKind)
      : undefined,
    "mesacode.execution.agent_name": options.includeAgent
      ? safeIdentifier(correlation.agentName)
      : undefined,
    "mesacode.execution.identity_state": options.includeIdentity
      ? safeEnum(correlation.identityState)
      : undefined,
    "mesacode.execution.user_subject_id": options.includeIdentity
      ? safeId(correlation.userSubjectId)
      : undefined,
    "mesacode.execution.launch_surface": safeEnum(correlation.launchSurface),
  };
}
