...
export function createOwnedAgentTelemetryRuntime(
  ...
  const tracer = provider.getTracer("@mesacode/cli-agent-telemetry", String(TELEMETRY_SCHEMA_VERSION));
  const meter = meterProvider.getMeter(
    "@mesacode/cli-agent-telemetry",
...
function metricViews(): ViewOptions[] {
  return [
    histogramView("mesacode.agent.turn.duration", [1, 5, 10, 30, 60, 300, 600]),
    histogramView("mesacode.model.*.duration", [0.25, 1, 2, 5, 10, 30, 60, 120]),
    histogramView(
      "mesacode.context.compaction.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView(
      "mesacode.detached.operation.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView("mesacode.agent.step.duration", [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600]),
    histogramView("mesacode.tool.execution.duration", [0.01, 0.1, 0.5, 1, 5, 30, 120, 300]),
    histogramView(
      "mesacode.command.execution.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView("mesacode.*.time_to_first_*", [0.25, 0.5, 1, 2, 5, 10, 30, 60]),
    histogramView("mesacode.model.call.attempts", [1, 2, 3, 5, 8]),
    cardinalityView("mesacode.model.attempt.tokens"),
    cardinalityView("mesacode.model.attempt.stream_stall.count"),
    cardinalityView("mesacode.telemetry.creation_drop.count"),
    cardinalityView("mesacode.telemetry.abandoned.count"),
  ];
}
...
function telemetryResourceAttributes(
  ...
  return compactResourceAttributes({
    ...
    "mesacode.build.commit_id": resource.buildCommitId,
    "mesacode.device.installation_id": options.includeInstallationId
      ? resource.installationId
      : undefined,
    "mesacode.product.version": resource.productVersion,
    "mesacode.runtime.distribution": resource.runtimeDistribution,
    "mesacode.runtime.surface": resource.runtimeSurface,
    "mesacode.telemetry.schema_owner": "cli",
    "mesacode.telemetry.schema_version": TELEMETRY_SCHEMA_VERSION,
  });
}
