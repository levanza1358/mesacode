...
import type { ModelStatusSink } from "@mesacode/contracts/model";
...
export async function prepareModelTelemetryEnv(
  ...
  if (!resolveOtlpTraceEndpoint(env) || isExplicitlyDisabled(env.MESACODE_MODEL_TELEMETRY_ENABLED)) {
    return env;
  }
  const existingInstallationId = normalizeTelemetryDeviceMid(env.MESACODE_TELEMETRY_DEVICE_MID);
  const installationId =
    existingInstallationId ?? (await resolveStandaloneDeviceMid(env.MESACODE_HOME?.trim()));
  const preparedEnv = installationId ? { ...env, MESACODE_TELEMETRY_DEVICE_MID: installationId } : env;
...
async function createPreparedOwner(
  ...
  const resource: TelemetryResourceContext = {
    buildCommitId: normalizeBuildCommitId(options.buildCommitId ?? env.MESACODE_BUILD_COMMIT_ID),
    cliVersion: normalizeVersion(options.cliVersion),
    deploymentEnvironment:
      env.MESACODE_ENV ??
      env.MESACODE_RUNTIME_ENV ??
      resourceAttributesFromEnv(env)["deployment.environment.name"],
    installationId: normalizeTelemetryDeviceMid(env.MESACODE_TELEMETRY_DEVICE_MID),
    productVersion: normalizeVersion(options.productVersion ?? env.MESACODE_APP_VERSION),
    runtimeDistribution:
      options.runtimeDistribution ??
      resolveRuntimeDistribution(env.MESACODE_TELEMETRY_RUNTIME_DISTRIBUTION),
    runtimeSurface:
      options.runtimeSurface ?? resolveRuntimeSurface(env.MESACODE_TELEMETRY_RUNTIME_SURFACE),
    serviceInstanceId: randomUUID(),
    serviceName: env.OTEL_SERVICE_NAME?.trim() || "mesacode-cli-agent",
  };
  try {
...
function resolveTelemetryIdentity(env: EnvRecord): TelemetryIdentitySnapshot {
  const userSubjectId = normalizeTelemetryId(env.MESACODE_TELEMETRY_USER_SUBJECT_ID);
  const configuredState = env.MESACODE_TELEMETRY_IDENTITY_STATE?.trim();
  const identityState =
    ...
    configuredState === "unknown"
      ...
      : userSubjectId
        ? "authenticated"
        : normalizeTelemetryDeviceMid(env.MESACODE_TELEMETRY_DEVICE_MID)
          ? "anonymous"
          : "unknown";
...
async function resolveStandaloneDeviceMid(
  mesacodeHome: string | undefined,
): Promise<string | undefined> {
  const stateFile = mesacodeHome
    ? join(mesacodeHome, "v2", "telemetry-state.json")
    : join(homedir(), ".mesacode", "v2", "telemetry-state.json");
