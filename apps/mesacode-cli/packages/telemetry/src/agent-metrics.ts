...
export class OtelAgentTelemetryMetrics implements AgentTelemetryMetricRecorder {
  ...
  constructor(meter: Meter) {
    this.durations = {
      agent_turn: duration(meter, "mesacode.agent.turn.duration"),
      agent_step: duration(meter, "mesacode.agent.step.duration"),
      tool_execution: duration(meter, "mesacode.tool.execution.duration"),
      command_execution: duration(meter, "mesacode.command.execution.duration"),
      context_compaction: duration(meter, "mesacode.context.compaction.duration"),
      detached_operation: duration(meter, "mesacode.detached.operation.duration"),
      model_call: duration(meter, "mesacode.model.call.duration"),
      model_attempt: duration(meter, "mesacode.model.attempt.duration"),
    };
    this.modelCallAttempts = meter.createHistogram("mesacode.model.call.attempts", {
      description: "Physical provider attempts per logical model call",
      unit: "{attempt}",
    });
    this.modelTokens = meter.createCounter("mesacode.model.attempt.tokens", {
      description: "Observed provider token delta by token type",
      unit: "{token}",
    });
    this.modelAttemptFirstProviderEvent = duration(
      meter,
      "mesacode.model.attempt.time_to_first_provider_event",
    );
    this.modelAttemptFirstContent = duration(meter, "mesacode.model.attempt.time_to_first_content");
    this.modelAttemptFirstText = duration(meter, "mesacode.model.attempt.time_to_first_text");
    this.modelAttemptStreamStalls = meter.createCounter("mesacode.model.attempt.stream_stall.count", {
      description: "Observed stream stalls",
      unit: "{stall}",
    });
    this.modelAttemptStreamMaxIdle = duration(meter, "mesacode.model.attempt.stream_max_idle");
    this.commandFirstOutput = duration(meter, "mesacode.command.execution.time_to_first_output");
    this.creationDrop = meter.createCounter("mesacode.telemetry.creation_drop.count", {
      description: "Spans rejected before creation by runtime capacity protection",
      unit: "{span}",
    });
    this.abandoned = meter.createCounter("mesacode.telemetry.abandoned.count", {
