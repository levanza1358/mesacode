...
import { JsInputJsonSchema } from "@mesacode/contracts/tools/node-repl";
// 值导入必须走 @mesacode/core/repl 这条深路径：barrel 会把 core 的整张图拖进 bundle
// （tool handlers → @mesacode/dynamic-workflow → typescript，实测 21.7MB 且求值即崩
...
} from "@mesacode/core/repl";
import { createComputerUseRuntime, type ComputerUseRuntime } from "@mesacode/cua";
import { z } from "zod";
...
const WORKER_KIND = "mesacode-node-repl-call";
export const NODE_REPL_MCP_PROCESS_TITLE = "mesacode-node-repl-mcp";
const pluginRoot = process.env.MESACODE_PLUGIN_ROOT ?? process.cwd();
...
const cuaDocumentationRoot = resolve(
  process.env.MESACODE_CUA_PLUGIN_ROOT ?? pluginRoot,
  "docs",
);
...
function buildRequestMeta(meta: Record<string, unknown> | undefined): NodeReplRequestMeta {
  const parsed = requestContextSchema.safeParse(meta?.["com.mesacode/request-context"]);
  // 安全边界：顶层 MCP _meta 是第三方可扩展字段，不能成为 Mesacode session 路由凭据。
...
export function captureComputerUseRuntimeFromEnvironment(
  ...
  const socketPath = env.MESACODE_CUA_PERMISSION_BROKER_SOCKET?.trim();
  if (!socketPath) return undefined;
  return createComputerUseRuntime({
    brokerSocketPath: socketPath,
    refreshMarkerPath: env.MESACODE_CUA_PERMISSION_BROKER_REFRESH_MARKER?.trim(),
  });
}
