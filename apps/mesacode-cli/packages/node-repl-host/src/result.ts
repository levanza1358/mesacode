import {
  MESACODE_MCP_BROWSER_SCREENSHOT_CONTENT_INDICES_META_KEY,
  MESACODE_MCP_NODE_REPL_CUA_APP_META_KEY,
} from "@mesacode/contracts/mcp";
import { isOfficialCuaImageRefText } from "@mesacode/cua/frame-contract";
import { CUA_APP_ASSOCIATIONS_META_KEY } from "@mesacode/cua/host-display-contract";
import type { NodeReplRunResult } from "@mesacode/core/repl";
 ...
 * （mesacode-cua 的 frame-pixel-latest-only 测试即此契约），旧帧对模型没有使用价值。
...
export function toMcpRunResult(run: NodeReplRunResult): CallToolResult {
  ...
  delete responseMeta[MESACODE_MCP_BROWSER_SCREENSHOT_CONTENT_INDICES_META_KEY];
  ...
  delete responseMeta[MESACODE_MCP_NODE_REPL_CUA_APP_META_KEY];
  if (run.cuaApp) responseMeta[MESACODE_MCP_NODE_REPL_CUA_APP_META_KEY] = run.cuaApp;
      ...
      ? {
          _meta: {
            ...
            ...((run.images?.length ?? 0) > 0 || structuredContent.some((block) => block.type === "image")
              ? { "mesacode/nodeReplEmittedImage": true }
              : {}),
