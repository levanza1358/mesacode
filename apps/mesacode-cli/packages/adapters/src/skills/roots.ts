...
import type { SkillRoot, SkillSource } from "@mesacode/contracts";
...
function skillRootsForBase(
  ...
  // 合并而不是 fallback：用户可能同时安装原生 `.mesacode` skill 和兼容 `.agents` skill。
  // 同一级别仍保持 `.mesacode` 优先，后续同名按 root 顺序解析。
  return [
    root(join(baseDirectory, MESACODE_DIR, SKILLS_DIR), scope, "mesacode", nextPriority()),
