/* eslint-disable max-lines -- mesacode-cli 配置 schema 需要集中维护文件解析和 provider 继承，拆散会让配置语义更难对齐。 */
import { z } from "zod";
...
export const LEGACY_CUA_PLUGIN_ID = "mesacode-cua@mesacode-plugins-official";
export const CANONICAL_CUA_PLUGIN_ID = "computer-use@mesacode-plugins-official";
...
export const MesacodeConfigFileSchema = z
  .object({
...
export type MesacodeConfigFile = z.infer<typeof MesacodeConfigFileSchema>;
...
function normalizeMcpServerConfigInput(value: unknown): unknown {
  ...
  // External MCP configs may carry provider-specific timeout fields. Mesacode does not migrate
  ...
  if (server.type === "remote") {
    // 外部 Agent 配置常把 HTTP MCP 标记为 remote；Mesacode 运行态协议类型是 http。
    server.type = "http";
...
export function parseConfigFileToRuntimePatchWithDiagnostics(
  ...
  const parsed = MesacodeConfigFileSchema.parse(normalized);
  return {
...
function parsedConfigFileToRuntimePatch(parsed: MesacodeConfigFile): RuntimeConfigPatch {
...
function normalizePluginConfig(
  plugins: MesacodeConfigFile["plugins"],
...
function parseSkillsRuntimeConfig(
  skills: MesacodeConfigFile["skills"],
