...
import type { McpServerConfig, RuntimeConfigPatch } from "@mesacode/contracts";
import {
    ...
    ...(hooks
      ? {
          hookCandidate: createWorkspaceHookSourceInput({
            ...
            // hooks 字段已由 loadFileConfig 经 MesacodeConfigFileSchema（shared 单源
...
function getProjectConfigBaseDir(path: string): string {
  ...
  return basename(configDirectory) === ".mesacode" ? dirname(configDirectory) : configDirectory;
}
