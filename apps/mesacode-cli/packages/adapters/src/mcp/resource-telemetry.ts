...
import { MESACODE_MCP_RESOURCE_SAMPLE_INTERVAL_MS, type MesacodeMcpResourceSample } from "@mesacode/shared";
import {
  createProcessProbe,
  type ProcessProbe,
...
export interface McpResourceTelemetryOptions {
  arch: MesacodeMcpResourceSample["arch"];
  platform: MesacodeMcpResourceSample["platform"];
  now(): number;
  getProcesses(): McpResourceProcess[];
  onResourceSamples?(samples: MesacodeMcpResourceSample[]): void;
...
export function createMcpResourceTelemetry(options: McpResourceTelemetryOptions) {
  ...
  const sampleNow = async (): Promise<void> => {
    ...
    try {
      ...
      const intervalMs =
        previousAt === undefined
          ? MESACODE_MCP_RESOURCE_SAMPLE_INTERVAL_MS
          : Math.max(1, sampledAt - previousAt);
      const next = new Map<string, Map<number, number>>();
      const groups = new Map<string, MesacodeMcpResourceSample>();
