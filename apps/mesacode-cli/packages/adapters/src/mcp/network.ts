...
import { sanitizeMesacodeRuntimeEnv } from "@mesacode/shared";
...
export function buildMcpStdioEnv(options: {
  ...
  return prependRunningNodeDirectory(
    applyNetworkEgressEnv(sanitizeMesacodeRuntimeEnv(filterStringEnv(sourceEnv)), {
...
function prependRunningNodeDirectory(
  ...
  // remote Agent 由 ~/.mesacode/server/node 启动，但登录环境 PATH 不含该目录，
