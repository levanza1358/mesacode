...
import { acquireFileLock } from "@mesacode/shared/node";
import { MESACODE_FILE_LOCK_TIMEOUT_ERROR_CODE } from "@mesacode/shared";
import type { SharedMesacodeCredentialStore } from "../auth/shared-credentials.js";
...
export async function tryAcquireAuthorizationLease(input: {
  ...
  try {
    ...
    if (getErrorCode(error) === MESACODE_FILE_LOCK_TIMEOUT_ERROR_CODE) return undefined;
