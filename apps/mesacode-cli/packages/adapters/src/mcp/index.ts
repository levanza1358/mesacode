...
import { MESACODE_MCP_SERVER_REQUEST_ID_META_KEY } from "@mesacode/contracts";
...
import {
  ...
  MESACODE_OFFICIAL_MCP_AUTH_TYPE,
...
export interface CreateMcpAdapterOptions {
  ...
  officialMcpAuth?: {
    ...
    /**
     * 当前 Mesacode API origin。stdio 形态没有 `url` 可供校验，targetOrigin 只能由宿主给出
     * ——插件因此无法把身份头导向别的 origin。
     * 与 trustedOrigins 的 `resolveMesacodeApiOrigin` 必须同源，否则两侧判定会分叉。
     */
    resolveMesacodeApiOrigin?: () => string;
 ...
 * 由 adapter 发起无身份的 tools/call，让 Mesacode server 返回权威结构化错误。把 reason 交给 stdio
...
class NodeMcpAdapter implements McpPort {
  ...
  private credentialStore?: SharedMesacodeCredentialStore;
  /**
  ...
  constructor(options: CreateMcpAdapterOptions) {
    this.clientName = options.clientName ?? "mesacode";
  ...
  private async resolveOfficialStdioAuthMeta(
    ...
    const resolveMesacodeApiOrigin = this.officialMcpAuth?.resolveMesacodeApiOrigin;
    const logBase = {
    ...
    if (!authHeadersPort || !trustedOrigins || !resolveMesacodeApiOrigin) {
    ...
    try {
      targetOrigin = resolveMesacodeApiOrigin();
      ...
      return {
        ...
        _meta:
          isError && serverRequestId
            ? { ...meta, [MESACODE_MCP_SERVER_REQUEST_ID_META_KEY]: serverRequestId }
            : meta,
      };
    } catch (error) {
      const durationMs = Date.now() - startedAt;
    ...
    try {
      ...
      const tools = listed.tools.map((tool) =>
        normalizeMcpToolDescriptor(
          ...
          //   - http：结果来自 Mesacode 后端。fetch wrapper 对每次请求校验 origin；登录态只在
          ...
          config.type === "http" && config.auth?.type === MESACODE_OFFICIAL_MCP_AUTH_TYPE,
        ),
      );
  ...
  private async runInteractiveOAuthAuthorization(input: {
    ...
    try {
      ...
      const credentialStore = oauthOptions?.credentialStore ?? createSharedMesacodeCredentialStore();
  ...
  private createOAuthClientProvider(
    ...
    // 官方鉴权与 OAuth 互斥：官方 MCP 的失败只能由 Mesacode 登录/套餐解决，
  ...
  private resolveCredentialStore(): SharedMesacodeCredentialStore {
    this.credentialStore ??= this.mcpOAuth?.credentialStore ?? createSharedMesacodeCredentialStore();
...
function mcpRequestMeta(request: McpCallToolRequest): Record<string, unknown> {
  // nodeRepl.requestMeta 暴露。Mesacode 所有 MCP server 都可忽略这些扩展键；node_repl browser
  ...
  return {
    ...requestContext,
    "com.mesacode/request-context": requestContext,
  };
}
...
function resolveVersionNegotiation(
  ...
  return {
    ...
    probe: {
      ...
      // Mesacode 的总连接超时可能先结束并让 probe 残留，也不给 legacy initialize 留预算。
...
function resolveAuthorizationCodeOAuthConfig(
  ...
  // 只能由 Mesacode 自身的登录/套餐解决，不可能由目标 MCP 的 OAuth 授权解决。
  if (isOfficialAuthConfig(config)) return undefined;
...
function isOfficialAuthConfig(config: McpServerConfig): boolean {
  return (
    ...
    config.auth?.type === "mesacode_official" &&
