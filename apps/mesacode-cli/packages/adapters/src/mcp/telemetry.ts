...
import type { MesacodeMcpTelemetryEvent } from "@mesacode/shared";
import {
...
type McpTelemetryEvent = MesacodeMcpTelemetryEvent;
...
interface CreateMcpTelemetryTrackerOptions {
  arch?: MesacodeMcpTelemetryEvent["arch"];
  idSalt: string;
  now?: () => number;
  onEvent(event: McpTelemetryEvent): void;
  platform?: MesacodeMcpTelemetryEvent["platform"];
  randomId?: () => string;
...
export function createMcpTelemetryTracker(
  ...
  const arch = options.arch ?? (process.arch as MesacodeMcpTelemetryEvent["arch"]);
  const now = options.now ?? Date.now;
  const platform = options.platform ?? (process.platform as MesacodeMcpTelemetryEvent["platform"]);
