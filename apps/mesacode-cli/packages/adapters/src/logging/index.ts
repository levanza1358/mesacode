...
import type { LogContext, LogEntry, Logger, LoggerFactory, LogRedactor } from "@mesacode/contracts";
import { LogLevel, LogLevelName } from "@mesacode/contracts";
...
export function createNodeLoggerFactory(options: NodeLoggerFactoryOptions = {}): NodeLoggerFactory {
  ...
  const logDir = options.logDir ?? options.env?.MESACODE_LOG_DIR ?? getDefaultLogDir();
  const consoleStream =
    typeof options.console === "object"
      ? options.console.stream
      : options.console === true || options.env?.MESACODE_LOG_CONSOLE === "1"
        ? process.stderr
        : undefined;
...
export function getDefaultLogDir(): string {
  return join(homedir(), ".mesacode", "cli", "log");
}
...
function isDevelopmentMode(env: NodeJS.ProcessEnv): boolean {
  const runtimeEnv = normalizeMesacodeRuntimeEnv(env[MESACODE_RUNTIME_ENV_KEY]);
...
function getLogFileName(): string {
  return `mesacode-${formatLocalLogDate(new Date())}.jsonl`;
}
