...
export function createCwdCapturePlan(
  ...
  const cwdFilePath = join(cwdCaptureDir, `mesacode-${crypto.randomUUID()}-cwd`);
...
function createPosixCwdCaptureCommand(command: string, cwdFilePath: string): string {
  return [
    command,
    "__mesacode_status=$?",
    `if [ "$__mesacode_status" -eq 0 ]; then pwd -P > ${shellQuote(cwdFilePath)}; fi`,
    'exit "$__mesacode_status"',
  ].join("\n");
}
...
function createWindowsCmdCwdCaptureCommand(command: string, cwdFilePath: string): string {
  return [
    command,
    'set "__mesacode_status=%ERRORLEVEL%"',
    `if "%__mesacode_status%"=="0" cd > ${cmdQuote(cwdFilePath)}`,
    "exit /b %__mesacode_status%",
  ].join("\r\n");
}
