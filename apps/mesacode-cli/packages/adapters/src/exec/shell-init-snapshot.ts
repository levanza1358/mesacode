...
import { windowsPathToGitBashPath } from "@mesacode/contracts";
import type { ExecutionShellDialect } from "@mesacode/contracts";
...
function pathExportLines(pathValue: string): string[] {
  return [
    ...
    "cat >> \"$SNAPSHOT_FILE\" << 'PATH_END_MESACODE_SHELL_INIT_SNAPSHOT'",
    `export PATH=${shellQuoteAlways(pathValue)}`,
    "PATH_END_MESACODE_SHELL_INIT_SNAPSHOT",
  ];
}
