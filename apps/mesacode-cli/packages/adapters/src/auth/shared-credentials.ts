...
import { atomicWritePrivateTextFile, backupCorruptFile, withFileLock } from "@mesacode/shared/node";
import { createMesacodeCredentialCipher, type MesacodeCredentialCipher } from "./credential-cipher.js";

const MESACODE_DATA_BASE_DIR_ENV_KEY = "MESACODE_DATA_BASE_DIR";
...
export const SHARED_MESACODE_CREDENTIAL_KEYS = {
  ...
  mesacodeJwtToken: "mesacodejwttoken",
} as const;

export interface SharedMesacodeCredentialStoreOptions {
  baseDir?: string;
  cipher?: MesacodeCredentialCipher;
...
export interface SharedMesacodeCredentialStore {
...
export function createSharedMesacodeCredentialStore(
  options: SharedMesacodeCredentialStoreOptions = {},
): SharedMesacodeCredentialStore {
  const env = options.env ?? process.env;
  const filePath = resolveSharedMesacodeCredentialsPath(options);
  const cipher = options.cipher ?? createMesacodeCredentialCipher({ env });
  ...
  return {
    ...
    async clearZaiLoginCredentials(): Promise<void> {
      await mutateRawCredentialRecord(filePath, async (rawCredentials) => {
        const activeProviderRaw = rawCredentials[SHARED_MESACODE_CREDENTIAL_KEYS.activeProvider];
        const activeProvider = activeProviderRaw ? cipher.decrypt(activeProviderRaw) : null;
        delete rawCredentials[SHARED_MESACODE_CREDENTIAL_KEYS.zaiAccessToken];
        delete rawCredentials[SHARED_MESACODE_CREDENTIAL_KEYS.zaiRefreshToken];
        delete rawCredentials[SHARED_MESACODE_CREDENTIAL_KEYS.zaiUserInfo];
        delete rawCredentials[SHARED_MESACODE_CREDENTIAL_KEYS.mesacodeJwtToken];
        if (activeProvider === ZAI_PROVIDER_ID) {
          delete rawCredentials[SHARED_MESACODE_CREDENTIAL_KEYS.activeProvider];
        }
      });
    },
    ...
    async saveZaiLoginCredentials(payload: ZaiLoginCredentialPayload): Promise<void> {
      ...
      await mutateRawCredentialRecord(filePath, (rawCredentials) => {
        rawCredentials[SHARED_MESACODE_CREDENTIAL_KEYS.activeProvider] =
          encryptedCredentials.activeProvider;
        rawCredentials[SHARED_MESACODE_CREDENTIAL_KEYS.zaiAccessToken] =
          encryptedCredentials.accessToken;
        rawCredentials[SHARED_MESACODE_CREDENTIAL_KEYS.mesacodeJwtToken] = encryptedCredentials.jwtToken;
        rawCredentials[SHARED_MESACODE_CREDENTIAL_KEYS.zaiUserInfo] = encryptedCredentials.userInfo;
      });
    },
  };
}

export function loadSharedMesacodeCredentialSync(
  key: string,
  options: SharedMesacodeCredentialStoreOptions = {},
): string | undefined {
  const filePath = resolveSharedMesacodeCredentialsPath(options);
  ...
  try {
    ...
    const cipher = options.cipher ?? createMesacodeCredentialCipher({ env: options.env });
...
export function resolveSharedMesacodeCredentialsPath(
  options: SharedMesacodeCredentialStoreOptions = {},
): string {
  if (options.filePath) {
  ...
  const env = options.env ?? process.env;
  const baseDir = options.baseDir ?? env[MESACODE_DATA_BASE_DIR_ENV_KEY] ?? homedir();
  return join(resolveUserPath(baseDir), ".mesacode", "v2", "credentials.json");
}
...
async function readRawCredentialRecord(filePath: string): Promise<Record<string, string>> {
  ...
  try {
    ...
    throw new Error(`Unable to read shared Mesacode credentials: ${filePath}`, { cause: error });
  }
  ...
  try {
    ...
    throw new Error(`Shared Mesacode credentials are corrupt: ${filePath}.${evidence}`, {
      cause: error,
    });
  }
}
