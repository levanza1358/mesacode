...
const CREDENTIAL_SECRET_ENV_KEY = "MESACODE_CREDENTIAL_SECRET";

export interface MesacodeCredentialCipher {
  decrypt(value: string): string;
  encrypt(value: string): string;
}

export interface MesacodeCredentialCipherOptions {
  env?: Record<string, string | undefined>;
}

export function createMesacodeCredentialCipher(
  options: MesacodeCredentialCipherOptions = {},
): MesacodeCredentialCipher {
  const key = deriveCipherKey(resolveCredentialSecret(options.env ?? process.env));

  return {
    decrypt(value: string): string {
      if (!isEncryptedMesacodeCredentialValue(value)) {
        return value;
      }
...
export function isEncryptedMesacodeCredentialValue(value: string): boolean {
...
function resolveCredentialSecret(env: Record<string, string | undefined>): string {
  ...
  return `mesacode-credential-fallback:${platform()}:${homedir()}:${username}`;
}
