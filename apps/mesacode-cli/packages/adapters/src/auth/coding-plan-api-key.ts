import type { HttpClientPort, HttpClientRunOptions, TraceContext } from "@mesacode/contracts";
import { resolveBigModelApiOrigin } from "@mesacode/shared";

const ZAI_API_HOST = "https://api.z.ai";
const JSON_CONTENT_TYPE = "application/json";
const MESACODE_API_KEY_NAME = "mesacode-api-key";
...
async function resolveBizApiKey(
  ...
  const keyEntry =
    keys.find((item) => item.name === MESACODE_API_KEY_NAME) ??
    (await requestRemoteData<RemoteApiKeySummary>(
      input.httpClient,
      {
        body: new TextEncoder().encode(JSON.stringify({ name: MESACODE_API_KEY_NAME })),
        headers: createBizAuthHeaders(input.authorization),
        method: "POST",
        trace: input.trace,
        url: listUrl,
