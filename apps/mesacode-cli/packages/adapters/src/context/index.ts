...
import { formatLocalIsoDate } from "@mesacode/contracts";
import type {
...
async function findSoulFile(
  ...
  // The global persona lives at `~/.mesacode/SOUL.md` to match the existing
  // `~/.mesacode/AGENTS.md` convention; the workspace persona sits at the project root.
  const filePath = scope === "user" ? join(directory, ".mesacode", "SOUL.md") : join(directory, "SOUL.md");
...
async function findDefaultUserInstructionFile(
  ...
  const filePath = join(resolveUserHomeDir(env), ".mesacode", "AGENTS.md");
  if (await isFile(filePath)) {
