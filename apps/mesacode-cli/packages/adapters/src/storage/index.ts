...
// 内存 event store 实现已下沉到 @mesacode/contracts，
// 这里保持 `@mesacode/adapters/storage` 的导出路径不变，避免调用方改 import。
export {
...
export class NodeToolArtifactStore implements ToolArtifactStorePort {
  ...
  async writeToolResultArtifact(
    ...
    return {
      id: artifactId,
      uri: `mesacode-artifact://${encodeURIComponent(request.sessionId)}/${encodeURIComponent(artifactId)}`,
      path,
...
function parseArtifactUri(uri: string): { artifactId: string; sessionId: string } {
  ...
  if (parsed.protocol !== "mesacode-artifact:") {
