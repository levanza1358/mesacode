...
import type { WorkspaceHookTrustRecord, WorkspaceHookTrustStoreFile } from "@mesacode/contracts";
import {
...
export async function resolveWorkspaceHookTrustStorePath(
  ...
  const userConfigPath = resolve(
    options.userConfigPath ?? join(home, ".mesacode", "cli", "config.json"),
  );
  ...
  const storageRoot = configured ? resolveTrustedUserPath(configured, home) : join(home, ".mesacode");
