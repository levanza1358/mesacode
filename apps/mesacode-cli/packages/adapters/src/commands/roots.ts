...
import type { CustomCommandRoot, CustomCommandSource } from "@mesacode/contracts";

const COMMANDS_DIR = "commands";
const GIT_MARKER = ".git";
const HOME_PREFIX = "~/";
const PRIORITY_STEP = 10;
const MESACODE_DIR = ".mesacode";
...
function commandRootsForBase(
  ...
  // 合并而不是 fallback：兼容 `.agents` 命令和原生 `.mesacode` 命令需要同时可见。
  // 同一级别 `.mesacode` 先扫描，命令同名时仍按“先到先赢”处理。
  return [
    root(join(baseDirectory, MESACODE_DIR, COMMANDS_DIR), scope, "mesacode", nextPriority()),
