import {
  ...
  MESACODE_TOOL_ENV_PASSTHROUGH_ENV_KEY,
  readMesacodeToolEnvPassthroughEnv,
...
const EXPLICIT_CA_SOURCE_KEYS = [MESACODE_AGENT_CA_CERT_ENV_KEY] as const;
const EXPLICIT_NO_PROXY_SOURCE_KEYS = [MESACODE_NO_PROXY_ENV_KEY] as const;
...
export function applyNetworkEgressEnv(
  ...
  deleteEnvKey(env, MESACODE_TOOL_ENV_PASSTHROUGH_ENV_KEY, platform);
...
function applyToolEnvPassthroughEnv(
  ...
  const passthroughEnv = readMesacodeToolEnvPassthroughEnv(sourceEnv);
...
function applyProxyEnv(
  ...
  const mesacodeProxy = normalizeProxyValue(
    getFirstEnvValue(sourceEnv, [MESACODE_HTTP_PROXY_ENV_KEY], platform),
  );
  if (mesacodeProxy) {
    for (const key of ALL_PROXY_KEYS) {
      setEnvKey(env, key, mesacodeProxy, platform);
    }
    return;
  }
}
