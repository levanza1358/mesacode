...
if (check) {
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "mesacode-bash-registry-"));
  ...
  try {
    ...
    if (!actual.equals(expected)) {
      throw new Error(
        "Generated Bash command registry is stale. Run `pnpm --dir apps/mesacode-cli registry:generate`.",
      );
    }
  } finally {
