...
- `npm run build` produces `dist/mesacode.cjs`, which works anywhere Node.js 24.14.0 is installed.
...
mesacode plugins are local bundles that can contribute skills, custom commands, and MCP servers.

Plugin state lives under `~/.mesacode/cli/plugins`:
...
- `marketplaces/mesacode-plugins-official/`: bundled and CDN partitions plus the merged metadata for the single official marketplace.

This repository also ships built-in official plugins as workspace packages. The bundled Browser Use, Document Skills, Skill Creator, and Mesacode Guide content plugins are default-enabled and appear as `browser-use@mesacode-plugins-official`, `document-skills@mesacode-plugins-official`, `skill-creator@mesacode-plugins-official`, and `mesacode-guide@mesacode-plugins-official`. Runtime-heavy official plugins, and local-data migration plugins such as `ios-simulator@mesacode-plugins-official`, `android-emulator@mesacode-plugins-official`, and `restore-legacy-sessions@mesacode-plugins-official`, are discovered by mesacode but stay disabled until the user enables them.

```sh
mesacode plugins list
mesacode plugins enable ios-simulator
mesacode plugins disable browser-use
mesacode plugins enable restore-legacy-sessions
mesacode plugins disable ios-simulator
```
...
MCP config can live directly in `.mesacode-plugin/plugin.json` through `mcpServers`. A plugin may provide both `.mcp.json` and manifest `mcpServers`; when the same server name appears in both places, `mcpServers` from the selected manifest wins.

Supported fields in the current mesacode plugin surface:
...
Example `.mesacode-plugin/plugin.json` with inline MCP config:
...
```json
{
  ...
  "mcpServers": {
    "ios-simulator": {
      "command": "node",
      "args": ["${MESACODE_PLUGIN_ROOT}/dist/mcp/server.js"],
      "cwd": "${MESACODE_PROJECT_DIR}",
      "env": {
        "PLUGIN_DATA": "${MESACODE_PLUGIN_DATA}",
...
- `${MESACODE_PLUGIN_ROOT}`
- `${MESACODE_PLUGIN_DATA}`
- `${MESACODE_PROJECT_DIR}`
- `${user_config.key}`
- `${MESACODE_SOME_ENV}`

Only environment variables with the `MESACODE_` prefix are expanded. Missing variables disable the affected MCP server and produce a plugin diagnostic.

### Recommended Layout

```txt
my-plugin/
  .mesacode-plugin/plugin.json
  .mcp.json
  skills/
    my-skill/SKILL.md
  commands/
...
For MCP servers, prefer Node's normal package build and `bin` output when targeting mesacode-cli, and keep all process/file/network side effects inside the MCP server boundary.

## MCP Configuration

mesacode reads MCP servers from the main JSON config. The default user config path is `~/.mesacode/cli/config.json`; MCP entries live under `mcp.servers`. MCP is enabled by default, so `features.mcp` only needs to be set when you want an explicit on/off switch. The current CLI does not auto-discover standalone `mcp.json` or `.mcp.json` files outside enabled plugins.
...
- `stdio`: requires `command`; accepts `args`, `cwd`, `env`, `enabled`, and `timeoutMs`. `cwd` is resolved from the active working directory, and the server process inherits mesacode's environment plus any `env` overrides.
...
mesacode reads hooks from the same main JSON config file as MCP, usually `~/.mesacode/cli/config.json`. Hooks are disabled by default; set `hooks.enabled` to `true` and add process hooks under `hooks.events`.
