# @mesacode/prompt-trajectory

OpenAI protocol trajectory recorder for inspecting mesacode-cli prompt assembly.
...
```bash
pnpm --filter @mesacode/bootstrap^... build
pnpm --filter @mesacode/bootstrap build

pnpm --filter @mesacode/prompt-trajectory record -- \
  --fixture /path/to/recording.json \
  --out /tmp/mesacode-prompt-trajectory/basic-live

pnpm --filter @mesacode/prompt-trajectory record:prompt -- \
  --prompt "Say hello in one short sentence."

pnpm --filter @mesacode/prompt-trajectory derive -- \
  --out /tmp/mesacode-prompt-trajectory/basic-live

pnpm --filter @mesacode/prompt-trajectory model-io -- \
  --input ~/.mesacode/cli/debug/model-io-<session>.jsonl \
  --out /tmp/mesacode-prompt-trajectory/model-io-session
```
...
uses the same mesacode model config resolution as the CLI. The upstream request is
...
`model-io` reads a real Mesacode `model-io-*.jsonl` file and turns the main
