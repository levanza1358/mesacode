...
interface MesacodeBootstrapModule {
  createModelAdapter(input: unknown): unknown;
  createMesacodeApp(input: unknown): Promise<{
    close?: () => Promise<void>;
...
interface MesacodeContractsModule {
...
interface MesacodeProviderModule {
...
export async function recordPromptTrajectoryFromFixture(input: {
  ...
  try {
    const { bootstrap, contracts, provider } = await loadMesacodeModules();
    ...
    const app = await bootstrap.createMesacodeApp({
...
async function loadMesacodeModules(): Promise<{
  bootstrap: MesacodeBootstrapModule;
  contracts: MesacodeContractsModule;
  provider: MesacodeProviderModule;
}> {
  ...
  try {
    const [bootstrap, contracts, providerConfig, providerRegistry] = await Promise.all([
      import(bootstrapPath) as Promise<MesacodeBootstrapModule>,
      import(contractsPath) as Promise<MesacodeContractsModule>,
      import(providerConfigPath) as Promise<Omit<MesacodeProviderModule, "ProviderRegistry">>,
      import(providerRegistryPath) as Promise<Pick<MesacodeProviderModule, "ProviderRegistry">>,
    ]);
    return {
      bootstrap,
      contracts,
    ...
    throw new Error(
      "Missing built mesacode-cli packages. Run `pnpm --filter @mesacode/bootstrap^... build && pnpm --filter @mesacode/bootstrap build` before `record`.",
      { cause: error },
    );
  }
}
