...
- Use the `ZCODE_` prefix for existing project environment variables. Do not add one without a spec defining its purpose, precedence, errors, and tests.
...
- User interactions must use stable request/response interfaces or session events for TUI and ZCode Protocol V4 clients.
...
- Before finishing, run `pnpm typecheck` and `pnpm lint` from the repository root. For CLI changes also run `pnpm --dir apps/zcode-cli typecheck` and `pnpm --dir apps/zcode-cli lint`.
