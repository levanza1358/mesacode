...
assert.ok(archive, "Usage: node scripts/mesacode-distribution-smoke.mjs <archive.tar.gz>");
const directory = await realpath(await mkdtemp(join(tmpdir(), "mesacode-release-smoke-")));
const root = join(directory, "mesacode");
const runner = join(root, "bin/mesacode.mjs");
const workspace = join(directory, "workspace");
const env = {
  ...process.env,
  MESACODE_DATA_BASE_DIR: join(directory, "data"),
  NODE_PATH: "",
  NODE_OPTIONS: "",
  TERM: "xterm-256color",
};
...
try {
  ...
  await writeFile(
    runtimeCheck,
    'import { runTui } from "@mesacode/tui"; if (typeof runTui !== "function") throw new Error("Missing TUI export"); console.log("tui-runtime-ok");',
  );
  ...
  await until(
    () => /Mesacode/.test(screen) && /(?:登录|\/login|输入提示词|Type a prompt)/i.test(screen),
