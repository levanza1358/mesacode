...
export async function resolveBuiltinProviderBuildEnvironment({
  ...
  if (!value?.trim()) {
    ...
    for (const file of files) {
      ...
      if (parsed.MESACODE_ENV !== undefined) value = parsed.MESACODE_ENV;
    }
  }
  const normalized = value?.trim().toLowerCase() || "test";
  if (normalized !== "test" && normalized !== "production") {
    throw new Error(`Invalid MESACODE_ENV for Built-in Provider build: ${normalized}`);
  }
  return normalized;
}
...
export async function loadBuiltinProviderConfig({ root = repositoryRoot, env = process.env } = {}) {
  ...
  const sourcePath = resolve(
    root,
    env.MESACODE_BUILTIN_PROVIDER_CONFIG_FILE?.trim() || "config/provider/mesacode-builtin.json",
  );
  try {
    ...
    const { decodeMesacodeBuiltinRelease } = await tsImport(
      pathToFileURL(resolve(repositoryRoot, "packages/provider-node/src/mesacode-builtin-release.ts"))
        .href,
      import.meta.url,
    );
    decodeMesacodeBuiltinRelease(JSON.parse(content));
...
export async function stageBuiltinProviderConfig({ directory, ...options }) {
  ...
  await writeFile(resolve(directory, "mesacode-builtin.json"), config.content, "utf8");
  return config;
}
