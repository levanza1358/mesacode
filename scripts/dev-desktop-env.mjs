  ...
  return new Promise((resolveRun, rejectRun) => {
    ...
    const child = spawn(command, spawnArgs, {
      ...
      env: withPinnedNodePath(
        {
          ...process.env,
          MESACODE_ENV: requestedEnv,
          MESACODE_DESKTOP_AGENT_BYTECODE: agentBytecode ? "1" : "0",
        },
        process.execPath,
      ),
      stdio: "inherit",
...
try {
  ...
  await run(pnpmCommand, ["--filter", "@mesacode/desktop", "pre-dev"]);
  ...
  await run(pnpmCommand, ["--filter", "@mesacode/desktop", "dev:runtime"]);
} catch (error) {
