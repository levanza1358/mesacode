...
function resolveBootstrapWithRemoteEnv(baseEnv = process.env) {
  ...
  return {
    ...
    MESACODE_BOOTSTRAP_WITH_REMOTE: "1",
...
function runBootstrapServerBuild() {
  ...
  // bootstrap:with-remote 的最终构建过去复用 build:bootstrap，导致 @mesacode/server build
...
function runBootstrapWithRemoteBuild() {
  for (const filter of ["@mesacode/rpc", "@mesacode/web", "@mesacode/formal-proof"]) {
...
runGit(["submodule", "update", "--init", "--recursive", "apps/mesacode-cli"]);
