...
export async function packageNativeSearchTools({
  platform = process.env.MESACODE_TARGET_OS || process.platform,
  arch = process.env.MESACODE_TARGET_ARCH || process.arch,
...
async function main() {
  await packageNativeSearchTools({
    platform: readOption("platform") ?? process.env.MESACODE_TARGET_OS ?? process.platform,
    arch: readOption("arch") ?? process.env.MESACODE_TARGET_ARCH ?? process.arch,
