...
const defaultEntryPath = join(repoRoot, "apps/mesacode-cli/packages/cli/dist/mesacode.cjs");
...
export async function buildDesktopAgentBytecode({
  ...
  if (env.MESACODE_E2E_COVERAGE === "1") throw new Error("coverage 构建不能启用字节码试验");
  const directory = dirname(entryPath);
  const temporary = join(directory, `.bytecode-${randomUUID()}`);
  const loaderPath = join(directory, "mesacode.bytecode.cjs");
  try {
    ...
    const bytecodeFile = `mesacode.bytecode-${metadata.bytecodeSha256}.jsc`;
    ...
    const runtimeFile = `mesacode.bytecode-runtime-${runtimeHash}.cjs`;
