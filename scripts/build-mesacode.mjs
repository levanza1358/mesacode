...
import {
  ...
  stageTuiRuntime,
} from "./mesacode-distribution/assets.mjs";
import { installScriptSource } from "./mesacode-distribution/installer.mjs";

const root = resolve(import.meta.dirname, "..");
const defaultOutDir = resolve(root, "dist", "mesacode");
const defaultBaseUrl = (await loadEndpointEnv()).MESACODE_DIST_BASE_URL?.trim() || "";
const packageDirName = "mesacode";
const usage = `Usage:
  pnpm build:mesacode
  node scripts/build-mesacode.mjs --skip-build
  node scripts/build-mesacode.mjs --version 3.3.3-dev.1
  node scripts/build-mesacode.mjs --out-dir dist/mesacode
  node scripts/build-mesacode.mjs --base-url http://host/mesacode/deps/mesacode/
...
Options:
  ...
  --out-dir <path>    Output directory. Defaults to dist/mesacode.
...
function run(command, args, options = {}) {
  console.log(`[mesacode] ${commandText(command, args)}`);
...
async function buildOutputs(skipBuild) {
  if (skipBuild) {
    console.log("[mesacode] skipping build; reusing existing outputs");
    return;
  }

  run("pnpm", ["--filter", "@mesacode/cli...", "build"]);
  await rm(resolve(root, "packages", "server", "dist"), {
    force: true,
    recursive: true,
  });
  run("pnpm", ["--filter", "@mesacode/server", "build"]);
  run("pnpm", ["--filter", "@mesacode/web", "build"]);
}

async function stageMesacodePackage({ packageRoot, version }) {
  const webDist = resolve(root, "packages", "web", "dist");
  const serverDist = resolve(root, "packages", "server", "dist");
  const agentBundle = resolve(root, "apps", "mesacode-cli", "packages", "cli", "dist", "mesacode.cjs");
  const agentProvider = resolve(root, "apps/mesacode-cli/packages/cli/dist/provider");
  ...
  await assertFile(resolve(agentProvider, "mesacode-builtin.json"), "Agent provider config");
  ...
  await cp(agentBundle, resolve(packageRoot, "agent", "mesacode.cjs"));
  // TUI 入口通过真正的 CLI 路径定位伴随配置；只复制 JS 会在仓库外启动失败。
  await cp(agentProvider, resolve(packageRoot, "agent/provider"), { recursive: true });
  await cp(
    resolve(root, "apps/mesacode-cli/packages/cli/dist/THIRD-PARTY-NOTICES.md"),
    resolve(packageRoot, "agent/THIRD-PARTY-NOTICES.md"),
  );
  await chmod(resolve(packageRoot, "agent", "mesacode.cjs"), 0o755);
  ...
  const runner = resolve(packageRoot, "bin", "mesacode.mjs");
  await cp(resolve(root, "scripts/mesacode-distribution/runner.mjs"), runner);
...
async function main() {
  ...
  if (!options.help && !options.baseUrl)
    throw new Error("Configure MESACODE_DIST_BASE_URL in .env or pass --base-url");
  if (options.help) {
    console.log(usage);
    return;
  }
  ...
  if (!version || typeof version !== "string") {
    throw new Error("Unable to resolve Mesacode version.");
  }
  ...
  console.log(`[mesacode] release directory: ${outDir}`);
  console.log(`[mesacode] tarball: ${tarball}`);
  console.log(`[mesacode] sha256: ${sha256}`);
}
