...
const agentEntry = join(root, "agent", "mesacode.cjs");

function usage() {
  return `Usage:
  mesacode --web [--host <host>] [--port <port>] [--workspace <path>] [--open|--no-open] [--token <token>|--no-token]
  mesacode --version
`;
}
...
async function serve(options) {
  ...
  const child = spawn(process.execPath, [serverEntry], {
    ...
    env: {
      ...process.env,
      PORT: String(port),
      MESACODE_AGENT_SERVER_ARGS_JSON: JSON.stringify([agentEntry, "app-server", "--stdio"]),
      MESACODE_AGENT_SERVER_COMMAND: process.execPath,
      MESACODE_SERVER_HOST: options.host,
      MESACODE_SERVER_WORKSPACE: options.workspace,
      MESACODE_WEB_STATIC_ROOT: webRoot,
      // 显式关闭 token 时必须清空继承值，否则 --no-token 仍会开启后端鉴权。
      MESACODE_SERVER_AUTH_TOKEN: token,
    },
  ...
  console.log("");
  console.log("Mesacode Web is running");
...
try {
  ...
  if (argv.length === 1 && ["--version", "-v"].includes(argv[0])) {
    ...
    if (argv.length === 1 && ["--help", "-h"].includes(argv[0])) {
      console.log("Web mode: mesacode --web [options] (mesacode --web --help for details)\n");
    }
